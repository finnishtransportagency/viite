...
class OAGProxyServlet extends ProxyServlet {
  ...
  def regex: Regex = "/(viite-kmtk|viite)".r
...
class ArcGisProxyServlet extends ProxyServlet {
  ...
  override def rewriteURI(req: HttpServletRequest): java.net.URI = {
    val uri = req.getRequestURI
    val url = "http://aineistot.esri.fi" + uri.replaceFirst("/(viite-kmtk|viite)", "")
    logger.info(url)
    java.net.URI.create(url)
  }
...
class VKMUIProxyServlet extends ProxyServlet {
  def regex: Regex = "/(viite-kmtk|viite)/viitekehysmuunnin/".r
