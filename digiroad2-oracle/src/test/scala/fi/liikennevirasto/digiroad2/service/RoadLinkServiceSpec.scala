...
class RoadLinkServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Test getRoadLinksFromVVHByMunicipality() When supplying a specific municipality Id Then return the correct return of a ViiteRoadLink of that Municipality") {
    ...
    val roadLink = RoadLink(linkId, Nil, 0, AdministrativeClass.Municipality, TrafficDirection.TowardsDigitizing, None, None, attributes = Map("MUNICIPALITYCODE" -> BigInt(municipalityId)), LifecycleStatus.UnknownLifecycleStatus, municipalityCode = municipalityId)
...
//    val municipalityId = 235
//    val linkId = 2l.toString
//    val roadLink: RoadLinkFetched = RoadLinkFetched(linkId, municipalityId, Nil, AdministrativeClass.Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.AllOthers, attributes = Map("MUNICIPALITYCODE" -> BigInt(235)))
...
//      new RoadLinkFetched(id, municipalityId, Nil, AdministrativeClass.Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.AllOthers, attributes = Map("MUNICIPALITYCODE" -> BigInt(235)))
//    )
//    val linkIdComp = Seq(3l, 4l).map(_.toString)
//    val roadLinksComp = linkIdComp.map(id =>
//      new RoadLinkFetched(id, municipalityId, Nil, AdministrativeClass.Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.AllOthers, attributes = Map("MUNICIPALITYCODE" -> BigInt(235), "SUBTYPE" -> BigInt(3)))
//    )
//
//    val mockVVHClient = MockitoSugar.mock[KgvRoadLink]
