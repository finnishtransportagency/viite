...
object Extractor {
  ...
  def extractFeature(feature: Feature, path: List[List[Double]], linkGeomSource: LinkGeomSource): RoadLinkFetched = {
    ...
    val municipalityCode = Try(attributes.getOrElse("municipalitycode", attributes("MUNICIPALITYCODE")).asInstanceOf[BigInt].toInt).getOrElse(throw new NoSuchElementException(s"Missing mandatory municipalityCode. Check data for linkId: $linkId from $linkGeomSource."))
