...
ALTER TABLE NODE_POINT ADD ("TYPE" NUMBER DEFAULT 99);
...
UPDATE NODE_POINT NP SET "TYPE" = (SELECT CASE
	WHEN (rp.ADDR_M = (SELECT MIN(roadAddr.START_ADDR_M) FROM ROADWAY roadAddr
		...
		OR rp.ADDR_M = (SELECT MIN(roadAddr.START_ADDR_M) FROM ROADWAY roadAddr
			WHERE roadAddr.ROAD_NUMBER = rw.ROAD_NUMBER AND roadAddr.TRACK = rw.TRACK AND roadAddr.ROAD_PART_NUMBER = rw.ROAD_PART_NUMBER
			AND roadAddr.VALID_TO IS NULL AND roadAddr.END_DATE IS NULL)
		OR point.NODE_NUMBER IS NULL) THEN 1
		ELSE 2
	END AS NODE_POINT_TYPE
	FROM NODE node
	LEFT JOIN NODE_POINT point ON node.NODE_NUMBER = point.NODE_NUMBER AND point.VALID_TO IS NULL
	...
	LEFT JOIN ROADWAY rw ON rp.ROADWAY_NUMBER = rw.ROADWAY_NUMBER AND rw.VALID_TO IS NULL AND rw.END_DATE IS NULL
	  WHERE node.VALID_TO IS NULL AND node.END_DATE IS NULL AND point.ID = NP.ID);

UPDATE NODE_POINT SET "TYPE" = 99 WHERE "TYPE" IS NULL;

ALTER TABLE NODE_POINT MODIFY "TYPE" NUMBER DEFAULT 99 NOT NULL;