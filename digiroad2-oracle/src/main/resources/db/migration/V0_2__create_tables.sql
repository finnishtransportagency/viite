CREATE TABLE ROAD_ADDRESS (
  ...
  CONSTRAINT ROAD_ADDRESS_HISTORY_UK UNIQUE (ROAD_NUMBER, ROAD_PART_NUMBER, START_ADDR_M, END_ADDR_M,
...
CREATE TABLE LINEAR_LOCATION (
  ...
  CAL_START_M NUMBER(8,0),
  CAL_END_M NUMBER(8,0),
  ...
  CONSTRAINT LINEAR_LOCATION_UK UNIQUE (ROADWAY_ID, ORDER_NUMBER, LINK_ID, START_MEASURE, END_MEASURE,
    VALID_TO, CAL_START_M, CAL_END_M, FLOATING, SIDE_CODE, LINK_SOURCE) USING INDEX
);
...
CREATE TABLE PROJECT_RESERVED_ROAD_PART (
  ...
  CONSTRAINT COMBINED_UK UNIQUE (PROJECT_ID, ROAD_NUMBER, ROAD_PART_NUMBER) USING INDEX,
...
CREATE TABLE PROJECT_LINK (
  ...
  MODIFIED_DATE DATE, 
  STATUS NUMBER(2,0), 
  CALIBRATION_POINTS NUMBER(1,0) DEFAULT 0 NOT NULL, /* TODO Should this be changed to two separate fields like in Linear_location -table? */
  ROAD_TYPE NUMBER(2,0) DEFAULT 99 NOT NULL,
  ROAD_ADDRESS_ID NUMBER(38,0),
  LINEAR_LOCATION_ID NUMBER(38,0), /* TODO Is this needed here? Road_address has roadway_id and with that we can get the current linear_locations. */
  ...
  CONSTRAINT PROJECT_LINK_ROAD_ADDRESS_FK FOREIGN KEY (ROAD_ADDRESS_ID) REFERENCES ROAD_ADDRESS (ID),
...
CREATE TABLE PUBLISHED_ROAD_ADDRESS (
  ...
  ROAD_ADDRESS_ID NUMBER(38,0) NOT NULL,
  CONSTRAINT PUBLISHED_ROAD_ADDRESS_PK PRIMARY KEY (NETWORK_ID, ROAD_ADDRESS_ID), 
  CONSTRAINT PUBROADADDR_PUBROADNETWORK_FK FOREIGN KEY (NETWORK_ID) REFERENCES PUBLISHED_ROAD_NETWORK (ID),
  CONSTRAINT PUBROADADDR_ROAD_ADDRESS_FK FOREIGN KEY (ROAD_ADDRESS_ID) REFERENCES ROAD_ADDRESS (ID)
);
...
CREATE TABLE ROAD_NETWORK_ERRORS (
  ...
  CONSTRAINT ROAD_NETWORK_ERRORS_PK PRIMARY KEY (ID),
  CONSTRAINT ADDRESS_ERROR_VERSION_UK UNIQUE (ROAD_ADDRESS_ID, ERROR_CODE, ROAD_NETWORK_VERSION) USING INDEX,
  CONSTRAINT RAE_RA_FK FOREIGN KEY (ROAD_ADDRESS_ID) REFERENCES ROAD_ADDRESS (ID),
  CONSTRAINT RAE_LL_FK FOREIGN KEY (LINEAR_LOCATION_ID) REFERENCES LINEAR_LOCATION (ID),
...
-- TODO Should be renamed to ROAD_NAME
CREATE TABLE ROAD_NAMES (
  ...
  CONSTRAINT ROAD_NAMES_PK PRIMARY KEY (ID)
);
