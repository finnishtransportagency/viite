...
class TwoTrackRoadUtilsSpec extends FunSuite with Matchers {
  ...
  private def setUpProjectWithLinks(
    ...
    def projectLink(
      ...
      ProjectLink(NewIdValue, roadPart, track, discontinuity, addrMRange, addrMRange, startDate, None, Some("User"), linkId, 0.0, (addrMRange.endAddrM - addrMRange.startAddrM).toDouble, SideCode.TowardsDigitizing, (NoCP, NoCP), (NoCP, NoCP), geom: Seq[Point], projectId, status, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, (addrMRange.endAddrM - addrMRange.startAddrM).toDouble, roadwayId, linearLocationId, ely, reversed = false, None, 0L)
    }

    def withTrack(testTrack: TestTrack): Seq[ProjectLink] = {
      testTrack.status.zipWithIndex.map {
        case (status, index) =>
          val addrMs = AddrMRange(testTrack.geom(index).minBy(_.x).x.toLong, testTrack.geom(index).maxBy(_.x).x.toLong)

          projectLink(addrMs, testTrack.track, id, status, roadPart, discontinuity, ely, geom = testTrack.geom(index), linkId = index.toString, roadwayId = roadwayId,
            startDate = startDate)
      }
    }
