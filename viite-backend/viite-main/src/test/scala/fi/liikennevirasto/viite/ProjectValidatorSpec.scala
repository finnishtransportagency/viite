...
class ProjectValidatorSpec extends FunSuite with Matchers {
  ...
  test("Test checkRoadContinuityCodes " +
    ...
    runWithRollback {
      ...
      val projectLinks   = Seq(
        ProjectLink(1001,roadNumber,roadPartNumber,Track.Combined,Discontinuity.Continuous,        0,0,0,0,None,None,Some(user),5175001.toString,0.0,72.789,SideCode.Unknown,(NoCP,NoCP),(NoCP,NoCP),List(Point(536938.0,6984394.0,0.0), Point(536865.0,6984398.0,0.0)),project_id,LinkStatus.New,AdministrativeClass.Municipality, LinkGeomSource.FrozenLinkInterface,72.789,0,0,8,false,None,1500418814000L,0,Some("testroad"),None,None,None,None,None,None),
        ProjectLink(1002,roadNumber,roadPartNumber,Track.Combined,Discontinuity.MinorDiscontinuity,0,0,0,0,None,None,Some(user),5174998.toString,0.0,84.091,SideCode.Unknown,(NoCP,NoCP),(NoCP,NoCP),List(Point(536781.0,6984396.0,0.0), Point(536865.0,6984398.0,0.0)),project_id,LinkStatus.New,AdministrativeClass.Municipality, LinkGeomSource.FrozenLinkInterface,84.091,0,0,8,false,None,1500418814000L,0,Some("testroad"),None,None,None,None,None,None),
  ...
  test("Test checkRoadContinuityCodes " +
    ...
    runWithRollback {
      ...
      val projectLinks   = Seq(
        ...
        ProjectLink(1005,roadNumber,roadPartNumber,Track.Combined,Discontinuity.EndOfRoad,         0,0,0,0,None,None,Some(user),5174996.toString,0.0,138.959,SideCode.Unknown,(NoCP,NoCP),(NoCP,NoCP),List(Point(536694.0,6984375.0,0.0), Point(536684.0,6984513.0,0.0)),project_id,LinkStatus.New,AdministrativeClass.Municipality, LinkGeomSource.FrozenLinkInterface,138.959,0,0,8,false,None,1551999616000L,0,Some("testroad"),None,None,None,None,None,None)
      )
      projectDAO.create(project)
      projectReservedPartDAO.reserveRoadPart(project_id, roadNumber, roadPartNumber, user)
      projectLinkDAO.create(projectLinks)
  ...
  test("Test validateProject When Checking end of road with first part being continuous and not terminated in project with multiple parts Then should not return TerminationContinuity error (value 18)") {
    ...
    runWithRollback {
      ...
      projectLinkDAO.create(Seq(util.projectLink(0L, 10L, Track.Combined, id, LinkStatus.Unchanged, 19999L, 1L, discontinuity = Discontinuity.Continuous, 8, 12345.toString, raId, linearLocationId).copy(geometry = roadAddresses.head.geometry),
        util.projectLink(0L, 10L, Track.Combined, id, LinkStatus.Termination, 19999L, 2L, discontinuity = Discontinuity.EndOfRoad, 8, 12346.toString, raId + 1, linearLocationId + 1).copy(geometry = roadAddresses.last.geometry)))
  ...
  test("Test validateProject When Checking end of road after first not reserved part being Continuous and not terminated, and second reserved part being EndOfRoad but terminated in project with multiple parts Then should return TerminationContinuity error") {
    runWithRollback {
      ...
      projectDAO.create(project)
      projectReservedPartDAO.reserveRoadPart(id, 19999L, 2L, "u")
      projectLinkDAO.create(Seq(util.projectLink(0L, 10L, Track.Combined, id, LinkStatus.Termination, 19999L, 2L, discontinuity = Discontinuity.EndOfRoad, 8, 12346.toString, raId + 1, linearLocationId + 1).copy(geometry = roadAddresses.last.geometry)))
  ...
  test("Test validateProject When reserving part 2 (which has EndOfRoad) and Terminate it, Reserve and Transfer part 1 to part 2 (with and without EndOfRoad) Then different results should be returned") {
    runWithRollback {
      ...
      projectLinkDAO.create(Seq(util.projectLink(0L, 10L, Track.Combined, project.id, LinkStatus.Termination, roadAddresses.last.roadNumber, roadAddresses.last.roadPartNumber, discontinuity = Discontinuity.EndOfRoad).copy(roadwayId = ra.last.id)))
  ...
  test("Test validateProject When reserve part 2 (which has EndOfRoad) and Terminate it. Create new part 2 (with and without EndOfRoad) Then different results should be expected") {
    runWithRollback {
      ...
      projectLinkDAO.create(Seq(util.projectLink(0L, 10L, Track.Combined, project.id, LinkStatus.Termination).copy(roadNumber = roadAddresses.last.roadNumber, roadPartNumber = roadAddresses.last.roadPartNumber, roadwayId = ra.last.id)))
  ...
  test("Test validateProject When reserve part 2 (which has EndOfRoad) and Terminate it. Create new part 3 (with and without EndOfRoad) Then different results should be expected") {
    runWithRollback {
      ...
      projectLinkDAO.create(Seq(util.projectLink(0L, 10L, Track.Combined, project.id, LinkStatus.Termination).copy(roadNumber = ra.last.roadNumber, roadPartNumber = ra.last.roadPartNumber, roadwayId = ra.last.id)))
  ...
  test("Test validateProject When Terminate all links for all parts in a roadNumber Then should not exist any error") {
    runWithRollback {
      ...
      projectReservedPartDAO.reserveRoadPart(id, 19999L, 1L, "u")
      projectReservedPartDAO.reserveRoadPart(id, 19999L, 2L, "u")
      projectLinkDAO.create(Seq(util.projectLink(0L, 10L, Track.Combined, id, LinkStatus.Termination, 19999L, 1L, discontinuity = Discontinuity.Continuous, 8, 12345.toString, raId, linearLocationId).copy(geometry = roadAddresses.head.geometry),
        util.projectLink(0L, 10L, Track.Combined, id, LinkStatus.Termination, 19999L, 2L, discontinuity = Discontinuity.EndOfRoad, 8, 12346.toString, raId + 1, linearLocationId + 1).copy(geometry = roadAddresses.last.geometry)))
      when(mockRoadAddressService.getRoadAddressWithRoadAndPart(19999L, 1L, false, false, false)).thenReturn(Seq.empty[RoadAddress])
      when(mockRoadAddressService.getRoadAddressWithRoadAndPart(19999L, 2L, false, false, false)).thenReturn(Seq.empty[RoadAddress])
  ...
  test("Test validateProject When there are some Not handled links Then should return HasNotHandledLinks error") {
    runWithRollback {
      ...
      projectLinkDAO.create(Seq(util.projectLink(0L, 10L, Track.Combined, id, LinkStatus.NotHandled, roadNumber = 19999L, 1L, discontinuity = Discontinuity.EndOfRoad, 8, 12345.toString, raId, linearLocationId).copy(geometry = roadAddresses.head.geometry),
        util.projectLink(0L, 10L, Track.Combined, id, LinkStatus.Termination, roadNumber = 19999L, 2L, discontinuity = Discontinuity.EndOfRoad, 8, 12346.toString, raId + 1, linearLocationId + 1).copy(geometry = roadAddresses.last.geometry)))
  ...
  test("Test checkForInvalidUnchangedLinks When it is connected after any other action Then should return invalid unchanged links error") {
    runWithRollback {
      ...
      /*
       |---Numbering--->|---Unchanged--->|
        */
      val updatedProjectLinkToNumbering = Seq(projectLinks.head.copy(startAddrMValue = 10, endAddrMValue = 20, status = LinkStatus.Renumeration))
      ...
      /*
        ...
        */
      val updatedProjectLinkToTerminated = Seq(projectLinks.head.copy(startAddrMValue = 10, endAddrMValue = 20, status = LinkStatus.Termination))
...
Left|      |Right
  ...
  test("Test checkDiscontinuityOnPreviousRoadPart When a road part is terminated from between two road parts causing a discontinuity on that road number Then should be a validation error") {
    runWithRollback {
      ...
      val (project, projectLinks) = util.setUpProjectWithLinks(LinkStatus.Termination, Seq(0L, 10L), roads= Seq((20000L, 2L, "Test road")), discontinuity = Discontinuity.Continuous)
  ...
  test("Test checkDiscontinuityOnPreviousRoadPart When two road parts are terminated from between two road parts causing a discontinuity on that road number Then should be only one validation error") {
    runWithRollback {
      ...
      val (project, projectLinks) = util.setUpProjectWithLinks(LinkStatus.Termination, Seq(0L, 10L), roads= Seq((20000L, 2L, "Test road"), (20000L, 3L, "Test road")), discontinuity = Discontinuity.Continuous)
  ...
  test("Test checkDiscontinuityOnPreviousRoadPart When a road part that is disconnected from the previous road part is terminated and the next road part is connected to the previous road part Then should be a validation error") {
    runWithRollback {
      ...
      val (project, projectLinks) = util.setUpProjectWithLinks(LinkStatus.Termination, Seq(0L, 10L), roads= Seq((20000L, 2L, "Test road")), discontinuity = Discontinuity.Discontinuous)
  ...
  test("Test projectValidator.checkProjectElyCodes When converting all of ely codes to a new one and putting the correct link status then validator should not return an error") {
    runWithRollback {
      val project = setUpProjectWithLinks(LinkStatus.Renumeration, Seq(0L, 10L, 20L, 30L, 40L), discontinuity = Discontinuity.Continuous, lastLinkDiscontinuity = Discontinuity.ChangingELYCode)
      val originalProjectLinks = projectLinkDAO.fetchProjectLinks(project.id)
      addProjectLinksToProject(LinkStatus.Renumeration, Seq(40L, 50L, 60L), discontinuity = Discontinuity.Continuous, lastLinkDiscontinuity = Discontinuity.EndOfRoad, project = project, roadPartNumber = 2L, ely = 50L)
  ...
  test("Test checkOriginalRoadPartAfterTransfer When a road part with EndOfRoad discontinuity is numbered to another road number where the original road number has another road part" +
    ...
    runWithRollback {
      ...
      val (project, projectLinks) = util.setUpProjectWithLinks(LinkStatus.Renumeration, Seq(10L, 20L, 30L), roads = Seq((newRoadNumber, 1L, "Test road")), discontinuity = Discontinuity.Continuous)
