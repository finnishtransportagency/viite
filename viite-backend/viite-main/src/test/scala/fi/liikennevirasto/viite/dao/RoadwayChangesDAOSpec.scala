...
class RoadwayChangesDAOSpec extends FunSuite with Matchers {
  ...
  test("Test RoadwayChangesDAO().insertDeltaToRoadChangeTable() When a road is transferred to another road with reverse then roadway changetable should have one road reversed and the others roadnumber changed .") {
    ...
    runWithRollback {
      ...
      changes.foreach(c => {
        //c.changeInfo.source.addrMRange.get.endAddrM should be > c.changeInfo.source.addrMRange.get.startAddrM // AddrMRange enforces this, unless addrMRange is undefined ("0-0")
        //c.changeInfo.target.addrMRange.get.endAddrM should be > c.changeInfo.target.addrMRange.get.startAddrM // AddrMRange enforces this, unless addrMRange is undefined ("0-0")
  ...
  test("Test RoadwayChangesDAO().insertDeltaToRoadChangeTable() When administrative class changes in the middle of an existing road.") {
    ...
    runWithRollback {
      ...
      val (changesToMunicipality, changesToState) = changes.sortBy(_.changeInfo.orderInChangeTable).partition(_.changeInfo.target.addrMRange.get.startAddrM == 0) // TODO weird partition names? Should be related to addresses?
