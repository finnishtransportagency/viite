...
class RoadwayChangesDAOSpec extends AnyFunSuite with Matchers with BaseDAO {
 ...
 test("Test RoadwayChangesDAO().fetchRoadwayChanges() When searching for changes on a project with roadway changes Then return said changes."){
   runWithRollback{
     //inserts one case
     val addresses = List(ProjectReservedPart(5:Long, RoadPart(203, 203), Some(6L), Some(Discontinuity.apply("jatkuva")), Some(8L), newLength = None, newDiscontinuity = None))
     ...
     runUpdateToDb(
       sql"""
            INSERT INTO roadway_changes(project_id,change_type,new_road_number,new_road_part_number,new_track,new_start_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,new_ely, roadway_change_id, new_road_maintainer, old_road_maintainer)
            values(100,1,6,1,1,0,10.5,1,1,8,1,'EVK1','EVK1')
            """
     )
 ...
 test("Test RoadwayChangesDAO().insertDeltaToRoadChangeTable() When inserting the results of the delta calculation for a project Then when querying directly the roadway_changes it should confirm data insertion.") {
   runWithRollback {
     ...
     val reservedParts = Seq(ProjectReservedPart(0, RoadPart(1, 1), Some(0), Some(Discontinuity.Continuous), Some(8L), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, Some(12345L.toString)))
 ...
 test("Test RoadwayChangesDAO().insertDeltaToRoadChangeTable() When inserting the results of the delta calculation for a project, the inserted ely code should be the roadway ely instead of project ely") {
   val newProjectLink = ProjectLink(1, RoadPart(1, 1), Track.Unknown, Discontinuity.Continuous, AddrMRange(0, 1), AddrMRange(0, 0), None, None, None, 0.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), List(), 1, RoadAddressChangeType.New, AdministrativeClass.Unknown, LinkGeomSource.NormalLinkInterface, 0.0, 0, 0, 5, ArealRoadMaintainer.getEVK(8), reversed = false, None, 748800L)
   runWithRollback {
     addprojects()
     val project1 = projectDAO.fetchById(1).get
     ...
     val ra = Seq(
       RoadAddress(12345, projectLink1.linearLocationId, projectLink1.roadPart, projectLink1.administrativeClass, projectLink1.track, projectLink1.discontinuity, projectLink1.addrMRange, projectLink1.startDate, projectLink1.endDate, projectLink1.createdBy, projectLink1.linkId, projectLink1.startMValue, projectLink1.endMValue, projectLink1.sideCode, DateTime.now().getMillis, projectLink1.calibrationPoints, projectLink1.geometry, projectLink1.linkGeomSource,
         8 /*ELY*/, ArealRoadMaintainer.getEVK(8), NoTermination, projectLink1.roadwayNumber, None, None, None)
     )
 ...
 test("Test RoadwayChangesDAO().insertDeltaToRoadChangeTable() When a road is transferred to another road with reverse then roadway changetable should have one road reversed and the others roadnumber changed .") {
   ...
   runWithRollback {
     ...
     val rws           = Seq(Roadway(rw1, projectLink1.roadwayNumber, targetRoadPart,          AdministrativeClass.Municipality, Track.RightSide, Discontinuity.Continuous,    AddrMRange( 0, 10), reversed = false, DateTime.parse("2020-01-03"), None, "test", None, 5L, ArealRoadMaintainer.getEVK(5), NoTermination),
                             Roadway(rw2, projectLink2.roadwayNumber, targetRoadPart,          AdministrativeClass.Municipality, Track.LeftSide,  Discontinuity.Continuous,    AddrMRange( 0, 10), reversed = false, DateTime.parse("2020-01-03"), None, "test", None, 5L, ArealRoadMaintainer.getEVK(5), NoTermination),
                             Roadway(rw3, projectLink5.roadwayNumber, targetRoadPart,          AdministrativeClass.Municipality, Track.Combined,  Discontinuity.Discontinuous, AddrMRange(10, 20), reversed = false, DateTime.parse("2020-01-03"), None, "test", None, 5L, ArealRoadMaintainer.getEVK(5), NoTermination),
                             Roadway(rw4, projectLink6.roadwayNumber, otherRoadSourceRoadPart, AdministrativeClass.Municipality, Track.LeftSide,  Discontinuity.Discontinuous, AddrMRange(10, 50), reversed = false, DateTime.parse("2020-01-03"), None, "test", None, 5L, ArealRoadMaintainer.getEVK(5), NoTermination),
                             Roadway(rw5, projectLink7.roadwayNumber, otherRoadSourceRoadPart, AdministrativeClass.Municipality, Track.RightSide, Discontinuity.Discontinuous, AddrMRange(10, 50), reversed = false, DateTime.parse("2020-01-03"), None, "test", None, 5L, ArealRoadMaintainer.getEVK(5), NoTermination)
                            )
 ...
 test("Test RoadwayChangesDAO().insertDeltaToRoadChangeTable() When administrative class changes in the middle of an existing road.") {
   ...
   runWithRollback {
     ...
     val project1   = projectDAO.fetchById(projId1).get
     val rw         = Seq(Roadway(0, 1111, projectLink4.roadPart, AdministrativeClass.Municipality, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0, 40), reversed = false, DateTime.parse("2020-01-03"), None, "test", None, 5L, ArealRoadMaintainer.getEVK(5), NoTermination))
     val roadwayDAO = new RoadwayDAO
     roadwayDAO.create(rw)
     projectLinkDAO.create(Seq(projectLink1, projectLink2, projectLink3, projectLink4))
     val reservedParts     = Seq(ProjectReservedPart(0, RoadPart(1, 1), Some(0), Some(Discontinuity.Discontinuous), Some(8L), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, Some(12345L.toString)))
     val roadwayChangesDAO = new RoadwayChangesDAO()
 ...
 test("Test query for Roadway_change changes api") {
   runWithRollback {
     val testRoadway1 = Roadway(NewIdValue, roadwayNumber1, RoadPart(roadNumber1, roadPartNumber1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, 100), reversed = false, DateTime.parse("2000-01-02"), None, "test", Some("TEST ROAD 1"), 1, ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     val dao = new RoadwayChangesDAO()
     val roadwayDAO = new RoadwayDAO
...
//    val rwcs = dao.fetchRoadwayChanges(Set(projId1))  // for debug
 ...
 test("When roadway is partially transferred to another roadaddress " +
   ...
   runWithRollback {
     ...
     val oldroadway1 = Roadway(NewIdValue, roadwayNumber1, RoadPart(roadNumber, initialRoadPart),    AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(100, 200), reversed = false, DateTime.parse("2000-01-02"), Some(DateTime.parse("2010-01-01")), "test", Some("TEST ROAD 1"), 1, ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     val oldroadway2 = Roadway(NewIdValue, roadwayNumber1, RoadPart(roadNumber, initialRoadPart),    AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(  0, 100), reversed = false, DateTime.parse("2000-01-02"), Some(DateTime.parse("2010-01-01")), "test", Some("TEST ROAD 1"), 1, ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)

     val roadway1 = Roadway(NewIdValue, roadwayNumber1,   RoadPart(roadNumber, initialRoadPart),     AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(  0, 100), reversed = false, DateTime.parse("2010-01-02"), None, "test", Some("TEST ROAD 1"), 1, ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     val roadway2 = Roadway(NewIdValue, roadwayNumber1+1, RoadPart(roadNumber, transferredRoadPart), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(  0, 100), reversed = false, DateTime.parse("2010-01-02"), None, "test", Some("TEST ROAD 2"), 1, ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
 ...
 test("[ChangeInfoForRoadAddressChangesBrowser] When fetching road address change infos for road address browser then return change infos based on the parameters given") {
   runWithRollback {
     ...
     val result1 = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(startDate), None, Some(dateTargetProjectAccepted), None, None, Some(roadNumber), None, None)
     result1 should have size(4)
     result1.head shouldBe a [ChangeInfoForRoadAddressChangesBrowser]

     val result2 = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(startDate), None, Some(dateTargetRoadAddress), None, None, Some(roadNumber), None, None)
     ...
     val result3 = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(startDate2), None, Some(dateTargetRoadAddress), None, None, Some(roadNumber), None, None)
 ...
 test("[ChangeInfoForRoadAddressChangesBrowser] When the road has been terminated (-> type of change: termination) then return the change info of the terminated layer") {
   runWithRollback {
     ...
     val result = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(startDateInput), None, Some(dateTargetRoadAddressStartDate), None, None, Some(roadNumber), None, None)
 ...
 test("[ChangeInfoForRoadAddressChangesBrowser] When a road has no road name then return the change info without a road name") {
   runWithRollback {
     ...
     val result = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(startDate), None, Some(dateTargetProjectAccepted), None, None, Some(roadNumber), None, None)
 ...
 test("[ChangeInfoForRoadAddressChangesBrowser] When whole road part is transferred to be" +
   ...
   runWithRollback {
     ...
     val dateTargetProjectAccepted = "ProjectAcceptedDate"
     val result = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(startDate), None, Some(dateTargetProjectAccepted), None, None, Some(newRoadNumber), None, None)
 ...
 test("[ChangeInfoForRoadAddressChangesBrowser] When fetching roadway changes with roadPartNumber, then return ALL changes related to projects that have changes on the roadPartNumber") {
   runWithRollback {
     ...
     val dateTargetProjectAccepted = "ProjectAcceptedDate"
     val result = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(queryStartDate1), None, Some(dateTargetProjectAccepted), None, None, Some(roadNumber), Some(firstRoadPartNumber), Some(firstRoadPartNumber))
     result.size should be >= 4 // Expecting 4 change infos

     val result2 = dao.fetchChangeInfosForRoadAddressChangesBrowser(Some(queryStartDate2), None, Some(dateTargetProjectAccepted), None, None, Some(roadNumber), Some(firstRoadPartNumber), Some(firstRoadPartNumber))
