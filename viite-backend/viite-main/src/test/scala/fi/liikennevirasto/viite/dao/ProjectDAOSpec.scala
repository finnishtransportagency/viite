...
class ProjectDAOSpec extends AnyFunSuite with Matchers with BaseDAO{
 ...
 def dummyProjectLink(id: Long, projectId: Long, linkId: String, roadwayId: Long = 0, roadwayNumber: Long = roadwayNumber1,
   ...
   ProjectLink(id, roadPart, Track.Combined, Discontinuity.Continuous, addrMRange, addrMRange,
     ...
     roadwayId, linearLocationId, ArealRoadMaintainer.getEVK(0), reversed, connectedLinkId = None, 631152000, roadwayNumber, roadAddressLength = Some(addrMRange.length))
 ...
 test("Test update When Update project info Then should update the project infos such as project name, additional info, startDate") {
   val reservedPart = ProjectReservedPart(id = 5L, roadPart = RoadPart(203, 203), addressLength = None, discontinuity = Some(Discontinuity.apply("jatkuva")), roadMaintainer = Some(ArealRoadMaintainer.apply("EVK6")), newLength = None, newDiscontinuity = None, newRoadMaintainer = Some(ArealRoadMaintainer.apply("EVK8")), startingLinkId = None)
   runWithRollback {
     val id = Sequences.nextViiteProjectId
     val rap = dummyProject(id, ProjectState.Incomplete, List(), None)
 ...
 test("Test updateProjectEly When updating Ely for project Then ely should change") {
   runWithRollback {
     ...
     projectDAO.create(rap)
     projectDAO.fetchById(id).nonEmpty should be(true)
     projectDAO.fetchProjectEvkById(id).isEmpty should be (true)
   }
 }
 ...
 test("Test fetchProjectIdsWithToBePreservedStatus " +
   ...
   val reservedPart = ProjectReservedPart(id = 5L, roadPart = RoadPart(203, 203), addressLength = Some(6L), discontinuity = Some(Discontinuity.apply("jatkuva")), roadMaintainer = Some(ArealRoadMaintainer.apply("EVK8")), newLength = None, newDiscontinuity = None)
   runWithRollback {
 ...
 test("Test updateProjectElys " +
   ...
   runWithRollback {
     ...
     projectDAO.updateProjectRoadMaintainers(id, Seq("EVK2", "EVK1"))

     // Directly query the database to verify the elys array
     val query =
       sql"""
       SELECT road_maintainers
       FROM project
       WHERE id = $id
     """
     val roadMaintainersArray = runSelectFirst(query.map(rs => rs.arrayOpt("road_maintainers"))).flatten

     // Extract the values from the array
     val roadMaintainersValues = roadMaintainersArray.map(_.getArray.asInstanceOf[Array[String]])

     // Verify the contents in correct order
     roadMaintainersValues.get should contain theSameElementsInOrderAs Array("EVK1", "EVK2")
   }
 }
}
