...
//      roadwayDAO.create(
//        Seq(
//          Roadway(Sequences.nextRoadwayId, Sequences.nextRoadwayNumber, RoadPart(roadNumber, roadPartNumber), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0L, 1000L), reversed=false, DateTime.parse(date), None, "test", Some("testRoad"), 1L, 1L, validFrom = DateTime.parse(date), validTo = None)
//        )
//      )
//
//      // create road name
...
//      val resForFetch1 = RoadNameDAO.fetchRoadNamesForRoadAddressBrowser(Some(date), None, Some(roadNumber), None, None, None )
...
//      val resForFetch2 = RoadNameDAO.fetchRoadNamesForRoadAddressBrowser(Some(olderDate), None, Some(roadNumber), None, None, None)
...
//      val historyRoadway = Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(roadNumber, roadPartNumber), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0L, 1000L), reversed=false, DateTime.parse(rwHistoryStartDate),Some(DateTime.parse(rwHistoryEndDate)), "test", Some("oldName"),ely1, evk1, validFrom = DateTime.parse(validFrom), validTo = None)
//      val currentRoadway = Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(roadNumber, roadPartNumber), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0L, 1000L), reversed=false, DateTime.parse(rwCurrentStartDate), None, "test", Some("newName"),ely9, evk9, validFrom = DateTime.parse(validFrom), validTo = None)
//
//      // create roadways
//      roadwayDAO.create(
//        Seq(
...
//      val res = RoadNameDAO.fetchRoadNamesForRoadAddressBrowser(Some(situationDate), None, Some(roadNumber), None, None, None)
...
//      val res2 = RoadNameDAO.fetchRoadNamesForRoadAddressBrowser(Some(situationDateDayBeforeChanges), None, Some(roadNumber), None, None, None)
