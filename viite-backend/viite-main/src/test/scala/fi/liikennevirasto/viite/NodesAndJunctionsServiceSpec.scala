...
class NodesAndJunctionsServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Test getAllValidNodesWithJunctions When valid nodes and junctions exist Then return valid nodes with only junctions") {
    runWithRollback {
      // test data
      val nodeNumber = 1
      val nodes: Seq[Node] = Seq(Node(NewIdValue, nodeNumber, Point(100, 100), Some("Test node"), NodeType.NormalIntersection,
        DateTime.parse("2019-01-01"), None, DateTime.parse("2019-01-01"), None, "Test", None, registrationDate = new DateTime()))
      nodeDAO.create(nodes)
      ...
      //mock data
      val junctions: Seq[JunctionWithLinearLocation] = Seq(JunctionWithLinearLocation(1, None, Some(nodeNumber), DateTime.now(), None, DateTime.now(), None, "TestUser", None, Seq(llId1, llId2, llId3)), JunctionWithLinearLocation(2, None, Some(NewIdValue), DateTime.now(), None, DateTime.now(), None, "TestUser", None, Seq()))
      when(mockLinearLocationDAO.fetchJunctionsByNodeNumbersWithLinearLocation(any[Seq[Long]])).thenReturn(junctions)
      ...
      val nodesWithJunctions: Seq[NodeWithJunctions] = nodesAndJunctionsService.getAllValidNodesWithJunctions
