...
class NodesAndJunctionsServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates roadsToHead case When creating projectlinks Then junction template and junctions points should be handled/created properly") {
    runWithRollback {
     ...
     R: Existing road
      ...
      val roadJunctionPoints = junctionPointDAO.fetchByRoadwayPoint(roadway.roadwayNumber, roadway.addrMRange.endAddrM, BeforeAfter.Before)
      val junction1 = junctionDAO.fetchByIds(Seq(roadJunctionPoints.head.junctionId))

      val link1JunctionPoints = junctionPointDAO.fetchByRoadwayPoint(link1.roadwayNumber, link1.addrMRange.startAddrM, BeforeAfter.After)
      val link2JunctionPoints = junctionPointDAO.fetchByRoadwayPoint(link2.roadwayNumber, link2.addrMRange.startAddrM, BeforeAfter.After)
      val junction2 = junctionDAO.fetchByIds(Seq(link1JunctionPoints.head.junctionId))

      junction1 should be(junction2)

      val roadwayPoint1 = roadwayPointDAO.fetch(roadway.roadwayNumber, roadway.addrMRange.endAddrM)
      val roadwayPoint2 = roadwayPointDAO.fetch(link1.roadwayNumber, link1.addrMRange.startAddrM)

      roadwayPoint1.head.addrMValue should be(roadway.addrMRange.endAddrM)
      roadwayPoint2.head.addrMValue should be(link1.addrMRange.startAddrM)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates roadsToHead case When creating projectlinks Then junction template and junctions points should be handled/created properly and" +
    ...
    runWithRollback {
     ...
     |--R-->0|0--L-->
      ...
      val roadJunctionPoints = junctionPointDAO.fetchByRoadwayPoint(roadway1.roadwayNumber, roadway1.addrMRange.endAddrM, BeforeAfter.Before)
      val junction1 = junctionDAO.fetchByIds(Seq(roadJunctionPoints.head.junctionId))

      val linkJunctionPoints = junctionPointDAO.fetchByRoadwayPoint(link1.roadwayNumber, link1.addrMRange.startAddrM, BeforeAfter.After)
      val junction2 = junctionDAO.fetchByIds(Seq(linkJunctionPoints.head.junctionId))

      junction1 should be(junction2)

      val roadwayPoint1 = roadwayPointDAO.fetch(roadway1.roadwayNumber, roadway1.addrMRange.endAddrM)
      val roadwayPoint2 = roadwayPointDAO.fetch(link1.roadwayNumber, link1.addrMRange.startAddrM)

      roadwayPoint1.head.addrMValue should be(roadway1.addrMRange.endAddrM)
      roadwayPoint2.head.addrMValue should be(link1.addrMRange.startAddrM)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates roadsFromHead case When creating projectLinks Then junction template and junctions points should be handled/created properly and " +
    ...
    runWithRollback {
     ...
     <--R--0|0--L-->
      ...
      val roadJunctionPoints = junctionPointDAO.fetchByRoadwayPoint(roadway1.roadwayNumber, roadway1.addrMRange.startAddrM, BeforeAfter.After)
      val junction1 = junctionDAO.fetchByIds(Seq(roadJunctionPoints.head.junctionId))
      ...
      val roadwayPoint1 = roadwayPointDAO.fetch(roadway1.roadwayNumber, roadway1.addrMRange.startAddrM)
      val roadwayPoint2 = roadwayPointDAO.fetch(link1.roadwayNumber, link1.addrMRange.startAddrM)

      roadwayPoint1.head.addrMValue should be(roadway1.addrMRange.startAddrM)
      roadwayPoint2.head.addrMValue should be(link1.addrMRange.startAddrM)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates roadsToTail case When creating projectLinks Then junction template and junctions points should be handled/created properly and " +
    ...
    runWithRollback {
      ...
      val linkJunctionPoints = junctionPointDAO.fetchByRoadwayPoint(link1.roadwayNumber, link1.addrMRange.endAddrM, BeforeAfter.Before)
      val junction2 = junctionDAO.fetchByIds(Seq(linkJunctionPoints.head.junctionId))

      junction1 should be(junction2)

      val roadwayPoint1 = roadwayPointDAO.fetch(roadway1.roadwayNumber, roadway1.addrMRange.endAddrM)
      val roadwayPoint2 = roadwayPointDAO.fetch(link1.roadwayNumber, link1.addrMRange.endAddrM)

      roadwayPoint1.head.addrMValue should be(roadway1.addrMRange.endAddrM)
      roadwayPoint2.head.addrMValue should be(link1.addrMRange.endAddrM)
...
//    val llId6 = Sequences.nextLinearLocationId
      ...
      val projectChanges = List(
        ...
        ProjectRoadwayChange(projectId1, Some("project name"), 8L, "test user", DateTime.now,
          RoadwayChangeInfo(RoadAddressChangeType.New,
            RoadwayChangeSection(None, None, None, None, None, Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(8L)),
            RoadwayChangeSection(Some(road), Some(Track.LeftSide.value.toLong), startRoadPartNumber = Some(part), endRoadPartNumber = Some(part), addrMRange = Some(AddrMRange(0L, 5L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(8L)),
            Discontinuity.Continuous, AdministrativeClass.State, reversed = false, 1, 8)
          , DateTime.now),
        //  right
        ProjectRoadwayChange(projectId1, Some("project name"), 8L, "test user", DateTime.now,
          RoadwayChangeInfo(RoadAddressChangeType.New,
            RoadwayChangeSection(None, None, None, None, None, Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(8L)),
            RoadwayChangeSection(Some(road), Some(Track.RightSide.value.toLong), startRoadPartNumber = Some(part), endRoadPartNumber = Some(part), addrMRange = Some(AddrMRange(0L, 5L)), Some(AdministrativeClass.State), Some(Discontinuity.Discontinuous), Some(8L)),
      ...
      val terminatingProjectChanges = List(
        ...
        ProjectRoadwayChange(projectId1, Some("project name"), 8L, "test user", DateTime.now,
          RoadwayChangeInfo(RoadAddressChangeType.Transfer,
            RoadwayChangeSection(Some(road), Some(Track.Combined.value.toLong), startRoadPartNumber = Some(part), endRoadPartNumber = Some(part), addrMRange = Some(AddrMRange(5L, 15L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(8L)),
            RoadwayChangeSection(Some(road), Some(Track.Combined.value.toLong), startRoadPartNumber = Some(part), endRoadPartNumber = Some(part), addrMRange = Some(AddrMRange(5L, 15L)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Some(8L)),
            Discontinuity.EndOfRoad, AdministrativeClass.State, reversed = false, 3, 8)
          , DateTime.now),
        ProjectRoadwayChange(projectId1, Some("project name"), 8L, "test user", DateTime.now,
      ...
      val rwTransfer   = rw3WithId.copy(id =   transferCombLink1.roadwayId, roadwayNumber =   transferCombLink1.roadwayNumber, addrMRange = AddrMRange(rw3WithId.addrMRange.startAddrM, 15))
      val rwTerminated = rw3WithId.copy(id = terminatedCombLink2.roadwayId, roadwayNumber = terminatedCombLink2.roadwayNumber, addrMRange = AddrMRange(rw3WithId.addrMRange.startAddrM, 20))
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating one road part that intersects itself and still all links are continuous Then junction template and junctions points should be handled/created properly." +
    ...
    runWithRollback {
      ...
      buildTestDataForProject(Some(project2),
        Some(Seq(
          rw1WithId.copy(id =  unchangedLink1.roadwayId, addrMRange = AddrMRange(rw1WithId.addrMRange.startAddrM, 15)),
          rw1WithId.copy(id = terminatedLink4.roadwayId, addrMRange = AddrMRange(rw1WithId.addrMRange.startAddrM, 10), endDate = Some(DateTime.now())))),
    ...
    runWithRollback {
      ...
      val projectChanges = List(
        ...
        ProjectRoadwayChange(0, Some("project name"), 8L, "test user", DateTime.now,
          RoadwayChangeInfo(RoadAddressChangeType.New,
            RoadwayChangeSection(None, None, None, None, None, Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(8L)),
            RoadwayChangeSection(Some(999), Some(Track.Combined.value.toLong), startRoadPartNumber = Some(999L), endRoadPartNumber = Some(999L), addrMRange = Some(AddrMRange(50L,150L)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Some(8L)),
            Discontinuity.Continuous, AdministrativeClass.State, reversed = false, 3, 8)
          , DateTime.now)
      )
      ...
      */
      ...
      val reversedRoadways = Seq(
        Roadway(roadwayId + 4, left.roadwayNumber,      RoadPart(999, 999), AdministrativeClass.State, Track.LeftSide,  Discontinuity.EndOfRoad, AddrMRange(100, 150), reversed = true, DateTime.parse("2000-01-01"), None, "test", Some("TEST ROAD 1"), 1, TerminationCode.NoTermination),
        Roadway(roadwayId + 5, right.roadwayNumber,     RoadPart(999, 999), AdministrativeClass.State, Track.RightSide, Discontinuity.EndOfRoad, AddrMRange(100, 150), reversed = true, DateTime.parse("2000-01-01"), None, "test", Some("TEST ROAD 1"), 1, TerminationCode.NoTermination),
        Roadway(roadwayId + 6, combined1.roadwayNumber, RoadPart(999, 999), AdministrativeClass.State, Track.Combined,  Discontinuity.Continuous,AddrMRange( 50, 100), reversed = true, DateTime.parse("2000-01-01"), None, "test", Some("TEST ROAD 1"), 1, TerminationCode.NoTermination),
        Roadway(roadwayId + 7, combined2.roadwayNumber, RoadPart(999, 999), AdministrativeClass.State, Track.Combined,  Discontinuity.Continuous,AddrMRange(  0,  50), reversed = true, DateTime.parse("2000-01-01"), None, "test", Some("TEST ROAD 1"), 1, TerminationCode.NoTermination)
      )
  ...
  test("Test expireObsoleteNodesAndJunctions case When road part is terminated Then also node points for terminated road should be expired") {
    runWithRollback {
      ...
      val linearLocations = Seq(
        LinearLocation(NewIdValue, 1, roadLink1.linkId, roadLink1.addrMRange.startAddrM, roadLink1.addrMRange.endAddrM, roadLink1.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(roadLink1.addrMRange.startAddrM)), CalibrationPointReference(Some(roadLink1.addrMRange.endAddrM))), roadGeom1, LinkGeomSource.NormalLinkInterface, roadLink1.roadwayNumber, Some(DateTime.now), None),
        LinearLocation(NewIdValue, 1, roadLink2.linkId, roadLink2.addrMRange.startAddrM, roadLink2.addrMRange.endAddrM, roadLink2.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(roadLink2.addrMRange.startAddrM)), CalibrationPointReference(Some(roadLink2.addrMRange.endAddrM))), roadGeom2, LinkGeomSource.NormalLinkInterface, roadLink2.roadwayNumber, Some(DateTime.now), None))
      ...
      nodePointTemplates.foreach(
        np => {
          if (np.roadwayNumber == roadLink1.roadwayNumber) {
            if (np.addrM == roadLink1.addrMRange.startAddrM)
              runUpdateToDb(s"""UPDATE NODE_POINT SET NODE_NUMBER = $nodeNumber1 WHERE ID = ${np.id}""")
            else if (np.addrM == roadLink1.addrMRange.endAddrM)
            ...
            if (np.addrM == roadLink2.addrMRange.startAddrM)
              runUpdateToDb(s"""UPDATE NODE_POINT SET NODE_NUMBER = $nodeNumber2 WHERE ID = ${np.id}""")
            else if (np.addrM == roadLink2.addrMRange.endAddrM)
      ...
      nodePoints.exists(node => node.roadwayNumber == roadLink1.roadwayNumber && node.beforeAfter == BeforeAfter.After && node.addrM == roadLink1.addrMRange.startAddrM) should be(true)
      nodePoints.exists(node => node.roadwayNumber == roadLink1.roadwayNumber && node.beforeAfter == BeforeAfter.Before && node.addrM == roadLink1.addrMRange.endAddrM) should be(true)
      nodePoints.exists(node => node.roadwayNumber == roadLink2.roadwayNumber && node.beforeAfter == BeforeAfter.After && node.addrM == roadLink2.addrMRange.startAddrM) should be(true)
      nodePoints.exists(node => node.roadwayNumber == roadLink2.roadwayNumber && node.beforeAfter == BeforeAfter.Before && node.addrM == roadLink2.addrMRange.endAddrM) should be(true)
      ...
      nodePointsAfterExpiration.exists(node => node.roadwayNumber == roadLink1.roadwayNumber && node.beforeAfter == BeforeAfter.After && node.addrM == roadLink1.addrMRange.startAddrM) should be(true)
      nodePointsAfterExpiration.exists(node => node.roadwayNumber == roadLink1.roadwayNumber && node.beforeAfter == BeforeAfter.Before && node.addrM == roadLink1.addrMRange.endAddrM) should be(true)
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is extended after the existing road") {
    runWithRollback {
      ...
      val linearLocations = Seq(
        LinearLocation(NewIdValue, 1, roadLink.linkId, roadLink.addrMRange.startAddrM, roadLink.addrMRange.endAddrM, roadLink.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(roadLink.addrMRange.startAddrM)), CalibrationPointReference(Some(roadLink.addrMRange.endAddrM))), roadGeom1, LinkGeomSource.NormalLinkInterface, roadLink.roadwayNumber, Some(DateTime.now), None))
      ...
      nodePointTemplates.foreach(
        np => {
          if (np.addrM == roadLink.addrMRange.startAddrM)
            runUpdateToDb(s"""UPDATE NODE_POINT SET NODE_NUMBER = ${node1.nodeNumber} WHERE ID = ${np.id}""")
          else if (np.addrM == roadLink.addrMRange.endAddrM)
      ...
      nodePoints.exists(node => node.roadwayNumber == roadLink.roadwayNumber && node.beforeAfter == BeforeAfter.After && node.addrM == roadLink.addrMRange.startAddrM) should be(true)
      nodePoints.exists(node => node.roadwayNumber == roadLink.roadwayNumber && node.beforeAfter == BeforeAfter.Before && node.addrM == roadLink.addrMRange.endAddrM) should be(true)
      ...
      val newLinearLocations = Seq(
        LinearLocation(NewIdValue, 1, roadLink.linkId, roadLink.addrMRange.startAddrM, roadLink.addrMRange.endAddrM, roadLink.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(roadLink.addrMRange.startAddrM)), CalibrationPointReference(Some(roadLink.addrMRange.endAddrM))), roadGeom1, LinkGeomSource.NormalLinkInterface, roadLink.roadwayNumber, Some(DateTime.now), None),
        LinearLocation(NewIdValue, 2, newRoadLink.linkId, newRoadLink.addrMRange.startAddrM, newRoadLink.addrMRange.endAddrM, newRoadLink.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(newRoadLink.addrMRange.startAddrM)), CalibrationPointReference(Some(newRoadLink.addrMRange.endAddrM))), roadGeom2, LinkGeomSource.NormalLinkInterface, newRoadLink.roadwayNumber, Some(DateTime.now), None))
      ...
      nodePointsAfterExpiration.exists(node => node.roadwayNumber == roadLink.roadwayNumber && node.beforeAfter == BeforeAfter.After && node.addrM == roadLink.addrMRange.startAddrM) should be(true)
      nodePointsAfterExpiration.exists(node => node.roadwayNumber == roadLink.roadwayNumber && node.beforeAfter == BeforeAfter.Before && node.addrM == roadLink.addrMRange.endAddrM) should be(false)
    }
  }
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is extended before the existing road") {
    runWithRollback {
      ...
      val nodePointsAfterExpiration = nodePointDAO.fetchByNodeNumbers(Seq(node1.nodeNumber, node2.nodeNumber))
      nodePointsAfterExpiration.length should be(1)
      nodePointsAfterExpiration.exists(node => node.roadwayNumber == roadLink.roadwayNumber && node.beforeAfter == BeforeAfter.After && node.addrM == roadLink.addrMRange.startAddrM) should be(false)
      nodePointsAfterExpiration.exists(node => node.roadwayNumber == roadLink.roadwayNumber && node.beforeAfter == BeforeAfter.Before && node.addrM == roadLink.addrMRange.endAddrM) should be(true)
    }
  }
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is terminated Then also junction points should be expired") {
    runWithRollback {
      ...
      val linearLocations = Seq(
        LinearLocation(NewIdValue, 1, road1Link.linkId, road1Link.addrMRange.startAddrM, road1Link.addrMRange.endAddrM, road1Link.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(road1Link.addrMRange.startAddrM)), CalibrationPointReference(Some(road1Link.addrMRange.endAddrM))), roadGeom1, LinkGeomSource.NormalLinkInterface, road1Link.roadwayNumber, Some(DateTime.now), None),
        LinearLocation(NewIdValue, 1, road2Link.linkId, road2Link.addrMRange.startAddrM, road2Link.addrMRange.endAddrM, road2Link.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(road2Link.addrMRange.startAddrM)), CalibrationPointReference(Some(road2Link.addrMRange.endAddrM))), roadGeom2, LinkGeomSource.NormalLinkInterface, road2Link.roadwayNumber, Some(DateTime.now), None))
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is terminated Then also junction and junction points should be expired if they are in the same road") {
    runWithRollback {
      ...
      val roadways = Seq(
        Roadway(NewIdValue, road1Link1.roadwayNumber, road1Link1.roadPart, road1Link1.administrativeClass, road1Link1.track, road1Link1.discontinuity, road1Link1.addrMRange, reversed = false, road1Link1.startDate.get, road1Link1.endDate, "user", None, 8L, TerminationCode.NoTermination, DateTime.now, None),
        Roadway(NewIdValue,  road2Link.roadwayNumber,  road2Link.roadPart, road2Link.administrativeClass,  road2Link.track,  road2Link.discontinuity,   road2Link.addrMRange, reversed = false,  road2Link.startDate.get,  road2Link.endDate, "user", None, 8L, TerminationCode.NoTermination, DateTime.now, None)
      )
      val linearLocations = Seq(
        LinearLocation(NewIdValue, 1, road1Link1.linkId, road1Link1.addrMRange.startAddrM, road1Link1.addrMRange.endAddrM, road1Link1.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(road1Link1.addrMRange.startAddrM)), CalibrationPointReference(Some(road1Link1.addrMRange.endAddrM))), roadGeom1Link1, LinkGeomSource.NormalLinkInterface, road1Link1.roadwayNumber, Some(DateTime.now), None),
        LinearLocation(NewIdValue, 2, road1Link2.linkId, road1Link2.addrMRange.startAddrM, road1Link2.addrMRange.endAddrM, road1Link2.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some(road1Link2.addrMRange.startAddrM)), CalibrationPointReference(Some(road1Link2.addrMRange.endAddrM))), roadGeom1Link2, LinkGeomSource.NormalLinkInterface, road1Link2.roadwayNumber, Some(DateTime.now), None),
        LinearLocation(NewIdValue, 1,  road2Link.linkId,  road2Link.addrMRange.startAddrM,  road2Link.addrMRange.endAddrM,  road2Link.sideCode, 0L, calibrationPoints = (CalibrationPointReference(Some( road2Link.addrMRange.startAddrM)), CalibrationPointReference(Some( road2Link.addrMRange.endAddrM))),      roadGeom2, LinkGeomSource.NormalLinkInterface,  road2Link.roadwayNumber, Some(DateTime.now), None)
      )
  ...
  def dummyHorizontalRoad(projectId: Long, numberOfLinearLocations: Int, roadPart: RoadPart, rwNumber: Long, track: Track = Track.Combined, startAddrAt: Long = 0, administrativeClass: AdministrativeClass = AdministrativeClass.State, discontinuity: Discontinuity = Discontinuity.Continuous, firstPointAt: Point = Point(0.0, 0.0), orderNumber: Double = 1.0, linkId: Long = 0, plId: Long = 0, rw: (Long, Option[Roadway]) = (0, None), llId: Long = 0, size: Int = 10): (Seq[ProjectLink], Seq[LinearLocation], Roadway) = {
    ...
    val projectLinks = for (i: Int <- 0 until numberOfLinearLocations) yield {
      ...
      val projectLink = dummyProjectLink(roadPart, track, if (i == numberOfLinearLocations-1) discontinuity else Discontinuity.Continuous, addrRange, addrRange, Some(DateTime.now()), None, (linkId + i).toString, 0, size, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, administrativeClass, geom, rwNumber).copy(id = plId + i, roadwayId = rwId, linearLocationId = llId + i)
      projectLink
    }
    ...
    val rwOrHead = roadway.getOrElse(roadways.head)
    (projectLinks, orderedLinearLocations, rwOrHead.copy(addrMRange = AddrMRange(rwOrHead.addrMRange.startAddrM, roadways.last.addrMRange.endAddrM)))
  }
  ...
  def dummyVerticalRoad(projectId: Long, numberOfLinearLocations: Int, roadPart: RoadPart, rwNumber: Long, track: Track = Track.Combined, startAddrAt: Long = 0, administrativeClass: AdministrativeClass = AdministrativeClass.State, discontinuity: Discontinuity = Discontinuity.Continuous, firstPointAt: Point = Point(0.0, 0.0), orderNumber: Double = 1.0, linkId: Long = 0, plId: Long = 0, rw: (Long, Option[Roadway]) = (0, None), llId: Long = 0, size: Int = 10): (Seq[ProjectLink], Seq[LinearLocation], Roadway) = {
    ...
    val projectLinks = for (i: Int <- 0 until numberOfLinearLocations) yield {
      ...
      dummyProjectLink(roadPart, track, discontinuityCode, addrRange, addrRange, Some(DateTime.now()), None, (linkId + i).toString, 0, size, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, administrativeClass, geom, rwNumber).copy(id = plId + i, roadwayId = rwId, linearLocationId = llId + i)
    }
    val (linearLocations, roadways) = projectLinks.map(toRoadwayAndLinearLocation).unzip
    val orderedLinearLocations: Seq[LinearLocation] = linearLocations.zipWithIndex.map { case (ll, i) => ll.copy(orderNumber = orderNumber + i) }

    val rwOrHead = roadway.getOrElse(roadways.head)
    (projectLinks, orderedLinearLocations, rwOrHead.copy(discontinuity = discontinuity, addrMRange = AddrMRange(rwOrHead.addrMRange.startAddrM, roadways.last.addrMRange.endAddrM)))
  }
...
//    val roadwayPointId2 = roadwayPointDAO.create(testRoadwayPoint2.copy(roadwayNumber = roadwayNumber2, addrMValue = 20))
  ...
  test("Test calculateNodePointsForNode When there is a road with 2 roadparts, 2 junctions and the roadpart changes on a junction. The section between the junctions is transfered and NodePoints should update accordingly without dublicate NodePoints or templates") {
    runWithRollback {
      ...
      val roadAddress1 = RoadAddress(newRoadway1WithId.id, newOrderedll1.id, newRoadway1WithId.roadPart, newRoadway1WithId.administrativeClass, newRoadway1WithId.track, newRoadway1WithId.discontinuity, newRoadway1WithId.addrMRange, Some(newRoadway1WithId.startDate), Some(newRoadway1WithId.startDate), Some(newRoadway1WithId.createdBy), newPl1.linkId, newPl1.startMValue, newPl1.endMValue, newPl1.sideCode, newll1.adjustedTimestamp, (None, None), newPl1.geometry, LinkGeomSource.NormalLinkInterface, newRoadway1WithId.ely, newRoadway1WithId.terminated, newRoadway1WithId.roadwayNumber, Some(newRoadway1WithId.validFrom), newRoadway1WithId.validTo, newRoadway1WithId.roadName)
      val roadAddress2 = RoadAddress(newRoadway2WithId.id, newOrderedll2.id, newRoadway2WithId.roadPart, newRoadway2WithId.administrativeClass, newRoadway2WithId.track, newRoadway2WithId.discontinuity, newRoadway2WithId.addrMRange, Some(newRoadway2WithId.startDate), Some(newRoadway2WithId.startDate), Some(newRoadway2WithId.createdBy), newPl2.linkId, newPl2.startMValue, newPl2.endMValue, newPl2.sideCode, newll2.adjustedTimestamp, (None, None), newPl2.geometry, LinkGeomSource.NormalLinkInterface, newRoadway2WithId.ely, newRoadway2WithId.terminated, newRoadway2WithId.roadwayNumber, Some(newRoadway2WithId.validFrom), newRoadway2WithId.validTo, newRoadway2WithId.roadName)
      val roadAddress3 = RoadAddress(newRoadway3WithId.id, newOrderedll3.id, newRoadway3WithId.roadPart, newRoadway3WithId.administrativeClass, newRoadway3WithId.track, newRoadway3WithId.discontinuity, newRoadway3WithId.addrMRange, Some(newRoadway3WithId.startDate), Some(newRoadway3WithId.startDate), Some(newRoadway3WithId.createdBy), newPl3.linkId, newPl3.startMValue, newPl3.endMValue, newPl3.sideCode, newll3.adjustedTimestamp, (None, None), newPl3.geometry, LinkGeomSource.NormalLinkInterface, newRoadway3WithId.ely, newRoadway3WithId.terminated, newRoadway3WithId.roadwayNumber, Some(newRoadway3WithId.validFrom), newRoadway3WithId.validTo, newRoadway3WithId.roadName)
      val rwaddressCross1 = RoadAddress(rw4WithId.id, orderedll4.id, rw4WithId.roadPart, rw4WithId.administrativeClass, rw4WithId.track, rw4WithId.discontinuity, rw4WithId.addrMRange, Some(rw4WithId.startDate), Some(rw4WithId.startDate), Some(rw4WithId.createdBy), pl4.linkId, pl4.startMValue, pl4.endMValue, pl4.sideCode, orderedll4.adjustedTimestamp, (None, None), pl4.geometry, LinkGeomSource.NormalLinkInterface, rw4WithId.ely, rw4WithId.terminated, rw4WithId.roadwayNumber, Some(rw4WithId.validFrom), rw4WithId.validTo, rw4WithId.roadName)
      val rwaddressCross2 = RoadAddress(rw5WithId.id, orderedll5.id, rw5WithId.roadPart, rw5WithId.administrativeClass, rw5WithId.track, rw5WithId.discontinuity, rw5WithId.addrMRange, Some(rw5WithId.startDate), Some(rw5WithId.startDate), Some(rw5WithId.createdBy), pl5.linkId, pl5.startMValue, pl5.endMValue, pl5.sideCode, orderedll5.adjustedTimestamp, (None, None), pl5.geometry, LinkGeomSource.NormalLinkInterface, rw5WithId.ely, rw5WithId.terminated, rw5WithId.roadwayNumber, Some(rw5WithId.validFrom), rw5WithId.validTo, rw5WithId.roadName)
