...
class NodesAndJunctionsServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  def dummyProjectLink(roadPart: RoadPart, trackCode: Track, discontinuityType: Discontinuity, addrMRange: AddrMRange, originalStartAddr: Long = 0, originalEndAddr: Long = 0, startDate: Option[DateTime], endDate: Option[DateTime] = None, linkId: String = "0", startMValue: Double = 0, endMValue: Double = 0, sideCode: SideCode = SideCode.Unknown, status: RoadAddressChangeType, projectId: Long = 0, administrativeClass: AdministrativeClass = AdministrativeClass.State, geometry: Seq[Point] = Seq(), roadwayNumber: Long): ProjectLink = {
    ProjectLink(0L, roadPart, trackCode, discontinuityType, addrMRange, AddrMRange(originalStartAddr, originalEndAddr), startDate, endDate, Some("user"), linkId, startMValue, endMValue, sideCode, (NoCP, NoCP), (NoCP, NoCP), geometry, projectId, status, administrativeClass, LinkGeomSource.NormalLinkInterface, geometryLength = 0.0, roadwayId = 0, linearLocationId = 0, ely = 8, reversed = false, None, linkGeometryTimeStamp = 0, roadwayNumber)
  }
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates roadsToHead case When creating projectlinks Then junction template and junctions points should be handled/created properly") {
    runWithRollback {
     ...
     R: Existing road
      ...
      val link1 = dummyProjectLink(RoadPart(road1000, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 0, 11),  0, 11, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, geom2, roadwayNumber + 1).copy(id = plId1 + 1, roadwayId = rwId + 1, linearLocationId = linearLocationId + 1)
      val link2 = dummyProjectLink(RoadPart(road1000, part), Track.Combined, Discontinuity.Continuous, AddrMRange(11, 20), 11, 20, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, geom3, roadwayNumber + 1).copy(id = plId1 + 2, roadwayId = rwId + 1, linearLocationId = linearLocationId + 2)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates roadsToHead case When creating projectlinks Then junction template and junctions points should be handled/created properly and" +
    ...
    runWithRollback {
     ...
     |--R-->0|0--L-->
      ...
      val link1 = dummyProjectLink(RoadPart(road1000, part), Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, geom2, roadwayNumber2).copy(id = plId2, roadwayId = rwId2, linearLocationId = linearLocationId2)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates roadsFromHead case When creating projectLinks Then junction template and junctions points should be handled/created properly and " +
    ...
    runWithRollback {
     ...
     <--R--0|0--L-->
      ...
      val link1 = dummyProjectLink(RoadPart(road1000, part), Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, geom2, roadwayNumber2).copy(id = projectId + 1, roadwayId = rwId2, linearLocationId = linearLocationId2)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating one road that intersects itself with discontinuous address values Then junction template and junctions points should be handled/created properly") {
    runWithRollback {
     ...
     |--R1-->|0     |
      ...
      val leftLink1 = dummyProjectLink(RoadPart(road, part), Track.LeftSide, Discontinuity.Continuous,          AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom1,  rwNumber    ).copy(id = plId,     projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val leftLink2 = dummyProjectLink(RoadPart(road, part), Track.LeftSide, Discontinuity.Continuous,          AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12346.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom2,  rwNumber    ).copy(id = plId + 1, projectId = projectId, roadwayId = rwId,     linearLocationId = llId + 1)
      val leftLink3 = dummyProjectLink(RoadPart(road, part), Track.LeftSide, Discontinuity.EndOfRoad,           AddrMRange(10, 20), 10, 20, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom3,  rwNumber + 1).copy(id = plId + 2, projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 2)
      val rightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.Continuous,        AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12348.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom1, rwNumber + 2).copy(id = plId + 3, projectId = projectId, roadwayId = rwId + 2, linearLocationId = llId + 3)
      val rightLink2 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.MinorDiscontinuity,AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12349.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom2, rwNumber + 2).copy(id = plId + 4, projectId = projectId, roadwayId = rwId + 2, linearLocationId = llId + 4)
      val rightLink3 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.Continuous,        AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12350.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom3, rwNumber + 3).copy(id = plId + 5, projectId = projectId, roadwayId = rwId + 3, linearLocationId = llId + 5)
      val rightLink4 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.EndOfRoad,         AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12351.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom4, rwNumber + 3).copy(id = plId + 6, projectId = projectId, roadwayId = rwId + 3, linearLocationId = llId + 6)
...
//    val llId6 = Sequences.nextLinearLocationId
      ...
      val leftLink1  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.Continuous,   AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId1, AdministrativeClass.State, leftGeom1,  rwNumber1).copy(id = plId1, projectId = projectId1, roadwayId = rwId1, linearLocationId = llId1)
      val rightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.Discontinuous,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12346.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId1, AdministrativeClass.State, rightGeom1, rwNumber2).copy(id = plId2, projectId = projectId1, roadwayId = rwId2, linearLocationId = llId2)
      val combLink1  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.Continuous,   AddrMRange( 5, 15),  5, 15, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId1, AdministrativeClass.State, combGeom1,  rwNumber3).copy(id = plId3, projectId = projectId1, roadwayId = rwId3, linearLocationId = llId3)
      val combLink2  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,    AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12348.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId1, AdministrativeClass.State, combGeom2,  rwNumber3).copy(id = plId4, projectId = projectId1, roadwayId = rwId3, linearLocationId = llId4)
      ...
      val unchangedLeftLink1  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.Continuous,   AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Unchanged,   projectId2, AdministrativeClass.State, leftGeom1,  rwNumber1).copy(id = plId5, roadwayId = rwId1, linearLocationId = llId1)
      val unchangedRightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.Discontinuous,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12346.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Unchanged,   projectId2, AdministrativeClass.State, rightGeom1, rwNumber2).copy(id = plId6, roadwayId = rwId2, linearLocationId = llId2)
      val transferCombLink1   = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,    AddrMRange( 5, 15),  5, 15, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer,    projectId2, AdministrativeClass.State, combGeom1,  rwNumber4).copy(id = plId7, roadwayId = rwId4, linearLocationId = llId7)
      val terminatedCombLink2 = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,    AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12348.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Termination, projectId2, AdministrativeClass.State, combGeom2,  rwNumber5).copy(id = plId8, roadwayId = rwId5, linearLocationId = llId8)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating two track Discontinuous links and one combined EndOfRoad that are not connected by geometry, only by address Then junction template and junctions points should not be created") {
    runWithRollback {
     ...
     |--R*-->|
      ...
      val leftLink1  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.MinorDiscontinuity, AddrMRange(0,  5), 0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom1,  rwNumber1).copy(id = plId1, projectId = projectId, roadwayId = rwId1, linearLocationId = llId1)
      val rightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.MinorDiscontinuity, AddrMRange(0,  5), 0,  5, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom1, rwNumber2).copy(id = plId2, projectId = projectId, roadwayId = rwId2, linearLocationId = llId2)
      val combLink1  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,          AddrMRange(5, 10), 5, 10, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1,  rwNumber3).copy(id = plId3, projectId = projectId, roadwayId = rwId3, linearLocationId = llId3)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating two track MinorDiscontinuous links and one combined MinorDiscontinuous and one combined EndOfRoad that are not connected by geometry, only by address Then junction template and junctions points should not be created") {
    runWithRollback {
     ...
     |--R*-->|
      ...
      val leftLink1  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.MinorDiscontinuity,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom1,  rwNumber1).copy(id = plId1, projectId = projectId, roadwayId = rwId1, linearLocationId = llId1)
      val rightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.MinorDiscontinuity,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom1, rwNumber2).copy(id = plId2, projectId = projectId, roadwayId = rwId2, linearLocationId = llId2)
      val combLink1  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.MinorDiscontinuity,AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1,  rwNumber3).copy(id = plId3, projectId = projectId, roadwayId = rwId3, linearLocationId = llId3)
      val combLink2  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,         AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2,  rwNumber3).copy(id = plId4, projectId = projectId, roadwayId = rwId3, linearLocationId = llId4)
  ...
  def loopRoadTest(roadNumber: Long):Unit = {
      runWithRollback {
        ...
        * */
        ...
        val normalProjectLink = dummyProjectLink(RoadPart(roadNumber, roadPartNumber), Track.Combined, Discontinuity.Continuous,AddrMRange( 0,  30), 0, 30, Some(DateTime.now()), None, linkId1, 0, 30, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, geom1, roadwayNumber).copy(id = plId1, projectId = projectId, roadwayId = roadwayId, linearLocationId = llId1)
        val loopProjectLink   = dummyProjectLink(RoadPart(roadNumber, roadPartNumber), Track.Combined, Discontinuity.EndOfRoad, AddrMRange(30, 120), 30, 120, Some(DateTime.now()), None, linkId2, 0, 120, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, geom2, roadwayNumber).copy(id = plId2, projectId = projectId, roadwayId = roadwayId, linearLocationId = llId2)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating two track Discontinuous links and one combined Continuous and one combined EndOfRoad that are not connected by geometry, only by address Then junction template and junctions points should not be created.") {
    runWithRollback {
     ...
     |--R*-->|
      ...
      val leftLink1  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.MinorDiscontinuity,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom1,  rwNumber1).copy(id = plId1, projectId = projectId, roadwayId = rwId1, linearLocationId = llId1)
      val rightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.MinorDiscontinuity,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom1, rwNumber2).copy(id = plId2, projectId = projectId, roadwayId = rwId2, linearLocationId = llId2)
      val combLink1  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.Continuous,        AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1,  rwNumber3).copy(id = plId3, projectId = projectId, roadwayId = rwId3, linearLocationId = llId3)
      val combLink2  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,         AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2,  rwNumber3).copy(id = plId4, projectId = projectId, roadwayId = rwId3, linearLocationId = llId4)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating two track where one of them is Discontinuous and the other is not Then one junction and its junctions points should be created.") {
    runWithRollback {
     ...
     |--R*-->|
      ...
      val leftLink1  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.MinorDiscontinuity,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, leftGeom1,  rwNumber    ).copy(id = plId,     projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val rightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.MinorDiscontinuity,AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12346.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, rightGeom1, rwNumber + 1).copy(id = plId + 1, projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 1)
      val combLink1  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.Continuous,        AddrMRange( 5, 15),  5, 15, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom1,  rwNumber + 2).copy(id = plId + 2, projectId = projectId, roadwayId = rwId + 2, linearLocationId = llId + 2)
      val combLink2  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,         AddrMRange(15, 25), 15, 25, Some(DateTime.now()), None, 12348.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom2,  rwNumber + 2).copy(id = plId + 3, projectId = projectId, roadwayId = rwId + 2, linearLocationId = llId + 3)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating one track Discontinuous links and one opposite track Continuous and one combined EndOfRoad that are not connected by geometry with the discontinuous link, only by address, Then junction template and junctions points should be not be created.") {
    runWithRollback {
     ...
     |----R*---->|
      ...
      val leftLink1  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.Continuous,        AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom1,  rwNumber    ).copy(id = plId,     projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val leftLink2  = dummyProjectLink(RoadPart(road, part), Track.LeftSide,  Discontinuity.Continuous,        AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, leftGeom2,  rwNumber    ).copy(id = plId + 1, projectId = projectId, roadwayId = rwId,     linearLocationId = llId + 1)
      val rightLink1 = dummyProjectLink(RoadPart(road, part), Track.RightSide, Discontinuity.MinorDiscontinuity,AddrMRange( 0, 10),  0, 10, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, rightGeom1, rwNumber + 1).copy(id = plId + 2, projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 2)
      val combLink1  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.Continuous,        AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1,  rwNumber + 2).copy(id = plId + 3, projectId = projectId, roadwayId = rwId + 2, linearLocationId = llId + 3)
      val combLink2  = dummyProjectLink(RoadPart(road, part), Track.Combined,  Discontinuity.EndOfRoad,         AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12349.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2,  rwNumber + 2).copy(id = plId + 4, projectId = projectId, roadwayId = rwId + 2, linearLocationId = llId + 4)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating new road parts where one of them ends in the same road and which link is EndOfRoad Then junction template and junctions points should be handled/created properly") {
    runWithRollback {
     ...
     |--C1-->|0|--C2-->|
      ...
      val combLink1 = dummyProjectLink(RoadPart(road, part1), Track.Combined, Discontinuity.Continuous,   AddrMRange( 0, 10), 0, 10, Some(DateTime.now()), None, 12345.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1, rwNumber    ).copy(id = plId,     projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val combLink2 = dummyProjectLink(RoadPart(road, part2), Track.Combined, Discontinuity.Discontinuous,AddrMRange( 0, 10), 0, 10, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2, rwNumber + 1).copy(id = plId + 1, projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road, part3), Track.Combined, Discontinuity.EndOfRoad,    AddrMRange( 0, 10), 0, 10, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom3, rwNumber + 2).copy(id = plId + 2, projectId = projectId, roadwayId = rwId + 2, linearLocationId = llId + 2)
  ...
  test("Test nodesAndJunctionsService.expireObsoleteNodesAndJunctions When changing the EndOfRoad link of last part to Discontinuous and creating a new one with EndOfRoad that does not connect to the same road Then the existing Junction and his points should not be expired.") {
    runWithRollback {
      ...
      val combLink1 = dummyProjectLink(RoadPart(road, part1), Track.Combined, Discontinuity.Continuous,    AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12345.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom1, Sequences.nextRoadwayNumber).copy(id = Sequences.nextProjectLinkId, projectId = projectId, roadwayId = Sequences.nextRoadwayId, linearLocationId = Sequences.nextLinearLocationId)
      val combLink2 = dummyProjectLink(RoadPart(road, part2), Track.Combined, Discontinuity.Discontinuous, AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom2, Sequences.nextRoadwayNumber).copy(id = Sequences.nextProjectLinkId, projectId = projectId, roadwayId = Sequences.nextRoadwayId, linearLocationId = Sequences.nextLinearLocationId)
      val combLink3 = dummyProjectLink(RoadPart(road, part3), Track.Combined, Discontinuity.Discontinuous, AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom3, Sequences.nextRoadwayNumber).copy(id = Sequences.nextProjectLinkId, projectId = projectId, roadwayId = Sequences.nextRoadwayId, linearLocationId = Sequences.nextLinearLocationId)
      val combLink4 = dummyProjectLink(RoadPart(road, part4), Track.Combined, Discontinuity.EndOfRoad,     AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom4, Sequences.nextRoadwayNumber).copy(id = Sequences.nextProjectLinkId, projectId = projectId, roadwayId = Sequences.nextRoadwayId, linearLocationId = Sequences.nextLinearLocationId)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating new road parts that connects to other existing part in the beginning point of its geometry which link is EndOfRoad and ends in some link of this new roads in same road number Then junction template and junctions points should be handled/created properly." +
    ...
    runWithRollback {
      ...
      val transferLink = dummyProjectLink(RoadPart(road, part3), Track.Combined, Discontinuity.Discontinuous, AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Unchanged, projectId + 1, AdministrativeClass.State, combGeom3, rwNumber + 2).copy(id = plId + 3, roadwayId = rwId + 3, linearLocationId = llId + 3)
      val newLink      = dummyProjectLink(RoadPart(road,     4), Track.Combined, Discontinuity.EndOfRoad,     AddrMRange(0, 10), 0, 10, Some(DateTime.now()), None, 12348.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New,       projectId + 1, AdministrativeClass.State, combGeom4, rwNumber + 3).copy(id = plId + 4, roadwayId = rwId + 4, linearLocationId = llId + 4)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating one road part that intersects itself and still all links are continuous Then junction template and junctions points should be handled/created properly." +
    ...
    runWithRollback {
      ...
      val combLink1 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom1, rwNumber).copy(id = plId,     projectId = projectId, roadwayId = rwId, linearLocationId = llId)
      val combLink2 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom2, rwNumber).copy(id = plId + 1, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom3, rwNumber).copy(id = plId + 2, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 2)
      val combLink4 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom4, rwNumber).copy(id = plId + 3, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 3)
      val combLink5 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(20, 25), 20, 25, Some(DateTime.now()), None, 12349.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom5, rwNumber).copy(id = plId + 4, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 4)
      ...
      projectLinkDAO.moveProjectLinksToHistory(projectId)
      val unchangedLink1  = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Unchanged,   projectId + 1, AdministrativeClass.State, combGeom1, rwNumber + 1).copy(id = plId + 5, roadwayId = rwId + 1, linearLocationId = llId + 5)
      val unchangedLink2  = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Unchanged,   projectId + 1, AdministrativeClass.State, combGeom2, rwNumber + 1).copy(id = plId + 6, roadwayId = rwId + 1, linearLocationId = llId + 6)
      val unchangedLink3  = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Unchanged,   projectId + 1, AdministrativeClass.State, combGeom3, rwNumber + 1).copy(id = plId + 7, roadwayId = rwId + 1, linearLocationId = llId + 7)
      val terminatedLink4 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Termination, projectId + 1, AdministrativeClass.State, combGeom4, rwNumber + 2).copy(id = plId + 8, roadwayId = rwId + 2, linearLocationId = llId + 8)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating new road that connects to other road Then junction template and junctions points should be handled/created properly." +
    ...
    runWithRollback {
      ...
      val combLink1 = dummyProjectLink(RoadPart(road999,  part1), Track.Combined, Discontinuity.Continuous,    AddrMRange( 0, 10),  0, 10, Some(DateTime.now()), None, 12345.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom1, rwNumber    ).copy(id = plId,     projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val combLink2 = dummyProjectLink(RoadPart(road999,  part1), Track.Combined, Discontinuity.Discontinuous, AddrMRange(10, 20), 10, 20, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom2, rwNumber    ).copy(id = plId + 1, projectId = projectId, roadwayId = rwId,     linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road1000, part1), Track.Combined, Discontinuity.EndOfRoad,     AddrMRange( 0, 10),  0, 10, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom3, rwNumber + 1).copy(id = plId + 2, projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 2)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating new different road numbers that connects each other Then junction template and junctions points should be handled/created properly." +
    ...
    runWithRollback {
      ...
      */
      ...
      val combLink1 = dummyProjectLink(RoadPart(road, part1), Track.Combined, Discontinuity.Continuous, AddrMRange( 0, 10),  0, 10, Some(DateTime.now()), None, 12345.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1, rwNumber    ).copy(id = plId,     calibrationPointTypes = (RoadAddressCP, NoCP         ), projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val combLink2 = dummyProjectLink(RoadPart(road, part1), Track.Combined, Discontinuity.Continuous, AddrMRange(10, 20), 10, 20, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2, rwNumber    ).copy(id = plId + 1, calibrationPointTypes = (NoCP,          RoadAddressCP), projectId = projectId, roadwayId = rwId,     linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road, part2), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange( 0, 15),  0, 15, Some(DateTime.now()), None, 12347.toString, 0, 15, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom3, rwNumber + 1).copy(id = plId + 2, calibrationPointTypes = (RoadAddressCP, RoadAddressCP), projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 2)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating new ramps road part that connects to other part in same road number Then junction template and junctions points should be handled/created properly." +
    ...
    runWithRollback {
      ...
      */
      ...
      val combLink1 = dummyProjectLink(RoadPart(road, part1), Track.Combined, Discontinuity.Continuous,    AddrMRange( 0, 10),  0, 10, Some(DateTime.now()), None, 12345.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1, rwNumber    ).copy(id = plId,     calibrationPointTypes = (RoadAddressCP, NoCP         ), projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val combLink2 = dummyProjectLink(RoadPart(road, part1), Track.Combined, Discontinuity.Discontinuous, AddrMRange(10, 20), 10, 20, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2, rwNumber    ).copy(id = plId + 1, calibrationPointTypes = (NoCP,          RoadAddressCP), projectId = projectId, roadwayId = rwId,     linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road, part2), Track.Combined, Discontinuity.EndOfRoad,     AddrMRange( 0, 15),  0, 15, Some(DateTime.now()), None, 12347.toString, 0, 15, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom3, rwNumber + 1).copy(id = plId + 2, calibrationPointTypes = (RoadAddressCP, RoadAddressCP), projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 2)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating one roundabout road part that connects to same part in same road number and the connecting link is EndOfRoad Then junction template and junctions points should be handled/created properly") {
    runWithRollback {
      ...
      val combLink1 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom1, rwNumber).copy(id = plId,     projectId = projectId, roadwayId = rwId, linearLocationId = llId)
      val combLink2 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2, rwNumber).copy(id = plId + 1, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous, AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom3, rwNumber).copy(id = plId + 2, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 2)
      val combLink4 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom4, rwNumber).copy(id = plId + 3, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 3)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating one roundabout road part that connects to same part in same road number and the connecting link is MinorDiscontinuity Then junction template and junctions points should be handled/created properly") {
    runWithRollback {
      ...
      val combLink2 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous,         AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom2, rwNumber).copy(id = plId + 1, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.Continuous,         AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom3, rwNumber).copy(id = plId + 2, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 2)
      val combLink4 = dummyProjectLink(RoadPart(road, part), Track.Combined, Discontinuity.MinorDiscontinuity, AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, combGeom4, rwNumber).copy(id = plId + 3, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 3)
  ...
  test("Test nodesAndJunctionsService.handleJunctionTemplates When creating a roundabout road part that connects to the same part of the same road number with an EndOfRoad connecting link, " +
    ...
    runWithRollback {
      ...
      */
      ...
      // project links
      val roundaboutPLink1 = dummyProjectLink(RoadPart(roadForRoundabout, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roundaboutGeom1, rwNumber).copy(id = plId, projectId = projectId, roadwayId = rwId, linearLocationId = llId)
      val roundaboutPLink2 = dummyProjectLink(RoadPart(roadForRoundabout, part), Track.Combined, Discontinuity.Continuous, AddrMRange( 5, 10),  5, 10, Some(DateTime.now()), None, 12346.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roundaboutGeom2, rwNumber).copy(id = plId + 1, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 1)
      val roundaboutPLink3 = dummyProjectLink(RoadPart(roadForRoundabout, part), Track.Combined, Discontinuity.Continuous, AddrMRange(10, 15), 10, 15, Some(DateTime.now()), None, 12347.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roundaboutGeom3, rwNumber).copy(id = plId + 2, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 2)
      val roundaboutPLink4 = dummyProjectLink(RoadPart(roadForRoundabout, part), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(15, 20), 15, 20, Some(DateTime.now()), None, 12348.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roundaboutGeom4, rwNumber).copy(id = plId + 3, projectId = projectId, roadwayId = rwId, linearLocationId = llId + 3)
      val connectedPLink   = dummyProjectLink(RoadPart(connectedRoad,     part), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange( 0,  5),  0,  5, Some(DateTime.now()), None, 12350.toString, 0, 5, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, connectingRoadGeom, rwNumber2).copy(id = plId + 4, projectId = projectId, roadwayId = rwId2, linearLocationId = llId + 4)
  ...
  test("Test nodesAndJunctionsService.handleNodePointTemplates When creating projectLinks Then node points template should be handled/created properly and" +
    ...
    runWithRollback {
      ...
      val left      = dummyProjectLink(RoadPart(999, 999), Track.LeftSide,  Discontinuity.Continuous,AddrMRange(  0,  50),   0,  50, Some(DateTime.now()), None, 12345.toString, 0, 50, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, 0L, AdministrativeClass.State,        leftGeom,  roadwayNumber    ).copy(id = id,     projectId = projectId, roadwayId = roadwayId,     linearLocationId = linearLocationId)
      val right     = dummyProjectLink(RoadPart(999, 999), Track.RightSide, Discontinuity.Continuous,AddrMRange(  0,  50),   0,  50, Some(DateTime.now()), None, 12346.toString, 0, 50, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, 0L, AdministrativeClass.State,        rightGeom, roadwayNumber + 1).copy(id = id + 1, projectId = projectId, roadwayId = roadwayId + 1, linearLocationId = linearLocationId + 1)
      ...
      val combined2 = dummyProjectLink(RoadPart(999, 999), Track.Combined,  Discontinuity.EndOfRoad, AddrMRange(100, 150), 100, 150, Some(DateTime.now()), None, 12348.toString, 0, 50, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, 0L, AdministrativeClass.State,        combGeom2, roadwayNumber + 3).copy(id = id + 3, projectId = projectId, roadwayId = roadwayId + 3, linearLocationId = linearLocationId + 3)
  ...
  test("Test expireObsoleteNodesAndJunctions case When road part is terminated Then also node points for terminated road should be expired") {
    runWithRollback {
      ...
      val roadLink1 = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0, 100), 0, 100, Some(DateTime.now()), None, 12345.toString, 0, 100.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom1, roadwayNumber    ).copy(id = plId1)
      val roadLink2 = dummyProjectLink(RoadPart(1, 2), Track.Combined, Discontinuity.Continuous, AddrMRange(0, 150), 0, 150, Some(DateTime.now()), None, 12346.toString, 0, 150.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom2, roadwayNumber + 1).copy(id = plId2)
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is extended after the existing road") {
    runWithRollback {
      ...
      val roadwayNumber = Sequences.nextRoadwayNumber
      val projectId = Sequences.nextViiteProjectId
      val roadLink = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0, 100), 0, 100, Some(DateTime.now()), None, 12345.toString, 0, 100.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom1, roadwayNumber)
      ...
      val newRoadLink = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(100, 250), 100, 250, Some(DateTime.now()), None, 12346.toString, 0, 150.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, 0, AdministrativeClass.State, roadGeom2, roadwayNumber + 1)
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is extended before the existing road") {
    runWithRollback {
      ...
      val roadLink = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(100, 250), 100, 250, Some(DateTime.now()), None, 12345.toString, 100, 250.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom1, roadwayNumber)
      ...
      val newRoadLink = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0, 100), 0, 100, Some(DateTime.now()), None, 12346.toString, 0, 100.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, 0, AdministrativeClass.State, roadGeom2, roadwayNumber + 1)
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is terminated Then also junction points should be expired") {
    runWithRollback {
      ...
      val plId1 = projectId + 1
      val plId2 = projectId + 2
      val road1Link = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0, 100), 0, 100, Some(DateTime.now()), None, 12345.toString, 0, 100.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom1, roadwayNumber1).copy(id = plId1)
      val road2Link = dummyProjectLink(RoadPart(2, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0, 150), 0, 150, Some(DateTime.now()), None, 12346.toString, 0, 150.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom2, roadwayNumber2).copy(id = plId2)
  ...
  test("Test expireObsoleteNodesAndJunctions case When road is terminated Then also junction and junction points should be expired if they are in the same road") {
    runWithRollback {
      ...
      val project = Project(projectId, ProjectState.Incomplete, "f", "s", DateTime.now(), "", DateTime.now(), DateTime.now(),
        "", Seq(), Seq(), None, None)
      val road1Link1 = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0,  5), 0,  5, Some(DateTime.now()), None, 12345.toString,   0,  5.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom1Link1, roadwayNumber    ).copy(id = plId1)
      val road1Link2 = dummyProjectLink(RoadPart(1, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(5, 10), 5, 10, Some(DateTime.now()), None, 12346.toString, 5.0, 10.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom1Link2, roadwayNumber    ).copy(id = plId2)
      val road2Link  = dummyProjectLink(RoadPart(2, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0,  5), 0,  5, Some(DateTime.now()), None, 12347.toString,   0,  5.0, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, roadGeom2,      roadwayNumber + 1).copy(id = plId3)
  ...
  test("Test expireObsoleteNodesAndJunctions When there is complex changes in the project Then Junction and its Junction points should be properly expired") {
    runWithRollback {
      ...
      val combLink1 = dummyProjectLink(RoadPart(road999,  part1), Track.Combined, Discontinuity.Continuous,    AddrMRange( 0, 10),  0, 10, Some(DateTime.now()), None, 12345.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom1, rwNumber    ).copy(id = plId,     projectId = projectId, roadwayId = rwId,     linearLocationId = llId)
      val combLink2 = dummyProjectLink(RoadPart(road999,  part1), Track.Combined, Discontinuity.Discontinuous, AddrMRange(10, 20), 10, 20, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom2, rwNumber    ).copy(id = plId + 1, projectId = projectId, roadwayId = rwId,     linearLocationId = llId + 1)
      val combLink3 = dummyProjectLink(RoadPart(road1000, part1), Track.Combined, Discontinuity.Discontinuous, AddrMRange( 0,  9),  0,  9, Some(DateTime.now()), None, 12347.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, AdministrativeClass.State, combGeom3, rwNumber + 1).copy(id = plId + 2, projectId = projectId, roadwayId = rwId + 1, linearLocationId = llId + 2)
      ...
      val transferLinkRoad999        = dummyProjectLink(RoadPart(road999, part1), Track.Combined, Discontinuity.Continuous,    AddrMRange( 0, 10),  0, 10, Some(DateTime.now()), None, 12345.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer,    projectId + 1, AdministrativeClass.State, combGeom1, rwNumber + 3).copy(id = plId + 4, roadwayId = rwId + 3, linearLocationId = llId + 7)
      val terminatedLinkRoad999      = dummyProjectLink(RoadPart(road999, part1), Track.Combined, Discontinuity.Discontinuous, AddrMRange(10, 20), 10, 20, Some(DateTime.now()), None, 12346.toString, 0, 10, SideCode.TowardsDigitizing, RoadAddressChangeType.Termination, projectId + 1, AdministrativeClass.State, combGeom2, rwNumber + 2).copy(id = plId + 3, roadwayId = rwId + 2, linearLocationId = llId + 6)
      val transferLinkRoadExRoad1000 = dummyProjectLink(RoadPart(road999, part1), Track.Combined, Discontinuity.Discontinuous, AddrMRange(10, 19),  0,  9, Some(DateTime.now()), None, 12347.toString, 0,  9, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer,    projectId + 1, AdministrativeClass.State, combGeom3, rwNumber + 1).copy(id = plId + 5, roadwayId = rwId + 4, linearLocationId = llId + 8)
  ...
  def dummyVerticalRoad(projectId: Long, numberOfLinearLocations: Int, roadPart: RoadPart, rwNumber: Long, track: Track = Track.Combined, startAddrAt: Long = 0, administrativeClass: AdministrativeClass = AdministrativeClass.State, discontinuity: Discontinuity = Discontinuity.Continuous, firstPointAt: Point = Point(0.0, 0.0), orderNumber: Double = 1.0, linkId: Long = 0, plId: Long = 0, rw: (Long, Option[Roadway]) = (0, None), llId: Long = 0, size: Int = 10): (Seq[ProjectLink], Seq[LinearLocation], Roadway) = {
    ...
    val projectLinks = for (i: Int <- 0 until numberOfLinearLocations) yield {
      ...
      dummyProjectLink(roadPart, track, discontinuityCode, AddrMRange(startAddrM, endAddrM), startAddrM, endAddrM, Some(DateTime.now()), None, (linkId + i).toString, 0, size, SideCode.TowardsDigitizing, RoadAddressChangeType.New, projectId, administrativeClass, geom, rwNumber).copy(id = plId + i, roadwayId = rwId, linearLocationId = llId + i)
    }
    val (linearLocations, roadways) = projectLinks.map(toRoadwayAndLinearLocation).unzip
    val orderedLinearLocations: Seq[LinearLocation] = linearLocations.zipWithIndex.map { case (ll, i) => ll.copy(orderNumber = orderNumber + i) }
...
//    val roadwayPointId2 = roadwayPointDAO.create(testRoadwayPoint2.copy(roadwayNumber = roadwayNumber2, addrMValue = 20))
  ...
  test("Test calculateNodePointsForNode When there is a road with 2 roadparts, 2 junctions and the roadpart changes on a junction. The section between the junctions is transfered and NodePoints should update accordingly without dublicate NodePoints or templates") {
    runWithRollback {
      ...
      val newPl3 = dummyProjectLink(RoadPart(mainroad, 2), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(  0, 100), 100, 200, Some(DateTime.now()), None, 12347.toString, 200, 300, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer, projectId2, AdministrativeClass.State, geom3, newRwNumber3).copy(id = plId1 + 12, projectId = projectId2, roadwayId = rwId1 + 12, linearLocationId = llId1 + 12, reversed = false)
