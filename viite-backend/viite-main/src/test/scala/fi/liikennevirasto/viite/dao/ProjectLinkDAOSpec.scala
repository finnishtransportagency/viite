...
class ProjectLinkDAOSpec extends FunSuite with Matchers {
  ...
  test("Test updateAddrMValues When having changed addr values for some reason (e.g. recalculate) Then should still return original addr values") {
    runWithRollback {
      ...
      returnedProjectLinks.head.originalAddrMRange should be(projectLink.originalAddrMRange)
    }
  }
  ...
  test("Test updateProjectLinkAdministrativeClassDiscontinuity When Administrative Class or discontinuity got updated Then update should be made with success") {
    runWithRollback {
      ...
      projectLinkDAO.create(projectLinks)
      val returnedProjectLinks = projectLinkDAO.fetchProjectLinks(id)
      val biggestProjectLink = returnedProjectLinks.maxBy(_.addrMRange.endAddrM)
  ...
  test("Test updateProjectLinks When giving one new projectLink with new values Then it should update project link") {
    runWithRollback {
      ...
      val newStartAddrMValue = header.addrMRange.startAddrM + 1
      val newEndAddrMValue = header.addrMRange.endAddrM + 1
  ...
  test("Test updateProjectLinkNumbering When changing the discontinuity value of Left track specific id only Then the discontinuity value for that Left track ids should be updated the other should remain") {
    runWithRollback {
      ...
      projectLinkDAO.updateProjectLinkAdministrativeClassDiscontinuity(Set(links.filter(_.track == Track.LeftSide).maxBy(_.addrMRange.endAddrM).id), RoadAddressChangeType.Renumeration, "test", links.filter(_.track == Track.LeftSide).head.administrativeClass.value, Some(Discontinuity.MinorDiscontinuity.value))
