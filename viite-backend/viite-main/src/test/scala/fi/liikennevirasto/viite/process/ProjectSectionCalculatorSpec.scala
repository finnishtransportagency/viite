...
class ProjectSectionCalculatorSpec extends FunSuite with Matchers {
  ...
  test("Test assignMValues When giving new + transfer links Then Project section calculator should assign values properly") {
    runWithRollback {
      ...
      output.head.calibrationPoints should be(Some(ProjectCalibrationPoint(12347.toString, 0.0, 0, RoadAddressCP)), None)
    }
  }
 ...
 */
  ...
  test("Test assignMValues When new link with discontinuity on both sides is added in the between of two other links having calibration points at the beginning and end and discontinuity places Then the direction should stay same and new address values should be properly assigned") {
    runWithRollback {
      ...
      val projectLink1 = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(idRoad1.toLong, 0, 5, 1, AdministrativeClass.Municipality, Track.Combined, Discontinuity.Continuous,        10L, 20L, Some(DateTime.parse("1901-01-01")), None, Option("tester"), 12345L.toString, 0.0, 10.6, SideCode.TowardsDigitizing, 0, (Some(ProjectCalibrationPoint(12345L.toString, 0, 10L)), Some(ProjectCalibrationPoint(12345L.toString, 10.6, 20L))), geom1, LinkGeomSource.NormalLinkInterface, 8, NoTermination, 0))
      val projectLink2 = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(idRoad2.toLong, 0, 5, 1, AdministrativeClass.Municipality, Track.Combined, Discontinuity.MinorDiscontinuity, 0L, 10L, Some(DateTime.parse("1901-01-01")), None, Option("tester"), 12346L.toString, 0.0, 11.2, SideCode.TowardsDigitizing, 0, (Some(ProjectCalibrationPoint(12346L.toString, 0,  0L)), Some(ProjectCalibrationPoint(12346L.toString, 11.2, 10L))), geom2, LinkGeomSource.NormalLinkInterface, 8, NoTermination, 0))
