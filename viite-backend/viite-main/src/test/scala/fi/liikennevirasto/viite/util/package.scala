...
package object util {
  ...
  def setUpProjectWithLinks(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roads: Seq[(RoadPart, String)] = Seq((RoadPart(1999, 1), "Test road")),
                            discontinuity: Discontinuity = Discontinuity.Continuous, roadMaintainer: ArealRoadMaintainer = ArealRoadMaintainer.apply("EVK6"), withRoadAddress: Boolean = true): (Project, Seq[ProjectLink]) = {
    ...
    def createRoadAddresses(roadPart: RoadPart, track: Long, start: Long, end: Long): (Long, String) = {
      ...
      runUpdateToDb(sql"""
                    ...
                    VALUES (
                      ...
                      0, 0, current_date, NULL, ${roadMaintainer.id}
                    )
                    """)
    ...
    def withTrack(t: Track, roadPart: RoadPart): Seq[ProjectLink] = {
      addrM.init.zip(addrM.tail).map {
        case (st, en) =>
          ...
          projectLink(AddrMRange(st, en), t, projectId, roadAddressChangeType, roadPart, discontinuity, roadMaintainer, linkId, roadwayId)
      }
    }
