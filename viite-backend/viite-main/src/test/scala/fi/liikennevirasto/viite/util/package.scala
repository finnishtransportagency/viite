...
package object util {
  ...
  def setUpProjectWithLinks(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roads: Seq[(RoadPart, String)] = Seq((RoadPart(1999, 1), "Test road")),
                            discontinuity: Discontinuity = Discontinuity.Continuous, ely: Long = 8L, roadMaintainer: ArealRoadMaintainer = ArealRoadMaintainer.apply("EVK6"), withRoadAddress: Boolean = true): (Project, Seq[ProjectLink]) = {
    ...
    def withTrack(t: Track, roadPart: RoadPart): Seq[ProjectLink] = {
      addrM.init.zip(addrM.tail).map {
        case (st, en) =>
          ...
          projectLink(AddrMRange(st, en), t, projectId, roadAddressChangeType, roadPart, discontinuity, ely, roadMaintainer, linkId, roadwayId)
      }
    }
