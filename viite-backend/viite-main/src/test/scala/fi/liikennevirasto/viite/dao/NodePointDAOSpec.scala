...
//  private val testNodePoint1 = NodePoint(NewIdValue, BeforeAfter.Before, -1, None, NodePointType.UnknownNodePointType, None, None,
//    DateTime.parse("2019-01-01"), None, "Test", None, -1, 10, RoadPart(0, 0), Track.Combined, 0, 0)
//  private val testNodePoint2 = NodePoint(NewIdValue, BeforeAfter.After, -1, None, NodePointType.UnknownNodePointType, None, None,
//    DateTime.parse("2019-01-01"), None, "Test", None, -1, 10, RoadPart(0, 0), Track.Combined, 0, 0)
...
//      val roadway = Roadway(NewIdValue, newRoadwayNumber, RoadPart(1999, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 10L), reversed = false, DateTime.now(), None, "test_user", None, 8, 8, NoTermination, DateTime.now(), None)
...
//      val roadway = Roadway(NewIdValue, roadwayNumber, RoadPart(1, 2), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 10L), reversed = false, DateTime.now, None, "user", None, 8L, 8L, TerminationCode.NoTermination, DateTime.now, None)
//      roadwayDAO.create(Seq(roadway))
...
//    runWithRollback {
//      val newRoadwayNumber = Sequences.nextRoadwayNumber
//      val roadway = Roadway(NewIdValue, newRoadwayNumber, RoadPart(1, 2), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 10L), reversed = false, DateTime.now, None, "user", None, 8L, 8L, TerminationCode.NoTermination, DateTime.now, None)
//      roadwayDAO.create(Seq(roadway))
//      val roadwayPointId1 = roadwayPointDAO.create(testRoadwayPoint1.copy(roadwayNumber = newRoadwayNumber))
//      val ids = dao.create(Seq(testNodePoint1.copy(roadwayPointId = roadwayPointId1),
//        testNodePoint2.copy(roadwayPointId = roadwayPointId1)))
