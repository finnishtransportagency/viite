...
class ProjectReservedPartDAOSpec extends AnyFunSuite with Matchers {
 ...
 def dummyProjectLink(id: Long, projectId: Long, linkId: String, roadwayId: Long = 0, roadwayNumber: Long = roadwayNumber1, roadPart: RoadPart = RoadPart(roadNumber1, roadPartNumber1), addrMRange: AddrMRange, startMValue: Double, endMValue: Double, endDate: Option[DateTime] = None, calibrationPointTypes: (CalibrationPointType, CalibrationPointType) = (NoCP, NoCP), geometry: Seq[Point] = Seq(), status: RoadAddressChangeType, administrativeClass: AdministrativeClass, reversed: Boolean): ProjectLink =
   ProjectLink(id, roadPart, Track.Combined, Discontinuity.Continuous, addrMRange, addrMRange, Some(DateTime.parse("1901-01-01")), endDate, Some("testUser"), linkId, startMValue, endMValue, SideCode.TowardsDigitizing, calibrationPointTypes, (NoCP, NoCP), geometry, projectId, status, administrativeClass, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geometry), roadwayId, linearLocationId,  ArealRoadMaintainer.getEVK(0), reversed, connectedLinkId = None, 631152000, roadwayNumber, roadAddressLength = addrMRange.lengthOption)

 private def dummyRoadways: Seq[Roadway] = {
   Seq(
     Roadway(NewIdValue, roadwayNumber1, RoadPart(roadNumber1, roadPartNumber1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, 100),reversed = false, DateTime.parse("2000-01-01"), None, "testUser", Some("Test Rd. 1"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination),
     Roadway(NewIdValue, roadwayNumber2, RoadPart(roadNumber1, roadPartNumber2), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, 100),reversed = false, DateTime.parse("2000-01-01"), None, "testUser", Some("Test Rd. 1"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
   )
 }
  ...
  */
   ...
   test("Test isNotAvailableForProject case (2a) When START_DATE > PROJ_DATE, END_DATE = null Then should NOT be reservable") {
     runWithRollback {
       ...
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(5,205),id)
   ...
   test("Test isNotAvailableForProject case (3a) When START_DATE < PROJ_DATE, END_DATE < PROJ_DATE - 1 Then should be reservable") {
     runWithRollback {
       ...
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(roadNumber1, roadPartNumber1), id4)
       reserveNotAvailable should be (false)
     }
   }
   ...
   test("Test isNotAvailableForProject case (3b) When START_DATE < PROJ_DATE, END_DATE == PROJ_DATE - 1 Then should be reservable") {
     runWithRollback {
       ...
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(roadNumber1, roadPartNumber1), id5)
       reserveNotAvailable should be (false)
     }
   }
   ...
   test("Test isNotAvailableForProject case (4a) When START_DATE < PROJ_DATE, END_DATE > PROJ_DATE - 1 Then should NOT be reservable") {
     runWithRollback {
       ...
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(roadNumber1, roadPartNumber1), id6)
       reserveNotAvailable should be (true)
     }
   }
   ...
   test("Test isNotAvailableForProject case (4b) When START_DATE == PROJ_DATE, END_DATE > PROJ_DATE - 1 Then should NOT be reservable") {
     runWithRollback {
       ...
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(roadNumber1, roadPartNumber1), id7)
       reserveNotAvailable should be (true)
     }
   }
   ...
   test("Test isNotAvailableForProject case (5a) When START_DATE > PROJ_DATE, END_DATE > PROJ_DATE - 1 Then should NOT be reservable") {
     runWithRollback {
       ...
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(roadNumber1, roadPartNumber1), id8)
       reserveNotAvailable should be (true)
     }
   }
   ...
   test("Test isNotAvailableForProject case (5b) When START_DATE > PROJ_DATE, END_DATE > PROJ_DATE - 1 Then should NOT be reservable") {
     runWithRollback {
       ...
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(roadNumber1, roadPartNumber1), id9)
       reserveNotAvailable should be(false)
     }
   }
   ...
   test("Test isNotAvailableForProject case (6) When START_DATE > PROJ_DATE, END_DATE > PROJ_DATE - 1 Then should be reservable") {
     runWithRollback {
       ...
       projectDAO.create(rap)
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(roadNumber1, roadPartNumber1), id)
       reserveNotAvailable should be (false)
     }
   }
   ...
   test("Test isNotAvailableForProject When there is no reserved part for project Then road part can be reserved") {
     runWithRollback {
       ...
       projectDAO.create(rap)
       val reserveNotAvailable = projectReservedPartDAO.isNotAvailableForProject(RoadPart(12345,1),id)
       reserveNotAvailable should be (false)
     }
   }
 ...
 test("Test reserveRoadPart When having reserved one project with that part Then should fetch it without any problems") {
   runWithRollback {
     ...
     val reservedParts = Seq(ProjectReservedPart(id: Long, RoadPart(roadNumber1, roadPartNumber1), Some(6L), Some(Discontinuity.apply("jatkuva")), Some(ArealRoadMaintainer.apply("EVK8")), newLength = None, newDiscontinuity = None))
     ...
     val projectLinks = Seq(dummyProjectLink(projectLinkId, id, linkId1, roadwayIds.head, roadwayNumber1, RoadPart(roadNumber1, roadPartNumber1), AddrMRange(0, 100), 0.0, 100.0, None, (NoCP, NoCP), Seq(), RoadAddressChangeType.Transfer, AdministrativeClass.State, reversed = false)
     )
     projectLinkDAO.create(projectLinks)
     val fetchedPart = projectReservedPartDAO.fetchReservedRoadPart(RoadPart(roadNumber1, roadPartNumber1))
     fetchedPart.nonEmpty should be (true)
     fetchedPart.get.roadPart should be (reservedParts.head.roadPart)
   }
 }
 ...
 test("Test roadPartReservedByProject When removeReservedRoadPart is done in that same road part Then should not be returning that removed part") {
   runWithRollback {
     ...
     projectLinkDAO.create(projectLinks)
     val project = projectReservedPartDAO.fetchProjectReservedPart(RoadPart(roadNumber1, roadPartNumber1))
     project should be(Some(id, "TestProject"))
     val reserved = projectReservedPartDAO.fetchReservedRoadPart(RoadPart(roadNumber1, roadPartNumber1))
     reserved.nonEmpty should be(true)
     projectReservedPartDAO.fetchReservedRoadParts(id) should have size 1
     projectReservedPartDAO.removeReservedRoadPartAndChanges(id, reserved.get.roadPart)
     val projectAfter = projectReservedPartDAO.fetchProjectReservedPart(RoadPart(roadNumber1, roadPartNumber1))
     projectAfter should be(None)
     projectReservedPartDAO.fetchReservedRoadPart(RoadPart(roadNumber1, roadPartNumber1)).isEmpty should be(true)
   }
 }
 ...
 test("Test roadPartReservedTo When getting the road parts reserved Then should return the project that has the roads") {
     runWithRollback {
       ...
       projectLinkDAO.create(projectLinks)
       projectReservedPartDAO.roadPartReservedTo(RoadPart(roadNumber1, roadPartNumber1)).get._1 should be (id)
     }
 }
   ...
   runWithRollback {
     ...
     runUpdateToDb(sql"""INSERT INTO project VALUES ($projectId, 1, 'Test Project', 'Test', to_date('01.01.2018','DD.MM.YYYY'),'-',to_date('01.01.2018','DD.MM.YYYY'), null, to_date('01.01.2018','DD.MM.YYYY'), null, 0, 0, 0)""")
       ...
       INSERT INTO project_link_history
         ...
         reversed, side, start_measure, end_measure, link_id, adjusted_timestamp, link_source,
       ...
       VALUES (11111111, $projectId, 0, 1, $roadNumber, $roadPartNumber, 0, 10,
         ...
         8, null, 2, 0, 10, 99999, 1533576206000, 1,
         null, 0, 10, NULL,
         3, 3, 3, 3, 'EVK1')
     """)
 ...
 test("Test fetchReservedRoadParts When finding reserved parts by road number and part Then it should return the project reserved parts") {
   runWithRollback {
     ...
     projectLinkDAO.create(projectLinks)
     val fetched = projectReservedPartDAO.fetchReservedRoadPart(RoadPart(roadNumber1, roadPartNumber1))
     fetched.size should be (1)
     fetched.head.roadPart should be (RoadPart(roadNumber1,roadPartNumber1))
   }
 }
 ...
 test("Roadways with junction points that share junctions belonging to reserved roadways can not be reserved.") {
   runWithRollback {
     ...
     val intersectingRoadway = Roadway(NewIdValue, NewIdValue, RoadPart(roadNumber1, roadPartNumber1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous,
       AddrMRange(0, 100),reversed = false, roadwayStartDate, None, "test", Some("Test road 1"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     val reservedRoadway1 = Roadway(NewIdValue, NewIdValue, RoadPart(roadNumber2, roadPartNumber1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous,
       AddrMRange(0, 100),reversed = false, roadwayStartDate, None, "test", Some("Test road 2"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     val reservedRoadway2 = Roadway(NewIdValue, NewIdValue, RoadPart(roadNumber2 + 1, roadPartNumber1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous,
       AddrMRange(0, 100),reversed = false, roadwayStartDate, None, "test", Some("Test road 3"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     ...
     val outsideRoadway1 = Roadway(NewIdValue, NewIdValue, RoadPart(roadNumber2, roadPartNumber2), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous,
       AddrMRange(0, 100),reversed = false, roadwayStartDate, None, "test", Some("Test road, no junctions"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     ...
     val roadways = connectedRoadways.flatMap(roadway =>
       roadwayDAO.fetchAllBySection(roadway.roadPart)
     )
     ...
     val junctionPoint = JunctionPoint(NewIdValue, BeforeAfter.Before, -1, -1, Some(junction.startDate), junction.endDate, junctionStartDate, None, "Test", None, -1, 10, RoadPart(0, 0), Track.Combined, Discontinuity.Continuous)
     ...
     val projectLinks = Seq(dummyProjectLink(projectLinkId, id, linkId1 + 0, roadways.head.id, roadways.head.roadwayNumber, RoadPart(roadNumber1, roadPartNumber1), AddrMRange(0, 100), 0.0, 100.0, None, (NoCP, NoCP), Seq(), RoadAddressChangeType.Transfer, AdministrativeClass.State, reversed = false)
     )
     projectLinkDAO.create(projectLinks)

     connectedRoadways.flatMap(roadway => projectReservedPartDAO.fetchProjectReservedJunctions(roadway.roadPart, 0)).size should be (connectedRoadways.size)
     notConnectedroadways.flatMap(roadway => projectReservedPartDAO.fetchProjectReservedJunctions(roadway.roadPart, 0)).size should be (0)
   }
 }
 ...
 test("Check for reserved junctions take road part numbers into consideration") {
   runWithRollback {
     ...
     val newReservedRoadway2 = Roadway(NewIdValue, NewIdValue, RoadPart(roadNumber4, roadPartNumber1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous,
       AddrMRange(0, 100),reversed = false, roadwayStartDate, None, "test", Some("Test road 4"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination)
     ...
     val intersectingRoadway  = roadwayDAO.fetchAllBySection(newIntersectingRoadway.roadPart).head
     val intersectingRoadway2 = roadwayDAO.fetchAllBySection(newIntersectingRoadway2.roadPart).head
     val reservedRoadway      = roadwayDAO.fetchAllBySection(newReservedRoadway.roadPart).head
     val notReservedRoadway   = roadwayDAO.fetchAllBySection(newNotReservedRoadway.roadPart).head
     val reservedRoadway2     = roadwayDAO.fetchAllBySection(newReservedRoadway2.roadPart).head
     ...
     projectReservedPartDAO.reserveRoadPart(id1, reservedRoadway.roadPart, rap1.createdBy)
     projectReservedPartDAO.reserveRoadPart(id2, reservedRoadway2.roadPart, rap2.createdBy)

     val projectLinks1 = Seq(dummyProjectLink(projectLinkId1, id1, reservedRoadwayLocation.linkId,  reservedRoadway.id,  reservedRoadway.roadwayNumber,  reservedRoadway.roadPart,  AddrMRange(0, 100), 0.0, 100.0, None, (NoCP, NoCP), Seq(), RoadAddressChangeType.Transfer, AdministrativeClass.State, reversed = false))
     val projectLinks2 = Seq(dummyProjectLink(projectLinkId2, id2, reservedRoadway2Location.linkId, reservedRoadway2.id, reservedRoadway2.roadwayNumber, reservedRoadway2.roadPart, AddrMRange(0, 100), 0.0, 100.0, None, (NoCP, NoCP), Seq(), RoadAddressChangeType.Transfer, AdministrativeClass.State, reversed = false))

     projectLinkDAO.create(projectLinks1 ++ projectLinks2)

     projectReservedPartDAO.fetchProjectReservedJunctions(intersectingRoadway.roadPart, 0L).head should be (projectName1)
     projectReservedPartDAO.fetchProjectReservedJunctions(intersectingRoadway2.roadPart, 0L).head should be (projectName2)
   }
 }
}
