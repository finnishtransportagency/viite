...
class TrackCalculatorStrategySpec extends FunSuite with Matchers {
  test("Test TrackCalculatorStrategy.setCalibrationPoints() When having different road types between links on same Track Then one calibration point should be created") {
    ...
    val projectLinkRight3 = ProjectLink(-1000L, 9999L, 1L, Track.apply(1), Discontinuity.Continuous, 20L, 30L, 20L, 30L, None, None, None, 12350L.toString, 0.0, 10.0, SideCode.TowardsDigitizing, (NoCP, NoCP), (NoCP, NoCP), geomRight2, 0L, RoadAddressChangeType.Transfer, AdministrativeClass.Municipality, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomRight3), 0L, 0, 1L, reversed = false, None, 86400L)
