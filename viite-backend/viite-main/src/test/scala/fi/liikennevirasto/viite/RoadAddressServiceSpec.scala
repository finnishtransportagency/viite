...
class RoadAddressServiceSpec extends AnyFunSuite with Matchers{
 ...
 test("Test getRoadAddress When the biggest address in section is greater than the parameter $addressM Then should filter out all the road addresses with start address less than $addressM") {
   ...
   val result = roadAddressService.getRoadAddress(roadPart = RoadPart(1, 1), addressM = 200, None)

   result.size should be (3)
   result.count(_.addrMRange.start < 200) should be (1)
 }
 ...
 test("Test getRoadAddressesFiltered When roadway have less and greater addresses than given measures Then returns only road addresses in between given address measures") {
   ...
   when(mockRoadwayDAO.fetchAllBySection(any[RoadPart])).thenReturn(roadways)
   when(mockLinearLocationDAO.fetchByRoadways(any[Set[Long]])).thenReturn(linearLocations)

   val result = roadAddressService.getRoadAddressesFiltered(roadPart = RoadPart(1, 1))

   verify(mockRoadwayDAO, times(1)).fetchAllBySection(roadPart = RoadPart(1, 1))
 ...
 test("Test SearchService") {
   ...
   val roadways = Seq(
     dummyRoadway(roadwayNumber = 1L, roadPart = RoadPart(1, 1), addrMRange = AddrMRange(0L, 10L), DateTime.now(), None)
   )
   ...
   result.size should be(2)
   result(1)("road").head.asInstanceOf[RoadAddress].roadPart should be(RoadPart(1, 1))
   ...
   result.size should be(1)
   result.head("road").head.asInstanceOf[RoadAddress].roadPart should be(RoadPart(1, 1))
...
//    when(mockRoadLinkService.getComplementaryRoadLinksFromVVH(99999)).thenReturn(Seq())
 ...
 test("Test handleRoadwayPoints When Terminating one link and Transfer the rest Then roadway points should be handled/created properly") {
   runWithRollback {
     ...
     val link1 = dummyProjectLink(RoadPart(99, 2), Track.Combined, Discontinuity.Continuous, AddrMRange(0,  5), Some(DateTime.now()), None, 12345.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Termination, projectId, AdministrativeClass.State, geom1, roadwayNumber1)
     val link2 = dummyProjectLink(RoadPart(99, 2), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(5, 20), Some(DateTime.now()), None, 12346.toString, 0, 15, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer,    projectId, AdministrativeClass.State, geom2, roadwayNumber2)
     ...
     val rw1WithId = rw1.copy(id = id1, roadMaintainer = ArealRoadMaintainer.getEVK(8))
     val rw2WithId = rw2.copy(id = id2, roadMaintainer = ArealRoadMaintainer.getEVK(8))
     val lc2WithId = lc2
     ...
     when(mockRoadwayDAO.fetchAllByRoadPart(RoadPart(99, 2))).thenReturn(Seq(rw2WithId))

     projectReservedPartDAO.reserveRoadPart(projectId, RoadPart(99, 2), "u")
     projectLinkDAO.create(pls.map(_.copy(id = NewIdValue)))
     val projectLinks = projectLinkDAO.fetchByProjectRoad(99, projectId).sortBy(_.addrMRange.start)

     projectService.updateProjectLinks(projectId, Set(), Seq(projectLinks.head.linkId), RoadAddressChangeType.Termination, "-", RoadPart(99, 2), 0, Option.empty[Int])
     ...
     roadwayPointDAO.create(link1.roadwayNumber, link1.addrMRange.start, link1.createdBy.getOrElse("test"))
     roadwayPointDAO.create(link1.roadwayNumber, link1.addrMRange.end, link2.createdBy.getOrElse("test"))
     roadwayPointDAO.create(link2.roadwayNumber, link2.addrMRange.start, link2.createdBy.getOrElse("test"))
     roadwayPointDAO.create(link2.roadwayNumber, link2.addrMRange.end, link2.createdBy.getOrElse("test"))

     val afterUpdateProjectLinks = projectLinkDAO.fetchByProjectRoad(99, projectId).sortBy(_.addrMRange.start)
     ...
     val reservedParts = Seq(ProjectReservedPart(0, RoadPart(99, 2), Some(20), Some(Discontinuity.Continuous), Option(ArealRoadMaintainer.getEVK(8)), None, None, None, Some(12345L.toString))) //TODO: Muutos evk => None ei jäljitettävissä, mahdollinen ongelmapaikka
     ...
     when(mockRoadwayDAO.fetchAllBySectionAndTracks(any[RoadPart], any[Set[Track]])).thenReturn(Seq(rw1WithId, rw2WithId))
     ...
     roadwayPointsAfterChangesTransferLink.head.addrMValue should be (generatedProperRoadwayNumbersAfterChanges.last.addrMRange.start)
     roadwayPointsAfterChangesTransferLink.last.addrMValue should be (generatedProperRoadwayNumbersAfterChanges.last.addrMRange.end)
   }
 }
 ...
 test("Test handleRoadwayPoints When Terminating one link and Transfer the rest and reverse it Then roadway points should be handled/created properly") {
   runWithRollback {
     ...
     val linearLocationId = Sequences.nextLinearLocationId
     val link1 = dummyProjectLink(RoadPart(99, 2), Track.Combined, Discontinuity.Continuous, AddrMRange(0,  5), Some(DateTime.now()), None, 12345.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Termination, projectId, AdministrativeClass.State, geom1, roadwayNumber1)
     val link2 = dummyProjectLink(RoadPart(99, 2), Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(5, 20), Some(DateTime.now()), None, 12346.toString, 0, 15, SideCode.AgainstDigitizing, RoadAddressChangeType.Transfer, projectId, AdministrativeClass.State, geom2, roadwayNumber2)
     ...
     projectService.changeDirection(projectId, RoadPart(99, 2), Seq(LinkToRevert(link2.id, link2.linkId, RoadAddressChangeType.Transfer.value, Seq())), ProjectCoordinates(link2.geometry.head.x, link2.geometry.head.y), "testuser")
     ...
     val terminated = Termination(Seq(
       (
         dummyRoadAddress(roadwayNumber1, RoadPart(99, 2), AddrMRange(0, 5), Some(DateTime.now()), None, 12345.toString, 0, 5, LinkGeomSource.NormalLinkInterface, geom1),
     ...
     val transferred = Transferred(Seq(
       (
         dummyRoadAddress(roadwayNumber1, RoadPart(99, 2), AddrMRange(5, 20), Some(DateTime.now()), None, 12346.toString, 0, 15, LinkGeomSource.NormalLinkInterface, geom2),
     ...
     val formedParts = Seq(ProjectReservedPart(
       id = 0L,
       roadPart = RoadPart(99, 2),
       addressLength = None,
       discontinuity = None,
       roadMaintainer = None,
       newLength = Some(15L),
       newDiscontinuity = Some(Discontinuity.Continuous),
       newRoadMaintainer = Some(ArealRoadMaintainer.getEVK(8)),
       startingLinkId = Some(12345L.toString)
     ))
 ...
 test("Test handleRoadwayPoints When dual roadway point is no longer dual roadway point then addrM's and roadway number in that roadway point should change correctly and new roadway point should be created NEW + TRANSFER + TRANSFER") {
   ...
   <---R3-----O*------R1-----                         <---R3-----O<-----R1--------------R1-------
     ...
     val roadMaintainer = ArealRoadMaintainer.getEVK(10)
     val user = "silari"
     ...
     val newRoadwaysFor19510 = Seq(
       Roadway(Sequences.nextRoadwayId, newRoadwayNumber1, RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous,AddrMRange(0, 5), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), roadMaintainer, TerminationCode.NoTermination, DateTime.now(), None),
       Roadway(Sequences.nextRoadwayId, newRoadwayNumber2,      RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(5,15), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), roadMaintainer, TerminationCode.NoTermination, DateTime.now(), None)
     )

     val newRoadwaysFor19527 = Seq(
       Roadway(Sequences.nextRoadwayId, newRoadwayNumber3,      RoadPart(19527, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0, 10), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), roadMaintainer, TerminationCode.NoTermination, DateTime.now(), None)
     )
     ...
     val roadwayChanges = List(
       ProjectRoadwayChange(
         projectId, Some(projectName), roadMaintainer, user, DateTime.now(),
         RoadwayChangeInfo(
           RoadAddressChangeType.New,
           RoadwayChangeSection(None, None, None, None, None, Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Option(ArealRoadMaintainer.getEVK(10))),
           RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(0L, 5L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Option(ArealRoadMaintainer.getEVK(10))),
           Discontinuity.Continuous, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer.getEVK(10)),
         date),
       ProjectRoadwayChange(
         projectId, Some(projectName), roadMaintainer, user, DateTime.now(),
         RoadwayChangeInfo(
           RoadAddressChangeType.Transfer,
           RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(0L, 10L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Option(ArealRoadMaintainer.getEVK(10))),
           RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(5L, 15L)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Option(ArealRoadMaintainer.getEVK(10))),
           Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer.getEVK(10)),
         date),
       ProjectRoadwayChange(
         projectId, Some(projectName), ArealRoadMaintainer.getEVK(10), user, DateTime.now(),
         RoadwayChangeInfo(
           RoadAddressChangeType.Transfer,
           RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(10, 20)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Option(ArealRoadMaintainer.getEVK(10))),
           RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange( 0, 10)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Option(ArealRoadMaintainer.getEVK(10))),
           Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer.getEVK(10)),
         date)
     )

     when(mockRoadwayDAO.fetchAllBySectionAndTracks(RoadPart(19510, 1), Set(Track.Combined))).thenReturn(newRoadwaysFor19510)
     when(mockRoadwayDAO.fetchAllBySectionAndTracks(RoadPart(19527, 1), Set(Track.Combined))).thenReturn(newRoadwaysFor19527)
 ...
 test("Test handleRoadwayPoints When dual roadway point is no longer dual roadway point then addrM's and roadway number in that roadway point should change correctly and new roadway point should be created UNCHANGED + TRANSFER") {
   ...
   <---R3-----O*------R1-----                         <---R3-----O<-----R1-----
     ...
     val newRoadwaysFor19510 = Seq(
       Roadway(Sequences.nextRoadwayId, originalRoadwayNumber0, RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, 5), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ArealRoadMaintainer.getEVK(10), TerminationCode.NoTermination, DateTime.now(), None),
       Roadway(Sequences.nextRoadwayId, newRoadwayNumber1,      RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(5,10), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ArealRoadMaintainer.getEVK(10), TerminationCode.NoTermination, DateTime.now(), None)
     )

     val newRoadwaysFor19527 = Seq(
       Roadway(Sequences.nextRoadwayId, newRoadwayNumber2,      RoadPart(19527, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, 3), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ArealRoadMaintainer.getEVK(10), TerminationCode.NoTermination, DateTime.now(), None),
       Roadway(Sequences.nextRoadwayId, originalRoadwayNumber2, RoadPart(19527, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(3,13), reversed=false, DateTime.now(), None, "silari", Some("testRoadName2"), ArealRoadMaintainer.getEVK(10), TerminationCode.NoTermination, DateTime.now(), None)
     )
     ...
     val roadwayChanges = List(
       ...
       ProjectRoadwayChange(
         ...
         RoadwayChangeInfo(
           RoadAddressChangeType.Unchanged,
           RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(5L, 10L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Option(ArealRoadMaintainer.getEVK(10))),
           RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(5L, 10L)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Option(ArealRoadMaintainer.getEVK(10))),
           Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer.getEVK(10)),
         date),
       ProjectRoadwayChange(
         projectId, Some(projectName), roadMaintainer, user, DateTime.now(),
         RoadwayChangeInfo(
           RoadAddressChangeType.Transfer,
           RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(10, 13)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Option(ArealRoadMaintainer.getEVK(10))),
           RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange( 0,  3)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous),  Option(ArealRoadMaintainer.getEVK(10))),
           Discontinuity.Continuous, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer.getEVK(10)),
         date),
       ProjectRoadwayChange(
         projectId, Some(projectName), roadMaintainer, user, DateTime.now(),
         RoadwayChangeInfo(
           RoadAddressChangeType.Transfer,
           RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange(0, 10)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Option(ArealRoadMaintainer.getEVK(10))),
           RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange(3, 13)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Option(ArealRoadMaintainer.getEVK(10))),
           Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer.getEVK(10)),
         date)
     )
 ...
 test("Test handleRoadwayPoints When dual roadway point is handled") {
   runWithRollback {
     ...
     val projectId = Sequences.nextViiteProjectId
     val rap =  dummyProject(projectId, ProjectState.Incomplete, Seq(), None).copy(startDate = DateTime.parse("2019-10-10"))
     val link1 = dummyProjectLink(RoadPart(99, 1), Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0,  5), Some(DateTime.now()), None, 12345.toString, 0,  5, SideCode.TowardsDigitizing, RoadAddressChangeType.Unchanged, projectId, AdministrativeClass.State, geom1, roadwayNumber2)
     val link2 = dummyProjectLink(RoadPart(99, 2), Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0, 15), Some(DateTime.now()), None, 12346.toString, 0, 15, SideCode.TowardsDigitizing, RoadAddressChangeType.Transfer,  projectId, AdministrativeClass.State, geom2, roadwayNumber3)

     //Roadways and linear location generated AFTER changes
     val (lc1, rw1): (LinearLocation, Roadway) = Seq(link1).map(toRoadwayAndLinearLocation).head
     val (lc2, rw2): (LinearLocation, Roadway) = Seq(link2).map(toRoadwayAndLinearLocation).head
     val rw1WithId = rw1.copy(id = Sequences.nextRoadwayId, roadMaintainer = ArealRoadMaintainer.getEVK(8))
     val rw2WithId = rw2.copy(id = Sequences.nextRoadwayId, roadMaintainer = ArealRoadMaintainer.getEVK(8))
     ...
     val ra1 = RoadAddress(0, lc1WithId.id, rw1WithId.roadPart, rw1WithId.administrativeClass, rw1WithId.track, rw1WithId.discontinuity, rw1WithId.addrMRange, Some(rw1WithId.startDate), rw1WithId.endDate, Some(rw1WithId.createdBy), lc1WithId.linkId, lc1WithId.startMValue, lc1WithId.endMValue, lc1WithId.sideCode, 1000000, (None, None), lc1WithId.geometry, lc1WithId.linkGeomSource, rw1WithId.roadMaintainer, NoTermination, rw1WithId.roadwayNumber, None, None, None)

     val ra2 = RoadAddress(0, lc2WithId.id, rw2WithId.roadPart, rw2WithId.administrativeClass, rw2WithId.track, rw2WithId.discontinuity, rw2WithId.addrMRange, Some(rw2WithId.startDate), rw2.endDate, Some(rw2.createdBy), lc2WithId.linkId, lc2WithId.startMValue, lc2WithId.endMValue, lc2WithId.sideCode, 1000000, (None, None), lc2WithId.geometry, lc2WithId.linkGeomSource, rw2WithId.roadMaintainer, NoTermination, rw2WithId.roadwayNumber, None, None, None)
     ...
     projectReservedPartDAO.reserveRoadPart(projectId, RoadPart(99, 1), "u")
     projectReservedPartDAO.reserveRoadPart(projectId, RoadPart(99, 2), "u")
     projectLinkDAO.create(pls.map(_.copy(id = NewIdValue)))
     ...
     projectService.updateProjectLinks(projectId, Set(), Seq(projectLinks.head.linkId), RoadAddressChangeType.Unchanged, "-", RoadPart(99, 1), 0, Option.empty[Int])
     projectService.updateProjectLinks(projectId, Set(), Seq(projectLinks.last.linkId), RoadAddressChangeType.Transfer,  "-", RoadPart(99, 2), 0, Option.empty[Int])
     ...
     when(mockRoadwayDAO.fetchAllBySectionAndTracks(any[RoadPart], any[Set[Track]])).thenReturn(Seq(rw1WithId, rw2WithId))
     val t1 = roadwayChanges.last.changeInfo.source.copy(addrMRange = Some(AddrMRange(5, 20)), startRoadPartNumber = Some(1), endRoadPartNumber = Some(1))
     ...
     roadwayPointsBeforeDual.head.addrMValue should be (beforeDualPoint.addrMRange.start)
     roadwayPointsBeforeDual.last.addrMValue should be (beforeDualPoint.addrMRange.end)

     roadwayPointsAfterDual.size should be (2)
     roadwayPointsAfterDual.head.addrMValue should be (afterDualPoint.addrMRange.start)
     roadwayPointsAfterDual.last.addrMValue should be (afterDualPoint.addrMRange.end)
   }
 }
}
