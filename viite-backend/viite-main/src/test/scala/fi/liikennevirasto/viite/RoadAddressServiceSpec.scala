...
//    when(mockRoadLinkService.getComplementaryRoadLinksFromVVH(99999)).thenReturn(Seq())
  ...
  test("Test handleRoadwayPoints When Terminating one link and Transfer the rest Then roadway points should be handled/created properly") {
    runWithRollback {
      ...
      val rw1WithId = rw1.copy(id = id1, arealRoadMaintainer = ArealRoadMaintainer("ELY8"))
      val rw2WithId = rw2.copy(id = id2, arealRoadMaintainer = ArealRoadMaintainer("ELY8"))
      val lc2WithId = lc2
  ...
  test("Test handleRoadwayPoints When dual roadway point is no longer dual roadway point then addrM's and roadway number in that roadway point should change correctly and new roadway point should be created NEW + TRANSFER + TRANSFER") {
    ...
    <---R3-----O*------R1-----                         <---R3-----O<-----R1--------------R1-------
      ...
      val newRoadwaysFor19510 = Seq(
        Roadway(Sequences.nextRoadwayId, newRoadwayNumber1, RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous,AddrMRange(0, 5), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ely14, TerminationCode.NoTermination, DateTime.now(), None),
        Roadway(Sequences.nextRoadwayId, newRoadwayNumber2, RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(5,15), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ely14, TerminationCode.NoTermination, DateTime.now(), None)
      )

      val newRoadwaysFor19527 = Seq(
        Roadway(Sequences.nextRoadwayId, newRoadwayNumber3, RoadPart(19527, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0, 10), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ely14, TerminationCode.NoTermination, DateTime.now(), None)
      )
      ...
      val roadwayChanges = List(
        ProjectRoadwayChange(
          projectId, Some(projectName), ely14, user, DateTime.now(),
          RoadwayChangeInfo(
            RoadAddressChangeType.New,
            RoadwayChangeSection(None,        None,    None,    None,    None,                  Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(ely14)),
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(0,5)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(ely14)),
            Discontinuity.Continuous, AdministrativeClass.State, reversed=false, 4481L, ely14),
          date),
        ProjectRoadwayChange(
          projectId, Some(projectName), ely14, user, DateTime.now(),
          RoadwayChangeInfo(
            RoadAddressChangeType.Transfer,
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(0,10)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(ely14)),
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(5,15)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad),  Some(ely14)),
            Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ely14),
          date),
        ProjectRoadwayChange(
          projectId, Some(projectName), ely14, user, DateTime.now(),
          RoadwayChangeInfo(
            RoadAddressChangeType.Transfer,
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(10, 20)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Some(ely14)),
            RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange( 0, 10)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Some(ely14)),
            Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ely14),
          date)
      )
  ...
  test("Test handleRoadwayPoints When dual roadway point is no longer dual roadway point then addrM's and roadway number in that roadway point should change correctly and new roadway point should be created UNCHANGED + TRANSFER") {
    ...
    <---R3-----O*------R1-----                         <---R3-----O<-----R1-----
      ...
      val projectId = Sequences.nextViiteProjectId
      val projectName = "dualRoadwayPoint"
      val ely14 = ArealRoadMaintainer.getELY(14)
      val user = "silari"
      val date = DateTime.parse("2022-05-09")
      ...
      val newRoadwaysFor19510 = Seq(
        Roadway(Sequences.nextRoadwayId, originalRoadwayNumber0, RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, 5), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ely14, TerminationCode.NoTermination, DateTime.now(), None),
        Roadway(Sequences.nextRoadwayId, newRoadwayNumber1,      RoadPart(19510, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(5,10), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"), ely14, TerminationCode.NoTermination, DateTime.now(), None)
      )

      val newRoadwaysFor19527 = Seq(
        Roadway(Sequences.nextRoadwayId, newRoadwayNumber2,      RoadPart(19527, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, 3), reversed=false, DateTime.now(), None, "silari", Some("testRoadName"),  ely14, TerminationCode.NoTermination, DateTime.now(), None),
        Roadway(Sequences.nextRoadwayId, originalRoadwayNumber2, RoadPart(19527, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(3,13), reversed=false, DateTime.now(), None, "silari", Some("testRoadName2"), ely14, TerminationCode.NoTermination, DateTime.now(), None)
      )
      ...
      val roadwayChanges = List(
        ProjectRoadwayChange(
          ...
          RoadwayChangeInfo(
            ...
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(0L, 5L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(ely14)),
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(0L, 5L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(ely14)),
            Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer(s"ELY14")),
          date),
        ProjectRoadwayChange(
          projectId, Some(projectName), ely14, user, DateTime.now(),
          RoadwayChangeInfo(
            RoadAddressChangeType.Unchanged,
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(5L, 10L)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(ely14)),
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(5L, 10L)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad),  Some(ely14)),
            Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer("ELY14")),
          date),
        ProjectRoadwayChange(
          projectId, Some(projectName), ely14, user, DateTime.now(),
          RoadwayChangeInfo(
            RoadAddressChangeType.Transfer,
            RoadwayChangeSection(Some(19510), Some(0), Some(1), Some(1), Some(AddrMRange(10, 13)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad),  Some(ely14)),
            RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange( 0,  3)), Some(AdministrativeClass.State), Some(Discontinuity.Continuous), Some(ely14)),
            Discontinuity.Continuous, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer("ELY14")),
          date),
        ProjectRoadwayChange(
          projectId, Some(projectName), ely14, user, DateTime.now(),
          RoadwayChangeInfo(
            RoadAddressChangeType.Transfer,
            RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange(0, 10)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Some(ely14)),
            RoadwayChangeSection(Some(19527), Some(0), Some(1), Some(1), Some(AddrMRange(3, 13)), Some(AdministrativeClass.State), Some(Discontinuity.EndOfRoad), Some(ely14)),
            Discontinuity.EndOfRoad, AdministrativeClass.State, reversed=false, 4481L, ArealRoadMaintainer("ELY14")),
          date)
      )
  ...
  test("Test handleRoadwayPoints When dual roadway point is handled") {
    runWithRollback {
      ...
      val (lc1, rw1): (LinearLocation, Roadway) = Seq(link1).map(toRoadwayAndLinearLocation).head
      val (lc2, rw2): (LinearLocation, Roadway) = Seq(link2).map(toRoadwayAndLinearLocation).head
      val rw1WithId = rw1.copy(id = Sequences.nextRoadwayId, arealRoadMaintainer = ArealRoadMaintainer("ELY8"))
      val rw2WithId = rw2.copy(id = Sequences.nextRoadwayId, arealRoadMaintainer = ArealRoadMaintainer("ELY8"))
