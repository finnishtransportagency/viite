...
class ProjectServiceSpec extends AnyFunSuite with Matchers with BeforeAndAfter with BaseDAO {
  ...
  private val ely1  = ArealRoadMaintainer.ELYUusimaa
  private val ely4  = ArealRoadMaintainer.ELYPirkanmaa
  ...
  val defaultChangeInfo = RoadwayChangeInfo(
    ...
    RoadwayChangeSection(Option(403), Option(0), Option(8), Option(0), Option(AddrMRange(8,1001)),
      Option(AdministrativeClass.State), Option(Discontinuity.Continuous), Option(ArealRoadMaintainer.ELYLappi)
    ),
  ...
  private def createProjectLinks(linkIds: Seq[String], projectId: Long, roadPart: RoadPart, track: Int, discontinuity: Int, administrativeClass: Int, roadLinkSource: Int, roadARM: ArealRoadMaintainer, user: String, roadName: String) = {
    projectService.createProjectLinks(linkIds, projectId, roadPart, Track.apply(track), Discontinuity.apply(discontinuity), AdministrativeClass.apply(administrativeClass), LinkGeomSource.apply(roadLinkSource), roadARM, user, roadName)
  }

  private def toProjectLink(project: Project, status: RoadAddressChangeType)(roadAddress: RoadAddress): ProjectLink = {
    ProjectLink(id = NewIdValue, roadAddress.roadPart, roadAddress.track, roadAddress.discontinuity,
      AddrMRange(roadAddress.addrMRange.start, roadAddress.addrMRange.end), AddrMRange(roadAddress.addrMRange.start, roadAddress.addrMRange.end),
      roadAddress.startDate, roadAddress.endDate, createdBy = Option(project.createdBy), roadAddress.linkId, roadAddress.startMValue, roadAddress.endMValue,
      ...
      AdministrativeClass.State, roadAddress.linkGeomSource, GeometryUtils.geometryLength(roadAddress.geometry), if (status == RoadAddressChangeType.New) 0 else roadAddress.id,
      if (status == RoadAddressChangeType.New) 0 else roadAddress.linearLocationId, roadAddress.arealRoadMaintainer, reversed = false, None, roadAddress.adjustedTimestamp)
  }
  ...
  private def setUpProjectWithLinks(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roadPart: RoadPart = RoadPart(19999, 1),
    ...
    def projectLink(addrMRange: AddrMRange, track: Track, projectId: Long, status: RoadAddressChangeType = RoadAddressChangeType.NotHandled, roadPart: RoadPart = RoadPart(19999, 1), discontinuity: Discontinuity = Discontinuity.Continuous, arealRoadMaintainer: ArealRoadMaintainer = ely8, linkId: String = 0L.toString, roadwayId: Long = 0L, linearLocationId: Long = 0L, startDate: Option[DateTime] = None) = {
    ...
    def withTrack(t: Track): Seq[ProjectLink] = {
      addrM.init.zip(addrM.tail).map { case (st, en) =>
        projectLink(AddrMRange(st, en), t, id, roadAddressChangeType, roadPart, discontinuity, ArealRoadMaintainer.getELY(ely), roadwayId = roadwayId, startDate = startDate)
      }
    }
  ...
  test("Test saveProject When two projects with same road part Then throw exception on unique key constraint violation") {
    runWithRollback {
      ...
      val addr1 = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(5, 207), Some(0L), Some(Discontinuity.Continuous), Some(ely8), None, None, None, None))
  ...
  test("Test saveProject When new part is added Then return project with new reservation") {
    ...
    runWithRollback {
      ...
      val addresses: List[ProjectReservedPart] = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, RoadPart(5, 207), Some(8L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None))
  ...
  test("Test checkRoadPartsExist When roadway exist Then return None") {
    runWithRollback {
      ...
      val ra = Seq(Roadway(id1, roadwayNumber, RoadPart(roadNumber, roadStartPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("test road"), ArealRoadMaintainer(s"ELY1")))
  ...
  test("Test checkRoadPartsExist When start road way does not exist and end exists then return error message") {
    runWithRollback {
      ...
      val roadwayNumber = 8000
      val id1 = Sequences.nextRoadwayId
      val ra = Seq(Roadway(id1, roadwayNumber, RoadPart(roadNumber, roadEndPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("test road"), ArealRoadMaintainer(s"ELY1")))
      roadwayDAO.create(ra)
      val check = projectService.checkRoadPartsExist(roadNumber, roadStartPart, roadEndPart)
      check should be(Some(ErrorStartingRoadPartNotFound))
    }
  ...
  test("Test validateReservations When road parts don't exist Then return error") {
    runWithRollback {
      ...
      val reservedPart = ProjectReservedPart(0L, roadPart, Some(1000L), Some(Discontinuity.Continuous), Some(ely8))
  ...
  test("Test checkRoadPartsReservable When road can Then return on right part 2") {
    ...
    runWithRollback {
      val id1 = Sequences.nextRoadwayId
      val ra = Seq(Roadway(id1, roadwayNumber, RoadPart(roadNumber, roadStartPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("test road"), ArealRoadMaintainer("ELY8")))
      ...
      val rb = Seq(Roadway(id2, roadwayNumber, RoadPart(roadNumber, roadEndPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("Test road 2"), ArealRoadMaintainer("ELY8")))
  ...
  test("Test checkRoadPartsReservable When road part is already reserved to the current project Then should return reserved info.") {
    ...
    runWithRollback {
      roadwayDAO.create(
                        Seq(Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(roadNumber, roadStartPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, testUser, Some("test road"),   ArealRoadMaintainer("ELY8")),
                            Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(roadNumber, roadEndPart),   AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, testUser, Some("Test road 2"), ArealRoadMaintainer("ELY8")))
                       )
  ...
  test("Test getRoadAddressAllProjects When project is created Then return project") {
    ...
    runWithRollback {
      ...
      projectService.saveProject(project.copy(reservedParts = Seq(ProjectReservedPart(0L, RoadPart(5, 207), Some(0L), Some(Discontinuity.Continuous), Some(ely8), None, None, None, None)), elys = Set()))
      val countAfterInsertProjects = projectService.getAllProjects
      count = countCurrentProjects.size + 1
      countAfterInsertProjects.size should be(count)
    }
  ...
  test("Test projectService.saveProject When after project creation we reserve roads to it Then return the count of created project should be increased AND Test projectService.getAllProjects When not specifying a projectId Then return the count of returned projects should be 0 ") {
    ...
    runWithRollback {
      ...
      projectService.saveProject(project.copy(reservedParts = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, RoadPart(1130, 4), Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.ELYUusimaa), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None)), elys = Set()))
      val countAfterInsertProjects = projectService.getAllProjects
      count = countCurrentProjects.size + 1
      countAfterInsertProjects.size should be(count)
    }
  ...
  test("Test projectService.createRoadLinkProject(), projectService.deleteProject When a user wants to create a new, simple project and then delete it Then return the number of existing projects should increase by 1 and then decrease by 1 as well") {
    ...
    runWithRollback {
      ...
      projectService.saveProject(project.copy(reservedParts = Seq(ProjectReservedPart(0L, RoadPart(5, 203), Some(100L), Some(Discontinuity.Continuous), Some(ely8), None, None, None, None)), elys = Set()))
      val countAfterInsertProjects = projectService.getAllProjects
  ...
  test("Test projectService.validateProjectDate() When evaluating project links with start date are equal as the project start date Then return no error message") {
    runWithRollback {
      ...
      val addresses = List(ProjectReservedPart(5: Long, RoadPart(5, 205), Some(5L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None))
  ...
  test("Test projectService.getSingleProjectById() When after creation of a new project, with reserved parts Then return only 1 project whose name is the same that was defined. ") {
    ...
    runWithRollback {
      val countCurrentProjects = projectService.getAllProjects
      val addresses: List[ProjectReservedPart] = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, RoadPart(5, 206), Some(5L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None))
  ...
  test("Test getRoadAddressSingleProject When project with reserved parts is created Then return the created project") {
    ...
    runWithRollback {
      ...
      val ra = Seq(Roadway(id, roadwayNumber, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "Tester", Option("test name"), ArealRoadMaintainer("ELY8")))
      ...
      val project = Project(projectId, ProjectState.Incomplete, "testiprojekti", "Test", DateTime.now(), "Test", DateTime.now(), DateTime.now(), "info", List(
        ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, roadPart: RoadPart, Some(5L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None)), Seq(), None, elys = Set())
...
//    Project1 projectlink is connected to Project2 projectlink at point 1000, 10 and there is no junction which means that Project2 projectlink cannot be created with new action
    runWithRollback {
      val id = Sequences.nextViiteProjectId
      val ra1 = Seq(
        RoadAddress(12345, linearLocationId,     RoadPart(19999, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L,  5L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1000.toString, 0, 5, SideCode.TowardsDigitizing,  DateTime.now().getMillis, (None, None), Seq(Point(1000.0,  0.0), Point(1000.0,  5.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, roadwayNumber1, Some(DateTime.parse("1901-01-01")), None, None),
        RoadAddress(12346, linearLocationId + 1, RoadPart(19999, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(5L, 10L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1001.toString, 0, 5, SideCode.TowardsDigitizing,  DateTime.now().getMillis, (None, None), Seq(Point(1000.0,  5.0), Point(1000.0, 10.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, roadwayNumber2, Some(DateTime.parse("1901-01-01")), None, None)
      )
      val ra2 = Seq(
        RoadAddress(12347, linearLocationId + 2, RoadPart(12345, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(10L, 15L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1002.toString, 0, 5, SideCode.TowardsDigitizing, DateTime.now().getMillis, (None, None), Seq(Point(1000.0, 10.0), Point(1000.0, 25.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, roadwayNumber3, Some(DateTime.parse("1901-01-01")), None, None),
        RoadAddress(12348, linearLocationId + 3, RoadPart(12345, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(15L, 20L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1003.toString, 0, 5, SideCode.TowardsDigitizing, DateTime.now().getMillis, (None, None), Seq(Point(1000.0, 25.0), Point(1000.0, 30.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, roadwayNumber4, Some(DateTime.parse("1901-01-01")), None, None)
      )
      ...
      val response1 = projectService.createProjectLinks(Seq(1000L.toString, 1001L.toString), rap1.id, RoadPart(56, 207), Track.Combined, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ely8, "test", "road name")
      ...
      val response2 = projectService.createProjectLinks(Seq(1002L.toString,1003L.toString), rap2.id, RoadPart(55, 206), Track.Combined, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ely8, "test", "road name")
  ...
  test("Test addNewLinksToProject When reserving part already used in other project Then return error message") {
    runWithRollback {
      ...
      val projectLink = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(idr, 123, RoadPart(5, 207), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 10L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 9.8, SideCode.TowardsDigitizing, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0, 9.8)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0))
      projectDAO.create(rap)
      projectService.saveProject(rap)


      val rap2 = Project(id + 1, ProjectState.apply(1), "TestProject2", "TestUser", DateTime.parse("2700-01-01"), "TestUser", DateTime.now(), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, elys = Set())
      val projectLink2 = toProjectLink(rap2, RoadAddressChangeType.New)(RoadAddress(idr, 1234, RoadPart(5, 999), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 10L), Some(DateTime.parse("1901-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 9.8, SideCode.TowardsDigitizing, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0, 9.8)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0))
      projectDAO.create(rap2)
      projectService.saveProject(rap2)
      ...
      val p = ProjectAddressLink(idr, projectLink.linkId, projectLink.geometry, 1, AdministrativeClass.apply(1), LifecycleStatus.apply(1), projectLink.linkGeomSource, AdministrativeClass.State, None, 111, "Heinola", Some(""), Some(modifiedBy), projectLink.roadPart, 2,
        ARMInvalid, projectLink.discontinuity.value, projectLink.addrMRange, projectLink.startMValue, projectLink.endMValue, projectLink.sideCode, calibrationPoints._1, calibrationPoints._2, projectLink.status, 12345, 123456, sourceId = "", originalAddrMRange = Some(projectLink.originalAddrMRange))
  ...
  test("Test createProjectLinks When adding new links to a new road and roadpart Then discontinuity should be on correct link.") {
    def runTest(reversed: Boolean) = {
      ...
      val projectLinks   = Seq(
        toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 123, roadPart, AdministrativeClass.Unknown, Track.Combined, continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0,  9.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0,  9.8)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0)),
        toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 124, roadPart, AdministrativeClass.Unknown, Track.Combined, continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 19.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 9.8), Point(9.8, 29.6)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0))
      )
      projectDAO.create(rap)
      projectService.saveProject(rap)
      ...
      val message1project1 = projectService.createProjectLinks(longs, rap.id, roadPart, Track.Combined, discontinuity, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ely8, user, "new road")
  ...
  test("Test addNewLinksToProject When adding new links to a new road and roadpart Then values should be correct.") {
    runWithRollback {
      ...
      val user           = "TestUser"
      val rap            = Project(projectId, ProjectState.apply(1), "TestProject", user, DateTime.parse("2700-01-01"), user, DateTime.parse("1972-03-03"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, elys = Set())
      val projectLink1   = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 123, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0,  9.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0,  9.8)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0))
      val projectLink2   = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 124, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 19.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 9.8), Point(9.8, 29.6)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0))
      projectDAO.create(rap)
      projectService.saveProject(rap)
      projectReservedPartDAO.reserveRoadPart(projectId, roadPart, user)
  ...
  test("Test addNewLinksToProject When adding new links to existing road and new roadpart Then values should be correct.") {
    runWithRollback {
      ...
      val rap = Project(projectId, ProjectState.apply(1), "TestProject", user, DateTime.parse("2700-01-01"), user, DateTime.parse("1972-03-03"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, elys = Set())
      val projectLinks = Seq(
       toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 125, newRoadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 10.4, SideCode.Unknown, 0, (None, None), Seq(Point(532427.945,6998488.475,0.0),Point(532395.164,6998549.616,0.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0)),
       toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 126, newRoadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 10.0, SideCode.Unknown, 0, (None, None), Seq(Point(532427.945,6998488.475,0.0),Point(532495.0,  6998649.0,  0.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0))
      )
      projectDAO.create(rap)
      projectService.saveProject(rap)
      projectReservedPartDAO.reserveRoadPart(projectId, roadPart,    user)
  ...
  test("Test addNewLinksToProject When adding new links to existing road and existing roadpart Then values should be correct.") {
    runWithRollback {
      ...
      val projectLinks = Seq(
        toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 125, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 10.4, SideCode.Unknown, 0, (None, None), Seq(Point(0,20),    Point(0,30,0.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0)),
        toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 126, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 10.0, SideCode.Unknown, 0, (None, None), Seq(Point(0.0,30.0),Point(0.0,40.0)), LinkGeomSource.NormalLinkInterface, ely8, NoTermination, 0))
      )
      projectDAO.create(rap)
  ...
  test("Test projectService.fetchPreFillData " +
    ...
    runWithRollback {
      ...
      projectService.fetchPreFillData(projectLinks.head.linkId, roadAddressProject.id) should be(Right(PreFillInfo(roadPart.roadNumber, roadPart.partNumber, "", RoadNameSource.UnknownSource, projectLinks.head.arealRoadMaintainer.number)))
    }
  }
  ...
  test("Test projectService.fetchPreFillData " +
    ...
    runWithRollback {
      ...
      val projectLinks = roadwayAddressMapper.getRoadAddressesByRoadway(roadwayDAO.fetchAllByRoadPart(roadPart)).map(toProjectLink(roadAddressProject))
      projectLinkDAO.create(projectLinks)
      projectService.fetchPreFillData(projectLinks.head.linkId, roadAddressProject.id) should be(Right(PreFillInfo(roadPart.roadNumber, roadPart.partNumber, "road name test", RoadNameSource.RoadAddressSource, projectLinks.head.arealRoadMaintainer.number)))
    }
  }
  ...
  test("Test projectService.fetchPreFillData() " +
    ...
    runWithRollback{
      ...
      val projectLinks = roadwayAddressMapper.getRoadAddressesByRoadway(roadwayDAO.fetchAllByRoadPart(roadPart)).map(toProjectLink(roadAddressProject))
      projectLinkDAO.create(projectLinks)
      projectService.fetchPreFillData(projectLinks.head.linkId, roadAddressProject.id) should be(Right(PreFillInfo(roadPart.roadNumber, roadPart.partNumber, "TestRoadName_Project_Link", RoadNameSource.ProjectLinkSource, projectLinks.head.arealRoadMaintainer.number)))
    }
  }
  ...
  test("Test projectService.fetchPreFillData() " +
    ...
    runWithRollback{
      ...
      val projectLinks = roadwayAddressMapper.getRoadAddressesByRoadway(roadwayDAO.fetchAllByRoadPart(roadPart)).map(toProjectLink(roadAddressProject))
      projectLinkDAO.create(projectLinks)
      projectService.fetchPreFillData(projectLinks.head.linkId, roadAddressProject.id) should be(Right(PreFillInfo(roadPart.roadNumber,
        roadPart.partNumber, "road name test", RoadNameSource.RoadAddressSource, projectLinks.head.arealRoadMaintainer.number)))
    }
  }
  ...
  test("Test projectService.updateProjectLinks When applying the operation \"Numerointi\" to a road part that is ALREADY reserved in a different project Then return an error message") {
    runWithRollback {
      ...
      val addr1 = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(5, 207), Some(0L), Some(Discontinuity.Continuous), Some(ely8), None, None, None, None))
      val addr2 = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(5, 206), Some(5L), Some(Discontinuity.Continuous), Some(ely8), None, None, None, None))
      val project1 = projectService.createRoadLinkProject(rap1)
      ...
      val response = projectService.updateProjectLinks(project1.id, Set(), projectLinkDAO.fetchProjectLinks(project1.id).map(_.linkId), RoadAddressChangeType.Renumeration, "TestUser", RoadPart(5, 206), 0, None, AdministrativeClass.State.value, Discontinuity.Continuous.value, Some(ely8))
  ...
  test("Test projectService.updateProjectLinks When applying the operation \"Numerointi\" to a road part that is ALREADY reserved in a different project Then return an error message" +
    ...
    runWithRollback {
      ...
      mockForProject(project1.id, roadAddressServiceRealRoadwayAddressMapper.getRoadAddressWithRoadPart(RoadPart(5, 207)).map(toProjectLink(project1)))
      projectService.saveProject(project1.copy(reservedParts = addr1, elys = Set()))
      val response = projectService.updateProjectLinks(project1.id, Set(), projectLinkDAO.fetchProjectLinks(project1.id).map(_.linkId), RoadAddressChangeType.Renumeration, "TestUser", RoadPart(5, 203), 0, None, AdministrativeClass.State.value, Discontinuity.Continuous.value, Some(ely8))
      response.get should be("Antamasi tienumero ja tieosanumero ovat jo käytössä. Tarkista syöttämäsi tiedot.")
    }
  }
  ...
  test("Test projectService.updateProjectLinks When applying the operation \"Numerointi\" to a road part that already has some other action applied Then return an error message") {
    runWithRollback {
      ...
      mockForProject(project1.id, roadAddressServiceRealRoadwayAddressMapper.getRoadAddressWithRoadPart(RoadPart(5, 207)).map(toProjectLink(project1)))
      projectService.saveProject(project1.copy(reservedParts = addr1, elys = Set()))
      projectService.updateProjectLinks(project1.id, Set(), projectLinkDAO.fetchProjectLinks(project1.id).map(_.linkId), RoadAddressChangeType.Termination, "TestUser", RoadPart(5, 207), 0, None, AdministrativeClass.State.value, Discontinuity.Continuous.value, Some(ely8))
      val response = projectService.updateProjectLinks(project1.id, Set(), projectLinkDAO.fetchProjectLinks(project1.id).map(_.linkId), RoadAddressChangeType.Renumeration, "TestUser", RoadPart(5, 308), 0, None, AdministrativeClass.State.value, Discontinuity.Continuous.value, Some(ely8))
  ...
  test("Test projectLinkDAO.getProjectLinks() When after the \"Numerointi\" operation on project links of a newly created project Then the last returned link should have a discontinuity of \"EndOfRoad\", all the others should be \"Continuous\" ") {
    runWithRollback {
      ...
      projectService.updateProjectLinks(project1.id, Set(fetchedProjectLinks.maxBy(_.addrMRange.end).id), fetchedProjectLinks.map(_.linkId), RoadAddressChangeType.Renumeration, "TestUser", RoadPart(6, 207), 0, None, AdministrativeClass.State.value, Discontinuity.EndOfRoad.value, Some(ely8))
...
//    }
  ...
  test("Test projectService.changeDirection() When after the creation of valid project links on a project Then the discontinuity of road addresses that are not the same road number and road part number should not be altered.") {
    runWithRollback {
      ...
      projectService.createProjectLinks(Seq(12346L.toString, 12347L.toString), project.id, RoadPart(9999, 1), Track.LeftSide, Discontinuity.Continuous, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ely8, "test", "road name")
      when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(Seq(pl5).map(toRoadLink))
      projectService.createProjectLinks(Seq(12349L.toString), project.id, RoadPart(9998, 1), Track.Combined,  Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ely8, "test", "road name")
    ...
    "Then the mAddressValues of the last links should be equal in both sides of the tracks for part 1.")
  {
    runWithRollback {
      ...
      projectService.saveProject(project.copy(reservedParts = Seq(
        ProjectReservedPart(0L, RoadPart(9999, 1), null, Some(Discontinuity.Continuous), Some(ely8), None, None, None, None),
        ProjectReservedPart(0L, RoadPart(9999, 2), null, Some(Discontinuity.Continuous), Some(ely8), None, None, None, None)), elys = Set())
      )
...
//        */
      ...
      val ely1 = ArealRoadMaintainer.EVKUusimaa
      projectService.updateProjectLinks(id, part1AdjacentToPart2LinkRightSide, Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 1, None, 1, 5, Some(ely1), reversed = false, None)
      projectService.updateProjectLinks(id, part1AdjacentToPart2LinkLeftSide,  Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 2, None, 1, 5, Some(ely1), reversed = false, None)
      ...
      projectService.updateProjectLinks(id, part2track1Links, Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 1, None, 1, 5, Some(ely1), reversed = false, None)
      projectService.updateProjectLinks(id, part2Track2Links, Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 2, None, 1, 5, Some(ely1), reversed = false, None)
    ...
    "Then the mAddressValues of the last links should be equal in both sides of the tracks for part 1." )
  {
    runWithRollback {
      ...
      projectService.saveProject(project.copy(reservedParts = Seq(
        ProjectReservedPart(0L, RoadPart(9999, 1), null, Some(Discontinuity.Continuous), Some(ely8), None, None, None, None),
        ProjectReservedPart(0L, RoadPart(9999, 2), null, Some(Discontinuity.Continuous), Some(ely8), None, None, None, None)
      ), elys = Set()))
      ...
      projectService.updateProjectLinks(id, part2track1Links, Seq(), RoadAddressChangeType.Transfer, "test",
        newRoadPart = RoadPart(9999, 2), newTrackCode = 1, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, arealRoadMaintainer = Some(ely1), roadName = None)
      projectService.updateProjectLinks(id, part2Track2Links, Seq(), RoadAddressChangeType.Transfer, "test",
        newRoadPart = RoadPart(9999, 2), newTrackCode = 2, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, arealRoadMaintainer = Some(ely1), roadName = None)
      /**
        * Tranfering adjacents of part1 to part2
        */
      val part1AdjacentToPart2IdRightSide = Set(12347L.toString)
      ...
      projectService.updateProjectLinks(id, part1AdjacentToPart2LinkRightSide, Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart = RoadPart(9999, 2), newTrackCode = 1, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, arealRoadMaintainer = Some(ely1), roadName = None)
      projectService.updateProjectLinks(id, part1AdjacentToPart2LinkLeftSide,  Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart = RoadPart(9999, 2), newTrackCode = 2, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, arealRoadMaintainer = Some(ely1), roadName = None)
  ...
  test("Test expireHistoryRows When link is renumbered Then set end date to old roadway") {
    runWithRollback {
      ...
      val newEndAddr = 110
      val projectLink = ProjectLink(Sequences.nextProjectLinkId, roadway.roadPart, roadway.track, roadway.discontinuity, AddrMRange(roadway.addrMRange.start, roadway.addrMRange.end + 10), roadway.addrMRange,
        Some(DateTime.now().plusMonths(1)), None, Some("test"), linkId, 0.0, newLength, SideCode.TowardsDigitizing, (RoadAddressCP, RoadAddressCP), (NoCP, NoCP), Seq(Point(0.0, 0.0), Point(0.0, newLength)), projectId,
        RoadAddressChangeType.Renumeration, roadway.administrativeClass, LinkGeomSource.NormalLinkInterface, newLength, roadway.id, 1234, roadway.arealRoadMaintainer, reversed = false, None,
        DateTime.now().minusMonths(10).getMillis, roadway.roadwayNumber, roadway.roadName, Some(roadAddressLength), Some(0), Some(newEndAddr), Some(roadway.track), Some(roadway.roadPart))
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1)
      )

      val changes = List(
        ProjectRoadwayChange(projectId, Some("test project"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos.head, DateTime.now())
      )
  ...
  test("Test fillRoadNames When creating one project link for one new road name Then method should get the road name of the created project links") {
    ...
    runWithRollback {
      ...
      projectService.saveProject(project.copy(reservedParts = Seq(ProjectReservedPart(0L, roadPart, Some(0L), Some(Discontinuity.Continuous), Some(ely8), None, None, None, None)), elys = Set()))
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.New,
          ...
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1),
        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(RoadPart(55555, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(RoadPart(55555, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1)
      )

      val changes = List(
        ProjectRoadwayChange(projectId, Some("test project"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos.head, DateTime.now()),
        ProjectRoadwayChange(projectId, Some("test project"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos(1), DateTime.now())
      )
  ...
  test("Test getProjectLinks When doing some operations (Unchanged with termination test, repeats termination update), Then the calibration points are cleared and moved to correct positions") {
    ...
    runWithRollback {
      ...
      val countCurrentProjects = projectService.getAllProjects
      val id = 0
      val addresses = Seq(ProjectReservedPart(5: Long, RP205, Some(5L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None),
        ProjectReservedPart(5: Long, RP206, Some(5L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None))
  ...
  test("Test getProjectLinks When doing some operations (Transfer and then Terminate), Then the calibration points are cleared and moved to correct positions") {
    ...
    runWithRollback {
      val countCurrentProjects = projectService.getAllProjects
      val id = 0
      val addresses = List(ProjectReservedPart(5: Long, RP207, Some(5L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None))
  ...
  test("Test getProjectLinks When doing some operations (Terminate then transfer), Then the calibration points are cleared and moved to correct positions") {
    ...
    runWithRollback {
      val countCurrentProjects = projectService.getAllProjects
      val id = 0
      val addresses = List(ProjectReservedPart(5: Long, RoadPart(5, 207), Some(5L), Some(Discontinuity.Continuous), Some(ely8), newLength = None, newDiscontinuity = None, newArealRoadMaintainer = None))
      val project = Project(id, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1970-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, elys = Set())
      val savedProject = projectService.createRoadLinkProject(project)
  ...
  test("Test handleNewRoadNames - Test if a new RoadName is created from a project link and without duplicates") {
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(  0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1),

        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 2),

        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
      ...
      val changes = List(
        ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos.head, projectStartTime),
        ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos(1), projectStartTime),
        ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos(2), projectStartTime)
      )
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.Termination,
          source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(0L),   Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1),

        RoadwayChangeInfo(RoadAddressChangeType.Unchanged,
          source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 2),

        RoadwayChangeInfo(RoadAddressChangeType.Unchanged,
          source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 3)
      )
      ...
      val changes = List(
        ProjectRoadwayChange(0L, Some("projectName"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos.head, projectStartTime),
        ProjectRoadwayChange(0L, Some("projectName"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos(1), projectStartTime),
        ProjectRoadwayChange(0L, Some("projectName"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos(2), projectStartTime)
      )
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.Transfer,
          source = dummyRoadwayChangeSection(Some(RoadPart(srcRoadNumber,    1)), Some(0L), Some(0L),   Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          target = dummyRoadwayChangeSection(Some(RoadPart(targetRoadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer("ELY8"))),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1)
      )

      val changes = List(
        ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer("ELY8"), "Test", DateTime.now(), changeInfos.head, project.startDate)
      )
  ...
  test("Test handleTransferAndRenumeration: Transfer to existing roadway - If source roadway exists expire its roadname") {
    runWithRollback {
      ...
      val roadways = List(
        Roadway( 0L, 0L, RoadPart(srcRoadNumber,    0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), reversed = false, DateTime.now, Some(DateTime.now), "dummy", None, ARMInvalid, NoTermination), // TODO VIITE-3416 invalid works?
        Roadway(-1L, 0L, RoadPart(targetRoadNumber, 0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), reversed = false, DateTime.now, Some(DateTime.now), "dummy", None, ARMInvalid, NoTermination)  // TODO VIITE-3416 invalid works?
      )
      roadwayDAO.create(roadways)
  ...
  test("Test handleTransferAndRenumeration: Renumeration to existing roadway - If source roadway exists expire its roadname") {
    runWithRollback {
      ...
      val roadways = List(
        Roadway( 0L, 0L, RoadPart(srcRoadNumber,    0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), startDate = DateTime.now, endDate = Some(DateTime.now), createdBy = "dummy", roadName = None, arealRoadMaintainer = ARMInvalid, terminated = NoTermination),
        Roadway(-1L, 0L, RoadPart(targetRoadNumber, 0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), reversed = false, DateTime.now, Some(DateTime.now), "dummy", None, ARMInvalid, NoTermination)
      )
      roadwayDAO.create(roadways)
  ...
  test("Test changeDirection When after the creation of valid project links on a project Then the side code of road addresses should be successfully reversed.") {
    runWithRollback {
      ...
      projectService.createProjectLinks(Seq(12352L, 12353L, 12354L).map(_.toString), project.id, roadPart = RoadPart(9999, 1), track = Track.LeftSide, userGivenDiscontinuity = Discontinuity.Continuous, administrativeClass = AdministrativeClass.State, roadLinkSource = LinkGeomSource.NormalLinkInterface, roadARM = ely8, user = "test", roadName = "road name")
      ...
      projectService.createProjectLinks(Seq(12355L, 12356L, 12357L).map(_.toString), project.id, RoadPart(9999, 1), Track.LeftSide, Discontinuity.Continuous, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ely8, "test", "road name")
  ...
  test("Test checkRoadPartsReservable When there are some transferred road links to that same road part.") {
    runWithRollback {
      ...
      val ra = Seq(
        //Combined
        Roadway(raId,     roadwayNumber1, roadPart,    AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0L, 20L), reversed = false, DateTime.now(), None, "test_user", None, ArealRoadMaintainer.EVKPohjanmaa, NoTermination, startDate, None),
        //RightSide
        Roadway(raId + 1, roadwayNumber2, newRoadPart, AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0L, 50L), reversed = false, DateTime.now(), None, "test_user", None, ArealRoadMaintainer.EVKPohjanmaa, NoTermination, startDate, None))
      ...
      projectService.updateProjectLinks(project_id, lastLink, Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart, 0, None, 1, 5, Some(ely8), reversed = false, None)
      ...
      val ely: ArealRoadMaintainer = ArealRoadMaintainer.EVKUusimaa
      val projectLinksSet = projectLinkDAO.fetchProjectLinks(project_id).filter(_.roadPart == newRoadPart).map(_.id).toSet
      projectService.updateProjectLinks(project_id, projectLinksSet, Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart, 0, None, 1, 5, Some(ely1), reversed = false, None)
  ...
  test("Test projectService.createProjectLinks() discontinuity assignment When creating some links Then " +
    ...
    runWithRollback {
      ...
      projectService.createProjectLinks(Seq(12347L.toString, 12346L.toString), project.id, RoadPart(9999, 1), Track.Combined, Discontinuity.MinorDiscontinuity, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ely8, "test", "road name")
  ...
  test("Test defaultSectionCalculatorStrategy.assignAddrMValues() " +
    ...
    runWithRollback {
      ...
      val createSuccessLeft  = projectService.createProjectLinks(rl1.map(_.linkId), project.id, RoadPart(9999, 1), Track.LeftSide,  Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ARMInvalid, project.createdBy, "9999_1")
      when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(rl2)
      val createSuccessRight = projectService.createProjectLinks(rl2.map(_.linkId), project.id, RoadPart(9999, 1), Track.RightSide, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ARMInvalid, project.createdBy, "9999_1")
  ...
  test("Test defaultSectionCalculatorStrategy.assignAddrMValues() " +
    ...
    runWithRollback {
      ...
      when(mockRoadLinkService.getRoadLinksHistoryFromVVH(any[Set[String]])).thenReturn(Seq())
      when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(rl1)
      val createSuccessLeft  = projectService.createProjectLinks(rl1.map(_.linkId), project.id, RoadPart(9999, newRoadPartNumber), Track.LeftSide,  Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ARMInvalid, project.createdBy, "9999_1")
      when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(rl2)
      val createSuccessRight = projectService.createProjectLinks(rl2.map(_.linkId), project.id, RoadPart(9999, newRoadPartNumber), Track.RightSide, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ARMInvalid, project.createdBy, "9999_1")
  ...
  test("Test projectService.recalculateProjectLinks() and the way roadway numbers are handled in the process") {
    ...
    runWithRollback {
      val arealRoadMaintainer = ArealRoadMaintainer.ELYKeskiSuomi
      ...
      roadwayDAO.create(Seq(
        Roadway(originalRoadwayId, originalRoadwayNumber, roadPart, AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0, 700), reversedBoolean, DateTime.now(),None,"Test", Some("Test"), arealRoadMaintainer,  TerminationCode.NoTermination, DateTime.now(),None)
      ))
  ...
  test("Test projectService.recalculateProjectLinks() Real life project on road part 110/1, recalculation should complete successfully") {
    ...
    runWithRollback {
      ...
      val roadways = Seq(
        Roadway(65784, 90509805,roadPart,AdministrativeClass.Municipality,Track.Combined, Discontinuity.Continuous,        AddrMRange(1800, 3940),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(65512, 90509888,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange(3940, 4960),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(65618, 90510413,roadPart,AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(3940, 4960),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75083,148600148,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.MinorDiscontinuity,AddrMRange(   0, 1800),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75344,148694049,roadPart,AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.MinorDiscontinuity,AddrMRange(   0, 1800),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
  ...
  test("Test projectService.recalculateProjectLinks() When terminating part of two track section and part of one track section. " +
    ...
    runWithRollback {
      ...
      val roadways = Seq(
        ...
        Roadway( 6187,    44928,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange( 615, 1135),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway( 7297,    46088,roadPart,AdministrativeClass.Municipality,Track.Combined, Discontinuity.Continuous,        AddrMRange(1365, 2510),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway( 7584,    46089,roadPart,AdministrativeClass.State,       Track.Combined, Discontinuity.Continuous,        AddrMRange(2510, 5638),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(70651,148121546,roadPart,AdministrativeClass.Municipality,Track.Combined, Discontinuity.Continuous,        AddrMRange(   0,  615),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(73189,148122112,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.MinorDiscontinuity,AddrMRange(1135, 1148),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(73470,148127435,roadPart,AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange( 615, 1135),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(73064,148127875,roadPart,AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.MinorDiscontinuity,AddrMRange(1135, 1148),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(83706,260730736,roadPart,AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(1148, 1365),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(83762,260730742,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.MinorDiscontinuity,AddrMRange(1148, 1365),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
  ...
  test("Test projectService.recalculateProjectLinks() Real life project on road parts 4/218, 4/219 and 11960/1, recalculation should complete successfully") {
    ...
    runWithRollback {
      ...
      val roadways = Seq(
        Roadway(7609,45461,roadPart1,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(0, 6104),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(7691,45462,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(0, 5624),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(7506,45463,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.ChangingArealRoadMaintainer,AddrMRange(5624, 6565),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
  ...
  test("Test recalculateProjectLinks() When creating roundabout from New and Transfer links then roadway numbers should be assigned correctly. (three different roadways for the roundabout in this case)") {
    ...
    runWithRollback{
      ...
      val roadways = Seq(
        Roadway(79881,190813844,roadPart,AdministrativeClass.Municipality,Track.Combined, Discontinuity.Continuous,        AddrMRange( 739,  957),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(79679,190813846,roadPart,AdministrativeClass.Municipality,Track.Combined, Discontinuity.EndOfRoad,         AddrMRange(1371, 2025),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(79381,190813848,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange( 957, 1371),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(78993,190813849,roadPart,AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange( 957, 1371),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(88915,306319328,roadPart,AdministrativeClass.Municipality,Track.Combined, Discontinuity.MinorDiscontinuity,AddrMRange(   0,  739),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
  ...
  test("Test projectService.recalculateProjectLinks() When creating two track road from single track road" +
    ...
    runWithRollback {
      ...
      val roadways = Seq(
        Roadway(19743,54807,roadPart1,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(   0, 6008),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(19619,54808,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(   0, 2459),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(19180,54809,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(2459, 7292),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
  ...
  private def addProjectLinksToProject(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roadPart: RoadPart = RoadPart(19999, 1),
    ...
    def withTrack(t: Track): Seq[ProjectLink] = {
      addrM.init.zip(addrM.tail).map { case (st, en) =>
        projectLink(AddrMRange(st, en), t, project.id, roadAddressChangeType, roadPart, discontinuity, ArealRoadMaintainer.getELY(ely), roadwayId)
      }
    }
   ...
   test("Test recalculateProjectLinks() When terminating last project links from two track section (that are of different lengths), creating a minor discontinuity on both tracks, then the tracks should be adjusted to match at the discontinuous part.") {
    ...
    runWithRollback {
      ...
      val roadways = Seq(
        Roadway(82725,225079136,RoadPart(49529,1),AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(  0,  696),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(82872,225079139,RoadPart(49529,1),AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,AddrMRange(  0,  696),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(82220,225079142,RoadPart(49529,1),AdministrativeClass.Municipality,Track.Combined, Discontinuity.EndOfRoad, AddrMRange(696, 1042),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
   ...
   test("Test projectService.recalculateProjectLinks() Real life project on road parts 40921/2 and 31329/1, recalculation should complete successfully") {
    ...
    runWithRollback {
      ...
      val roadways = Seq(
        Roadway(59551, 43168588,roadPart1,         AdministrativeClass.Municipality,Track.Combined, Discontinuity.EndOfRoad,         AddrMRange(5621, 5992),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(74878,148600188,roadPart1,         AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange(5209, 5528),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75119,148600207,roadPart1,         AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange(3481, 3915),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75007,148600384,roadPart1,         AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange(3400, 3481),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(74886,148600417,roadPart1,         AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange(3161, 3400),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75256,148600420,roadPart1,         AdministrativeClass.Municipality,Track.Combined, Discontinuity.MinorDiscontinuity,AddrMRange(1170, 1494),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(74889,148600423,roadPart1,         AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange( 937, 1170),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75025,148600426,roadPart1,         AdministrativeClass.Municipality,Track.Combined, Discontinuity.Continuous,        AddrMRange( 695,  937),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(74690,148600429,roadPart1,         AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange(   0,  695),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(74803,148694062,roadPart1,         AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(5209, 5528),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(74576,148694072,roadPart1,         AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(3481, 3915),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75072,148694086,roadPart1,         AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(3161, 3400),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75327,148694087,roadPart1,         AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange( 937, 1170),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(75100,148694088,roadPart1,         AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(   0,  695),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(76524,168748574,roadPart1,         AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(3400, 3481),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(79525,190813336,roundaboutRoadPart,AdministrativeClass.Municipality,Track.Combined, Discontinuity.EndOfRoad,         AddrMRange(   0,   67),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(79295,190960312,roadPart1,         AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,        AddrMRange(5528, 5621),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(79183,190960315,roadPart1,         AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(5528, 5621),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(81391,202221121,roadPart1,         AdministrativeClass.Municipality,Track.Combined, Discontinuity.Continuous,        AddrMRange(1494, 3161),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(85053,262360669,roadPart1,         AdministrativeClass.Municipality,Track.Combined, Discontinuity.MinorDiscontinuity,AddrMRange(3915, 5209),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
  ...
  test("Test assignAddrMValues() When Terminating a small section on a two track road in the middle of road part Then the terminated sections should be adjusted to match") {
    runWithRollback {
      ...
      val roadways = Seq(
        Roadway(85042,262301768,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.EndOfRoad,AddrMRange(0, 1463),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
        Roadway(85415,262301771,roadPart1,AdministrativeClass.Municipality,Track.LeftSide, Discontinuity.EndOfRoad,AddrMRange(0, 1463),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer("ELY14"),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
      )
  ...
  private def projectLink(addrMRange: AddrMRange, track: Track, projectId: Long, status: RoadAddressChangeType = RoadAddressChangeType.NotHandled,
                          roadPart: RoadPart = RoadPart(19999, 1), discontinuity: Discontinuity = Discontinuity.Continuous, arealRoadMaintainer: ArealRoadMaintainer = ArealRoadMaintainer.EVKPohjanmaa, roadwayId: Long = 0L, linearLocationId: Long = 0L) = {
    val startDate = if (status !== RoadAddressChangeType.New) Some(DateTime.now()) else None
    ProjectLink(NewIdValue, roadPart, track, discontinuity, addrMRange, addrMRange, startDate, None, Some("User"),
      addrMRange.start.toString, 0.0, addrMRange.length.toDouble, SideCode.TowardsDigitizing, (NoCP, NoCP), (NoCP, NoCP),
      Seq(Point(0.0, addrMRange.start), Point(0.0, addrMRange.end)), projectId, status, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,
      addrMRange.length.toDouble, roadwayId, linearLocationId, arealRoadMaintainer, reversed = false, None, 0L)
  }
