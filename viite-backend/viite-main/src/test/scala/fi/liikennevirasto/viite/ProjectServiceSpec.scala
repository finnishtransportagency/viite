...
class ProjectServiceSpec extends AnyFunSuite with Matchers with BeforeAndAfter with BaseDAO {
 ...
 val defaultChangeInfo = RoadwayChangeInfo(
   ...
   RoadwayChangeSection(Option(403), Option(0), Option(8), Option(0), Option(AddrMRange(8,1001)),
     Option(AdministrativeClass.State), Option(Discontinuity.Continuous), Option(ArealRoadMaintainer.getEVK(5))),
 ...
 private def setUpProjectWithLinks(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roadPart: RoadPart = RoadPart(19999, 1),
                                   discontinuity: Discontinuity = Discontinuity.Continuous, roadMaintainer: ArealRoadMaintainer = ArealRoadMaintainer.getEVK(8), roadwayId: Long = 0L, startDate: Option[DateTime] = None) = {
   val id = Sequences.nextViiteProjectId

   def projectLink(addrMRange: AddrMRange, track: Track, projectId: Long, status: RoadAddressChangeType = RoadAddressChangeType.NotHandled, roadPart: RoadPart = RoadPart(19999, 1), discontinuity: Discontinuity = Discontinuity.Continuous, roadMaintainer: ArealRoadMaintainer = ArealRoadMaintainer.getEVK(8), linkId: String = 0L.toString, roadwayId: Long = 0L, linearLocationId: Long = 0L, startDate: Option[DateTime] = None) = {
     ProjectLink(NewIdValue, roadPart, track, discontinuity, addrMRange, addrMRange, startDate, None, Some("User"), linkId, 0.0, addrMRange.length.toDouble, SideCode.TowardsDigitizing, (NoCP, NoCP), (NoCP, NoCP), Seq(Point(0.0, addrMRange.start), Point(0.0, addrMRange.end)), projectId, status, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, addrMRange.length.toDouble, roadwayId, linearLocationId, ArealRoadMaintainer.getEVK(8), reversed = false, None, 0L)
   }

   def withTrack(t: Track): Seq[ProjectLink] = {
     addrM.init.zip(addrM.tail).map { case (st, en) =>
       projectLink(AddrMRange(st, en), t, id, roadAddressChangeType, roadPart, discontinuity, ArealRoadMaintainer.getEVK(1), roadwayId = roadwayId, startDate = startDate)
     }
   }
 ...
 test("Test createRoadLinkProject When creating a road link project with same name as an existing project Then return error") {
   runWithRollback {
     val roadAddressProject1 = Project(0, ProjectState.apply(1), "TestProject", "TestUser1", DateTime.now(), "TestUser1", DateTime.parse("1901-01-01"), DateTime.now(), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     projectService.createRoadLinkProject(roadAddressProject1)

     val roadAddressProject2 = Project(0, ProjectState.apply(1), "TESTproject", "TestUser2", DateTime.now(), "TestUser2", DateTime.parse("1902-03-03"), DateTime.now(), "Some other info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     ...
     val roadAddressProject3 = Project(0, ProjectState.apply(1), "testproject", "TestUser3", DateTime.now(), "TestUser3", DateTime.parse("1903-03-03"), DateTime.now(), "Some other info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
 ...
 test("Test saveProject When two projects with same road part Then throw exception on unique key constraint violation") {
   runWithRollback {
     ...
     val rap1 = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1963-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val rap2 = Project(0L, ProjectState.apply(1), "TestProject2", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1963-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val addr1 = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(5, 207), Some(0L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None))
     ...
     projectService.saveProject(project1.copy(reservedParts = addr1, roadMaintainers = Set()))
     ...
     val exception = intercept[BatchUpdateException] {
       projectService.saveProject(project2.copy(reservedParts = addr1, roadMaintainers = Set()))
     }
 ...
 test("Test saveProject When new part is added Then return project with new reservation") {
   ...
   runWithRollback {
     ...
     val addresses: List[ProjectReservedPart] = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, RoadPart(5, 207), Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None))
     val roadAddressProject = Project(0, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1963-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     ...
     projectService.saveProject(saved.copy(reservedParts = addresses, roadMaintainers = Set()))
 ...
 test("Test checkRoadPartsExist When roadway exist Then return None") {
   runWithRollback {
     ...
     val ra = Seq(Roadway(id1, roadwayNumber, RoadPart(roadNumber, roadStartPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("test road"), ArealRoadMaintainer.getEVK(1)))
 ...
 test("Test checkRoadPartsExist When start road way does not exist and end exists then return error message") {
   runWithRollback {
     ...
     val roadwayNumber = 8000
     val id1 = Sequences.nextRoadwayId
     val ra = Seq(Roadway(id1, roadwayNumber, RoadPart(roadNumber, roadEndPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("test road"), ArealRoadMaintainer.getEVK(1)))
     roadwayDAO.create(ra)
     val check = projectService.checkRoadPartsExist(roadNumber, roadStartPart, roadEndPart)
     check should be(Some(ErrorStartingRoadPartNotFound))
   }
 ...
 test("Test validateReservations When road parts don't exist Then return error") {
   runWithRollback {
     ...
     val reservedPart = ProjectReservedPart(0L, roadPart, Some(1000L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)))
 ...
 test("Test checkRoadPartsReservable When road can Then return on right part 2") {
   ...
   runWithRollback {
     val id1 = Sequences.nextRoadwayId
     val ra = Seq(Roadway(id1, roadwayNumber, RoadPart(roadNumber, roadStartPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("test road"), ArealRoadMaintainer.getEVK(8)))
     ...
     val rb = Seq(Roadway(id2, roadwayNumber, RoadPart(roadNumber, roadEndPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "tester", Some("Test road 2"), ArealRoadMaintainer.getEVK(8)))
 ...
 test("Test checkRoadPartsReservable When road part is already reserved to the current project Then should return reserved info.") {
   ...
   runWithRollback {
     roadwayDAO.create(
                       Seq(Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(roadNumber, roadStartPart), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, testUser, Some("test road"), ArealRoadMaintainer.getEVK(8)),
                           Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(roadNumber, roadEndPart),   AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, testUser, Some("Test road 2"), ArealRoadMaintainer.getEVK(8)))
                      )
     ...
     val roadAddressProject = Project(0, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1963-01-01"), DateTime.now(), "Some additional info", Seq(reservedPart1, reservedPart2), Seq(), None, roadMaintainers = Set())
     val project            = projectService.createRoadLinkProject(roadAddressProject)
 ...
 test("Test getRoadAddressAllProjects When project is created Then return project") {
   ...
   runWithRollback {
     ...
     projectService.saveProject(project.copy(reservedParts = Seq(ProjectReservedPart(0L, RoadPart(5, 207), Some(0L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None)), roadMaintainers = Set()))
     val countAfterInsertProjects = projectService.getAllProjects
     count = countCurrentProjects.size + 1
     countAfterInsertProjects.size should be(count)
   }
 ...
 test("Test projectService.changeDirection When project links have a defined direction Then return project_links with adjusted M addresses and side codes due to the reversing ") {
   runWithRollback {
     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("1963-01-01"), "TestUser", DateTime.parse("1963-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
 ...
 test("Test projectService.preserveSingleProjectToBeTakenToRoadNetwork() " +
   ...
   runWithRollback {
     ...
     val rap = Project(projectId, ProjectState.apply(ProjectState.InUpdateQueue.value), "TestProject", "TestUser", DateTime.parse("2700-01-01"), "TestUser", DateTime.parse("2700-01-01"), DateTime.now(), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
 ...
 test("Test projectService.saveProject When after project creation we reserve roads to it Then return the count of created project should be increased AND Test projectService.getAllProjects When not specifying a projectId Then return the count of returned projects should be 0 ") {
   ...
   runWithRollback {
     val countCurrentProjects = projectService.getAllProjects
     val id = 0
     val roadAddressProject = Project(id, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1997-01-01"), DateTime.now(), "Some additional info", List(), Seq(), None, roadMaintainers = Set())
     val project = projectService.createRoadLinkProject(roadAddressProject)
     ...
     projectService.saveProject(project.copy(reservedParts = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, RoadPart(1130, 4), Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(1)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None)), roadMaintainers = Set()))
     val countAfterInsertProjects = projectService.getAllProjects
     count = countCurrentProjects.size + 1
     countAfterInsertProjects.size should be(count)
   }
 ...
 test("Test projectService.createRoadLinkProject(), projectService.deleteProject When a user wants to create a new, simple project and then delete it Then return the number of existing projects should increase by 1 and then decrease by 1 as well") {
   ...
   runWithRollback {
     val countCurrentProjects = projectService.getAllProjects
     val roadAddressProject = Project(0, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("2012-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val project = projectService.createRoadLinkProject(roadAddressProject)
     ...
     projectService.saveProject(project.copy(reservedParts = Seq(ProjectReservedPart(0L, RoadPart(5, 203), Some(100L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None)), roadMaintainers = Set()))
     val countAfterInsertProjects = projectService.getAllProjects
 ...
 test("Test projectService.validateProjectDate() When evaluating project links with start date are equal as the project start date Then return no error message") {
   runWithRollback {
     ...
     val addresses = List(ProjectReservedPart(5: Long, RoadPart(5, 205), Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None))
 ...
 test("Test projectService.getSingleProjectById() When after creation of a new project, with reserved parts Then return only 1 project whose name is the same that was defined. ") {
   ...
   runWithRollback {
     val countCurrentProjects = projectService.getAllProjects
     val addresses: List[ProjectReservedPart] = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, RoadPart(5, 206), Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None))
     val roadAddressProject = Project(0, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("2017-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val saved = projectService.createRoadLinkProject(roadAddressProject)
 ...
 test("Test getRoadAddressSingleProject When project with reserved parts is created Then return the created project") {
   ...
   runWithRollback {
     ...
     val ra = Seq(Roadway(id, roadwayNumber, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 1000L), reversed = false, DateTime.parse("1901-01-01"), None, "Tester", Option("test name"), ArealRoadMaintainer.getEVK(8)))
     ...
     val project = Project(projectId, ProjectState.Incomplete, "testiprojekti", "Test", DateTime.now(), "Test", DateTime.now(), DateTime.now(), "info", List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue: Long, roadPart: RoadPart, Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None)), Seq(), None, roadMaintainers = Set())
...
//    Project1 projectlink is connected to Project2 projectlink at point 1000, 10 and there is no junction which means that Project2 projectlink cannot be created with new action
   runWithRollback {
     val id = Sequences.nextViiteProjectId
     val ra1 = Seq(
       RoadAddress(12345, linearLocationId,     RoadPart(19999, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L,  5L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1000.toString, 0, 5, SideCode.TowardsDigitizing,  DateTime.now().getMillis, (None, None), Seq(Point(1000.0,  0.0), Point(1000.0,  5.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, roadwayNumber1, Some(DateTime.parse("1901-01-01")), None, None),
       RoadAddress(12346, linearLocationId + 1, RoadPart(19999, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(5L, 10L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1001.toString, 0, 5, SideCode.TowardsDigitizing,  DateTime.now().getMillis, (None, None), Seq(Point(1000.0,  5.0), Point(1000.0, 10.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, roadwayNumber2, Some(DateTime.parse("1901-01-01")), None, None)
     )
     val ra2 = Seq(
       RoadAddress(12347, linearLocationId + 2, RoadPart(12345, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(10L, 15L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1002.toString, 0, 5, SideCode.TowardsDigitizing, DateTime.now().getMillis, (None, None), Seq(Point(1000.0, 10.0), Point(1000.0, 25.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, roadwayNumber3, Some(DateTime.parse("1901-01-01")), None, None),
       RoadAddress(12348, linearLocationId + 3, RoadPart(12345, 1), AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad,  AddrMRange(15L, 20L), Some(DateTime.parse("1901-01-01")), None, Some("User"), 1003.toString, 0, 5, SideCode.TowardsDigitizing, DateTime.now().getMillis, (None, None), Seq(Point(1000.0, 25.0), Point(1000.0, 30.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, roadwayNumber4, Some(DateTime.parse("1901-01-01")), None, None)
     )

     val rap1 = Project(id, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("2700-01-01"), "TestUser", DateTime.parse("1972-03-03"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     ...
     val rap2 = Project(id + 1, ProjectState.apply(1), "TestProject2", "TestUser", DateTime.parse("2700-01-01"), "TestUser", DateTime.now(), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     ...
     val response1 = projectService.createProjectLinks(Seq(1000L.toString, 1001L.toString), rap1.id, RoadPart(56, 207), Track.Combined, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(8), "test", "road name")
     ...
     val response2 = projectService.createProjectLinks(Seq(1002L.toString,1003L.toString), rap2.id, RoadPart(55, 206), Track.Combined, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(8), "test", "road name")
 ...
 test("Test addNewLinksToProject When reserving part already used in other project Then return error message") {
   runWithRollback {
     ...
     val id = Sequences.nextViiteProjectId
     val rap = Project(id, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("2700-01-01"), "TestUser", DateTime.parse("1972-03-03"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     val projectLink = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(idr, 123, RoadPart(5, 207), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 10L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 9.8, SideCode.TowardsDigitizing, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0, 9.8)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0))
     projectDAO.create(rap)
     projectService.saveProject(rap)


     val rap2 = Project(id + 1, ProjectState.apply(1), "TestProject2", "TestUser", DateTime.parse("2700-01-01"), "TestUser", DateTime.now(), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     val projectLink2 = toProjectLink(rap2, RoadAddressChangeType.New)(RoadAddress(idr, 1234, RoadPart(5, 999), AdministrativeClass.Unknown, Track.Combined, Discontinuity.Discontinuous, AddrMRange(0L, 10L), Some(DateTime.parse("1901-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 9.8, SideCode.TowardsDigitizing, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0, 9.8)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0))
     projectDAO.create(rap2)
     projectService.saveProject(rap2)
 ...
 test("Test createProjectLinks When adding new links to a new road and roadpart Then discontinuity should be on correct link.") {
   def runTest(reversed: Boolean) = {
     ...
     val rap            = Project(projectId, ProjectState.apply(1), "TestProject", user, DateTime.parse("2700-01-01"), user, DateTime.parse("1972-03-03"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     val projectLinks   = Seq(
       toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 123, roadPart, AdministrativeClass.Unknown, Track.Combined, continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0,  9.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0,  9.8)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0)),
       toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 124, roadPart, AdministrativeClass.Unknown, Track.Combined, continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 19.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 9.8), Point(9.8, 29.6)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0))
     )
     projectDAO.create(rap)
     projectService.saveProject(rap)
     ...
     val message1project1 = projectService.createProjectLinks(longs, rap.id, roadPart, Track.Combined, discontinuity, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(8), user, "new road")
 ...
 test("Test addNewLinksToProject When adding new links to a new road and roadpart Then values should be correct.") {
   runWithRollback {
     ...
     val user           = "TestUser"
     val rap            = Project(projectId, ProjectState.apply(1), "TestProject", user, DateTime.parse("2700-01-01"), user, DateTime.parse("1972-03-03"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     val projectLink1   = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 123, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0,  9.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 0.0), Point(0.0,  9.8)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0))
     val projectLink2   = toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 124, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L,  0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 19.8, SideCode.Unknown, 0, (None, None), Seq(Point(0.0, 9.8), Point(9.8, 29.6)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0))
     projectDAO.create(rap)
     projectService.saveProject(rap)
     projectReservedPartDAO.reserveRoadPart(projectId, roadPart, user)
 ...
 test("Test addNewLinksToProject When adding new links to existing road and new roadpart Then values should be correct.") {
   runWithRollback {
     ...
     val rap = Project(projectId, ProjectState.apply(1), "TestProject", user, DateTime.parse("2700-01-01"), user, DateTime.parse("1972-03-03"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     val projectLinks = Seq(
      toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 125, newRoadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 10.4, SideCode.Unknown, 0, (None, None), Seq(Point(532427.945,6998488.475,0.0),Point(532395.164,6998549.616,0.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0)),
      toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 126, newRoadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 10.0, SideCode.Unknown, 0, (None, None), Seq(Point(532427.945,6998488.475,0.0),Point(532495.0,  6998649.0,  0.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0))
     )
     projectDAO.create(rap)
     projectService.saveProject(rap)
     projectReservedPartDAO.reserveRoadPart(projectId, roadPart,    user)
 ...
 test("Test addNewLinksToProject When adding new links to existing road and existing roadpart Then values should be correct.") {
   runWithRollback {
     ...
     val roadPart = RoadPart(5,206)
     val user = "TestUser"
     val rap = Project(projectId, ProjectState.apply(1), "TestProject", user, DateTime.parse("2700-01-01"), user, DateTime.parse("2700-01-01"), DateTime.parse("2700-01-01"), "Some additional info", List.empty[ProjectReservedPart], Seq(), None, roadMaintainers = Set())
     val projectLinks = Seq(
       toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 125, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12345L.toString, 0.0, 10.4, SideCode.Unknown, 0, (None, None), Seq(Point(0,20),Point(0,30,0.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0)),
       toProjectLink(rap, RoadAddressChangeType.New)(RoadAddress(Sequences.nextRoadwayId, 126, roadPart, AdministrativeClass.Unknown, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), Some(DateTime.parse("1963-01-01")), Some(DateTime.parse("1902-01-01")), Option("tester"), 12346L.toString, 0.0, 10.0, SideCode.Unknown, 0, (None, None), Seq(Point(0.0,30.0),Point(0.0,40.0)), LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), NoTermination, 0))
     )
     projectDAO.create(rap)
 ...
 test("Test projectService.fetchPreFillData " +
   ...
   runWithRollback {
     ...
     projectService.fetchPreFillData(projectLinks.head.linkId, roadAddressProject.id) should be(Right(PreFillInfo(roadPart.roadNumber, roadPart.partNumber, "", RoadNameSource.UnknownSource, projectLinks.head.roadMaintainer)))
   }
 }
 ...
 test("Test projectService.fetchPreFillData " +
   ...
   runWithRollback {
     ...
     val projectLinks = roadwayAddressMapper.getRoadAddressesByRoadway(roadwayDAO.fetchAllByRoadPart(roadPart)).map(toProjectLink(roadAddressProject))
     projectLinkDAO.create(projectLinks)
     projectService.fetchPreFillData(projectLinks.head.linkId, roadAddressProject.id) should be(Right(PreFillInfo(roadPart.roadNumber, roadPart.partNumber, "road name test", RoadNameSource.RoadAddressSource, projectLinks.head.roadMaintainer)))
   }
 }
 ...
 test("Test projectService.fetchPreFillData() " +
   ...
   runWithRollback{
     ...
     val projectLinks = roadwayAddressMapper.getRoadAddressesByRoadway(roadwayDAO.fetchAllByRoadPart(roadPart)).map(toProjectLink(roadAddressProject))
     projectLinkDAO.create(projectLinks)
     projectService.fetchPreFillData(projectLinks.head.linkId, roadAddressProject.id) should be(Right(PreFillInfo(roadPart.roadNumber, roadPart.partNumber, "TestRoadName_Project_Link", RoadNameSource.ProjectLinkSource, projectLinks.head.roadMaintainer)))
   }
 }
 ...
 test("Test createProjectLinks When project link is created for part not reserved Then should return error message") {
   runWithRollback {
     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1901-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val newLink = Seq(ProjectLink(-1000L, RoadPart(5, 206), Track.apply(99), Discontinuity.Continuous, AddrMRange(0L, 50L), AddrMRange(0L, 50L), None, None, None, 12345L.toString, 0.0, 43.1, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), Seq(Point(468.5, 0.5), Point(512.0, 0.0)), 0L, RoadAddressChangeType.Unknown, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, 43.1, 49704009, 1000570, ArealRoadMaintainer.getEVK(8), reversed = false, None, 123456L, 12345L))
     val project = projectService.createRoadLinkProject(rap)
     when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(newLink.map(toRoadLink))
     val response = projectService.createProjectLinks(Seq(12345L.toString), project.id, RoadPart(5, 206), Track.Combined, Discontinuity.Continuous, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), "test", "road name")
 ...
 test("Test projectService.updateProjectLinks When applying the operation \"Numerointi\" to a road part that is ALREADY reserved in a different project Then return an error message") {
   runWithRollback {
     val rap1 = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("1963-01-01"), "TestUser", DateTime.parse("1963-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val rap2 = Project(0L, ProjectState.apply(1), "TestProject2", "TestUser", DateTime.parse("2012-01-01"), "TestUser", DateTime.parse("2017-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val addr1 = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(5, 207), Some(0L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None))
     val addr2 = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(5, 206), Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None))
     val project1 = projectService.createRoadLinkProject(rap1)
     ...
     projectService.saveProject(project2.copy(reservedParts = addr2, roadMaintainers = Set()))

     val response = projectService.updateProjectLinks(
       project1.id,
       Set(),
       projectLinkDAO.fetchProjectLinks(project1.id).map(_.linkId),
       RoadAddressChangeType.Renumeration,
       "TestUser",
       RoadPart(5, 206),
       0,
       None,
       AdministrativeClass.State.value,
       Discontinuity.Continuous.value,
       roadMaintainer = Some(ArealRoadMaintainer.getEVK(8)),
       reversed = false,
       roadName = None,
       coordinates = None,
       devToolData = None
 ...
 test("Test projectService.updateProjectLinks When applying the operation \"Numerointi\" to a road part that is ALREADY reserved in a different project Then return an error message" +
   ...
   runWithRollback {
     ...
     val response = projectService.updateProjectLinks(
       ...
       AdministrativeClass.State.value,
       Discontinuity.Continuous.value,
       Some(ArealRoadMaintainer.apply("EVK8")))
     response.get should be("Antamasi tienumero ja tieosanumero ovat jo käytössä. Tarkista syöttämäsi tiedot.")
   }
 }
 ...
 test("Test projectService.updateProjectLinks When applying the operation \"Numerointi\" to a road part that already has some other action applied Then return an error message") {
   runWithRollback {
     ...
     mockForProject(project1.id, roadAddressServiceRealRoadwayAddressMapper.getRoadAddressWithRoadPart(RoadPart(5, 207)).map(toProjectLink(project1)))
     projectService.saveProject(project1.copy(reservedParts = addr1, roadMaintainers = Set()))
     projectService.updateProjectLinks(project1.id, Set(), projectLinkDAO.fetchProjectLinks(project1.id).map(_.linkId), RoadAddressChangeType.Termination, "TestUser", RoadPart(5, 207), 0, None, AdministrativeClass.State.value, Discontinuity.Continuous.value, Some(ArealRoadMaintainer.getEVK(8)))
     val response = projectService.updateProjectLinks(project1.id, Set(), projectLinkDAO.fetchProjectLinks(project1.id).map(_.linkId), RoadAddressChangeType.Renumeration, "TestUser", RoadPart(5, 308), 0, None, AdministrativeClass.State.value, Discontinuity.Continuous.value, Some(ArealRoadMaintainer.getEVK(8)))
 ...
 test("Test projectLinkDAO.getProjectLinks() When after the \"Numerointi\" operation on project links of a newly created project Then the last returned link should have a discontinuity of \"EndOfRoad\", all the others should be \"Continuous\" ") {
   runWithRollback {
     val rap1 = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.now().plusDays(1), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val addr1 = List(ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(5, 207), Some(0L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None))
     val project1 = projectService.createRoadLinkProject(rap1)
     mockForProject(project1.id, roadAddressServiceRealRoadwayAddressMapper.getRoadAddressWithRoadPart(RoadPart(5, 207)).map(toProjectLink(project1)))
     projectService.saveProject(project1.copy(reservedParts = addr1, roadMaintainers = Set()))
     val fetchedProjectLinks = projectLinkDAO.fetchProjectLinks(project1.id)
     projectService.updateProjectLinks(project1.id, Set(fetchedProjectLinks.maxBy(_.addrMRange.end).id), fetchedProjectLinks.map(_.linkId), RoadAddressChangeType.Renumeration, "TestUser", RoadPart(6, 207), 0, None, AdministrativeClass.State.value, Discontinuity.EndOfRoad.value, Some(ArealRoadMaintainer.getEVK(8)))
 ...
 test("Test projectService.saveProject() When updating a project to remove ALL it's reserved roads Then the project should lose it's associated road names.") {
   runWithRollback {
     ...
     ProjectLinkNameDAO.get(99999L, project.id).get.roadName should be("Test name")
     projectService.saveProject(project.copy(reservedParts = Seq(), roadMaintainers = Set()))
     ProjectLinkNameDAO.get(99999L, project.id) should be(None)
   }
 }
...
//    }
   ...
   runWithRollback {
     ...
     val pl2 = ProjectLink(-1000L, RoadPart(9999, 1), Track.apply(2), Discontinuity.Continuous, AddrMRange(0L,  0L), AddrMRange(0L,  0L), None, None, None, 12346L.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), Seq(Point(20.0, 10.0), Point(30.0, 15.0)), 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(Seq(Point(20.0, 10.0), Point(30.0, 15.0))), 0L, 0, ArealRoadMaintainer.getEVK(0), reversed = false, None, 86400L)
     val pl3 = ProjectLink(-1000L, RoadPart(9999, 1), Track.apply(2), Discontinuity.Continuous, AddrMRange(0L,  0L), AddrMRange(0L,  0L), None, None, None, 12347L.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), Seq(Point(30.0, 15.0), Point(45.0, 10.0)), 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(Seq(Point(30.0, 15.0), Point(45.0, 10.0))), 0L, 0, ArealRoadMaintainer.getEVK(0), reversed = false, None, 86400L)
     val pl4 = ProjectLink(-1000L, RoadPart(9999, 1), Track.apply(1), Discontinuity.Continuous, AddrMRange(0L,  0L), AddrMRange(0L,  0L), None, None, None, 12348L.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), Seq(Point(20.0, 10.0), Point(25.0, 5.0), Point(45.0, 10.0)), 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(Seq(Point(20.0, 10.0), Point(25.0, 5.0), Point(45.0, 10.0))), 0L, 0, ArealRoadMaintainer.getEVK(0), reversed = false, None, 86400L)
     val pl5 = ProjectLink(-1000L, RoadPart(9999, 1), Track.apply(0), Discontinuity.Continuous, AddrMRange(0L,  0L), AddrMRange(0L,  0L), None, None, None, 12349L.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), Seq(Point(45.0, 10.0), Point(60.0, 10.0)), 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(Seq(Point(45.0, 10.0), Point(60.0, 10.0))), 0L, 0, ArealRoadMaintainer.getEVK(0), reversed = false, None, 86400L)
 ...
 test("Test projectService.changeDirection() When after the creation of valid project links on a project Then the discontinuity of road addresses that are not the same road number and road part number should not be altered.") {
   runWithRollback {
     ...
     projectService.createProjectLinks(Seq(12346L.toString, 12347L.toString), project.id, RoadPart(9999, 1), Track.LeftSide, Discontinuity.Continuous, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(8), "test", "road name")
     when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(Seq(pl5).map(toRoadLink))
     projectService.createProjectLinks(Seq(12349L.toString), project.id, RoadPart(9998, 1), Track.Combined, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(8), "test", "road name")
 ...
 test("Test createRoadLinkProject When project in writable state Then  Service should identify states (Incomplete, and ErrorInViite)") {
   runWithRollback {
     val incomplete = Project(0L, ProjectState.apply(1), "I am Incomplete", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1901-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val incompleteProject = projectService.createRoadLinkProject(incomplete)
     val errorInViite = Project(0L, ProjectState.apply(1), "I am ErrorInViite", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1901-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
   ...
   "Then the mAddressValues of the last links should be equal in both sides of the tracks for part 1.")
 {
   runWithRollback {
     /**
       * Test data
       */
     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1995-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 123,9999,1,1,0,26,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL,'EVK8')""")
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 124,9999,1,2,0,26,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL, 'EVK8')""")
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 125,9999,2,1,0,7,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL, 'EVK8')""")
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 126,9999,2,2,0,11,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL, 'EVK8')""")
     ...
     projectService.saveProject(project.copy(reservedParts = Seq(
       ProjectReservedPart(0L, RoadPart(9999, 1), null, Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None),
       ProjectReservedPart(0L, RoadPart(9999, 2), null, Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None)), roadMaintainers = Set())
     )
...
//        */
     ...
     projectService.updateProjectLinks(id, part1AdjacentToPart2LinkRightSide, Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 1, None, 1, 5, Option(ArealRoadMaintainer.getEVK(1)), reversed = false, None)
     projectService.updateProjectLinks(id, part1AdjacentToPart2LinkLeftSide,  Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 2, None, 1, 5,  Option(ArealRoadMaintainer.getEVK(1)), reversed = false, None)
     ...
     projectService.updateProjectLinks(id, part2track1Links, Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 1, None, 1, 5, Option(ArealRoadMaintainer.getEVK(1)), reversed = false, None)
     projectService.updateProjectLinks(id, part2Track2Links, Seq(), RoadAddressChangeType.Transfer, "test", RoadPart(9999, 2), 2, None, 1, 5, Option(ArealRoadMaintainer.getEVK(1)), reversed = false, None)
   ...
   "Then the mAddressValues of the last links should be equal in both sides of the tracks for part 1." )
 {
   runWithRollback {
     ...
     val rap: Project = Project(
         id = 0L,
         projectState = ProjectState.apply(1),
         name = "TestProject",
         createdBy = "TestUser",
         createdDate = DateTime.parse("1901-01-01"),
         modifiedBy = "TestUser",
         startDate = DateTime.parse("1991-01-01"),
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 1234567,9999,1,1,0,26,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL,'EVK8')""")
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 1234568,9999,1,2,0,26,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL,'EVK8')""")
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 1234569,9999,2,1,0,7,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL,'EVK8')""")
     ...
     runUpdateToDb(sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
       values (nextval('roadway_seq'), 1234570,9999,2,2,0,11,0,1,to_date('22-10-90','DD-MM-YY'),NULL,'TR',to_timestamp('21-09-18 12.04.42.970245000','DD-MM-YY HH24.MI.SSXFF'),1,0,to_date('16-10-98','DD-MM-YY'),NULL,'EVK8')""")

     val project: Project = projectService.createRoadLinkProject(rap)
     val id = project.id
     val part1 = roadwayAddressMapper.getRoadAddressesByLinearLocation(linearLocationDAO.fetchByRoadways(roadwayDAO.fetchAllBySection(RoadPart(9999, 1)).map(_.roadwayNumber).toSet))
     val part2 = roadwayAddressMapper.getRoadAddressesByLinearLocation(linearLocationDAO.fetchByRoadways(roadwayDAO.fetchAllBySection(RoadPart(9999, 2)).map(_.roadwayNumber).toSet))
     val toProjectLinks = (part1 ++ part2).map(toProjectLink(rap))
     ...
     val projectX = project.copy(
       reservedParts = Seq(
         ProjectReservedPart(0L, RoadPart(9999, 1), null, Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None),
         ProjectReservedPart(0L, RoadPart(9999, 2), null, Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None, None)
       ),
     ...
     projectService.updateProjectLinks(id, part2track1Links, Seq(), RoadAddressChangeType.Transfer, "test",
       newRoadPart = RoadPart(9999, 2), newTrackCode = 1, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, roadMaintainer = Some(ArealRoadMaintainer.getEVK(1)), roadName = None)
     projectService.updateProjectLinks(id, part2Track2Links, Seq(), RoadAddressChangeType.Transfer, "test",
       newRoadPart = RoadPart(9999, 2), newTrackCode = 2, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, roadMaintainer = Some(ArealRoadMaintainer.getEVK(1)), roadName = None)
     /**
       * Tranfering adjacents of part1 to part2
       */
     val part1AdjacentToPart2IdRightSide = Set(12347L.toString)
     ...
     projectService.updateProjectLinks(id, part1AdjacentToPart2LinkRightSide, Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart = RoadPart(9999, 2), newTrackCode = 1, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, roadMaintainer = Some(ArealRoadMaintainer.getEVK(1)), roadName = None)
     projectService.updateProjectLinks(id, part1AdjacentToPart2LinkLeftSide,  Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart = RoadPart(9999, 2), newTrackCode = 2, userDefinedEndAddressM = None, administrativeClass = 1, discontinuity = 5, roadMaintainer = Some(ArealRoadMaintainer.getEVK(1)), roadName = None)
 ...
 test("Test expireHistoryRows " +
   ...
   runWithRollback {
     ...
     *  with history rows. */
     ...
     val roadwayNumber   = Sequences.nextRoadwayNumber
     val currentRoadway  = Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(9999, 1), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0, oldEndAddress), reversed = false, DateTime.now().minusYears(10).withTime(0, 0, 0, 0), None, "test", Some("Test Road"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination, DateTime.now().minusYears(10), None)
     val history_roadway = Seq(
       Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(9999, 1), AdministrativeClass.Municipality, Track.Combined, Discontinuity.Continuous, AddrMRange(0, oldEndAddress), reversed = false, DateTime.now().minusYears(15).withTime(0, 0, 0, 0), Some(DateTime.now().minusYears(10).withTime(0, 0, 0, 0)), "test", Some("Test Road"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination, DateTime.now().minusYears(15), None),
       Roadway(Sequences.nextRoadwayId, roadwayNumber, RoadPart(9999, 1), AdministrativeClass.State,        Track.Combined, Discontinuity.Continuous, AddrMRange(0, oldEndAddress), reversed = false, DateTime.now().minusYears(20).withTime(0, 0, 0, 0), Some(DateTime.now().minusYears(15).withTime(0, 0, 0, 0)), "test", Some("Test Road"), ArealRoadMaintainer.getEVK(1), TerminationCode.NoTermination, DateTime.now().minusYears(20), None)
     )
     roadwayDAO.create(Seq(currentRoadway) ++ history_roadway)
 ...
 test("Test updateRoadwaysAndLinearLocationsWithProjectLinks " +
   ...
   runWithRollback {
     ...
     // Create project
     val rap = Project(0L, ProjectState.UpdatingToRoadNetwork, "TestProject", "TestUser", DateTime.parse("1901-01-01"), "TestUser", roadway.startDate.plusYears(1), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val project = projectService.createRoadLinkProject(rap)
     val projectId = project.id
 ...
 test("Test updateRoadwaysAndLinearLocationsWithProjectLinks " +
   ...
   runWithRollback {
     ...
     val projectLinks = Seq(
       ...
       ProjectLink(1028,roadPart,Track.RightSide,Discontinuity.Continuous,AddrMRange(3699,3719),AddrMRange(3835,3855),None,None,Some(createdBy),linkIdToTest,10.117,30.351,SideCode.TowardsDigitizing,(NoCP,RoadAddressCP),(NoCP,JunctionPointCP),List(Point(393376.0,7288232.0,0.0), Point(393372.0,7288235.0,0.0), Point(393361.0,7288245.0,0.0)),projectId,RoadAddressChangeType.Transfer,AdministrativeClass.State, LinkGeomSource.FrozenLinkInterface,20.234,57052,393536,ArealRoadMaintainer.getEVK(10),reversed = false,Some(linkIdToTest),1615244419000L,13659596,roadName,None,None,None,None,None)
     )

     val roadways = Seq(
       Roadway(57052,13659596,RoadPart(19511,1),AdministrativeClass.State,Track.RightSide,Discontinuity.Continuous,AddrMRange(3659,3855),reversed = false,DateTime.parse("2016-03-01T00:00:00.000+02:00"),None,createdBy,roadName,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.parse("2016-03-30T00:00:00.000+03:00"),None)
     )
     ...
     // Create project
     val rap = Project(projectId, ProjectState.UpdatingToRoadNetwork, "TestProject", createdBy, DateTime.parse("1901-01-01"), createdBy, DateTime.parse("1971-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
 ...
 test("Test expireHistoryRows When expiring one roadway by id Then it should be expired by validTo date") {
   runWithRollback {
     ...
     val roadAddressProject = Project(projectId, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1970-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
 ...
 test("Test Road names should not have valid road name for any roadnumber after TR response") {
   runWithRollback {
     ...
     runUpdateToDb(sql"""INSERT INTO project_link (id, project_id, track, discontinuity_type, road_number, road_part_number,
         ...
         administrative_class, roadway_id, linear_location_id, connected_link_id, reversed, side, start_measure, end_measure,
       ...
       VALUES (${Sequences.nextProjectLinkId}, $projectId, 0, 5, 66666, 1,
         ...
         1, NULL, NULL, NULL, 0, NULL, 0, 85.617,
     ...
     val changeInfos = List(
       RoadwayChangeInfo(RoadAddressChangeType.New,
         source = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1, roadMaintainer = ArealRoadMaintainer.getEVK(1))
     )

     val changes = List(
       ProjectRoadwayChange(projectId, Some("test project"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos.head, DateTime.now())
     )
 ...
 test("Test fillRoadNames When creating one project link for one new road name Then method should get the road name of the created project links") {
   ...
   runWithRollback {
     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1971-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val project = projectService.createRoadLinkProject(rap)
     val projectId = project.id
     ...
     projectService.saveProject(project.copy(reservedParts = Seq(ProjectReservedPart(id = 0L, roadPart = roadPart, addressLength =  Some(0L), discontinuity =  Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), None, None, None)), roadMaintainers = Set()))
 ...
 test("Test fillRoadNames When creating one project link for one new road name Then through RoadNameDao the road name of the created project links should be same as the latest one") {
   val roadPart = RoadPart(5, 207)
   runWithRollback {
     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1970-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val project = projectService.createRoadLinkProject(rap)
     val id = project.id
     ...
     projectService.saveProject(project.copy(reservedParts = Seq(ProjectReservedPart(0L, roadPart, Some(0L), Some(Discontinuity.Continuous),  Some(ArealRoadMaintainer.getEVK(8)), None, None, None)), roadMaintainers = Set()))
     val projectLinks = projectLinkDAO.fetchProjectLinks(id)
   ...
   runWithRollback {
     ...
     val changeInfos = List(
       RoadwayChangeInfo(RoadAddressChangeType.New,
         source = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, orderInChangeTable = 1, roadMaintainer = ArealRoadMaintainer.getEVK(1))
     )
   ...
   runWithRollback {
     ...
     val changeInfos = List(
       RoadwayChangeInfo(RoadAddressChangeType.New,
         source = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1, ArealRoadMaintainer.getEVK(1)),
       RoadwayChangeInfo(RoadAddressChangeType.New,
         source = dummyRoadwayChangeSection(Some(RoadPart(55555, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(55555, 1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1, ArealRoadMaintainer.getEVK(1))
     )

     val changes = List(
       ProjectRoadwayChange(projectId, Some("test project"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos.head, DateTime.now()),
       ProjectRoadwayChange(projectId, Some("test project"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos(1), DateTime.now())
     )
 ...
 test("Test getProjectLinks When doing some operations (Unchanged with termination test, repeats termination update), Then the calibration points are cleared and moved to correct positions") {
   ...
   runWithRollback {
     ...
     val countCurrentProjects = projectService.getAllProjects
     val id = 0
     val addresses = Seq(ProjectReservedPart(5: Long, RP205, Some(5L), Some(Discontinuity.Continuous),  Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None),
       ProjectReservedPart(5: Long, RP206, Some(5L), Some(Discontinuity.Continuous), Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None))
     val project = Project(id, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("2017-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     ...
     projectService.saveProject(savedProject.copy(reservedParts = addresses, roadMaintainers = Set()))
     val countAfterInsertProjects = projectService.getAllProjects
     count = countCurrentProjects.size + 1
     countAfterInsertProjects.size should be(count)
 ...
 test("Test getProjectLinks When doing some operations (Transfer and then Terminate), Then the calibration points are cleared and moved to correct positions") {
   ...
   runWithRollback {
     val countCurrentProjects = projectService.getAllProjects
     val id = 0
     val addresses = List(ProjectReservedPart(5: Long, RP207, Some(5L), Some(Discontinuity.Continuous),  Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None))
     val project = Project(id, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1970-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val savedProject = projectService.createRoadLinkProject(project)
 ...
 test("Test getProjectLinks When doing some operations (Terminate then transfer), Then the calibration points are cleared and moved to correct positions") {
   ...
   runWithRollback {
     val countCurrentProjects = projectService.getAllProjects
     val id = 0
     val addresses = List(ProjectReservedPart(5: Long, RoadPart(5, 207), Some(5L), Some(Discontinuity.Continuous),  Some(ArealRoadMaintainer.getEVK(8)), newLength = None, newDiscontinuity = None, newRoadMaintainer = None))
     val project = Project(id, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1970-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val savedProject = projectService.createRoadLinkProject(project)
 ...
 test("Test revertLinksByRoadPart When reverting the road Then the road address geometry after reverting should be the same as KGV") {
   ...
   runWithRollback {
     ...
     val rap = Project(projectId, ProjectState.apply(1), "TestProject", user, DateTime.parse("1901-01-01"), "TestUser", DateTime.parse("1901-01-01"), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     projectDAO.create(rap)
 ...
 test("Test handleNewRoadNames - Test if a new RoadName is created from a project link and without duplicates") {
   runWithRollback {
     ...
     val changeInfos = List(
       RoadwayChangeInfo(RoadAddressChangeType.New,
         source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(  0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1, ArealRoadMaintainer.getEVK(8)),

       RoadwayChangeInfo(RoadAddressChangeType.New,
         source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 2, ArealRoadMaintainer.getEVK(8)),

       RoadwayChangeInfo(RoadAddressChangeType.New,
         source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 3, ArealRoadMaintainer.getEVK(8))
     )

     val projectStartTime = DateTime.now()

     val changes = List(
       ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos.head, projectStartTime),
       ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos(1), projectStartTime),
       ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos(2), projectStartTime)
     )
   ...
   runWithRollback {
     ...
     val changeInfos = List(
       RoadwayChangeInfo(RoadAddressChangeType.Termination,
         source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(0L),   Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1, ArealRoadMaintainer.getEVK(1)),

       RoadwayChangeInfo(RoadAddressChangeType.Unchanged,
         source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 2, ArealRoadMaintainer.getEVK(1)),

       RoadwayChangeInfo(RoadAddressChangeType.Unchanged,
         source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(200L), Some(400L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 3, ArealRoadMaintainer.getEVK(1))
     )

     val projectStartTime = DateTime.now()

     val changes = List(
       ProjectRoadwayChange(0L, Some("projectName"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos.head, projectStartTime),
       ProjectRoadwayChange(0L, Some("projectName"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos(1), projectStartTime),
       ProjectRoadwayChange(0L, Some("projectName"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos(2), projectStartTime)
     )
 ...
 test("Test handleTransferAndRenumeration: Transfer to new road - If source road exists expire its road name and create new road name for target road number") {
   runWithRollback {
     ...
     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.now(), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val project = projectService.createRoadLinkProject(rap)
     ...
     val changeInfos = List(
       RoadwayChangeInfo(RoadAddressChangeType.Transfer,
         source = dummyRoadwayChangeSection(Some(RoadPart(srcRoadNumber,    1)), Some(0L), Some(0L),   Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(targetRoadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1, ArealRoadMaintainer.getEVK(1))
     )

     val changes = List(
       ProjectRoadwayChange(project.id, Some("projectName"), ArealRoadMaintainer.getEVK(8), "Test", DateTime.now(), changeInfos.head, project.startDate)
     )
 ...
 test("Test handleTransferAndRenumeration: Transfer to existing roadway - If source roadway exists expire its roadname") {
   runWithRollback {
     ...
     val roadways = List(
       Roadway( 0L, 0L, RoadPart(srcRoadNumber,    0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), reversed = false, DateTime.now, Some(DateTime.now), "dummy", None, ArealRoadMaintainer.getEVK(0), NoTermination),
       Roadway(-1L, 0L, RoadPart(targetRoadNumber, 0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), reversed = false, DateTime.now, Some(DateTime.now), "dummy", None, ArealRoadMaintainer.getEVK(0), NoTermination)
     )
     roadwayDAO.create(roadways)

     val changeInfos = List(
       RoadwayChangeInfo(RoadAddressChangeType.Transfer,
         source = dummyRoadwayChangeSection(Some(RoadPart(srcRoadNumber,    1)), Some(0L), Some(0L), Some(100L), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Option("EVK1")),
         target = dummyRoadwayChangeSection(Some(RoadPart(targetRoadNumber, 1)), Some(0L), Some(100L), Some(200L), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Option("EVK1")),
         Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1, ArealRoadMaintainer.getEVK(1))
     )
 ...
 test("Test handleTransferAndRenumeration: Renumeration to existing roadway - If source roadway exists expire its roadname") {
   runWithRollback {
     ...
     val roadways = List(
       Roadway( 0L, 0L, RoadPart(srcRoadNumber,    0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), startDate = DateTime.now, endDate = Some(DateTime.now), createdBy = "dummy", roadName = None, roadMaintainer = ArealRoadMaintainer.getEVK(0), terminated = NoTermination),
       Roadway(-1L, 0L, RoadPart(targetRoadNumber, 0), AdministrativeClass.State, Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), reversed = false, DateTime.now, Some(DateTime.now), "dummy", None, ArealRoadMaintainer.getEVK(0), NoTermination)
     )
     roadwayDAO.create(roadways)
 ...
 test("Test save project with reserved road parts having different road maintainer codes should not update them. Formed road parts on other side," +
   " should get evk from project links") {
   runWithRollback {
     ...
     val roadMaintainer1 = Some(ArealRoadMaintainer.getEVK(1))
     val roadMaintainer2 = Some(ArealRoadMaintainer.getEVK(2))

     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.now(), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val reservations = List(
       ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(roadNumber, part1), Some(0L), Some(Discontinuity.Continuous), roadMaintainer1, None, None, None),
       ProjectReservedPart(Sequences.nextViitePrimaryKeySeqValue, RoadPart(roadNumber, part2), Some(0L), Some(Discontinuity.Continuous), roadMaintainer2, None, None, None)
     )
     val project = projectService.createRoadLinkProject(rap)
     ...
     val savedProject = projectService.saveProject(project.copy(reservedParts = reservations, formedParts = reservations, roadMaintainers = Set()))
     val originalEvkPart1 = roadwayDAO.fetchAllByRoadPart(RoadPart(roadNumber, part1)).map(_.roadMaintainer.id).toSet
     val originalEvkPart2 = roadwayDAO.fetchAllByRoadPart(RoadPart(roadNumber, part2)).map(_.roadMaintainer.id).toSet
     ...
     reservedPart1.get.roadMaintainer.get.id should be (originalEvkPart1.head)
     reservedPart2.nonEmpty should be (true)
     reservedPart2.get.roadMaintainer.get.id should be (originalEvkPart2.head)
 ...
 test("Test changeDirection When after the creation of valid project links on a project Then the side code of road addresses should be successfully reversed.") {
   runWithRollback {
     ...
     projectService.createProjectLinks(Seq(12352L, 12353L, 12354L).map(_.toString), project.id, roadPart = RoadPart(9999, 1), track = Track.LeftSide, userGivenDiscontinuity = Discontinuity.Continuous, administrativeClass = AdministrativeClass.State, roadLinkSource = LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), user = "test", roadName = "road name")
     ...
     projectService.createProjectLinks(Seq(12355L, 12356L, 12357L).map(_.toString), project.id, RoadPart(9999, 1), Track.LeftSide, Discontinuity.Continuous, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, ArealRoadMaintainer.getEVK(8), "test", "road name")
 ...
 test("Test checkRoadPartsReservable When there are some transferred road links to that same road part.") {
   runWithRollback {
     ...
     val ra = Seq(
       //Combined
       Roadway(raId,     roadwayNumber1, roadPart,    AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0L, 20L), reversed = false, DateTime.now(), None, "test_user", None, ArealRoadMaintainer.getEVK(8), NoTermination, startDate, None),
       //RightSide
       Roadway(raId + 1, roadwayNumber2, newRoadPart, AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0L, 50L), reversed = false, DateTime.now(), None, "test_user", None, ArealRoadMaintainer.getEVK(8), NoTermination, startDate, None))
     ...
     val rap = Project(0L, ProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.now().plusDays(5), DateTime.now(), "Some additional info", Seq(), Seq(), None, roadMaintainers = Set())
     val project = projectService.createRoadLinkProject(rap)
     ...
     projectService.saveProject(project.copy(reservedParts = Seq(reservedPart1, reservedPart2), roadMaintainers = Set()))
     ...
     projectService.updateProjectLinks(project_id, lastLink, Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart, 0, None, 1, 5, Option(ArealRoadMaintainer.getEVK(8)), reversed = false, None)
     ...
     projectService.updateProjectLinks(project_id, projectLinksSet, Seq(), RoadAddressChangeType.Transfer, "test", newRoadPart, 0, None, 1, 5, Option(ArealRoadMaintainer.getEVK(1)), reversed = false, None)
 ...
 test("Test projectService.createProjectLinks() discontinuity assignment When creating some links Then " +
   ...
   runWithRollback {
     ...
     projectService.createProjectLinks(Seq(12347L.toString, 12346L.toString), project.id, RoadPart(9999, 1), Track.Combined, Discontinuity.MinorDiscontinuity, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(8), "test", "road name")
 ...
 test("Test updateRoadwaysAndLinearLocationsWithProjectLinks When VIITE-2236 situation Then don't violate unique constraint on roadway_point table.") {
   runWithRollback {
     ...
     runUpdateToDb(
       sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
             values ('1052907','40998','22006','12','0','0','215','0','5',to_date('01.01.2017','DD.MM.YYYY'),NULL,'import',to_timestamp('27.12.2019 13:12:51','DD.MM.YYYY HH24:MI:SS'),'1','0',to_date('28.12.2016','DD.MM.YYYY'),NULL,'EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
             values ('1052911','40998','22006','12','0','0','215','0','2',to_date('01.01.1989','DD.MM.YYYY'),to_date('31.12.2016','DD.MM.YYYY'),'import',to_timestamp('27.12.2019 13:12:51','DD.MM.YYYY HH24:MI:SS'),'1','0',to_date('28.12.2016','DD.MM.YYYY'),NULL,'EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
             values ('1052912','40999','22006','12','0','215','216','0','2',to_date('01.01.1989','DD.MM.YYYY'),to_date('30.12.1997','DD.MM.YYYY'),'import',to_timestamp('27.12.2019 13:12:51','DD.MM.YYYY HH24:MI:SS'),'1','1',to_date('31.01.2013','DD.MM.YYYY'),NULL,'EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
             values ('1052910','41000','22006','34','0','0','310','0','2',to_date('01.01.1989','DD.MM.YYYY'),NULL,'import',to_timestamp('27.12.2019 13:12:51','DD.MM.YYYY HH24:MI:SS'),'1','0',to_date('14.06.2016','DD.MM.YYYY'),NULL,'EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
             values ('1052909','6446223','22006','23','0','0','45','0','2',to_date('20.05.2007','DD.MM.YYYY'),NULL,'import',to_timestamp('27.12.2019 13:12:51','DD.MM.YYYY HH24:MI:SS'),'1','0',to_date('14.06.2016','DD.MM.YYYY'),NULL,'EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
             values ('1052906','6446225','22006','45','0','0','248','0','1',to_date('20.05.2007','DD.MM.YYYY'),NULL,'import',to_timestamp('27.12.2019 13:12:51','DD.MM.YYYY HH24:MI:SS'),'1','0',to_date('25.01.2017','DD.MM.YYYY'),NULL,'EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway (id,roadway_number,road_number,road_part_number,track,start_addr_m,end_addr_m,reversed,discontinuity,start_date,end_date,created_by,created_time,administrative_class,terminated,valid_from,valid_to,road_maintainer)
             values ('1052908','166883589','22006','12','0','215','295','0','2',to_date('01.01.2017','DD.MM.YYYY'),NULL,'import',to_timestamp('27.12.2019 13:12:51','DD.MM.YYYY HH24:MI:SS'),'1','0',to_date('28.12.2016','DD.MM.YYYY'),NULL,'EVK2')""")
     ...
     runUpdateToDb(
       sql"""INSERT INTO roadway_changes (project_id,change_type,old_road_number,new_road_number,old_road_part_number,new_road_part_number,old_track,
                             new_track,old_start_addr_m,new_start_addr_m,old_end_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,
                             old_administrative_class,old_discontinuity,reversed,roadway_change_id, new_road_maintainer, old_road_maintainer)
             VALUES ('1000351','3','22006','22006','12','67','0','0','0','0','177','177','2','1','1','5','1','1000753','EVK2','EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway_changes (project_id,change_type,old_road_number,new_road_number,old_road_part_number,new_road_part_number,old_track,
                             new_track,old_start_addr_m,new_start_addr_m,old_end_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,
                             old_administrative_class,old_discontinuity,reversed,roadway_change_id, new_road_maintainer, old_road_maintainer)
             VALUES ('1000351','3','22006','22006','34','12','0','0','0','0','127','127','5','1','1','5','0','1000754','EVK2','EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway_changes (project_id,change_type,old_road_number,new_road_number,old_road_part_number,new_road_part_number,old_track,
                             new_track,old_start_addr_m,new_start_addr_m,old_end_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,
                             old_administrative_class,old_discontinuity,reversed,roadway_change_id, new_road_maintainer, old_road_maintainer)
             VALUES ('1000351','3','22006','22006','12','56','0','0','215','0','295','80','5','1','1','2','1','1000755','EVK2','EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway_changes (project_id,change_type,old_road_number,new_road_number,old_road_part_number,new_road_part_number,old_track,
                             new_track,old_start_addr_m,new_start_addr_m,old_end_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,
                             old_administrative_class,old_discontinuity,reversed,roadway_change_id, new_road_maintainer, old_road_maintainer)
             VALUES ('1000351','3','22006','22006','34','23','0','0','127','0','310','183','2','1','1','2','0','1000756','EVK2','EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway_changes (project_id,change_type,old_road_number,new_road_number,old_road_part_number,new_road_part_number,old_track,
                             new_track,old_start_addr_m,new_start_addr_m,old_end_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,
                             old_administrative_class,old_discontinuity,reversed,roadway_change_id, new_road_maintainer, old_road_maintainer)
             VALUES ('1000351','3','22006','22006','23','68','0','0','0','0','45','45','1','1','1','2','1','1000757','EVK2','EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway_changes (project_id,change_type,old_road_number,new_road_number,old_road_part_number,new_road_part_number,old_track,
                             new_track,old_start_addr_m,new_start_addr_m,old_end_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,
                             old_administrative_class,old_discontinuity,reversed,roadway_change_id, new_road_maintainer, old_road_maintainer)
             VALUES ('1000351','3','22006','22006','45','24','0','0','0','0','248','248','2','1','1','1','1','1000758','EVK2','EVK2')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway_changes (project_id,change_type,old_road_number,new_road_number,old_road_part_number,new_road_part_number,old_track,
                             new_track,old_start_addr_m,new_start_addr_m,old_end_addr_m,new_end_addr_m,new_discontinuity,new_administrative_class,
                             old_administrative_class,old_discontinuity,reversed,roadway_change_id, new_road_maintainer, old_road_maintainer)
             VALUEs ('1000351','3','22006','22006','12','56','0','0','177','80','215','118','5','1','1','5','1','1000759','EVK2','EVK2')""")
 ...
 test("Test defaultSectionCalculatorStrategy.assignAddrMValues() " +
   ...
   runWithRollback {
     ...
     val createSuccessLeft  = projectService.createProjectLinks(rl1.map(_.linkId), project.id, RoadPart(9999, 1), Track.LeftSide,  Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(0), project.createdBy, "9999_1")
     when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(rl2)
     val createSuccessRight = projectService.createProjectLinks(rl2.map(_.linkId), project.id, RoadPart(9999, 1), Track.RightSide, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(0), project.createdBy, "9999_1")
 ...
 test("Test defaultSectionCalculatorStrategy.assignAddrMValues() " +
   ...
   runWithRollback {
     ...
     when(mockRoadLinkService.getRoadLinksHistoryFromVVH(any[Set[String]])).thenReturn(Seq())
     when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(rl1)
     val createSuccessLeft  = projectService.createProjectLinks(rl1.map(_.linkId), project.id, RoadPart(9999, newRoadPartNumber), Track.LeftSide,  Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(0), project.createdBy, "9999_1")
     when(mockRoadLinkService.getRoadLinksByLinkIds(any[Set[String]])).thenReturn(rl2)
     val createSuccessRight = projectService.createProjectLinks(rl2.map(_.linkId), project.id, RoadPart(9999, newRoadPartNumber), Track.RightSide, Discontinuity.EndOfRoad, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface,  ArealRoadMaintainer.getEVK(0), project.createdBy, "9999_1")
 ...
 test("Test defaultSectionCalculatorStrategy.updateRoadwaysAndLinearLocationsWithProjectLinks() " +
   ...
   runWithRollback {
     ...
     runUpdateToDb(
       sql"""INSERT INTO roadway (id, roadway_number, road_number, road_part_number, track, start_addr_m, end_addr_m, reversed, discontinuity, start_date, end_date, created_by, created_time, administrative_class, terminated, valid_from, valid_to, road_maintainer)
            VALUES(107964, 335560416, 46020, 1, 0, 0, 785, 0, 1, '2022-01-01', NULL, $user, '2022-02-17 09:48:15.355', 2, 0, '2022-02-17 09:48:15.355', NULL, 'EVK3')""")
   ...
   runWithRollback {
     ...
     runUpdateToDb(
       sql"""INSERT INTO roadway (id, roadway_number, road_number, road_part_number, track, start_addr_m, end_addr_m, reversed, discontinuity, start_date, end_date, created_by, created_time, administrative_class, terminated, valid_from, valid_to, road_maintainer)
             VALUES(107964, 335560416, 46020, 1, 0, 0, 369, 0, 4, '2022-01-01', NULL, $user, '2022-02-17 09:48:15.355', 2, 0, '2022-02-17 09:48:15.355', NULL, 'EVK3')""")
     runUpdateToDb(
       sql"""INSERT INTO roadway (id, roadway_number, road_number, road_part_number, track, start_addr_m, end_addr_m, reversed, discontinuity, start_date, end_date, created_by, created_time, administrative_class, terminated, valid_from, valid_to, road_maintainer)
             VALUES(1111, 335560417, 46020, 1, 0, 369, 624, 0, 1, '2022-01-01', NULL, $user, '2022-02-17 09:48:15.355', 2, 0, '2022-02-17 09:48:15.355', NULL, 'EVK3')""")
     runUpdateToDb(sql"""INSERT INTO link (id, "source", adjusted_timestamp, created_time) VALUES(2621644, 4, 1634598047000, '2022-02-17 09:48:15.355'),(2621698, 4, 1533863903000, '2022-02-17 09:48:15.355'),(2621642, 4, 1634598047000, '2022-02-17 09:48:15.355'),(2621640, 4, 1634598047000, '2022-02-17 09:48:15.355'),(2621632, 4, 1634598047000, '2022-02-17 09:48:15.355'),(2621636, 4, 1533863903000, '2022-02-17 09:48:15.355'),(2621288, 4, 1533863903000, '2022-02-17 09:48:15.355'),(2621287, 4, 1533863903000, '2022-02-17 09:48:15.355')""")
 ...
 test("Test defaultSectionCalculatorStrategy.updateRoadwaysAndLinearLocationsWithProjectLinks() " +
   ...
   runWithRollback {
     ...
     val groupedByRoadway = newLinearLocs
       .groupBy(_.roadwayNumber)
       .toSeq
       .sortBy(_._1) // deterministic group iteration for stable tests



     groupedByRoadway.foreach { case (roadwayNumber, locationsInRoadway) =>
       val ordered = locationsInRoadway.sortBy(ll =>
         (ll.orderNumber, ll.startMValue, ll.id)
       )

       ordered.sliding(2).foreach {
         case Seq(first, next) =>
           withClue(
             s"""Continuity failed in roadway $roadwayNumber:
                |first(id=${first.id}, linkId=${first.linkId}, sideCode=${first.sideCode},
                |      order=${first.orderNumber}, first=${first.getFirstPoint}, last=${first.getLastPoint})
                |next (id=${next.id}, linkId=${next.linkId}, sideCode=${next.sideCode},
                |      order=${next.orderNumber}, first=${next.getFirstPoint}, last=${next.getLastPoint})
                |""".stripMargin
           ) {
             first.getLastPoint should be(next.getFirstPoint)
           }
         case _ => // one-element group, nothing to compare
       }
     }
   ...
   /*  val it = sortedLls.values.flatten.toSeq.sliding(2)
     while (it.hasNext) {
       it.next() match {
         case Seq(first, next) => {
           println(s"TESTING THE CONTINUITY OF A PAIR OF LINEAR LOCATIONS :::: ")
           println(s"FIRST :: ID :: ${first.id} ::: LINKID :: ${first.linkId}  ::: STARTMVALUE :: ${first.startMValue} ::: ENDMVALUE :: ${first.endMValue} ::: ORDER NUMBER ${first.orderNumber} :: GETFIRSTPOINT :: ${first.getFirstPoint} ::  GETLASTPOINT :: ${first.getLastPoint} :: SIDECODE :: ${first.sideCode}  :: ROADWAYNUMBER :: ${first.roadwayNumber} ")
           println(s"NEXT  :: ID :: ${next.id}  ::: LINKID ::  ${next.linkId} ::: STARTMVALUE ::  ${next.startMValue} ::: ENDMVALUE ::  ${next.endMValue} ::: ORDER NUMBER ${first.orderNumber} :: GETFIRSTPOINT :: ${next.getFirstPoint} ::  GETLASTPOINT :: ${next.getLastPoint} :: SIDECODE :: ${next.sideCode}  :: ROADWAYNUMBER :: ${first.roadwayNumber}  ")
           val lastOfFirst = first.getLastPoint
           val firstOfNext = next.getFirstPoint

           println(s"LAST OF FIRST ::: ${lastOfFirst}")
           println(s"FIRST OF NEXT ::: ${firstOfNext}")
 ...
 test("Test projectService.recalculateProjectLinks() and the way roadway numbers are handled in the process") {
   ...
   runWithRollback {
     ...
     val roadMaintainer = ArealRoadMaintainer.getEVK(9)

     // create the original roadway
     roadwayDAO.create(Seq(
       Roadway(originalRoadwayId, originalRoadwayNumber, roadPart, AdministrativeClass.State, Track.Combined, Discontinuity.EndOfRoad, AddrMRange(0, 700), reversedBoolean, DateTime.now(),None,"Test", Some("Test"), roadMaintainer, TerminationCode.NoTermination, DateTime.now(),None)
     ))
 ...
 test("Test projectService.recalculateProjectLinks() Real life project on road part 110/1, recalculation should complete successfully") {
   ...
   runWithRollback {
     ...
     val roadways = Seq(
       Roadway(65784,90509805,roadPart,AdministrativeClass.Municipality,Track.Combined,Discontinuity.Continuous,AddrMRange(1800, 3940),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(65512,90509888,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(3940, 4960),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(65618,90510413,roadPart,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(3940, 4960),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75083,148600148,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.MinorDiscontinuity,AddrMRange(0, 1800),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75344,148694049,roadPart,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.MinorDiscontinuity,AddrMRange(0, 1800),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
...
/* TODO: After changes to the average length calculation logic, the averages are rounded up resulting in a higher end value. The validity must be verified.
 ...
 test("Test projectService.recalculateProjectLinks() When terminating part of two track section and part of one track section. " +
   ...
   runWithRollback {
     ...
     val roadways = Seq(
       ...
       Roadway(6187,44928,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(615, 1135),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(7297,46088,roadPart,AdministrativeClass.Municipality,Track.Combined,Discontinuity.Continuous,AddrMRange(1365, 2510),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(7584,46089,roadPart,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(2510, 5638),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(70651,148121546,roadPart,AdministrativeClass.Municipality,Track.Combined,Discontinuity.Continuous,AddrMRange(0, 615),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(73189,148122112,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.MinorDiscontinuity,AddrMRange(1135, 1148),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(73470,148127435,roadPart,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(615, 1135),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(73064,148127875,roadPart,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.MinorDiscontinuity,AddrMRange(1135, 1148),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(83706,260730736,roadPart,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(1148, 1365),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(83762,260730742,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.MinorDiscontinuity,AddrMRange(1148, 1365),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
 ...
 test("Test projectService.recalculateProjectLinks() Real life project on road parts 4/218, 4/219 and 11960/1, recalculation should complete successfully") {
   ...
   runWithRollback {
     ...
     val roadways = Seq(
       Roadway(7609,45461,roadPart1,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(0, 6104),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(3),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(7691,45462,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(0, 5624),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(3),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(7506,45463,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.ChangingEVKCode,AddrMRange(5624, 6565),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(3),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
     ...
     roadPart1ProjectLinks.map(_.addrMRange.end).max should be (8899) // roadPart1ProjectLinks.map(_.addrMRange.end).max should be (8898) //TODO: After changes to the average length calculation logic, the averages are rounded up resulting in a higher end value. The validity must be verified.
     ...
     left1.map(_.addrMRange.end).max should be (8468)// left1.map(_.addrMRange.end).max should be (8467) //TODO: After changes to the average length calculation logic, the averages are rounded up resulting in a higher end value. The validity must be verified.
     right1.map(_.addrMRange.start).min should be (5688)
     right1.map(_.addrMRange.end).max should be (8468) //  //TODO: After changes to the average length calculation logic, the averages are rounded up resulting in a higher end value. The validity must be verified.
 ...
 test("Test recalculateProjectLinks() When creating roundabout from New and Transfer links then roadway numbers should be assigned correctly. (three different roadways for the roundabout in this case)") {
   ...
   runWithRollback{
     ...
     val roadways = Seq(
       Roadway(79881,190813844,roadPart,AdministrativeClass.Municipality,Track.Combined,Discontinuity.Continuous,AddrMRange(739, 957),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(79679,190813846,roadPart,AdministrativeClass.Municipality,Track.Combined,Discontinuity.EndOfRoad,AddrMRange(1371, 2025),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(79381,190813848,roadPart,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(957, 1371),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(78993,190813849,roadPart,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(957, 1371),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(88915,306319328,roadPart,AdministrativeClass.Municipality,Track.Combined,Discontinuity.MinorDiscontinuity,AddrMRange(0, 739),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
 ...
 test("Test projectService.recalculateProjectLinks() When creating two track road from single track road" +
   ...
   runWithRollback {
     ...
     val roadways = Seq(
       Roadway(19743,54807,roadPart1,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(0, 6008),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(19619,54808,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(0, 2459),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(19180,54809,roadPart2,AdministrativeClass.State,Track.Combined,Discontinuity.Continuous,AddrMRange(2459, 7292),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
 ...
 private def addProjectLinksToProject(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roadPart: RoadPart = RoadPart(19999, 1),
                                      discontinuity: Discontinuity = Discontinuity.Continuous, roadMaintainer: ArealRoadMaintainer = ArealRoadMaintainer.getEVK(8), roadwayId: Long = 0L,
   ...
   def withTrack(t: Track): Seq[ProjectLink] = {
     addrM.init.zip(addrM.tail).map { case (st, en) =>
       projectLink(AddrMRange(st, en), t, project.id, roadAddressChangeType, roadPart, discontinuity, roadMaintainer, roadwayId)
     }
   }
  ...
  test("Test recalculateProjectLinks() When terminating last project links from two track section (that are of different lengths), creating a minor discontinuity on both tracks, then the tracks should be adjusted to match at the discontinuous part.") {
   ...
   runWithRollback {
     ...
     val roadways = Seq(
       Roadway(82725,225079136,RoadPart(49529,1),AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(0, 696),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(82872,225079139,RoadPart(49529,1),AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(0, 696),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(82220,225079142,RoadPart(49529,1),AdministrativeClass.Municipality,Track.Combined,Discontinuity.EndOfRoad,AddrMRange(696, 1042),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
  ...
  test("Test projectService.recalculateProjectLinks() Real life project on road parts 40921/2 and 31329/1, recalculation should complete successfully") {
   ...
   runWithRollback {
     ...
     val roadways = Seq(
       Roadway(59551,43168588,roadPart1,AdministrativeClass.Municipality,Track.Combined,Discontinuity.EndOfRoad,AddrMRange(5621, 5992),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(74878,148600188,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(5209, 5528),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75119,148600207,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(3481, 3915),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75007,148600384,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(3400, 3481),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(74886,148600417,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(3161, 3400),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75256,148600420,roadPart1,AdministrativeClass.Municipality,Track.Combined,Discontinuity.MinorDiscontinuity,AddrMRange(1170, 1494),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(74889,148600423,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(937, 1170),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75025,148600426,roadPart1,AdministrativeClass.Municipality,Track.Combined,Discontinuity.Continuous,AddrMRange(695, 937),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(74690,148600429,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(0, 695),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(74803,148694062,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(5209, 5528),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(74576,148694072,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(3481, 3915),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75072,148694086,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(3161, 3400),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75327,148694087,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(937, 1170),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(75100,148694088,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(0, 695),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(76524,168748574,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(3400, 3481),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(79525,190813336,roundaboutRoadPart,AdministrativeClass.Municipality,Track.Combined,Discontinuity.EndOfRoad,AddrMRange(0, 67),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(79295,190960312,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.Continuous,AddrMRange(5528, 5621),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(79183,190960315,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.Continuous,AddrMRange(5528, 5621),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(81391,202221121,roadPart1,AdministrativeClass.Municipality,Track.Combined,Discontinuity.Continuous,AddrMRange(1494, 3161),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(85053,262360669,roadPart1,AdministrativeClass.Municipality,Track.Combined,Discontinuity.MinorDiscontinuity,AddrMRange(3915, 5209),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
 ...
 test("Test assignAddrMValues() When Terminating a small section on a two track road in the middle of road part Then the terminated sections should be adjusted to match") {
   runWithRollback {
     ...
     val roadways = Seq(
       Roadway(85042,262301768,roadPart1,AdministrativeClass.Municipality,Track.RightSide,Discontinuity.EndOfRoad,AddrMRange(0, 1463),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None),
       Roadway(85415,262301771,roadPart1,AdministrativeClass.Municipality,Track.LeftSide,Discontinuity.EndOfRoad,AddrMRange(0, 1463),false,DateTime.now().minusDays(2),None,"test",None,ArealRoadMaintainer.getEVK(10),TerminationCode.NoTermination,DateTime.now().minusDays(2),None)
     )
