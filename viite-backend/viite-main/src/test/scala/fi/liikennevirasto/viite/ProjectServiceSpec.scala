...
class ProjectServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
    ...
    new Answer[Seq[RoadLink]]() {
      override def answer(invocation: InvocationOnMock): Seq[RoadLink] = {
        ...
        projectLinks.groupBy(_.linkId).filterKeys(l => ids.contains(l)).mapValues { pl =>
          val startP = Point(pl.map(_.addrMRange.startAddrM).min, 0.0)
          val endP = Point(pl.map(_.addrMRange.endAddrM).max, 0.0)
          ...
          val forcedGeom = pl.filter(l => l.id == -1000L && l.geometry.nonEmpty).sortBy(_.addrMRange.startAddrM)
  ...
  private def setUpProjectWithLinks(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roadPart: RoadPart = RoadPart(19999, 1),
    ...
    def projectLink(addrMRange: AddrMRange, track: Track, projectId: Long, status: RoadAddressChangeType = RoadAddressChangeType.NotHandled, roadPart: RoadPart = RoadPart(19999, 1), discontinuity: Discontinuity = Discontinuity.Continuous, ely: Long = 8L, linkId: String = 0L.toString, roadwayId: Long = 0L, linearLocationId: Long = 0L, startDate: Option[DateTime] = None) = {
      ProjectLink(NewIdValue, roadPart, track, discontinuity, addrMRange, addrMRange, startDate, None, Some("User"), linkId, 0.0, (addrMRange.endAddrM-addrMRange.startAddrM).toDouble, SideCode.TowardsDigitizing, (NoCP, NoCP), (NoCP, NoCP), Seq(Point(0.0, addrMRange.startAddrM), Point(0.0, addrMRange.endAddrM)), projectId, status, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, (addrMRange.endAddrM-addrMRange.startAddrM).toDouble, roadwayId, linearLocationId, ely, reversed = false, None, 0L)
    }
  ...
  test("Test projectService.changeDirection When project links have a defined direction Then return project_links with adjusted M addresses and side codes due to the reversing ") {
    runWithRollback {
      ...
      val maxBefore = if (projectLinks.nonEmpty) calculatedProjectLinks.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM else 0
      val maxAfter = if (updatedProjectLinks.nonEmpty) updatedProjectLinks.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM else 0
      ...
      (combined ++ right).sortBy(_.addrMRange.startAddrM).foldLeft(Seq.empty[ProjectLink]) { case (seq, plink) =>
        if (seq.nonEmpty)
          seq.last.addrMRange.endAddrM should be(plink.addrMRange.startAddrM)
        seq ++ Seq(plink)
      }

      (combined ++ left).sortBy(_.addrMRange.startAddrM).foldLeft(Seq.empty[ProjectLink]) { case (seq, plink) =>
        if (seq.nonEmpty)
          seq.last.addrMRange.endAddrM should be(plink.addrMRange.startAddrM)
        seq ++ Seq(plink)
      }
      ...
      projectLinks.sortBy(_.addrMRange.endAddrM).map(_.geometry).zip(secondUpdatedProjectLinks.sortBy(_.addrMRange.endAddrM).map(_.geometry)).forall { case (x, y) => x == y }
...
//    Project1 projectlink is connected to Project2 projectlink at point 1000, 10 and there is no junction which means that Project2 projectlink cannot be created with new action
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    def runTest(reversed: Boolean) = {
      ...
      val maxLink                = calculatedProjectLinks.maxBy(_.addrMRange.endAddrM)
      if (reversed)
        maxLink.id should be(lastLink.id)
      else
        maxLink.id should be(headLink.id)
      maxLink.addrMRange.endAddrM should be > 0L
  ...
  test("Test projectLinkDAO.getProjectLinks() When after the \"Numerointi\" operation on project links of a newly created project Then the last returned link should have a discontinuity of \"EndOfRoad\", all the others should be \"Continuous\" ") {
    runWithRollback {
      ...
      projectService.updateProjectLinks(project1.id, Set(fetchedProjectLinks.maxBy(_.addrMRange.endAddrM).id), fetchedProjectLinks.map(_.linkId), RoadAddressChangeType.Renumeration, "TestUser", RoadPart(6, 207), 0, None, AdministrativeClass.State.value, Discontinuity.EndOfRoad.value, Some(8))

      //Descending order by end address
      val projectLinks = projectLinkDAO.fetchProjectLinks(project1.id).sortBy(-_.addrMRange.endAddrM)
  ...
  test("Test projectService.updateProjectLinks() When new user given addressMValues, even on Left/Right tracks Then all links should keep continuous and incremented address values (calibration ones included).") {
    ...
    runWithRollback {
      ...
      val links = projectLinkDAO.fetchProjectLinks(project.id).sortBy(_.addrMRange.startAddrM)

      links.filterNot(_.track == Track.RightSide).sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        m should be(pl.addrMRange.startAddrM)
        pl.addrMRange.endAddrM
      }
      links.filterNot(_.track == Track.LeftSide).sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        m should be(pl.addrMRange.startAddrM)
        pl.addrMRange.endAddrM
      }

      links.sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        if (pl.calibrationPoints._1.nonEmpty) {
          pl.calibrationPoints._1.get.addressMValue should be(pl.addrMRange.startAddrM)
          pl.calibrationPoints._1.get.segmentMValue should be(pl.endMValue - pl.startMValue)
        }
        if (pl.calibrationPoints._2.nonEmpty) {
          pl.calibrationPoints._2.get.addressMValue should be(pl.addrMRange.endAddrM)
      ...
      val newEndAddressValue = Seq(links.filter(_.linkId == linkidToIncrement).head.addrMRange.endAddrM.toInt, valueToIncrement.toInt).sum
      ...
      /**
        * Test 1.
        */
      linksAfterGivenAddrMValue.filterNot(_.track == Track.RightSide).sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        m should be(pl.addrMRange.startAddrM)
        pl.addrMRange.endAddrM
      }
      linksAfterGivenAddrMValue.filterNot(_.track == Track.LeftSide).sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        m should be(pl.addrMRange.startAddrM)
        pl.addrMRange.endAddrM
      }
      ...
      /**
        * Test 2.
        */
      linksAfterGivenAddrMValue.sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        if (pl.calibrationPoints._1.nonEmpty) {
          pl.calibrationPoints._1.get.addressMValue should be(pl.addrMRange.startAddrM)
          pl.calibrationPoints._1.get.segmentMValue should be(pl.endMValue - pl.startMValue)
        }
      ...
      val linksBefore = links.filter(_.addrMRange.endAddrM >= extendedLink.addrMRange.endAddrM).sortBy(_.addrMRange.endAddrM)
      val linksAfter = linksAfterGivenAddrMValue.filter(_.addrMRange.endAddrM >= extendedLink.addrMRange.endAddrM).sortBy(_.addrMRange.endAddrM)
      linksBefore.zip(linksAfter).foreach { case (st, en) =>
        liesInBetween(en.addrMRange.endAddrM, (st.addrMRange.endAddrM + valueToIncrement - coeff, st.addrMRange.endAddrM + valueToIncrement + coeff))
      }
      ...
      val secondNewEndAddressValue = Seq(links.filter(_.linkId == secondLinkidToIncrement).head.addrMRange.endAddrM.toInt, secondValueToIncrement.toInt).sum
      ...
      /**
        * Test 3.
        */
      linksAfterSecondGivenAddrMValue.filterNot(_.track == Track.RightSide).sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        m should be(pl.addrMRange.startAddrM)
        pl.addrMRange.endAddrM
      }
      linksAfterSecondGivenAddrMValue.filterNot(_.track == Track.LeftSide).sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        m should be(pl.addrMRange.startAddrM)
        pl.addrMRange.endAddrM
      }
      ...
      /**
        * Test 4.
        */
      linksAfterSecondGivenAddrMValue.sortBy(_.addrMRange.endAddrM).scanLeft(0.0) { case (m, pl) =>
        if (pl.calibrationPoints._1.nonEmpty) {
          pl.calibrationPoints._1.get.addressMValue should be(pl.addrMRange.startAddrM)
          pl.calibrationPoints._1.get.segmentMValue should be(pl.endMValue - pl.startMValue)
        }
      ...
      val secondLinksBefore = linksAfterGivenAddrMValue.filter(_.addrMRange.endAddrM >= secondExtendedLink.addrMRange.endAddrM).sortBy(_.addrMRange.endAddrM)
      val secondLinksAfter = linksAfterSecondGivenAddrMValue.filter(_.addrMRange.endAddrM >= secondExtendedLink.addrMRange.endAddrM).sortBy(_.addrMRange.endAddrM)
      secondLinksBefore.zip(secondLinksAfter).foreach { case (st, en) =>
        liesInBetween(en.addrMRange.endAddrM, (st.addrMRange.endAddrM + valueToIncrement + secondValueToIncrement - coeff, st.addrMRange.endAddrM + valueToIncrement + secondValueToIncrement + coeff))
      }
...
//    }
    ...
    runWithRollback {
      ...
      val links_ = projectLinkDAO.fetchProjectLinks(project.id).sortBy(_.addrMRange.startAddrM)
      val linksAfterGivenAddrMValue = ProjectSectionCalculator.assignMValues(links_).sortBy(_.addrMRange.endAddrM)
      ...
      val linksAfterFirstReverse = ProjectSectionCalculator.assignMValues(links_).sortBy(_.addrMRange.endAddrM)
      projectService.changeDirection(project.id, RoadPart(9999, 1), Seq(LinkToRevert(pl1.id, pl1.linkId, pl1.status.value, pl1.geometry)), ProjectCoordinates(0, 0, 0), "TestUserTwo")
      val linksAfterSecondReverse = ProjectSectionCalculator.assignMValues(links_).sortBy(_.addrMRange.endAddrM)

      linksAfterGivenAddrMValue.sortBy(pl => (pl.addrMRange.endAddrM, pl.addrMRange.startAddrM)).zip(linksAfterSecondReverse.sortBy(pl => (pl.addrMRange.endAddrM, pl.addrMRange.startAddrM))).foreach { case (st, en) =>
        (st.addrMRange.startAddrM, st.addrMRange.endAddrM) should be(en.addrMRange.startAddrM, en.addrMRange.endAddrM)
  ...
  test("Test projectService.changeDirection() When after the creation of valid project links on a project Then the discontinuity of road addresses that are not the same road number and road part number should not be altered.") {
    runWithRollback {
      ...
      val linksBeforeChange = projectLinkDAO.fetchProjectLinks(project.id).sortBy(_.addrMRange.startAddrM)
      ...
      projectService.changeDirection(project.id, RoadPart(9999, 1), Seq(LinkToRevert(pl1.id, pl1.linkId, pl1.status.value, pl1.geometry)), ProjectCoordinates(0, 0, 0), "TestUserTwo")
      val linksAfterChange = projectLinkDAO.fetchProjectLinks(project.id).sortBy(_.addrMRange.startAddrM)
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    runWithRollback {
      ...
      Track.values.foreach(track => {
        assignedTrackGrouped.getOrElse(track, Seq()).sortBy(_.addrMRange.startAddrM).zip(originalTrackGrouped.getOrElse(track, Seq()).sortBy(_.addrMRange.startAddrM)).foreach { case (calculated, original) => {
          calculated.addrMRange.startAddrM should be(original.addrMRange.startAddrM)
          calculated.addrMRange.endAddrM should be(original.addrMRange.endAddrM)
      ...
      val minAddress = updatedProjectLinks.minBy(_.addrMRange.startAddrM)
      val maxAddress = updatedProjectLinks.maxBy(_.addrMRange.endAddrM)
      minAddress.originalAddrMRange.endAddrM should be(projectLinks.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM)
      maxAddress.originalAddrMRange.startAddrM should be(0)
  ...
  test("Test projectService.updateProjectLinks() When transferring last ajr 1 & 2 links from part 1 to part 2 and adjust addrMRange.endAddrMs for last links from transferred part and transfer the rest of the part 2 " +
...
//        */
      ...
      val projectLinks3 = ProjectSectionCalculator.assignMValues(projectLinks2).sortBy(_.addrMRange.endAddrM)

      val parts = projectLinks3.partition(_.roadPart === RoadPart(9999, 1))
      val part1tracks = parts._1.partition(_.track === Track.RightSide)
      part1tracks._1.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM should be(part1tracks._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM)
      val part2tracks = parts._2.partition(_.track === Track.RightSide)
      part2tracks._1.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM should be(part2tracks._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM)
    }
  }

  test("Test projectService.updateProjectLinks When transferring the rest of the part 2 and then the last ajr 1 & 2 links from part 1 to part 2 and adjust addrMRange.endAddrMs for last links from transferred part " +
    "Then the mAddressValues of the last links should be equal in both sides of the tracks for part 1." )
  {
    runWithRollback {
      /**
  ...
  test("Test expireHistoryRows When link is renumbered Then set end date to old roadway") {
    runWithRollback {
      ...
      // Create project link
      val roadAddressLength = roadway.addrMRange.endAddrM - roadway.addrMRange.startAddrM
      val projectId = Sequences.nextViiteProjectId
      val newLength = 110.123
      val newEndAddr = 110
      val rAddr = roadway.addrMRange
      val projectLink = ProjectLink(Sequences.nextProjectLinkId, roadway.roadPart, roadway.track, roadway.discontinuity, AddrMRange(rAddr.startAddrM, rAddr.endAddrM + 10), rAddr, Some(DateTime.now().plusMonths(1)), None, Some("test"), linkId, 0.0, newLength, SideCode.TowardsDigitizing, (RoadAddressCP, RoadAddressCP), (NoCP, NoCP), Seq(Point(0.0, 0.0), Point(0.0, newLength)), projectId, RoadAddressChangeType.Renumeration, roadway.administrativeClass, LinkGeomSource.NormalLinkInterface, newLength, roadway.id, 1234, roadway.ely, reversed = false, None, DateTime.now().minusMonths(10).getMillis, roadway.roadwayNumber, roadway.roadName, Some(roadAddressLength), Some(AddrMRange(0,newEndAddr)), Some(roadway.track), Some(roadway.roadPart))
  ...
  test("Test expireHistoryRows " +
    ...
    runWithRollback {
      ...
      newCurrent.foreach(_.addrMRange.endAddrM should be(currentRoadway.addrMRange.endAddrM))
      historyRows.foreach(_.addrMRange.endAddrM should be(currentRoadway.addrMRange.endAddrM))
    ...
    runWithRollback {
      ...
      *  with history rows. */
      ...
      val linearLocation = Seq(
        dummyLinearLocation(currentRoadway.roadwayNumber, 0, linkId, 0.0, currentRoadway.addrMRange.endAddrM).copy(id = linkId.toLong),
        dummyLinearLocation(currentRoadway.roadwayNumber, 0, linkId+1, 0.0, currentRoadway.addrMRange.endAddrM).copy(id = (linkId+1).toLong),
        dummyLinearLocation(currentRoadway.roadwayNumber, 0, linkId+2, 0.0, currentRoadway.addrMRange.endAddrM).copy(id = (linkId+2).toLong)
      )
      ...
      newCurrent.filter(hr => hr.addrMRange.startAddrM ==   0 && hr.addrMRange.endAddrM == 100) should have size 1
      newCurrent.filter(hr => hr.addrMRange.startAddrM == 100 && hr.addrMRange.endAddrM == 200) should have size 1
      newCurrent.filter(hr => hr.addrMRange.startAddrM == 200 && hr.addrMRange.endAddrM == 300) should have size 1

      // The same validFrom date for all
      AllByRoadAndPart.map(_.validFrom).distinct should have size 1

      val historiesByAddrM = historyRows.groupBy(_.addrMRange.startAddrM)
      ...
      historyRows.filter(hr => hr.addrMRange.startAddrM ==   0 && hr.addrMRange.endAddrM == 100) should have size 2
      historyRows.filter(hr => hr.addrMRange.startAddrM == 100 && hr.addrMRange.endAddrM == 200) should have size 3
      historyRows.filter(hr => hr.addrMRange.startAddrM == 200 && hr.addrMRange.endAddrM == 300) should have size 2
    }
  }
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(AddrMRange(0L,100L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(RoadPart(66666, 1)), Some(0L), Some(AddrMRange(0L,100L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.New,
          ...
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1),
        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(RoadPart(55555, 1)), Some(0L), Some(AddrMRange(0L,100L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(RoadPart(55555, 1)), Some(0L), Some(AddrMRange(0L,100L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1)
      )
  ...
  test("Test getProjectLinks When doing some operations (Unchanged with termination test, repeats termination update), Then the calibration points are cleared and moved to correct positions") {
    ...
    runWithRollback {
      ...
      val sortedRoad206AfterTermination = updatedProjectLinks2.filter(_.roadPart == RP206).sortBy(_.addrMRange.startAddrM)
      ...
      sortedRoad206AfterTermination.filter(_.status != RoadAddressChangeType.Termination).last.calibrationPoints should be((None, Some(ProjectCalibrationPoint(lastValid.linkId, lastValid.endMValue, lastValid.addrMRange.endAddrM, RoadAddressCP))))
  ...
  test("Test getProjectLinks When doing some operations (Transfer and then Terminate), Then the calibration points are cleared and moved to correct positions") {
    ...
    runWithRollback {
      ...
      val highestDistanceEnd = projectLinks.map(p => p.addrMRange.endAddrM).max
      ...
      val updatedProjectLinks = ProjectSectionCalculator.assignMValues(links_).sortBy(_.addrMRange.endAddrM) ++ projectLinkDAO.fetchProjectLinks(savedProject.id).filter(_.status == RoadAddressChangeType.Termination)
        updatedProjectLinks.exists { x => x.status == RoadAddressChangeType.Transfer } should be(true)
      updatedProjectLinks.exists { x => x.status == RoadAddressChangeType.Termination } should be(true)
      val sortedProjectLinks = updatedProjectLinks.sortBy(_.addrMRange.startAddrM)
      ...
      sortedProjectLinks.last.calibrationPoints._2.get.addressMValue should be (updatedProjectLinks.map(p => p.addrMRange.endAddrM).max)
      ...
      val updatedProjectLinks2 = ProjectSectionCalculator.assignMValues(projectLinkDAO.fetchProjectLinks(savedProject.id)).sortBy(_.addrMRange.endAddrM) ++ projectLinkDAO.fetchProjectLinks(savedProject.id).filter(_.status == RoadAddressChangeType.Termination)
      val sortedProjectLinks2 = updatedProjectLinks2.sortBy(_.addrMRange.startAddrM)
      ...
      sortedProjectLinks2.last.calibrationPoints._2.get.addressMValue should be (updatedProjectLinks2.map(p => p.addrMRange.endAddrM).max)
  ...
  test("Test getProjectLinks When doing some operations (Terminate then transfer), Then the calibration points are cleared and moved to correct positions") {
    ...
    runWithRollback {
      ...
      sortedProjectLinks.last.calibrationPoints._2.nonEmpty should be (true)
      sortedProjectLinks.last.calibrationPoints._2.get.segmentMValue should be (442.89)
      val highestDistanceEnd2 = updatedProjectLinks.map(p => p.addrMRange.endAddrM).max
      sortedProjectLinks.last.calibrationPoints._2.get.addressMValue should be (updatedProjectLinks.map(p => p.addrMRange.endAddrM).max)
      sortedProjectLinks.last.endCalibrationPointType should be (RoadAddressCP)
      ...
      val updatedProjectLinks2 = ProjectSectionCalculator.assignMValues(links_).sortBy(_.addrMRange.endAddrM) ++ projectLinkDAO.fetchProjectLinks(savedProject.id).filter(_.status == RoadAddressChangeType.Termination)
      val sortedProjectLinks2 = updatedProjectLinks2.sortBy(_.addrMRange.startAddrM)
  ...
  test("Test changeDirection() When projectLinks are reversed the track codes must switch and start_addr_m and end_addr_m should be the same for the first and last links") {
    runWithRollback {
      ...
      val projectLinksBefore = projectLinkDAO.fetchProjectLinks(project.id).sortBy(pl => (pl.addrMRange.startAddrM, pl.track.value))
      ...
      val projectLinksAfter = projectLinkDAO.fetchProjectLinks(project.id).sortBy(pl => (pl.addrMRange.startAddrM, -pl.track.value))
      projectLinksAfter.size should be(projectLinksBefore.size)
      projectLinksAfter.head.addrMRange.startAddrM should be(projectLinksBefore.head.addrMRange.startAddrM)
      projectLinksAfter.last.addrMRange.endAddrM should be(projectLinksBefore.last.addrMRange.endAddrM)
      var i = 0
  ...
  test("Test handleNewRoadNames - Test if a new RoadName is created from a project link and without duplicates") {
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(AddrMRange(  0L,100L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(AddrMRange(100L,200L)), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(8L)),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1),

        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(AddrMRange(100L,200L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(AddrMRange(100L,200L)), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(8L)),
          Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 2),

        RoadwayChangeInfo(RoadAddressChangeType.New,
          source = dummyRoadwayChangeSection(Some(testRoad9999), Some(0L), Some(AddrMRange(200L,400L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(testRoad9998), Some(0L), Some(AddrMRange(200L,400L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.Termination,
          source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(AddrMRange(  0L,100L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(AddrMRange(100L,200L)), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(8L)),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1),

        RoadwayChangeInfo(RoadAddressChangeType.Unchanged,
          source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(AddrMRange(100L,200L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(AddrMRange(100L,200L)), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(8L)),
          Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 2),

        RoadwayChangeInfo(RoadAddressChangeType.Unchanged,
          source = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(AddrMRange(200L,400L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(RoadPart(roadNumber, 1)), Some(0L), Some(AddrMRange(200L,400L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          Discontinuity.Continuous, AdministrativeClass.apply(3), reversed = false, 3)
      )
    ...
    runWithRollback {
      ...
      val changeInfos = List(
        RoadwayChangeInfo(RoadAddressChangeType.Transfer,
          source = dummyRoadwayChangeSection(Some(RoadPart(srcRoadNumber,    1)), Some(0L), Some(AddrMRange(  0L,100L)), Some(AdministrativeClass.apply(1)), Some(Discontinuity.Continuous), Some(8L)),
          target = dummyRoadwayChangeSection(Some(RoadPart(targetRoadNumber, 1)), Some(0L), Some(AddrMRange(100L,200L)), Some(AdministrativeClass.apply(3)), Some(Discontinuity.Continuous), Some(8L)),
          Discontinuity.Continuous, AdministrativeClass.apply(1), reversed = false, 1)
      )
  ...
  test("Test checkRoadPartsReservable When there are some transferred road links to that same road part.") {
    runWithRollback {
      ...
      val lastLink = Set(projectLinks.filter(_.roadPart == roadPart).maxBy(_.addrMRange.endAddrM).id)
      ...
      reservation.filter(_.roadPart == roadPart   ).head.addressLength should be(Some(ra.head.addrMRange.endAddrM))
      reservation.filter(_.roadPart == newRoadPart).head.addressLength should be(Some(ra.last.addrMRange.endAddrM))
      ...
      reservation2.filter(_.roadPart == roadPart   ).head.addressLength should be(Some(ra.head.addrMRange.endAddrM))
      reservation.filter (_.roadPart == newRoadPart).head.addressLength should be(Some(ra.last.addrMRange.endAddrM))

      formed2.filter(_.roadPart == roadPart   ).head.newLength should be(Some(newLengthOfTheRoadPart1))
      formed2.filter(_.roadPart == newRoadPart).head.newLength should be(Some(ra.last.addrMRange.endAddrM+lengthOfTheTransferredPart))
  ...
  test("Test projectService.createProjectLinks() discontinuity assignment When creating some links Then " +
    ...
    runWithRollback {
      ...
      val afterAllProjectLinksCreation = projectLinkDAO.fetchProjectLinks(project.id).sortBy(_.addrMRange.startAddrM)
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    runWithRollback {
      ...
      val updatedRoadway1 = roadway1.copy(addrMRange = AddrMRange(roadway1.addrMRange.startAddrM, endValue), discontinuity = Discontinuity.EndOfRoad, id = projectLinkLeft1.roadwayId)
      val updatedRoadway2 = roadway2.copy(addrMRange = AddrMRange(roadway1.addrMRange.startAddrM, endValue), discontinuity = Discontinuity.EndOfRoad, id = projectLinkRight1.roadwayId)
      ...
      projectLinksWithAssignedValues.filter(pl => pl.status == RoadAddressChangeType.Unchanged).foreach(pl => {
        pl.addrMRange.startAddrM should be(0)
        pl.addrMRange.endAddrM should be(endValue)
      ...
      calculatedNewLinkLefts.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM should be (endValue)
      calculatedNewLinkRights.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM should be (endValue)

      val maxNewLeft  = calculatedNewLinkLefts.maxBy(_.addrMRange.endAddrM)
      val maxNewRight = calculatedNewLinkRights.maxBy(_.addrMRange.endAddrM)
      maxNewLeft.addrMRange.endAddrM should be(maxNewRight.addrMRange.endAddrM)
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    runWithRollback {
      ...
      val updatedRoadway1 = roadway1.copy(addrMRange = AddrMRange(roadway1.addrMRange.startAddrM, endValue), discontinuity = Discontinuity.EndOfRoad, id = projectLinkLeft1.roadwayId)
      val updatedRoadway2 = roadway2.copy(addrMRange = AddrMRange(roadway2.addrMRange.startAddrM, endValue), discontinuity = Discontinuity.EndOfRoad, id = projectLinkRight1.roadwayId)
      ...
      val calculatedNewLinkLefts = calculatedNewLinks.filter(_.track == Track.LeftSide)
      val calculatedNewLinkRights = calculatedNewLinks.filter(_.track == Track.RightSide)
      calculatedNewLinkLefts.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM should be (0)
      calculatedNewLinkRights.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM should be (0)
  ...
  test("Test defaultSectionCalculatorStrategy.updateRoadwaysAndLinearLocationsWithProjectLinks() " +
    ...
    runWithRollback {
      ...
      val roadways = roadwayDAO.fetchAllByRoadAndPart(roadPart)
      val firstRw = roadways.find(r => r.addrMRange.startAddrM == 0 && r.track == Track.Combined)
      firstRw should be ('defined)
      val secondRw = roadways.find(r => r.addrMRange.endAddrM == 785 && r.track == Track.RightSide)
      secondRw should be ('defined)
      val thirdRw = roadways.find(r => r.addrMRange.startAddrM == 369 && r.addrMRange.endAddrM == 1035 && r.track == Track.LeftSide)
      thirdRw should be ('defined)
      val fourthRw = roadways.find(r => r.addrMRange.startAddrM == 785 && r.addrMRange.endAddrM == 1035 && r.track == Track.RightSide)
  ...
  test("Test projectService.recalculateProjectLinks() and the way roadway numbers are handled in the process") {
    ...
    runWithRollback {
      ...
      val recalculatedProjectLinks = projectLinkDAO.fetchProjectLinks(projectId).sortBy(_.originalAddrMRange.startAddrM)

      recalculatedProjectLinks.foreach(pl => println(pl.addrMRange.startAddrM, pl.addrMRange.endAddrM, pl.status, pl.roadwayNumber))
  ...
  private def addProjectLinksToProject(roadAddressChangeType: RoadAddressChangeType, addrM: Seq[Long], changeTrack: Boolean = false, roadPart: RoadPart = RoadPart(19999, 1),
    ...
    val newLinksWithRoadwayInfo = if(withRoadInfo){
      ...
      newLinks.map(nl => {
        val roadway = roadways.find(r => r.roadPart == nl.roadPart && r.addrMRange.startAddrM == nl.addrMRange.startAddrM && r.addrMRange.endAddrM == nl.addrMRange.endAddrM)
        if (roadway.nonEmpty) {
