...
class JunctionDAOSpec extends AnyFunSuite with Matchers {
  ...
  test("Test When fetching junctions for road address browser then return junctions based on the query") {
    ...
    runWithRollback {
      ...
      // fetch and test
      val resultForQuery1 = dao.fetchJunctionsForRoadAddressBrowser(Some("2022-01-01"), None, Some(roadPart.roadNumber), None, None, None)
      ...
      val resultForQuery2 = dao.fetchJunctionsForRoadAddressBrowser(Some("1992-01-01"), None, Some(roadPart.roadNumber), None, None, None)
