...
Left     ^  ^   Right
  ...
  test("Test assignAddrMValues() When two track road part has terminated links at the track changing spot and new minor discontinuity on both tracks Then calculation should be able to adjust the tracks to match") {
    ...
    runWithRollback {
      ...
      nonTerminated.filter(_.track == Track.Combined).head.startAddrMValue should be (endAddrMValueOnMinorDiscontinuity.head) //combined section startAddressMValue should be the same as the endAddrMValue of the minor discontinuity links
      nonTerminated.filter(_.status != RoadAddressChangeType.Transfer).foreach(pl => pl.startAddrMValue should be (pl.originalStartAddrMValue)) // addressMValues should match original addressMValues on all links except Transferred links
      nonTerminated.filter(_.status != RoadAddressChangeType.Transfer).foreach(pl => pl.endAddrMValue should be (pl.originalEndAddrMValue))
    }
  }
}