...
class DefaultSectionCalculatorStrategySpec extends FunSuite with Matchers {
    ...
    test("Test defaultSectionCalculatorStrategy.assignMValues() " +
      ...
      */
      runWithRollback {
        ...
        val fl = projectLinksWithAssignedValues.filter(_.addrMRange.startAddrM == 0)
        fl should have size 1
        fl.head.addrMRange.endAddrM should be(roadways.find(_.addrMRange.startAddrM == 0).get.addrMRange.endAddrM)

        val mdl = projectLinksWithAssignedValues.filter(_.addrMRange.startAddrM == fl.head.addrMRange.endAddrM)
        mdl should have size 2
        mdl.head.addrMRange.endAddrM should be (mdl.last.addrMRange.endAddrM)

        val mx = projectLinksWithAssignedValues.filter(_.addrMRange.startAddrM == mdl.head.addrMRange.endAddrM)
        mx should have size 1
        mx.head.addrMRange.endAddrM - mx.head.addrMRange.startAddrM should be(mx.head.originalAddrMRange.endAddrM - mx.head.originalAddrMRange.startAddrM)
      }
    }
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() and findStartingPoints When there is MinorDiscontinuity + Continuous + EndOfRoad from east to west intention " +
    ...
    runWithRollback {
      ...
      projectLinksWithAssignedValues.sortBy(_.addrMRange.endAddrM).zip(intendedOrder).forall { case (pl1, pl2) => {pl1.id == pl2.id}} should be(true)
    }
    ...
    test("Test defaultSectionCalculatorStrategy.assignMValues() " +
      ...
      val pls = projectLinksWithAssignedValues.filter(_.track != Track.RightSide).sortBy(_.addrMRange.startAddrM)
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() and findStartingPoints When using 4 geometries that end up in a point " +
    ...
    runWithRollback {
      ...
      val projectLinkLeft1 = ProjectLink(-1000L, RoadPart(9999, 1), Track.apply(2), Discontinuity.Continuous, AddrMRange(0L, 0L), AddrMRange(0L, 0L), None, None, None, 12345L.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), geomLeft1, 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomLeft1), 0L, 0, 0, reversed = false, None, 86400L)
      val projectLinkLeft2 = ProjectLink(-1000L, RoadPart(9999, 1), Track.apply(2), Discontinuity.Continuous, AddrMRange(0L, 0L), AddrMRange(0L, 0L), None, None, None, 12346L.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), geomLeft2, 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomLeft2), 0L, 0, 0, reversed = false, None, 86400L)

      val geomRight1 = Seq(Point(10.0, 20.0), Point(20.0, 20.0))
      val geomRight2 = Seq(Point(20.0, 20.0), Point(30.0, 20.0))

      val projectLinkRight1 = ProjectLink(-1000L, RoadPart(9999, 1), Track.apply(1), Discontinuity.Continuous, AddrMRange(0L, 0L), AddrMRange(0L, 0L), None, None, None, 12347L.toString, 0.0, 0.0, SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), geomRight1, 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomRight1), 0L, 0, 0, reversed = false, None, 86400L)
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() When using 2 tracks with proper pairing administrative class sections Then they will calculate values properly") {
    runWithRollback {
      ...
      buildTestDataForProject(Some(project), Some(Seq(roadway1.copy(addrMRange = AddrMRange(roadway1.addrMRange.startAddrM, roadway2.addrMRange.endAddrM)))), Some(Seq(linearLocation1, linearLocation2)), Some(leftSideProjectLinks ++ rightSideProjectLinks))
      ...
      !projectLinksWithAssignedValues.exists(pl => pl.addrMRange.endAddrM == 0L) should be (true)
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    runWithRollback {
      ...
      buildTestDataForProject(Some(project), Some(Seq(roadway1.copy(addrMRange = AddrMRange(roadway1.addrMRange.startAddrM, roadway2.addrMRange.endAddrM), administrativeClass = AdministrativeClass.State))), Some(Seq(linearLocation1, linearLocation2)), Some(leftSideProjectLinks ++ rightSideProjectLinks))
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    runWithRollback {
      ...
      val projectLinkLeft1 = ProjectLink(nextPlId, RoadPart(9999, 1), Track.apply(2), Discontinuity.Continuous,    AddrMRange(0L, 30L), AddrMRange(0L, 30L), None, None, Some("user"), 12345L.toString, 0.0, getEndMValue(geomLeft1), SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), geomLeft1, 0L, RoadAddressChangeType.Unchanged, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomLeft1), roadwayId,   linearLocationId,   0, reversed = false, None, 86400L, roadwayNumber = 12345L)
      ...
      val projectLinkLeft3 = ProjectLink(nextPlId, RoadPart(9999, 1), Track.apply(2), Discontinuity.Discontinuous, AddrMRange(0L, 24L), AddrMRange(0L, 24L), None, None, Some("user"), 12347L.toString, 0.0, getEndMValue(geomLeft3), SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), geomLeft3, 0L, RoadAddressChangeType.Transfer,  AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomLeft3), roadwayId+1, linearLocationId+1, 0, reversed = false, None, 86400L, roadwayNumber = 12347L)
      ...
      val projectLinkRight2 = ProjectLink(nextPlId, RoadPart(9999, 1), Track.apply(1), Discontinuity.Continuous,   AddrMRange(20L, 62L), AddrMRange(20L, 62L), None, None, Some("user"), 12349L.toString, 0.0, getEndMValue(geomRight2), SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), geomRight2, 0L, RoadAddressChangeType.Unchanged, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomRight2), roadwayId+3, linearLocationId+3, 0, reversed = false, None, 86400L, roadwayNumber = 12349L)
      val projectLinkRight3 = ProjectLink(nextPlId, RoadPart(9999, 1), Track.apply(1), Discontinuity.Discontinuous,AddrMRange( 0L, 22L), AddrMRange( 0L, 22L), None, None, Some("user"), 12350L.toString, 0.0, getEndMValue(geomRight3), SideCode.Unknown, (NoCP, NoCP), (NoCP, NoCP), geomRight3, 0L, RoadAddressChangeType.Transfer,  AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomRight3), roadwayId+4, linearLocationId+4, 0, reversed = false, None, 86400L, roadwayNumber = 12350L)
      ...
      val roadwayLengths = Seq(roadway1, roadway2, roadway3).map(rw => rw.id → (rw.addrMRange.endAddrM - rw.addrMRange.startAddrM)).toMap
      ...
      val roadwayLengthsLeft = Seq(roadway4, roadway5).map(rw => rw.id → (rw.addrMRange.endAddrM - rw.addrMRange.startAddrM)).toMap
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    runWithRollback {
      ...
      leftCalculated.map(_.addrMRange.endAddrM  ).max should be(rightCalculated.map(_.addrMRange.endAddrM  ).max)
      leftCalculated.map(_.addrMRange.startAddrM).min should be(rightCalculated.map(_.addrMRange.startAddrM).min)
    ...
    runWithRollback {
      ...
      */
      ...
      calculated.foreach(pl => {
        ...
        pl.originalAddrMRange should be(pl.addrMRange)
      })

      calculated.filter(_.addrMRange.startAddrM == 0)    should have size 2
      calculated.filter(_.addrMRange.endAddrM == 2570) should have size 2
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() " +
    ...
    runWithRollback {
      ...
      val projectLinks = Seq(
        ...
        ProjectLink(1056,roadPart,Track.RightSide,Discontinuity.MinorDiscontinuity,AddrMRange(2790,2881),AddrMRange(2790,2881),None,None,Some(createdBy), 2618583.toString,  0.0  , 91.162,SideCode.AgainstDigitizing,(RoadAddressCP,  NoCP),(RoadAddressCP,NoCP),List(Point(566561.0,6770386.0,0.0), Point(566482.0,6770432.0,0.0)),projectId,RoadAddressChangeType.Unchanged,  AdministrativeClass.Municipality,NormalLinkInterface,       91.162,73430,574704,3,reversed = false,None,1630018852000L,148124318,roadName,None,None,None,None),
        ProjectLink(1057,roadPart,Track.LeftSide,Discontinuity.MinorDiscontinuity, AddrMRange(2790,2883),AddrMRange(2790,2883),None,None,Some(createdBy), 2618584.toString,  0.0  , 93.474,SideCode.AgainstDigitizing,(RoadAddressCP,  NoCP),(RoadAddressCP,NoCP),List(Point(566567.0,6770395.0,0.0), Point(566482.0,6770432.0,0.0)),projectId,RoadAddressChangeType.Unchanged,  AdministrativeClass.Municipality,NormalLinkInterface,       93.474,74754,564156,3,reversed = false,None,1630018852000L,148128092,roadName,None,None,None,None),
        ...
        ProjectLink(1058,roadPart,Track.LeftSide, Discontinuity.Continuous,        AddrMRange(2883,2917),AddrMRange(2883,2917),None,None,Some(createdBy),12675141.toString,  0.0  , 34.254,SideCode.AgainstDigitizing,(NoCP,           NoCP),(NoCP,         NoCP),List(Point(566595.0,6770376.0,0.0), Point(566567.0,6770395.0,0.0)),projectId,RoadAddressChangeType.Termination,AdministrativeClass.Municipality,ComplementaryLinkInterface,34.254,74754,564160,3,reversed = false,None,1652734800000L,148128092,roadName,None,None,None,None),
        ...
        ProjectLink(1080,roadPart,Track.LeftSide, Discontinuity.MinorDiscontinuity,AddrMRange(5179,5188),AddrMRange(5179,5188),None,None,Some(createdBy), 6929633.toString,  0.0  ,  8.487,SideCode.TowardsDigitizing,(JunctionPointCP,NoCP),(JunctionPointCP,RoadAddressCP  ),List(Point(568207.0,6771287.0,0.0), Point(568214.0,6771292.0,0.0)),projectId,RoadAddressChangeType.Transfer,AdministrativeClass.Municipality,NormalLinkInterface,  8.487,73190,563293,3,reversed = false,None,1630018852000L,148127472,roadName,None,None,None,None),
      ...
      val calculatedAddressValues = calculated.map(pl => (pl.addrMRange, pl.track.value))
      ...
      // Group by track and addrMRange.startAddrM
      val t = (calculatedAddressValues ++ targetAddresses).groupBy(v => (v._2, v._1.startAddrM))
  ...
  private def testFindStartingPointsWithTwoMinorDiscontinuitiesNewInMiddleTwoTrackOnOtherSide(sideCode: SideCode, roadAddressChangeType: RoadAddressChangeType): Unit = {
    runWithRollback {
      ...
      val addrMRange1 = if (sideCode == SideCode.TowardsDigitizing) AddrMRange(15L,30L) else AddrMRange(0,15L)
      val projectLink1 = ProjectLink(plId + 1, RoadPart(9999, 1), Track.Combined, Discontinuity.Continuous, addrMRange1, addrMRange1, None, None, None, 12345L.toString, 0.0, 15.0, sideCode, (NoCP, NoCP), (NoCP, NoCP), geom1, 0L, roadAddressChangeType, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geom1), 0L, 0, 0, reversed = false, None, 86400L)

      val addrMRange34 = if (sideCode == SideCode.TowardsDigitizing) AddrMRange(0L,15L) else AddrMRange(15L,30L)
      val projectLinkNew3 = ProjectLink(plId + 3, RoadPart(9999, 1), if (sideCode == SideCode.TowardsDigitizing) Track.LeftSide  else Track.RightSide, Discontinuity.Continuous, addrMRange34, addrMRange34, None, None, None, 12347L.toString, 0.0, 0.0, sideCode, (NoCP, NoCP), (NoCP, NoCP), geom3, 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geom3), 0L, 0, 0, reversed = false, None, 86400L)
      val projectLinkNew4 = ProjectLink(plId + 4, RoadPart(9999, 1), if (sideCode == SideCode.TowardsDigitizing) Track.RightSide else Track.LeftSide,  Discontinuity.Continuous, addrMRange34, addrMRange34, None, None, None, 12348L.toString, 0.0, 0.0, sideCode, (NoCP, NoCP), (NoCP, NoCP), geom4, 0L, RoadAddressChangeType.New, AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geom4), 0L, 0, 0, reversed = false, None, 86400L)
...
//      val groupedLeft1: ListMap[Long, Seq[ProjectLink]] = ListMap(left.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      val groupedRight1: ListMap[Long, Seq[ProjectLink]] = ListMap(right.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      groupedLeft1.size should be (groupedRight1.size)
//      groupedLeft1.size should be (1)
//      groupedLeft1.zip(groupedRight1).forall(zipped => zipped._1._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM == zipped._2._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM) should be (true)
//
...
//      val groupedLeft2: ListMap[Long, Seq[ProjectLink]] = ListMap(left2.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      val groupedRight2: ListMap[Long, Seq[ProjectLink]] = ListMap(right2.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      groupedLeft2.size should be (groupedRight2.size)
//      groupedLeft2.size should be (2)
//      groupedLeft2.zip(groupedRight2).forall(zipped => zipped._1._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM == zipped._2._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM) should be (true)
//
...
//      val groupedLeft3: ListMap[Long, Seq[ProjectLink]] = ListMap(left3.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      val groupedRight3: ListMap[Long, Seq[ProjectLink]] = ListMap(right3.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      groupedLeft3.size should be (groupedRight3.size)
//      groupedLeft3.size should be (3)
//      //groupedLeft3.zip(groupedRight3).forall(zipped => zipped._1._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM == zipped._2._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM) should be (true)
//
...
//      val groupedLeft4: ListMap[Long, Seq[ProjectLink]] = ListMap(left4.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      val groupedRight4: ListMap[Long, Seq[ProjectLink]] = ListMap(right4.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
//      groupedLeft4.size should be (groupedRight4.size)
//      groupedLeft4.size should be (2)
//      groupedLeft4.zip(groupedRight4).forall(zipped => zipped._1._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM == zipped._2._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM) should be (true)
//    }
//  }
  /* Unnecessary roadwaynumber checkings? VIITE-2348, DefaultSectionCalculatorStrategy.scala: 273, adjustableToRoadwayNumberAttribution. */
...
Left     |  ^   Right
      ...
      */
      ...
      assignedValues = assignedValues.filterNot(_.addrMRange.startAddrM == 14) :+  assignedValues.filter(_.addrMRange.startAddrM == 14).head.copy(discontinuity = Discontinuity.Continuous)

      val (left, right) = assignedValues.filterNot(_.track == Track.Combined).partition(_.track == Track.LeftSide)
      val groupedLeft: ListMap[Long, Seq[ProjectLink]] = ListMap(left.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
      val groupedRight: ListMap[Long, Seq[ProjectLink]] = ListMap(right.groupBy(_.roadwayNumber).toSeq.sortBy(r => r._2.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM): _*)
      groupedLeft.size should be (groupedRight.size)
      groupedLeft.size should be (2)
      groupedLeft.zip(groupedRight).forall(zipped => zipped._1._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM == zipped._2._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM) should be (true)
    }
  }*/
...
Left     ^  ^   Right
  ...
  test("Test findStartingPoints When adding new combined link before existing Unhandled links Then the starting point should be the loose candidate from the new link") {
    runWithRollback {
      ...
      val plId = Sequences.nextProjectLinkId
      val projectLinkNewComb1Before = ProjectLink(plId,     RoadPart(9999, 1), Track.Combined, Discontinuity.Continuous, AddrMRange(0L, 0L), AddrMRange(0L, 0L), None, None, None, 12344L.toString, 0.0, 5.0, SideCode.TowardsDigitizing, (NoCP, NoCP), (NoCP, NoCP), geomNewComb1,      0L, RoadAddressChangeType.New,        AdministrativeClass.State, LinkGeomSource.NormalLinkInterface, GeometryUtils.geometryLength(geomNewComb1),      0L, 0, 0, reversed = false, None, 86400L)
