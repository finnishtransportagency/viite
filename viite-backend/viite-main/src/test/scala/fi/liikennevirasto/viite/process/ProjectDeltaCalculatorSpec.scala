...
//    RoadAddress(id = start, linearLocationId = 1L, roadPart = RoadPart(5, 205), administrativeClass = AdministrativeClass.State, track = Track.Combined, discontinuity = Discontinuity.Continuous, addrMRange = AddrMRange(start, start + distance), linkId = start.toString, startMValue = 0.0, endMValue = distance.toDouble, sideCode = TowardsDigitizing, adjustedTimestamp = 0L, geometry = Seq(Point(0.0, start), Point(0.0, start + distance)), linkGeomSource = NormalLinkInterface, ely = 8, evk = 8, terminated = NoTermination, roadwayNumber = roadwayNumber)
//  }
//
...
//    Roadway(p.roadwayId, p.roadwayNumber, p.roadPart, p.administrativeClass, p.track, p.discontinuity, AddrMRange(ps.head.addrMRange.start, ps.last.addrMRange.end), p.reversed, startDate, p.endDate, p.createdBy.getOrElse("-"), p.roadName, p.ely, p.evk, TerminationCode.NoTermination, DateTime.now(), None)
//  }
//
