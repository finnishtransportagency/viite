...
class ProjectDeltaCalculatorSpec extends AnyFunSuite with Matchers {
  ...
  private def toProjectLinkWithMove(project: Project, status: RoadAddressChangeType)(roadAddress: RoadAddress): ProjectLink = {
    ProjectLink(roadAddress.id, roadAddress.roadPart, roadAddress.track, roadAddress.discontinuity,
      ...
      roadAddress.linkGeomSource, GeometryUtils.geometryLength(roadAddress.geometry), roadAddress.id, roadAddress.linearLocationId, roadAddress.arealRoadMaintainer, reversed = false, None, 748800L)
  }

  def toRoadway(ps: Seq[ProjectLink]): Roadway = {
    val p = ps.head
    val startDate = p.startDate.getOrElse(DateTime.now()).minusDays(1)
    Roadway(p.roadwayId, p.roadwayNumber, p.roadPart, p.administrativeClass, p.track, p.discontinuity, AddrMRange(ps.head.addrMRange.start, ps.last.addrMRange.end), p.reversed, startDate, p.endDate, p.createdBy.getOrElse("-"), p.roadName, p.arealRoadMaintainer, TerminationCode.NoTermination, DateTime.now(), None)
  }
  ...
  test("Test ProjectDeltaCalculator.partition " +
       ...
       "Then returns the correct From RoadSection -> To RoadSection mapping.") {
    val ely12 = ArealRoadMaintainer.ELYPohjoisPohjanmaa
    runWithRollback {
...
//  }
  ...
  test("Test ProjectDeltaCalculator.partition When executing a Unchanged operation but changing it's ArealRoadMaintainer value  Then returns the correct From RoadSection -> To RoadSection mapping, ensuring the new ARM is in effect.") {
    runWithRollback {
      ...
      val roadway205 = toRoadway(links.map(_._2).map(_.copy(arealRoadMaintainer = originalARM)))
      roadwayDAO.create(Seq(roadway205))
