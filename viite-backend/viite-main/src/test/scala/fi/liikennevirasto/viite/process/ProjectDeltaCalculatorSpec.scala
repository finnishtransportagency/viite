  ...
  test("Test ProjectDeltaCalculator.partition When transfering two roadparts a small roadpart number to the greater roadpart number" +
    ...
    runWithRollback {
      ...
      val addressLinks = addresses.filter(_.endAddrMValue <= addressMLength).map(a => {
        (a, toProjectLink(project, RoadAddressChangeType.Transfer)(a.copy(roadPartNumber = endRoadPart)).copy(roadwayId = 1))
      }) ++ addresses.filter(_.endAddrMValue > addressMLength).map(a => {
        (a, toProjectLink(project, RoadAddressChangeType.Transfer)(a.copy(roadPartNumber = endRoadPart)).copy(roadwayId = 2))
      })
  ...
  test("Test ProjectDeltaCalculator.partition " +
    ...
    runWithRollback {
      ...
      val linksForRoad206rw1 = addresses.filter(a => {
        ...
        toProjectLink(project, RoadAddressChangeType.Transfer)(a).copy(roadPartNumber = 206, roadwayId = 1)
      })
      val linksForRoad206rw2 = addresses.filter(a => {
        a.endAddrMValue > 90
      }).map(a => {
        toProjectLink(project, RoadAddressChangeType.Transfer)(a).copy(roadPartNumber = 206, roadwayId = 2)
      })
