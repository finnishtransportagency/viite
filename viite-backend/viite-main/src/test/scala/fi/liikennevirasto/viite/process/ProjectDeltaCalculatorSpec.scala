...
class ProjectDeltaCalculatorSpec extends FunSuite with Matchers {
  ...
  test("Test ProjectDeltaCalculator.partition When executing multiple transfers on single road part Then returns the correct From RoadSection -> To RoadSection mapping.") {
    ...
    mapping.foreach { elem =>
      elem._1.addrMRange.startAddrM should be(elem._2.addrMRange.startAddrM - project.id)
      elem._1.addrMRange.endAddrM should be(elem._2.addrMRange.endAddrM - project.id)
  ...
  test("Test ProjectDeltaCalculator.partition When executing a Unchanged and 2 transfer on single road part Then returns the correct From RoadSection -> To RoadSection mapping.") {
    runWithRollback {
      ...
      val addresses2 = (11 to 21).map(i => {
        ...
        a.copy(roadPart = RoadPart(a.roadPart.roadNumber, 206), addrMRange = AddrMRange(a.addrMRange.startAddrM - 110L, a.addrMRange.endAddrM - 110L))
      })
      val (transferLinks1, transferLinks2) = addresses2.map(toTransition(project, RoadAddressChangeType.Transfer)).partition(_._2.addrMRange.startAddrM == 0L)
      val projectLinks                     = addresses.map(toTransition (project, RoadAddressChangeType.Unchanged)) ++ transferLinks1.map(l => {
        ...
        (l._1, l._2.copy(addrMRange = AddrMRange(l._2.addrMRange.startAddrM - 10L, l._2.addrMRange.endAddrM - 10L)))
      })
      ...
      start205.map(x => (x._1.addrMRange, x._2.addrMRange)) should be(Some(AddrMRange(0L, 110L), AddrMRange(0L, 110L)))
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition " +
    ...
    implicit val ordering: Ordering[RoadAddress] = Ordering.by(_.addrMRange.endAddrM)
    def getMinAddress(pls: Seq[BaseRoadAddress]): Long = pls.map(_.addrMRange.startAddrM).min
    def getMaxAddress(pls: Seq[BaseRoadAddress]): Long = pls.map(_.addrMRange.endAddrM).max
    def addressTrackChanges(x: (RoadwaySection, RoadwaySection)): (AddrMRange, AddrMRange, Track, Track) = (x._1.addrMRange, x._2.addrMRange, x._1.track, x._2.track)
    def toProjectLinks(transferLinks: IndexedSeq[(RoadAddress, ProjectLink)], track: Track)(implicit addresses: Seq[RoadAddress]): IndexedSeq[ProjectLink] = {
      ...
      transferLinks.map(l => {
        l._2.copy(track = track, addrMRange = AddrMRange(addresses.max.addrMRange.endAddrM - l._2.addrMRange.endAddrM, addresses.max.addrMRange.endAddrM - l._2.addrMRange.startAddrM), roadwayId = roadwayId, reversed = true)
      })
    }
    runWithRollback {
      val distance = 10L
      ...
      val combinedMaxEndAddrM = combinedTrackAddresses.max.addrMRange.endAddrM
      val roadway0      = toRoadway(combinedLinks).copy(track = combinedTrackAddresses.head.track, addrMRange = AddrMRange(                  0, combinedMaxEndAddrM))
      val roadway1      = toRoadway(rightLinks   ).copy(track =     leftTrackAddresses.head.track, addrMRange = AddrMRange(combinedMaxEndAddrM, leftTrackAddresses.max.addrMRange.endAddrM))
      val roadway2      = toRoadway(leftLinks    ).copy(track =    rightTrackAddresses.head.track, addrMRange = AddrMRange(combinedMaxEndAddrM, rightTrackAddresses.max.addrMRange.endAddrM))
      ...
      combined.map(addressTrackChanges) should be(Some(AddrMRange(getMinAddress(combinedLinks), getMaxAddress(combinedLinks)), AddrMRange(getMinAddress(combinedTrackAddresses), getMaxAddress(combinedTrackAddresses)), Track.Combined,  Track.Combined))
      track1.map(addressTrackChanges)   should be(Some(AddrMRange(getMinAddress(rightLinks),    getMaxAddress(rightLinks)   ), AddrMRange(getMinAddress(rightTrackAddresses),    getMaxAddress(rightTrackAddresses)   ), Track.RightSide, Track.LeftSide))
      track2.map(addressTrackChanges)   should be(Some(AddrMRange(getMinAddress(leftLinks),     getMaxAddress(leftLinks)    ), AddrMRange(getMinAddress(leftTrackAddresses),     getMaxAddress(leftTrackAddresses)    ), Track.LeftSide,  Track.RightSide))
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition " +
    ...
    runWithRollback {
      ...
      val (combinedLinks, leftLinks) = projectLinks.filter(_.track == Track.Combined).partition(_.addrMRange.endAddrM < 120)
      val rightLinks    = projectLinks.filter(_.track == Track.RightSide)

      val combinedTrackMaxEnd = combinedTrackAddresses.max.addrMRange.endAddrM
      val roadway0      = toRoadway(combinedLinks).copy(track = combinedTrackAddresses.head.track, addrMRange = AddrMRange(0, combinedTrackMaxEnd))
      val roadway1      = toRoadway(leftLinks    ).copy(track =     leftTrackAddresses.head.track, addrMRange = AddrMRange(combinedTrackMaxEnd,  leftTrackAddresses.max.addrMRange.endAddrM))
      val roadway2      = toRoadway(rightLinks   ).copy(track =    rightTrackAddresses.head.track, addrMRange = AddrMRange(combinedTrackMaxEnd, rightTrackAddresses.max.addrMRange.endAddrM))
      ...
      combined.map(addressTrackChanges).get should be(AddrMRange(getMinAddress(combinedLinks), getMaxAddress(combinedLinks)), AddrMRange(getMinAddress(combinedTrackAddresses), getMaxAddress(combinedTrackAddresses)), Track.Combined,  Track.Combined )
      track1.map(addressTrackChanges).get   should be(AddrMRange(getMinAddress(rightLinks),    getMaxAddress(rightLinks   )), AddrMRange(getMinAddress(rightTrackAddresses),    getMaxAddress(rightTrackAddresses   )), Track.RightSide, Track.RightSide)
      track2.map(addressTrackChanges).get   should be(AddrMRange(getMinAddress(leftLinks),     getMaxAddress(leftLinks    )), AddrMRange(getMinAddress(leftTrackAddresses),     getMaxAddress(leftTrackAddresses    )), Track.Combined,  Track.LeftSide )
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition When executing a numbering operation on single road part Then returns the correct From RoadSection -> To RoadSection mapping with the new road number/road part number.") {
    runWithRollback {
      ...
      correctRoadNumber.map(x => { (x._1.addrMRange, x._2.addrMRange)  }) should be(Some(AddrMRange(0L, 110L), AddrMRange(0L, 110L)))
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition " +
    ...
    runWithRollback {
      ...
      transferredPaired.map(x => { (x._1.addrMRange, x._2.addrMRange) }).foreach(_ should be((AddrMRange(0L, 795L), AddrMRange(205L, 1000L))))

      adjustedTerminated.map(_.addrMRange).foreach(_ should be(AddrMRange(0L, 205L)))
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition " +
    ...
    runWithRollback {
      ...
      transferredPaired.filter(_._1.addrMRange.startAddrM == 0) should have size 1
      ...
      adjustedTerminated.map(_.addrMRange).foreach(_ should be(AddrMRange(0L, 762L)))
    }
  }
   ...
   */
    runWithRollback {
      ...
      val createdTargets = transferredPaired.asInstanceOf[List[(RoadwaySection, RoadwaySection)]].sortBy(rs => (rs._1.addrMRange.startAddrM, rs._1.track.value))
      val validTargets = Seq(
        (AddrMRange(  0L, 125L), Track.Combined,  Discontinuity.Continuous),
        (AddrMRange(125L, 241L), Track.RightSide, Discontinuity.MinorDiscontinuity),
        (AddrMRange(125L, 241L), Track.LeftSide,  Discontinuity.Continuous),
        (AddrMRange(241L, 367L), Track.Combined,  Discontinuity.EndOfRoad))

      createdTargets.map(x => {
        (x._1.addrMRange, x._1.track,x._1.discontinuity)
...
//      (t, toProjectLink(project, RoadAddressChangeType.Transfer)(t.copy(addrMRange.startAddrM = t.addrMRange.startAddrM - d,
//        addrMRange.endAddrM = t.addrMRange.endAddrM - d)))
//    })
//
...
//      to.addrMRange.startAddrM should be(AddrMRange( 0L,109L))
//    })
//  }
  ...
  test("Test ProjectDeltaCalculator.partition When executing a unchanged operation and terminating 2 different tracks, the links have different roadwayNumbers Then returns the correct From RoadSection -> To RoadSection mapping, roadways are not considered.") {
    runWithRollback {
      ...
      termPart2.foreach(_.addrMRange.endAddrM should be(120L))
  ...
  test("Test ProjectDeltaCalculator.partition When executing an AdministrativeClass change from single AdministrativeClass to two AdministrativeClasses on a single road part with unchanged status and creating a new section at the end" +
    ...
    runWithRollback {
      ...
      roadwaySectionPairs.foreach(x => {
        val (to, fr) = x
        (fr.addrMRange.startAddrM == 60 || fr.addrMRange.endAddrM == 60) should be(true)
        (to.addrMRange.startAddrM == 60 || to.addrMRange.endAddrM == 60) should be(true)
        if (fr.addrMRange.startAddrM == 0L) fr.administrativeClass should be(AdministrativeClass.Municipality) else fr.administrativeClass should be(AdministrativeClass.State)
        if (to.addrMRange.startAddrM == 0L) to.administrativeClass should be(AdministrativeClass.Municipality) else to.administrativeClass should be(AdministrativeClass.State)
      })
  ...
  test("Test ProjectDeltaCalculator.partition When executing an AdministrativeClass change to a single AdministrativeClass from two AdministrativeClasses on a single road part with unchanged status and creating a new section at the end" +
    ...
    runWithRollback {
      ...
      uncParts3.foreach(x => {
        ...
        (to.addrMRange.startAddrM == 60 || to.addrMRange.endAddrM == 60) should be(true)
        if (fr.addrMRange.startAddrM == 0L) fr.administrativeClass should be(AdministrativeClass.Municipality) else fr.administrativeClass should be(AdministrativeClass.State)
        if (to.addrMRange.startAddrM == 0L) to.administrativeClass should be(AdministrativeClass.State) else to.administrativeClass should be(AdministrativeClass.State)
      })
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition When end part of a single roadpart is transferred as righside track and a new is created for leftside track" +
    ...
    runWithRollback {
      ...
      (frComb.addrMRange.startAddrM == 0 && frComb.addrMRange.endAddrM == 800) should be(true)
      (toComb.addrMRange.startAddrM == 0 && toComb.addrMRange.endAddrM == 800) should be(true)

      val (toTrans, frTrans) = transfered.head
      (frTrans.addrMRange.startAddrM == 800 && frTrans.addrMRange.endAddrM == 1500) should be(true)
      (toTrans.addrMRange.startAddrM == 800 && toTrans.addrMRange.endAddrM == 1500) should be(true)

      val (toNew, _) = newpart.head
      (toNew.addrMRange.startAddrM == 800 && toNew.addrMRange.endAddrM == 1500) should be(true)
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition When a single roadpart with minor discontinuity and two track part at the end is reversed" +
    ...
    runWithRollback {
      ...
      (p1fr.addrMRange.startAddrM == 1524 && p1fr.addrMRange.endAddrM == 1987 && p1fr.track == Track.RightSide) should be(true)
      (p1to.addrMRange.startAddrM == 0 && p1to.addrMRange.endAddrM == 463 && p1to.track == Track.LeftSide) should be(true)
      // Check address lenght has not changed.
      p1fr.addrMRange.endAddrM - p1fr.addrMRange.startAddrM should be(p1to.addrMRange.endAddrM - p1to.addrMRange.startAddrM)

      val endOfRoadLeft   = uncParts3.find(_._2.track == Track.LeftSide)
      endOfRoadLeft shouldBe defined

      val (p2to, p2fr) = endOfRoadLeft.get
      (p2fr.addrMRange.startAddrM == 1524 && p2fr.addrMRange.endAddrM == 1987 && p2fr.track == Track.LeftSide) should be(true)
      (p2to.addrMRange.startAddrM == 0 && p2to.addrMRange.endAddrM == 463 && p2to.track == Track.RightSide) should be(true)
      p2fr.addrMRange.endAddrM - p2fr.addrMRange.startAddrM should be(p2to.addrMRange.endAddrM - p2to.addrMRange.startAddrM)

      val combinedMiddle   = uncParts3.find(_._2.addrMRange.startAddrM == 889)
      combinedMiddle shouldBe defined

      val (p4to, p4fr) = combinedMiddle.get
      (p4fr.addrMRange.startAddrM == 889 && p4fr.addrMRange.endAddrM == 1524 && p4fr.track == Track.Combined) should be(true)
      (p4to.addrMRange.startAddrM == 463 && p4to.addrMRange.endAddrM == 1098 && p4to.track == Track.Combined) should be(true)
      p4fr.addrMRange.endAddrM - p4fr.addrMRange.startAddrM should be(p4to.addrMRange.endAddrM - p4to.addrMRange.startAddrM)

      val combinedStart   = uncParts3.find(_._2.addrMRange.startAddrM == 0)
      combinedStart shouldBe defined

      val (p3to, p3fr) = combinedStart.get
      (p3fr.addrMRange.startAddrM == 0 && p3fr.addrMRange.endAddrM == 889 && p3fr.track == Track.Combined) should be(true)
      (p3to.addrMRange.startAddrM == 1098 && p3to.addrMRange.endAddrM == 1987 && p3to.track == Track.Combined) should be(true)
      p3fr.addrMRange.endAddrM - p3fr.addrMRange.startAddrM should be(p3to.addrMRange.endAddrM - p3to.addrMRange.startAddrM)
    }
  }
  ...
  test("Test ProjectDeltaCalculator.partition When transfering two roadparts a small roadpart number to the greater roadpart number" +
    ...
    runWithRollback {
      ...
      val addressLinks = addresses.filter(_.addrMRange.endAddrM <= addressMLength).map(a => {
      ...
      }) ++ addresses.filter(_.addrMRange.endAddrM > addressMLength).map(a => {
      ...
      links = links.filter(_.originalAddrMRange.endAddrM <= addressMLength) ++ links.filter(_.originalAddrMRange.endAddrM > addressMLength).map(pl => {
        pl.copy(originalAddrMRange = AddrMRange(pl.originalAddrMRange.startAddrM - addressMLength, pl.originalAddrMRange.endAddrM - addressMLength))
      })
      ...
      fr205.addrMRange should be(to205.addrMRange)
      ...
      fr206.addrMRange should be(AddrMRange(0,addressMLength))
      to206.addrMRange should be(AddrMRange(fr205.addrMRange.endAddrM, fr205.addrMRange.endAddrM + fr206.addrMRange.endAddrM))
  ...
  test("Test ProjectDeltaCalculator.partition " +
    ...
    runWithRollback {
      ...
      val addressLinks = addresses.filter(_.addrMRange.endAddrM <= addressMLengthFirst).map(a => {
        (a, toProjectLink(project, RoadAddressChangeType.Transfer)(a).copy(roadwayId = 1, reversed = true))
      }) ++ addresses.filter(_.addrMRange.endAddrM > addressMLengthFirst).map(a => {
      ...
      links = links.map(pl => {
        pl.copy(addrMRange = AddrMRange(addressMLengthSecond + addressMLengthFirst - pl.addrMRange.endAddrM, addressMLengthSecond + addressMLengthFirst - pl.addrMRange.startAddrM))
      })
  ...
  test("Test ProjectDeltaCalculator.partition When executing a Unchanged operation but changing it's ELY value Then returns the correct From RoadSection -> To RoadSection mapping, ensuring the new ELY is in effect.") {
    runWithRollback {
      ...
      val links       = addresses.filter(_.addrMRange.endAddrM < 61).map(a => {
  ...
  test("Test ProjectDeltaCalculator.partition " +
    ...
    runWithRollback {
      ...
      val splitAddress       = 60
      val linksForRoad205    = addresses.filter(_.addrMRange.endAddrM <= splitAddress).map(a => {
        toProjectLink(project, RoadAddressChangeType.Unchanged)(a).copy(roadwayId = 0)
      })
      val linksForRoad206rw1 = addresses.filter(a => {
        a.addrMRange.endAddrM > splitAddress && a.addrMRange.endAddrM < 90
      }).map(a => {
      ...
      val addressedRoad206Links      = (linksForRoad206rw1 ++ linksForRoad206rw2).map(pl => {
        pl.copy(addrMRange = AddrMRange(pl.addrMRange.startAddrM - splitAddress, pl.addrMRange.endAddrM - splitAddress))
      })
      ...
      fr1.addrMRange should be(AddrMRange(0,splitAddress))
      fr1.discontinuity should be(Discontinuity.Continuous)
      to1.addrMRange should be(AddrMRange(0,splitAddress))
      to1.discontinuity should be(Discontinuity.Continuous)

      val (to2, fr2) = road205and206Partitions._2.head
      fr2.addrMRange should be(AddrMRange(splitAddress,120))
      fr2.discontinuity should be(originalDiscontinuity)
      to2.addrMRange should be(AddrMRange(0,splitAddress))
  ...
  test("Test ProjectDeltaCalculator.partition When executing Multiple transfers with reversal and discontinuity change operations Then returns the correct From RoadSection -> To RoadSection mapping.") {
    ...
    mapping.foreach { case (from, to) =>
      from.addrMRange.endAddrM - from.addrMRange.startAddrM should be(to.addrMRange.endAddrM - to.addrMRange.startAddrM)
  ...
  test("Partitioner should separate links containing calibration points whose origin is ProjectLink") {
    runWithRollback {
      ...
      val projectLinksWithCp = addresses.sortBy(_.addrMRange.startAddrM).map(a => {
      ...
      firstSection.addrMRange  should be(AddrMRange(projectLinksWithCp.head._2.addrMRange.startAddrM, cutPoint.addrMRange.endAddrM))
      secondSection.addrMRange should be(AddrMRange(cutPoint.addrMRange.endAddrM, projectLinksWithCp.last._2.addrMRange.endAddrM))
    }
  }
  ...
  test("Partitioner should not separate links containing junction calibration points whose origin is ProjectLink") {
    runWithRollback {
      ...
      firstSection.addrMRange.startAddrM should be(projectLinksWithCp.head._2.addrMRange.startAddrM)
      firstSection.addrMRange.endAddrM   should be(projectLinksWithCp.last._2.addrMRange.endAddrM  )
    }
  }
  ...
  test("Test partitioner When a roadpart is combined to another with a new link having other part reversed " +
    ...
    runWithRollback {
      ...
      val transferLinks = addresses.take(5).sortBy(_.addrMRange.startAddrM).map(a => {
        val projectLink = toProjectLink(project, RoadAddressChangeType.Transfer)(a)
        projectLink.copy(roadwayId = 2, reversed = true)
      })

      val maxAddr       = transferLinks.last.addrMRange.endAddrM
      val lengthChange  = 2
      val reversedTrans = transferLinks.map(pl => {
        pl.copy(addrMRange = AddrMRange(maxAddr - pl.addrMRange.endAddrM, maxAddr - pl.addrMRange.startAddrM))
      })

      val transferLinks206 = addresses.sortBy(_.addrMRange.startAddrM).map(a => {
        val projectLink = toProjectLink(project, RoadAddressChangeType.Transfer)(a)
        projectLink.copy(addrMRange = AddrMRange(maxAddr + lengthChange + a.addrMRange.startAddrM, maxAddr + lengthChange + a.addrMRange.endAddrM), roadwayId = 3, reversed = false)
      })
      ...
      val roadway205 = toRoadway(Seq(transferLinks.head.copy(addrMRange = AddrMRange(0, addresses.take(5).last.addrMRange.endAddrM)))).copy(id = 2)
      val roadway206 = toRoadway(Seq(transferLinks.head.copy(roadPart = RoadPart(transferLinks.head.roadPart.roadNumber, 206), addrMRange = AddrMRange(0, addresses.last.addrMRange.endAddrM)))).copy(id = 3)
      roadwayDAO.create(Seq(roadway205, roadway206))
      ...
      firstSection.head.addrMRange  should be(AddrMRange(maxAddr, maxAddr + lengthChange))
      firstSection.head.reversed    should be(false)
      secondSection.head.addrMRange should be(AddrMRange(maxAddr + lengthChange, maxAddr + lengthChange + roadway206.addrMRange.endAddrM))
      secondSection.head.reversed   should be(false)
      secondSection.last.addrMRange should be(AddrMRange(0,maxAddr))
      ...
      originalSections205.addrMRange should be(AddrMRange(0,roadway205.addrMRange.endAddrM))

      val originalSections206 = partitioned.originalSections.find(_.roadPartNumberStart == 206).get
      originalSections206.addrMRange should be(AddrMRange(0,roadway206.addrMRange.endAddrM))
    }
  }
}
