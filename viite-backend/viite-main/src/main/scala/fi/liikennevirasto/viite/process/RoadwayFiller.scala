...
object RoadwayFiller {
  ...
  def applyRoadwayChanges(rwChanges: Seq[RwChanges]): Seq[Seq[(Seq[Roadway], Seq[LinearLocation], Seq[ProjectLink])]] = {
    ...
    rwChanges.map(changes => {
      ...
      val ARMChanged                       = if (others.nonEmpty) currentRoadway.arealRoadMaintainer != ArealRoadMaintainer.getELY(others.head.ely) else false
      ...
      val roadways = adminClassed.map { case (adminClassRoadwayNumber, projectLinkSeq) =>
        if (roadwayHasChanges(currentRoadway, adminClassRoadwayNumber, projectLinkSeq) || ARMChanged || addressChanged) {
          ...
          val existingRoadway = Seq(Roadway(NewIdValue, headPl.roadwayNumber, headPl.roadPart, headPl.administrativeClass, headPl.track, lastPl.discontinuity, AddrMRange(headPl.addrMRange.start, lastPl.addrMRange.end), headPl.reversed, currentRoadway.startDate, None, createdBy = headPl.createdBy.get, currentRoadway.roadName,
            ArealRoadMaintainer(s"ELY${headPl.ely}"), NoTermination)) ++ updateAddrMValuesOfHistoryRows(projectLinkSeq, currentRoadway, historyRoadways).map { historyRoadway =>
  ...
  def generateNewRoadwaysWithHistory2(projectLinks    : Seq[ProjectLink],
    ...
    val newAddrMRange = AddrMRange(
      if (reversed) lastProjectLink.originalAddrMRange.start else headProjectLink.originalAddrMRange.start,
      if (reversed) headProjectLink.originalAddrMRange.end   else lastProjectLink.originalAddrMRange.end
    )
    ...
    val noChanges        =  headProjectLink.roadPart == currentRoadway.roadPart &&
                            ...
                            headProjectLink.addrMRange.startsAt(newAddrMRange.start) &&
                            lastProjectLink.addrMRange.endsAt(newAddrMRange.end) &&
                            !reversed &&
                            ...
                            ArealRoadMaintainer.getELY(headProjectLink.ely) == currentRoadway.arealRoadMaintainer
    ...
    val newRoadway: Roadway = Roadway(
                            ...
                            ArealRoadMaintainer.getELY(headProjectLink.ely),
                            NoTermination
                          )
  ...
  def getNewAddrMRange(historyRoadway: Roadway, roadwaysHead: Roadway, currentRoadway: Roadway) = {
    val startAddr = historyRoadway.addrMRange.start + roadwaysHead.addrMRange.start - currentRoadway.addrMRange.start
    AddrMRange(startAddr, startAddr + roadwaysHead.addrMRange.length)
  }
    ...
    if (terminatedProjectLinksInRoadway.isEmpty) Seq.empty[Roadway] else {
      ...
      continuousGrouped.map(pls => {
        ...
        val newHistoryRoadways = currentRoadwayHistoryRoadways.map { historyRoadway =>
          val newAddrMRange = getNewAddrMRange(historyRoadway, roadway, currentRoadway)
          if (historyRoadway.addrMRange.length != roadway.addrMRange.length) {
            Roadway(NewIdValue, roadway.roadwayNumber, historyRoadway.roadPart, historyRoadway.administrativeClass, historyRoadway.track, roadway.discontinuity, newAddrMRange, historyRoadway.reversed, historyRoadway.startDate, historyRoadway.endDate, historyRoadway.createdBy, historyRoadway.roadName, historyRoadway.arealRoadMaintainer, Subsequent)
          } else {
  ...
  def generateNewRoadwaysWithHistory(changeSource: RoadwayChangeSection, changeTarget: RoadwayChangeSection, projectLinks: Seq[ProjectLink], currentRoadway: Roadway, projectStartDate: DateTime): Seq[Roadway] = {
    ...
    val historyRoadway = Roadway(NewIdValue, roadwayNumber, sourceRoadPart, changeSource.administrativeClass.get, Track.apply(changeSource.trackCode.get.toInt), changeSource.discontinuity.get, changeSource.addrMRange.get, projectLinks.head.reversed, currentRoadway.startDate, Some(projectStartDate.minusDays(1)),               createdBy = currentRoadway.createdBy,        currentRoadway.roadName, currentRoadway.arealRoadMaintainer, NoTermination, currentRoadway.validFrom, currentRoadway.validTo)
    val newRoadway =     Roadway(NewIdValue, roadwayNumber, targetRoadPart, changeTarget.administrativeClass.get, Track.apply(changeTarget.trackCode.get.toInt), changeTarget.discontinuity.get, AddrMRange(projectLinks.head.addrMRange.start, projectLinks.last.addrMRange.end), projectLinks.head.reversed, projectStartDate, None, createdBy = projectLinks.head.createdBy.get, currentRoadway.roadName, changeTarget.arealRoadMaintainer.getOrElse(ARMInvalid), NoTermination)
  ...
  private def applyUnchanged(change: ProjectRoadwayChange, projectLinks: Seq[ProjectLink], currentRoadways: Seq[Roadway],
    ...
    currentRoadways.map { currentRoadway =>
      ...
      val administrativeClassDiscontinuityOrElyChanged = currentRoadway.administrativeClass != changeTarget.administrativeClass.get ||
        currentRoadway.discontinuity != changeTarget.discontinuity.get ||
        !(currentRoadway.arealRoadMaintainer == changeTarget.arealRoadMaintainer.get)
      ...
      val roadways = if (administrativeClassDiscontinuityOrElyChanged || lengthChanged) {
        ...
        val targetRoadPart = RoadPart(changeTarget.roadNumber.get, changeTarget.startRoadPartNumber.get)
        Seq(Roadway(NewIdValue, projectLinksInRoadway.head.roadwayNumber, targetRoadPart, changeTarget.administrativeClass.get, Track.apply(changeTarget.trackCode.get.toInt), changeTarget.discontinuity.get, AddrMRange(projectLinks.head.addrMRange.start, projectLinks.last.addrMRange.end), projectLinks.head.reversed, currentRoadway.startDate, None, createdBy = projectLinks.head.createdBy.get, currentRoadway.roadName,
          changeTarget.arealRoadMaintainer.getOrElse(ArealRoadMaintainer.ARMInvalid), NoTermination))
      }

      val currentRoadwayHistoryRoadways = historyRoadways.filter(_.roadwayNumber == currentRoadway.roadwayNumber)
      val newHistoryRoadways = currentRoadwayHistoryRoadways.flatMap { historyRoadway =>
        val newAddrMRange = getNewAddrMRange(historyRoadway, roadways.head, currentRoadway)
        if (historyRoadway.addrMRange.length != roadways.head.addrMRange.length) {
          Seq(Roadway(NewIdValue, roadways.head.roadwayNumber, historyRoadway.roadPart, historyRoadway.administrativeClass, historyRoadway.track, historyRoadway.discontinuity, newAddrMRange, historyRoadway.reversed, historyRoadway.startDate, historyRoadway.endDate, historyRoadway.createdBy, historyRoadway.roadName, historyRoadway.arealRoadMaintainer, NoTermination))
        } else {
          Seq(historyRoadway)
        }
      }
  ...
  private def applyNew(change: ProjectRoadwayChange, projectLinks: Seq[ProjectLink]): Seq[(Seq[Roadway], Seq[LinearLocation], Seq[ProjectLink])] = {
    ...
    val targetRoadPart = RoadPart(changeTarget.roadNumber.get, changeTarget.startRoadPartNumber.get)
    val roadway = Roadway(NewIdValue, roadwayNumber, targetRoadPart, changeTarget.administrativeClass.get, Track.apply(changeTarget.trackCode.get.toInt), changeTarget.discontinuity.get, changeTarget.addrMRange.get, change.changeInfo.reversed, startDate = projectLinks.head.startDate.get, endDate = projectLinks.head.endDate, createdBy = projectLinks.head.createdBy.get, projectLinks.head.roadName, ArealRoadMaintainer.getELY(projectLinks.head.ely), NoTermination)
  ...
  def mergeRoadwayChanges(changesWithLinks: Seq[(ProjectRoadwayChange, Seq[ProjectLink])]): Seq[(ProjectRoadwayChange, Seq[ProjectLink])] = {
    def groupedSections(changes: Seq[(ProjectRoadwayChange, Seq[ProjectLink])]) = {
      changes.groupBy(c => (c._1.changeInfo.source.roadNumber, c._1.changeInfo.source.startRoadPartNumber, c._1.changeInfo.source.trackCode, c._1.changeInfo.source.administrativeClass, c._1.changeInfo.source.arealRoadMaintainer,
        c._1.changeInfo.target.roadNumber, c._1.changeInfo.target.startRoadPartNumber, c._1.changeInfo.target.trackCode, c._1.changeInfo.target.administrativeClass, c._1.changeInfo.target.arealRoadMaintainer))
        .flatMap {
        case (_, section) =>
          val sortedSections = section.sortBy(s => (s._1.changeInfo.changeType.value, s._1.changeInfo.target.addrMRange.get.start))
          sortedSections.foldLeft(Seq.empty[(ProjectRoadwayChange, Seq[ProjectLink])]) {(changeList, section) =>
            if (changeList.isEmpty)
              Seq(section)
            else if (changeList.last._1.changeInfo.target.addrMRange.get.end == section._1.changeInfo.target.addrMRange.get.start &&
              changeList.last._2.head.roadwayNumber == section._2.head.roadwayNumber) {
                val adjustedSource = changeList.last._1.changeInfo.source.copy(addrMRange = Some(AddrMRange(changeList.last._1.changeInfo.source.addrMRange.get.start, section._1.changeInfo.source.addrMRange.get.end)))
                val adjustedTarget = changeList.last._1.changeInfo.target.copy(addrMRange = Some(AddrMRange(changeList.last._1.changeInfo.target.addrMRange.get.start, section._1.changeInfo.target.addrMRange.get.end)))
