...
object ProjectSectionMValueCalculator {
  ...
  def calculateMValuesForTrack(seq: Seq[ProjectLink], calibrationPoints: Map[Long, UserDefinedCalibrationPoint]): Seq[ProjectLink] = {
    ...
    def isExtensionOf(ext: ProjectLink)(thisPL: ProjectLink) = {
      thisPL.originalAddrMRange.endAddrM == ext.originalAddrMRange.startAddrM &&
    ...
    def resetEndAddrMValue(pl: ProjectLink): ProjectLink = {
      val endAddrMValue = pl.addrMRange.startAddrM + pl.addrMLength
      if (endAddrMValue != pl.addrMRange.endAddrM)
        pl.copy(addrMRange = AddrMRange(pl.addrMRange.startAddrM, endAddrMValue))
      else
        pl
    }
    ...
    val mapped = unchanged.groupBy(_.addrMRange.startAddrM)
    ...
    if (!unchanged.forall(
      pl => {
        ...
        previousLinks.size match {
          case 0 => pl.addrMRange.startAddrM == 0
          case 1 => true
    ...
    unchanged.map(resetEndAddrMValue) ++ assignLinkValues(others, calibrationPoints, unchanged.map(_.addrMRange.endAddrM.toDouble).sorted.lastOption, None)
  }
  ...
  def assignLinkValues(seq: Seq[ProjectLink], cps: Map[Long, UserDefinedCalibrationPoint], addrSt: Option[Double], addrEn: Option[Double], coEff: Double = 1.0): Seq[ProjectLink] = {
    if (seq.isEmpty) Seq() else {
      val ordered = if (seq.exists(pl => pl.isNotCalculated)) {
        ...
        val endPoints = if (seq.exists(pl => pl.addrMRange.endAddrM == 0)) TrackSectionOrder.findChainEndpoints(seq) else  TrackSectionOrder.findChainEndpoints(seqOfEnds)
      ...
      val newAddressValues = ordered.scanLeft(addrSt.getOrElse(0.0)) {
        case (m, pl) =>
          ...
          pl.status match {
            ...
            case RoadAddressChangeType.Transfer | RoadAddressChangeType.NotHandled | RoadAddressChangeType.Renumeration | RoadAddressChangeType.Unchanged => m + (pl.originalAddrMRange.endAddrM - pl.originalAddrMRange.startAddrM)
            case RoadAddressChangeType.Termination => pl.addrMRange.endAddrM
  ...
  def calculateAddressingFactors(seq: Seq[ProjectLink]): TrackAddressingFactors = {
    seq.foldLeft[TrackAddressingFactors](TrackAddressingFactors(0, 0, 0.0)) { case (a, pl) =>
      pl.status match {
        ...
        case RoadAddressChangeType.Transfer | RoadAddressChangeType.NotHandled => a.copy(transferLength = a.transferLength + pl.addrMRange.endAddrM - pl.addrMRange.startAddrM)
