...
import fi.vaylavirasto.viite.model.{AdministrativeClass, Discontinuity, LinkGeomSource, RoadAddressChangeType, RoadLink, SideCode, Track}
      ...
      nProjectLinks.headOption match {
        case Some(nProjectLink) =>
          if (GeometryUtils.areAdjacent(pProjectLink.geometry, nProjectLink.geometry)) {
            merge(nProjectLink.copy(status = RoadAddressChangeType.Termination), nProjectLinks.tail, copy(pProjectLink, nProjectLink, result))
          } else {
  ...
  def split(roadLink: RoadLink, suravage: ProjectLink, templateLink: ProjectLink, projectLinksToTerminate: Seq[ProjectLink], split: SplitOptions): SplitResult = {
    ...
    def movedFromStart(suravageM: Double, templateM: Double, splitAddressM: Long, isReversed: Boolean) = {
      ...
      val splitT = templateLink.copy(
        ...
        status = RoadAddressChangeType.Termination, geometryLength = templateLink.endMValue - templateM, connectedLinkId = Some(suravage.linkId)
      )
    ...
    def movedFromEnd(suravageM: Double, templateM: Double, splitAddressM: Long, isReversed: Boolean) = {
      ...
      val splitT = templateLink.copy(
        ...
        status = RoadAddressChangeType.Termination, geometryLength = templateM, connectedLinkId = Some(suravage.linkId)
      )
      (splitA, splitB, splitT)
    }
...
case class SplitOptions(splitPoint: Point, statusA: RoadAddressChangeType, statusB: RoadAddressChangeType,
...
case class SplitResult(splitA: ProjectLink, splitB: ProjectLink, allTerminatedProjectLinks: Seq[ProjectLink], terminatedProjectLink: ProjectLink) {
  ...
  def toSeqWithAllTerminated: Seq[ProjectLink] = {
    ...
    (Seq(splitA, splitB)
      ...
      ).map(pl => pl.copy(status = RoadAddressChangeType.Termination, connectedLinkId = terminatedProjectLink.connectedLinkId)
