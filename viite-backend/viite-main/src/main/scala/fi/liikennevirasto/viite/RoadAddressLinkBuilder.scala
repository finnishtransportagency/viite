...
import fi.vaylavirasto.viite.model.{AddrMRange, AdministrativeClass, ArealRoadMaintainer, Discontinuity, LifecycleStatus, LinkGeomSource, RoadLink, RoadLinkLike, RoadPart, SideCode, Track}
...
class RoadAddressLinkBuilder(roadwayDAO: RoadwayDAO, linearLocationDAO: LinearLocationDAO) extends AddressLinkBuilder {
  ...
  def build(roadAddress: RoadAddress): RoadAddressLink = {
    ...
    RoadAddressLink(roadAddress.id, roadAddress.linearLocationId, roadAddress.linkId, geom, length, AdministrativeClass(1), LifecycleStatus.apply(0), LinkGeomSource.apply(1), administrativeClass, roadAddress.roadName, municipalityCode, "", Some(""), Some(modifiedBy), roadAddress.roadPart, roadAddress.track.value,
      ArealRoadMaintainer.ARMInvalid, roadAddress.discontinuity.value, roadAddress.addrMRange, roadAddress.startDate.map(finnishDateFormatter.print).getOrElse(""), roadAddress.endDate.map(finnishDateFormatter.print).getOrElse(""), roadAddress.startMValue, roadAddress.endMValue, roadAddress.sideCode, roadAddress.startCalibrationPoint, roadAddress.calibrationPoints._2, roadAddress.roadwayNumber, sourceId = "")
  }
  ...
  private def buildRoadLink(roadLink: RoadLink, unaddressedRoadLink: UnaddressedRoadLink): RoadAddressLink = {
    ...
    RoadAddressLink(0, 0, roadLink.linkId, geom, length, roadLink.administrativeClass, roadLink.lifecycleStatus, roadLink.linkSource, administrativeClass, None, municipalityCode, municipalityName, roadLink.modifiedAt, Some("kgv_modified"), RoadPart(0, 0), Track.Unknown.value,
      ArealRoadMaintainer(s"ELY${municipalityToViiteELYMapping.getOrElse(roadLink.municipalityCode, -1)}"), // TODO VIITE-3416 ELY-EVK Epic, include EVKs into this (refactor to functionality. Maybe VIITE-3456)
      Discontinuity.Continuous.value, AddrMRange(0, 0), "", "", 0.0, length, SideCode.Unknown, None, None, newGeometry = Some(roadLink.geometry), sourceId = roadLink.sourceId)
  }
}
