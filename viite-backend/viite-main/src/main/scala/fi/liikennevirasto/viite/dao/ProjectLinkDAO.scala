...
case class ProjectLink(id: Long, roadPart: RoadPart, track: Track, discontinuity: Discontinuity, addrMRange: AddrMRange, originalAddrMRange: AddrMRange, startDate: Option[DateTime] = None, endDate: Option[DateTime] = None, createdBy: Option[String] = None, linkId: String, startMValue: Double, endMValue: Double, sideCode: SideCode,
  ...
  lazy val isNotCalculated: Boolean = addrMRange.endAddrM == 0L
  ...
  private def isTheLastProjectLinkOnRoadway = roadway.isDefined && this.originalAddrMRange.endAddrM == roadway.get.addrMRange.endAddrM
  ...
  def addrAt(a: Double): Long = {
    val coefficient = (addrMRange.endAddrM - addrMRange.startAddrM) / (endMValue - startMValue)
    sideCode match {
      case SideCode.AgainstDigitizing =>
        addrMRange.endAddrM - Math.round((a-startMValue) * coefficient)
      case SideCode.TowardsDigitizing =>
        addrMRange.startAddrM + Math.round((a-startMValue) * coefficient)
      case _ => throw new InvalidAddressDataException(s"Bad sidecode $sideCode on project link")
    }
  }

  def addrMLength(): Long = {
    addrMRange.endAddrM - addrMRange.startAddrM
  }
  ...
  def toMeters(address: Long): Double = {
    val coefficient = (endMValue - startMValue) / (addrMRange.endAddrM - addrMRange.startAddrM)
    coefficient * address
  }
...
class ProjectLinkDAO extends BaseDAO {
  ...
  implicit val getProjectLinkRow: GetResult[ProjectLink] = new GetResult[ProjectLink] {
    def apply(r: PositionedResult): ProjectLink = {
      ...
      val originalAddrMRange  = AddrMRange(originalStartAddrMValue, originalEndAddrMValue)
      val roadAddressMRange   = if(roadAddressStartAddrM.isDefined && roadAddressEndAddrM.isDefined) Some(AddrMRange(roadAddressStartAddrM.get, roadAddressEndAddrM.get)) else None
  ...
  implicit val getProjectLinksChangeRow: GetResult[ProjectRoadLinkChange] = new GetResult[ProjectRoadLinkChange] {
    def apply(r: PositionedResult): ProjectRoadLinkChange = {
      ...
      val addrMRange = AddrMRange(r.nextLong(), r.nextLong())
      val status = RoadAddressChangeType.apply(r.nextInt())
      val reversed = r.nextBoolean()
      val roadwayNumber = r.nextLong()
      val projectRoadwayNumber = r.nextLong()

      ProjectRoadLinkChange(projectLinkId, roadwayId, originalLinearLocationId, 0, RoadPart(originalRoadNumber, originalRoadPartNumber), RoadPart(originalRoadNumber, originalRoadPartNumber), originalAddrMRange, addrMRange,
  ...
  def create(links: Seq[ProjectLink]): Seq[Long] = {
    ...
    time(logger, "Create project links") {
      ...
      projectLinks.toList.foreach { pl =>
        ...
        addressPS.setLong(7, pl.addrMRange.startAddrM)
        addressPS.setLong(8, pl.addrMRange.endAddrM)
        addressPS.setLong(9, pl.originalAddrMRange.startAddrM)
        addressPS.setLong(10, pl.originalAddrMRange.endAddrM)
  ...
  def updateProjectLinks(projectLinks: Seq[ProjectLink], modifier: String, addresses: Seq[RoadAddress]): Unit = {
      time(logger, "Update project links") {
        ...
        val maxInEachTracks = projectLinks.filter(pl => pl.status == RoadAddressChangeType.Unchanged).groupBy(_.track).map(p => p._2.maxBy(_.addrMRange.endAddrM).id).toSeq
        val links = projectLinks.map { pl =>
          if (!pl.isSplit && nonUpdatingStatus.contains(pl.status) && addresses.map(_.linearLocationId).contains(pl.linearLocationId) && !maxInEachTracks.contains(pl.id)) {
            ...
            pl.copy(roadPart = ra.roadPart, track = ra.track, addrMRange = ra.addrMRange, reversed = false)
          } else
            pl
        }
        ...
        for (projectLink <- links) {
          ...
          projectLinkPS.setLong(5, projectLink.addrMRange.startAddrM)
          projectLinkPS.setLong(6, projectLink.addrMRange.endAddrM)
          projectLinkPS.setLong(7, projectLink.originalAddrMRange.startAddrM)
          projectLinkPS.setLong(8, projectLink.originalAddrMRange.endAddrM)
  ...
  * */
  ...
  def updateAddrMValues(projectLink: ProjectLink): Unit = {
    runUpdateToDb(s"""
      update project_link
      set modified_date = current_timestamp, start_addr_m = ${projectLink.addrMRange.startAddrM}, end_addr_m = ${projectLink.addrMRange.endAddrM},
    ...
    time(logger, "Update project link values") {
      ...
      val updateProjectLink = s"""
        UPDATE PROJECT_LINK SET ROAD_NUMBER = ${roadAddress.roadPart.roadNumber},
          ...
          STATUS = ${RoadAddressChangeType.NotHandled.value}, START_ADDR_M = ${roadAddress.addrMRange.startAddrM}, END_ADDR_M = ${roadAddress.addrMRange.endAddrM},
    ...
    if (projectLinks.nonEmpty) {
      ...
      """)
        ...
        projectLinks.foreach { pl =>
          ...
          updatePS.setLong(6, pl.addrMRange.startAddrM)
          updatePS.setLong(7, pl.addrMRange.endAddrM)
