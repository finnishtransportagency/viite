    ...
    sortedLinearLocations.zip(addresses.zip(addresses.tail)).map {
      case (linearLocation, (st, en)) =>
        ...
        RoadAddress(
          ...
          AddrMRange(st, en),
          Some(roadway.startDate),
          roadway.endDate,
          Some(roadway.createdBy),
          linearLocation.linkId,
          linearLocation.startMValue,
          linearLocation.endMValue,
          linearLocation.sideCode,
          linearLocation.adjustedTimestamp,
          calibrationPoints,
          linearLocation.geometry,
          linearLocation.linkGeomSource,
          roadway.roadMaintainer,
          roadway.terminated,
          roadway.roadwayNumber,
          linearLocation.validFrom,
          linearLocation.validTo,
          roadway.roadName)
    }
  }
