...
//            .exists(o => u.addrMRange.startAddrM >= o.addrMRange.startAddrM)))
//    }.toSeq
//
...
//  }
  ...
  def checkForInvalidUnchangedLinks(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    def findInvalidUnchangedLinks(pls: Seq[ProjectLink]): Seq[ProjectLink] = {
      var errorLinkList: Seq[ProjectLink]  = Seq()
        ...
        // sort groups' project links by original addrMRange.startAddrM
        val sortedProjectLinks = pls.filterNot(_.isNotCalculated).sortBy(_.originalAddrMRange.startAddrM)
        ...
        if (firstOtherStatus.nonEmpty) {
          val limitAddrMValue = {
            if (firstOtherStatus.get.status == RoadAddressChangeType.New)
              firstOtherStatus.get.addrMRange.startAddrM // for New links we take the normal addrMRange.startAddrM
            else
              firstOtherStatus.get.originalAddrMRange.startAddrM // for anything else we take the originaladdrMRange.startAddrM
          }
          ...
          errorLinkList ++ sortedProjectLinks.filter(pl => pl.status == RoadAddressChangeType.Unchanged && pl.originalAddrMRange.startAddrM >= limitAddrMValue)
        } else
          errorLinkList
    }
  ...
  def isSameTrack(previous: ProjectLink, currentLink: ProjectLink): Boolean = {
    previous.track == currentLink.track && previous.addrMRange.endAddrM == currentLink.addrMRange.startAddrM
  }
  ...
  def getTrackInterval(links: Seq[ProjectLink], track: Track): Seq[ProjectLink] = {
    ...
    }.sortBy(_.addrMRange.startAddrM)
  }
  ...
  def checkTrackCodePairing(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    def checkMinMaxTrack(trackInterval: Seq[ProjectLink]): Option[ProjectLink] = {
      if (trackInterval.head.track != Track.Combined) {
        val minTrackLink = trackInterval.minBy(_.addrMRange.startAddrM)
        val maxTrackLink = trackInterval.maxBy(_.addrMRange.endAddrM)
        ...
        if (!notCombinedNonTerminatedLinksInRoadPart.exists(l => l.addrMRange.startAddrM == minTrackLink.addrMRange.startAddrM && l.track != minTrackLink.track)) {
          Some(minTrackLink)
        }
        else if (!notCombinedNonTerminatedLinksInRoadPart.exists(l => l.addrMRange.endAddrM == maxTrackLink.addrMRange.endAddrM && l.track != maxTrackLink.track)) {
          Some(maxTrackLink)
        } else None
      } else None
    }
    ...
    def checkMinMaxTrackAdministrativeClasses(trackInterval: Seq[ProjectLink]): Option[Seq[ProjectLink]] = {
        val diffLinks = trackInterval.groupBy(_.administrativeClass).flatMap { projectLinksByAdministrativeClass: (AdministrativeClass, Seq[ProjectLink]) =>
          ...
          case (left, right) if left.nonEmpty && right.nonEmpty =>
                val leftSection      = (projectLinksByAdministrativeClass._1,  left.map(_.addrMRange.startAddrM).min,  left.map(_.addrMRange.endAddrM).max)
                val rightSection     = (projectLinksByAdministrativeClass._1, right.map(_.addrMRange.startAddrM).min, right.map(_.addrMRange.endAddrM).max)
                ...
                if (leftSection != rightSection) {
                  val criticalLeftLinks  = left.filterNot(link => {link.addrMRange.startAddrM >= startSectionAdrr && link.addrMRange.endAddrM <= endSectionAddr}).filterNot(link => {right.map(_.addrMRange.startAddrM).contains(link.addrMRange.startAddrM) || right.map(_.addrMRange.endAddrM).contains(link.addrMRange.endAddrM)})
                  val criticalRightLinks = right.filterNot(link => {link.addrMRange.startAddrM >= startSectionAdrr && link.addrMRange.endAddrM <= endSectionAddr}).filterNot(link => {left.map(_.addrMRange.startAddrM).contains(link.addrMRange.startAddrM) || left.map(_.addrMRange.endAddrM).contains(link.addrMRange.endAddrM)})
        ...
        checkMinMaxTrack(validTrackInterval) match {
          ...
          case None => if (validTrackInterval.size > 1) {
            validTrackInterval.sliding(2).map { case Seq(first, second) =>
              if (first.addrMRange.endAddrM != second.addrMRange.startAddrM && first.id != second.id) {
                Some(first)
              } else None
            }.toSeq.flatten
          } else Seq.empty[ProjectLink]
    ...
    def recursiveCheckTrackChange(links: Seq[ProjectLink], errorLinks: Seq[ProjectLink] = Seq()): Option[ValidationErrorDetails] = {
      if (links.isEmpty) {
        ...
        val trackInterval = getTrackInterval(links.sortBy(o => (o.roadPart, o.track.value, o.addrMRange.startAddrM)), trackToCheck)
    ...
    def getTwoTrackInterval(links: Seq[ProjectLink], interval: Seq[(Long, Seq[ProjectLink])]): Seq[(Long, Seq[ProjectLink])] = {
      if (links.isEmpty || links.exists(_.isNotCalculated)) {
        ...
        val trackToCheck  = links.head.track
        val trackInterval = getTrackInterval(links.sortBy(o => (o.roadPart, o.track.value, o.addrMRange.startAddrM)), trackToCheck)
        val headerAddr    = trackInterval.minBy(_.addrMRange.startAddrM).addrMRange.startAddrM
    ...
    def recursiveCheckTwoTrackGeometryLength(roadPartLinks: Seq[ProjectLink],
      ...
      if (roadPartLinks.isEmpty) {
        ...
        val sortedRoadPartLinks  = roadPartLinks.sortBy(pl => (pl.track.value, pl.addrMRange.startAddrM))
  ...
  def checkActionsInRoadsNotInProject(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    val operationsOutsideProject: Seq[Roadway] = (project.reservedParts++project.formedParts).flatMap(r =>
      roadwayDAO.fetchAllByRoadAndPart(r.roadPart)).filterNot(
    ...
    if (erroredProjectLinks.nonEmpty) {
      erroredProjectLinks.flatMap{ l =>
        Seq(ValidationErrorDetails(project.id, alterShortMessage(ValidationErrorList.RoadNotReserved, currentRoadAndPart = Some((l._2.head.roadPart)))
  ...
  def checkDiscontinuityOnPreviousRoadPart(project: Project, allProjectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    def validateDiscontinuityBetweenTwoRoadAddresses(previousRoadAddress: Seq[BaseRoadAddress], nextRoadAddress: Seq[BaseRoadAddress]): Seq[ValidationErrorDetails] = {
      val prevLeftRoadAddress = Seq(previousRoadAddress.filter(_.track != Track.RightSide).maxBy(_.addrMRange.endAddrM))
      val prevRightRoadAddress = Seq(previousRoadAddress.filter(_.track != Track.LeftSide).maxBy(_.addrMRange.endAddrM))
      ...
      val nextLeftRoadAddress = if (leftRoadAddresses.nonEmpty) {
        Seq(leftRoadAddresses.minBy(_.addrMRange.startAddrM))
      } else Seq.empty[BaseRoadAddress]
      val nextRightRoadAddress = if (rightRoadAddresses.nonEmpty) {
        Seq(rightRoadAddresses.minBy(_.addrMRange.startAddrM))
      }  else Seq.empty[BaseRoadAddress]
      ...
      def validatePreviousRoadAddress(roadAddressList: Seq[BaseRoadAddress])(validationFunction: BaseRoadAddress => Boolean)(error: ValidationError): Seq[ValidationErrorDetails] = {
        ...
        if (foundError.nonEmpty) outsideOfProjectError(
          project.id,
          alterMessage(error, currentRoadAndPart = Some(roadAddressList.head.roadPart))
    ...
    def validateTheEndOfPreviousRoadPart(roadPart: RoadPart): Seq[ValidationErrorDetails] = {
      ...
      previousRoadPartNumber match {
        case Some(previousRoadPartNumber) =>
          val previousRoadwayRoadAddresses = roadAddressService.getRoadAddressWithRoadAndPart(RoadPart(roadPart.roadNumber, previousRoadPartNumber), fetchOnlyEnd = true, newTransaction = false)
            ...
            case _ =>
              nextRoadPartNumber match {
                case Some(nextRoadPartNumber) =>
                  roadAddressService.getRoadAddressWithRoadAndPart(RoadPart(roadPart.roadNumber, nextRoadPartNumber), newTransaction = false)
                case _ => Seq()
              }
          }
          ...
          if (project.formedParts.exists(rp => rp.roadPart.isBefore(roadPart) && previousRoadPartNumber < rp.roadPart.partNumber))
            ...
            if (previousRoadwayRoadAddresses.nonEmpty && nextRoadAddress.nonEmpty) {
              ...
              previousRoadwayRoadAddresses.maxBy(_.addrMRange.endAddrM).discontinuity != Discontinuity.EndOfRoad) {
              ...
              outsideOfProjectError(
                project.id,
                alterMessage(ValidationErrorList.TerminationContinuity, currentRoadAndPart = Some(RoadPart(roadPart.roadNumber, previousRoadPartNumber)))
  ...
  def checkProjectElyCodes(project: Project, allProjectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    @tailrec
    def recProjectGroupsEly(unprocessed: Map[RoadPart, Seq[ProjectLink]], processed: Map[RoadPart, Seq[ProjectLink]], acumulatedErrors: Seq[ValidationErrorDetails] = Seq.empty[ValidationErrorDetails]): Seq[ValidationErrorDetails] = {
      if (processed.isEmpty && unprocessed.nonEmpty) {
        ...
        if (unprocessed.isEmpty) {
          ...
          val biggestPrevious = processed.head._2.maxBy(_.addrMRange.endAddrM)
          val lowestCurrent = unprocessed.head._2.minBy(_.addrMRange.startAddrM)
          if (biggestPrevious.ely != lowestCurrent.ely) {
            ...
            if (biggestPrevious.discontinuity == Discontinuity.ChangingELYCode && biggestPrevious.roadPart.roadNumber == lowestCurrent.roadPart.roadNumber) {
              val affectedProjectLinks = Seq(biggestPrevious, lowestCurrent)
              val coords = prepareCoordinates(affectedProjectLinks)
              recProjectGroupsEly(unprocessed.tail, Map(unprocessed.head) ++ processed, acumulatedErrors ++ Seq(ValidationErrorDetails(project.id, alterMessage(ValidationErrorList.ElyCodeDiscontinuityChangeButNoElyChange, currentRoadAndPart = Some(biggestPrevious.roadPart), nextRoadAndPart = Some(lowestCurrent.roadPart)), affectedProjectLinks.map(_.id), affectedProjectLinks.map(_.linkId), coords, Option(biggestPrevious.roadPart.roadNumber.toString))))
            } else {
              recProjectGroupsEly(unprocessed.tail, Map(unprocessed.head) ++ processed, acumulatedErrors)
            }
        ...
        val (wrongProjectLinks, validationError) = condition match {
          case Left(originalElys) =>
            if (originalElys.nonEmpty)
              ...
              (allProjectLinks.groupBy(_.ely).map(_._2.maxBy(_.addrMRange.endAddrM)).toSeq, ValidationErrorList.MultipleElyInPart)
            }
      ...
      val validationErrors = groupedProjectLinks.flatMap(group => {
        ...
        val nonLastLinkHasChangeOfEly = if (group._2.exists(_.isNotCalculated))
                                          ...
                                          group._2.filter(p => p.discontinuity == Discontinuity.ChangingELYCode &&
                                                               p.addrMRange.endAddrM != group._2.maxBy(_.addrMRange.endAddrM).addrMRange.endAddrM)
        ...
        val tracksWithUnpairedChangeOfEly = twoTrackLinksWithChangeOfEly.flatMap(p => group._2.filter(q => q.addrMRange.endAddrM == p.addrMRange.endAddrM &&
      ...
      val unCalcElyCodeChangeDetectedErrors: Iterable[ValidationErrorDetails] =
        if (notCalculatedParts.isEmpty)
          ...
          def createValidationErrorDetails(roadPart: RoadPart,
            ...
            val message = alterMessage(ValidationErrorList.ElyCodeChangeDetected, currentRoadAndPart = Some(roadPart), nextRoadAndPart = Some(roadPart))
        ...
        if (validationErrorsWithUnCalculated.nonEmpty)
            ...
            val roadways = notCalculatedParts.keys.flatMap(rp => roadwayDAO.fetchAllByRoadAndPart(rp).filter(r => (r.roadPart.isAfter(rp))).sortBy(_.roadPart.partNumber).take(1))
    ...
    if (workedProjectLinks.nonEmpty) {
      val grouped = workedProjectLinks.groupBy(pl => pl.roadPart).map(group => group._1 -> group._2.sortBy(_.addrMRange.endAddrM))
  ...
  def checkRoadContinuityCodes(project: Project, roadProjectLinks: Seq[ProjectLink], isRampValidation: Boolean = false): Seq[ValidationErrorDetails] = {
    ...
    def isConnectingRoundabout(pls: Seq[ProjectLink]): Boolean = {
      ...
      roadParts.keys.exists(rp => TrackSectionOrder.isRoundabout(
        roadAddressService.getRoadAddressWithRoadAndPart(rp)))
    }
    ...
    def checkContinuityBetweenLinksOnParts: Seq[ValidationErrorDetails] = {
      def checkConnected(curr: ProjectLink, next: Option[ProjectLink]): Boolean = {
        if (next.isEmpty)
          ...
          curr.addrMRange.endAddrM == next.get.addrMRange.startAddrM && curr.connected(next.get)
      }
      ...
      val discontinuous: Seq[ProjectLink] = roadProjectLinks.groupBy(s => (s.roadPart)).flatMap { g =>
        ...
        trackIntervals.flatMap {
          interval => {
            if (interval.size > 1) {
              interval.sortBy(_.addrMRange.startAddrM).sliding(2).flatMap {
                case Seq(curr, next) =>
    ...
    def checkMinorDiscontinuityBetweenLinksOnPart: Seq[ValidationErrorDetails] = {
      ...
      val discontinuous: Seq[ProjectLink] = roadProjectLinks.groupBy(s => (s.roadPart)).flatMap { g =>
        ...
        trackIntervals.flatMap {
          interval => {
            if (interval.size > 1) {
              if (interval.exists(_.addrMRange.endAddrM == 0)) {
                ...
                interval.sortBy(_.addrMRange.startAddrM).sliding(2).flatMap { case Seq(curr, next) => /*
                ...
                Track 0  |
                  ...
                  t.track != next.track && t.addrMRange.startAddrM == next.addrMRange.startAddrM
                })
    ...
    def checkDiscontinuityBetweenLinksOnRamps: Seq[ValidationErrorDetails] = {
      val discontinuousErrors = if (isRampValidation) {
        error(project.id, ValidationErrorList.DiscontinuityOnRamp)(roadProjectLinks.filter { pl =>
          ...
          val nextLink = roadProjectLinks.find(pl2 => pl2.addrMRange.startAddrM == pl.addrMRange.endAddrM &&
          ...
          interval =>
            ...
            if (nonTerminated.nonEmpty) {
              ...
              val last = if (nonTerminated.forall(_.isNotCalculated)) {
                ...
                endLink._2
              } else {nonTerminated.maxBy(_.addrMRange.endAddrM)}
      ...
      val discontinuityErrors = roadProjectLinks.groupBy(_.roadPart.roadNumber).flatMap { g =>
        ...
        trackIntervals.flatMap {
          interval =>
            val nonTerminated = interval.filter(r => r.status != RoadAddressChangeType.Termination)
            if (nonTerminated.nonEmpty) {
              val last = nonTerminated.maxBy(_.addrMRange.endAddrM)
              val discontinuity = last.discontinuity
              val projectNextRoadParts = (project.reservedParts++project.formedParts).filter(rp =>
    ...
    def checkEndOfRoadBetweenLinksOnPart: Seq[ValidationErrorDetails] = {
      val endOfRoadErrors = roadProjectLinks.groupBy(_.roadPart.partNumber).flatMap {
        roadPart =>
          val addresses = roadPart._2.sortBy(_.addrMRange.startAddrM)
          val maxEndAddr = addresses.last.addrMRange.endAddrM
          error(project.id, ValidationErrorList.EndOfRoadMiddleOfPart)(addresses.filterNot(_.addrMRange.endAddrM == maxEndAddr).filter(_.discontinuity == Discontinuity.EndOfRoad))
      }.toSeq
    ...
    def getNextLinksFromParts(allProjectLinks: Seq[ProjectLink], road: Long, nextProjectPart: Option[Long], nextAddressPart: Option[Long]): Seq[BaseRoadAddress] = {
      if (nextProjectPart.nonEmpty && (nextAddressPart.isEmpty || nextProjectPart.get <= nextAddressPart.get))
        projectLinkDAO.fetchByProjectRoadPart(RoadPart(road, nextProjectPart.get), project.id).filter(l => RoadAddressChangeType.Termination.value != l.status.value && l.addrMRange.startAddrM == 0L)
      else {
        roadAddressService.getRoadAddressesFiltered(RoadPart(road, nextAddressPart.get))
          .filterNot(rp => allProjectLinks.exists(link => (rp.roadPart != link.roadPart) && rp.linearLocationId == link.linearLocationId)).filter(_.addrMRange.startAddrM == 0L)
      }
    }
  ...
  private def alterMessage(validationError: ValidationError,
                           ...
                           currentRoadAndPart: Option[RoadPart] = Option.empty[RoadPart],
                           nextRoadAndPart   : Option[RoadPart] = Option.empty[RoadPart],
    ...
    val formattedMessage =
      if (projectDate.nonEmpty && currentRoadAndPart.nonEmpty) {
        validationError.message.format(currentRoadAndPart.get, projectDate.get)
      } else if (currentRoadAndPart.nonEmpty && nextRoadAndPart.nonEmpty) {
        validationError.message.format(currentRoadAndPart.get, currentRoadAndPart.get, nextRoadAndPart.get.partNumber)
      } else {
        validationError.message.format(
          if (elyBorderData.nonEmpty)           elyBorderData.get.toSet.mkString(", ")
          else if (currentRoadAndPart.nonEmpty) currentRoadAndPart
  ...
  private def alterShortMessage(validationError: ValidationError, currentRoadAndPart: Option[RoadPart] = Option.empty[RoadPart]) = {
    val formattedMessage =
      if (currentRoadAndPart.nonEmpty) {
        val roadPart = currentRoadAndPart.get
