...
class ProjectValidator {
  ...
  def checkReservedExistence(currentProject: Project, newRoadPart: RoadPart, roadAddressChangeType: RoadAddressChangeType, projectLinks: Seq[ProjectLink]): Unit = {
    if (RoadAddressChangeType.New.value == roadAddressChangeType.value) {
      if (roadAddressService.getRoadAddressesFiltered(newRoadPart).nonEmpty) {
        ...
        val roadPartLinks = projectLinkDAO.fetchProjectLinksByProjectRoadPart(newRoadPart, currentProject.id)
  ...
  def checkAvailable(roadPart: RoadPart, currentProject: Project): Unit = {
    if (projectReservedPartDAO.isNotAvailableForProject(roadPart, currentProject.id)) {
      throw new ProjectValidationException(RoadNotAvailableMessage.format(roadPart, currentProject.startDate.toString(finnishDateFormatter)))
    }
  }

  def checkReservedPartInProject(roadPart: RoadPart, currentProject: Project, roadAddressChangeType: RoadAddressChangeType): Unit = {
    if (RoadAddressChangeType.Transfer.value == roadAddressChangeType.value && roadAddressService.getRoadAddressesFiltered(roadPart).nonEmpty && !currentProject.formedParts.map(fp => fp.roadPart).contains(roadPart)) {
      val partInCurrentProject = projectReservedPartDAO.fetchProjectReservedPart(roadPart, currentProject.id, withProjectId = Some(true))
      if (partInCurrentProject.isEmpty) {
        throw new ProjectValidationException(RoadNotAvailableMessage.format(roadPart, currentProject.name))
      }
    }
  }

  def checkReservedPartInOtherProject(roadPart: RoadPart, currentProject: Project): Unit = {
    val projectsWithPart = projectReservedPartDAO.fetchProjectReservedPart(roadPart, currentProject.id, withProjectId = Some(false))
    if (projectsWithPart.nonEmpty) {
      throw new ProjectValidationException(RoadReservedOtherProjectMessage.format(roadPart, currentProject.name))
    }
  }
  ...
  object ValidationErrorList {
    val values: Set[ValidationError] = Set(MinorDiscontinuityFound, DiscontinuousFound, InsufficientTrackCoverage, DiscontinuousAddressScheme,
      ...
      IncompatibleDiscontinuityCodes, EndOfRoadNotOnLastPart, ElinvoimakeskusCodeChangeDetected, DiscontinuityOnRamp, DiscontinuityInsideRoadPart,
      DiscontinuousCodeOnConnectedRoadPartOutside, NotDiscontinuousCodeOnDisconnectedRoadPartOutside, ElinvoimakeskusDiscontinuityCodeBeforeProjectButNoElinvoimakeskusChange,
      WrongDiscontinuityBeforeProjectWithElinvoimakeskusChangeInProject,
      ErrorInValidationOfUnchangedLinks, RoadNotEndingInElinvoimakeskusBorder, RoadContinuesInAnotherElinvoimakeskus,
      MultipleElinvoimakeskusInPart, IncorrectOperationTypeOnElinvoimakeskusCodeChange,
      ElinvoimakeskusCodeChangeButNoRoadPartChange, ElinvoimakeskusCodeChangeButNoElinvoimakeskusChange, ElinvoimakeskusCodeChangeButNotOnEnd, ElinvoimakeskusCodeDiscontinuityChangeButNoElinvoimakeskusChange, RoadNotReserved, DistinctAdministrativeClassesBetweenTracks, WrongDiscontinuityOutsideOfProject,
    ...
    case object ElinvoimakeskusCodeChangeDetected extends ValidationError {
      def value = 12
    ...
    case object RoadNotEndingInElinvoimakeskusBorder extends ValidationError {
      def value = 16
    ...
    case object RoadContinuesInAnotherElinvoimakeskus extends ValidationError {
      def value = 17

      def message: String = RoadContinuesInAnotherEvkMessage
    ...
    case object MultipleElinvoimakeskusInPart extends ValidationError {
      def value = 21

      def message: String = MultipleElinvoimakeskusInPartMessage

      def notification = true
    }

    case object IncorrectOperationTypeOnElinvoimakeskusCodeChange extends ValidationError {
      def value = 22

      def message: String = IncorrectOperationTypeOnElinvoimakeskusCodeChangeMessage

      def notification = true
    }

    case object ElinvoimakeskusCodeChangeButNoRoadPartChange extends ValidationError {
      def value = 23

      def message: String = ElinvoimakeskusCodeChangeButNoRoadPartChangeMessage

      def notification = true
    }

    case object ElinvoimakeskusCodeChangeButNoElinvoimakeskusChange extends ValidationError {
      def value = 24

      def message: String = ElinvoimakeskusCodeChangeButNoElinvoimakeskusChangeMessage

      def notification = true
    }

    case object ElinvoimakeskusCodeChangeButNotOnEnd extends ValidationError {
      def value = 25

      def message: String = ElinvoimakeskusCodeChangeButNotOnEndMessage

      def notification = true
    }

    case object ElinvoimakeskusCodeDiscontinuityChangeButNoElinvoimakeskusChange extends ValidationError {
      def value = 26

      def message: String = ElinvoimakeskusCodeDiscontinuityChangeButNoElinvoimakeskusChangeMessage
    ...
    //VIITE-2722
    case object UnpairedElinvoimakeskusCodeChange extends ValidationError {
      def value = 32

      def message: String = UnpairedElinvoimakeskusCodeChangeMessage
    ...
    case object ElinvoimakeskusDiscontinuityCodeBeforeProjectButNoElinvoimakeskusChange extends ValidationError {
      def value = 35

      def message: String = ElinvoimakeskusDiscontinuityCodeBeforeProjectButNoElinvoimakeskusChangeMessage

      def notification = false
    }

    case object WrongDiscontinuityBeforeProjectWithElinvoimakeskusChangeInProject extends ValidationError {
      def value = 36

      def message: String = WrongDiscontinuityBeforeProjectWithElinvoimakeskusChangeInProjectMessage
...
//    }
  ...
  def checkProjectContinuity(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    projectLinks.filter(_.status != RoadAddressChangeType.Termination).groupBy(pl => pl.roadPart).flatMap {
      case (roadPart, seq) =>
        if (roadPart.roadNumber < RampsMinBound || roadPart.roadNumber > RampsMaxBound) {
  ...
  def checkForNotHandledLinks(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    notHandled.groupBy(link => link.roadPart).foldLeft(Seq.empty[ValidationErrorDetails])((errorDetails, road) =>
      errorDetails :+ ValidationErrorDetails(project.id, ValidationErrorList.HasNotHandledLinks,
        ...
        Some(HasNotHandledLinksMessage.format(road._2.size, road._1.toString)))
    )
  }

//  def checkForInvalidUnchangedLinks(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
//    val invalidUnchangedLinks: Seq[ProjectLink] = projectLinks.groupBy(s => (s.roadPart)).flatMap { g =>
...
//  }
  ...
  def checkForInvalidUnchangedLinks(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    val groupedByRoadNumberAndPart = projectLinks.groupBy(pl => pl.originalRoadPart)
  ...
  def checkTrackCodePairing(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    val groupedLinks = notCombinedLinks.filterNot(_.status == RoadAddressChangeType.Termination).groupBy(pl => (pl.roadPart))
  ...
  def checkActionsInRoadsNotInProject(project: Project, projectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    val operationsOutsideProject: Seq[Roadway] = (project.reservedParts++project.formedParts).flatMap(r =>
      roadwayDAO.fetchAllByRoadPart(r.roadPart)).filterNot(
      l => projectLinks.exists(r => r.roadAddressRoadPart.nonEmpty && r.roadAddressRoadPart.get == l.roadPart)
    )
    val erroredProjectLinks = projectLinks.filter(pl => roadAddressChangeTypes.contains(pl.status) && operationsOutsideProject.exists(out => out.roadPart == pl.roadPart)).groupBy(pl => pl.roadPart)
    if (erroredProjectLinks.nonEmpty) {
      erroredProjectLinks.flatMap{ l =>
        Seq(ValidationErrorDetails(project.id, alterShortMessage(ValidationErrorList.RoadNotReserved, currentRoadPart = Some((l._2.head.roadPart)))
  ...
  def checkDiscontinuityOnPreviousRoadPart(project: Project, allProjectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    def validateDiscontinuityBetweenTwoRoadAddresses(previousRoadAddress: Seq[BaseRoadAddress], nextRoadAddress: Seq[BaseRoadAddress]): Seq[ValidationErrorDetails] = {
      ...
      def validatePreviousRoadAddress(roadAddressList: Seq[BaseRoadAddress])(validationFunction: BaseRoadAddress => Boolean)(error: ValidationError): Seq[ValidationErrorDetails] = {
        ...
        if (foundError.nonEmpty) outsideOfProjectError(
          project.id,
          alterMessage(error, currentRoadPart = Some(roadAddressList.head.roadPart))
      ...
      /**
       * Validates that the EVK has changed, if the previous RoadAddress has EvkCodeChange Discontinuity
       */
      ...
      /**
       * Validates that the previous RoadAddress has either Discontinuous or ChangingEvkCode Discontinuity code,
      ...
      val errors = notDiscontinuousCodeOnDisconnectedRoadAddress ++
        ...
        evkCodeChangeButSameEvkNumber ++ wrongDiscontinuityWithEvkChange
    ...
    def validateTheEndOfPreviousRoadPart(roadPart: RoadPart): Seq[ValidationErrorDetails] = {
      ...
      previousRoadPartNumber match {
        case Some(previousRoadPartNumber) =>
          val previousRoadwayRoadAddresses = roadAddressService.getRoadAddressWithRoadPart(RoadPart(roadPart.roadNumber, previousRoadPartNumber), fetchOnlyEnd = true, newTransaction = false)
          ...
          val nextRoadAddress: Seq[BaseRoadAddress] = allProjectLinks.filter(pl => pl.roadPart == roadPart && pl.status != RoadAddressChangeType.Termination) match {
            ...
            case _ =>
              nextRoadPartNumber match {
                case Some(nextRoadPartNumber) =>
                  roadAddressService.getRoadAddressWithRoadPart(RoadPart(roadPart.roadNumber, nextRoadPartNumber), newTransaction = false)
                case _ => Seq()
              }
          }
          ...
          if (project.formedParts.exists(rp => rp.roadPart.isBefore(roadPart) && previousRoadPartNumber < rp.roadPart.partNumber))
            Seq()
          else {
            if (previousRoadwayRoadAddresses.nonEmpty && nextRoadAddress.nonEmpty) {
              ...
              outsideOfProjectError(
                project.id,
                alterMessage(ValidationErrorList.TerminationContinuity, currentRoadPart = Some(RoadPart(roadPart.roadNumber, previousRoadPartNumber)))
    ...
    val roadPartsInProject = allProjectLinks.map(pl => pl.roadPart).distinct.toList
    val previousRoadPartValidationErrors = roadPartsInProject.flatMap {
      case (roadPart) => validateTheEndOfPreviousRoadPart(roadPart)
    }
    ...
    val roadPartsTransferredFromProject = allProjectLinks.groupBy(pl => pl.originalRoadPart).filter {
      case (originalRoadPart, pls) =>
        pls.forall(pl => (pl.status == RoadAddressChangeType.Transfer || pl.status == RoadAddressChangeType.Renumeration) && pl.roadPart.roadNumber != pl.originalRoadPart.roadNumber) &&
          !allProjectLinks.exists(pl => pl.roadPart.roadNumber == originalRoadPart.roadNumber)
    }.keys

    val previousOriginalRoadPartValidationErrors = roadPartsTransferredFromProject.flatMap {
      case (transferredRoadPart) => validateTheEndOfPreviousRoadPart(transferredRoadPart)
    }
  ...
  def checkProjectEvkCodes(project: Project, allProjectLinks: Seq[ProjectLink]): Seq[ValidationErrorDetails] = {
    ...
    */
    @tailrec
    def recProjectGroupsEvk(unprocessed: Map[RoadPart, Seq[ProjectLink]], processed: Map[RoadPart, Seq[ProjectLink]], acumulatedErrors: Seq[ValidationErrorDetails] = Seq.empty[ValidationErrorDetails]): Seq[ValidationErrorDetails] = {
      if (processed.isEmpty && unprocessed.nonEmpty) {
        recProjectGroupsEvk(unprocessed.tail, Map(unprocessed.head))
      } else {
        if (unprocessed.isEmpty) {
          ...
          if (biggestPrevious.roadMaintainer.id != lowestCurrent.roadMaintainer.id) {
            ...
            val errors = (lastLinkDoesNotHaveChangeOfEvk, roadNumbersAreDifferent) match {
              case (true, true) =>
                ...
                val sameEvkCoords = prepareCoordinates(projectLinksSameEvk)
                val sameRoadPartNumberCoords = prepareCoordinates(projectLinksSameRoadPartNumber)
                Seq(ValidationErrorDetails(project.id, ValidationErrorList.ElinvoimakeskusCodeChangeButNoElinvoimakeskusChange, projectLinksSameEvk.map(_.id), projectLinksSameEvk.map(_.linkId), sameEvkCoords, Option("")),
                  ValidationErrorDetails(project.id, ValidationErrorList.ElinvoimakeskusCodeChangeButNoRoadPartChange, projectLinksSameRoadPartNumber.map(_.id), projectLinksSameRoadPartNumber.map(_.linkId), sameRoadPartNumberCoords, Option("")))
              case (true, false) =>
            ...
            recProjectGroupsEvk(unprocessed.tail, Map(unprocessed.head) ++ processed, errors ++ acumulatedErrors)
          } else {
            if (biggestPrevious.discontinuity == Discontinuity.ChangingEVKCode && biggestPrevious.roadPart.roadNumber == lowestCurrent.roadPart.roadNumber) {
              val affectedProjectLinks = Seq(biggestPrevious, lowestCurrent)
              val coords = prepareCoordinates(affectedProjectLinks)
              recProjectGroupsEvk(unprocessed.tail, Map(unprocessed.head) ++ processed, acumulatedErrors ++ Seq(ValidationErrorDetails(project.id, alterMessage(ValidationErrorList.ElinvoimakeskusCodeDiscontinuityChangeButNoElinvoimakeskusChange, currentRoadPart = Some(biggestPrevious.roadPart), nextRoadPart = Some(lowestCurrent.roadPart)), affectedProjectLinks.map(_.id), affectedProjectLinks.map(_.linkId), coords, Option(biggestPrevious.roadPart.roadNumber.toString))))
            } else {
              recProjectGroupsEvk(unprocessed.tail, Map(unprocessed.head) ++ processed, acumulatedErrors)
            }
     ...
     * A. have all links transition to a new Elinvoimakeskus
    ...
    def checkChangeOfEvk(project: Project, groupedProjectLinks: Map[RoadPart, Seq[ProjectLink]]): Seq[ValidationErrorDetails] = {
        ...
        val (wrongProjectLinks, validationError) = condition match {
          case Left(originalEvks) =>
            if (originalEvks.nonEmpty)
              (allProjectLinks.filterNot(_.roadMaintainer.number.toLong == originalEvks.head), ValidationErrorList.MultipleElinvoimakeskusInPart)
            else {
          ...
          case Right(roadAddressChangeTypeSeq) =>
            (allProjectLinks.filterNot(pl => roadAddressChangeTypeSeq.contains(pl.status)), ValidationErrorList.IncorrectOperationTypeOnElinvoimakeskusCodeChange)
        }
      ...
      def findNonLastLinkHasChangeOfEvk(pls: Seq[ProjectLink]): Seq[ProjectLink] = {
      ...
      val validationErrors = groupedProjectLinks.flatMap(group => {
        ...
        val nonLastLinkHasChangeOfEvk = if (group._2.exists(_.isNotCalculated))
          findNonLastLinkHasChangeOfEvk(group._2)
        else
        ...
        val originalEvks = roadways.map(_.roadMaintainer.number.toLong).distinct
        val projectLinkEvks = group._2.map(_.roadMaintainer.number.toLong).distinct
        ...
        val errors = if (originalEvks.nonEmpty || (originalEvks.isEmpty && (projectLinkEvks.size > 1 || nonLastLinkHasChangeOfEvk.nonEmpty || tracksWithUnpairedChangeOfEvk.nonEmpty))) {
          ...
          val multi = if (projectLinkEvks.size > 1) {
            Seq(prepareValidationErrorDetails(Left(originalEvks)))
          }
          else Seq.empty

          val wrongStatusCode = if (!workableProjectLinks.forall(pl => pl.status == RoadAddressChangeType.Unchanged || pl.status == RoadAddressChangeType.Transfer || pl.status == RoadAddressChangeType.New || pl.status == RoadAddressChangeType.Renumeration) && !originalEvks.equals(projectLinkEvks)) {
          ...
          val changeEvkCodeNotInFinish = if (nonLastLinkHasChangeOfEvk.nonEmpty) {
            val coords = nonLastLinkHasChangeOfEvk.map(p => {
              val middlePoint = GeometryUtils.midPointGeometry(p.geometry)
              ProjectCoordinates(middlePoint.x, middlePoint.y, defaultZoomlevel)
            })
            Seq(ValidationErrorDetails(project.id, ValidationErrorList.ElinvoimakeskusCodeChangeButNotOnEnd, nonLastLinkHasChangeOfEvk.map(_.id), nonLastLinkHasChangeOfEvk.map(_.linkId), coords, Option.empty[String]))
          } else Seq.empty

          val changeEvkCodeNotInBothTracks = if (tracksWithUnpairedChangeOfEvk.nonEmpty) {
            val coords = tracksWithUnpairedChangeOfEvk.map(p => {
              val middlePoint = GeometryUtils.midPointGeometry(p.geometry)
              ProjectCoordinates(middlePoint.x, middlePoint.y, defaultZoomlevel)
            })
            Seq(ValidationErrorDetails(project.id, ValidationErrorList.UnpairedElinvoimakeskusCodeChange, tracksWithUnpairedChangeOfEvk.map(_.id), tracksWithUnpairedChangeOfEvk.map(_.linkId), coords, Option.empty[String]))
          } else Seq.empty

          multi ++ wrongStatusCode ++ changeEvkCodeNotInFinish ++ changeEvkCodeNotInBothTracks
        } else Seq.empty
        errors
      })
      ...
      val unCalcEvkCodeChangeDetectedErrors: Iterable[ValidationErrorDetails] =
        if (notCalculatedParts.isEmpty)
          ...
          def createValidationErrorDetails(roadPart: RoadPart,
                                           pls : Seq[ProjectLink]
          ...
          val evksInUnCalculated = getProjectLinksFromMap(notCalculatedParts).map(_.roadMaintainer.number.toLong).toSet
          ...
          if (validationErrorsWithUnCalculated.nonEmpty)
            ...
            notCalculatedParts.flatMap { case (roadPart, pls) =>
              if (existsSameRoadwayDifferentEvk(roadPart).booleanValue()) {
                createValidationErrorDetails(roadPart, pls)
              }
              else Seq.empty[ValidationErrorDetails] }
          }
        }

      val recErrors = recProjectGroupsEvk(calculatedParts, Map.empty)
      (validationErrors ++ recErrors ++ unCalcEvkCodeChangeDetectedErrors).toSeq
    }
    ...
    if (workedProjectLinks.nonEmpty) {
      ...
      val projectLinkEvkChangeErrors = checkChangeOfEvk(project, orderedProjectLinks)
      projectLinkEvkChangeErrors
    } else Seq.empty[ValidationErrorDetails]
  }
  ...
  /**
    ...
    * 3) End of road part, discontinuity 2 or 3 (major, evk change) is required if there is a gap
    ...
    * 5) If the next road part has differing evk code then there must be a discontinuity code 3 at the end
    *
  ...
  def checkRoadContinuityCodes(project: Project, roadProjectLinks: Seq[ProjectLink], isRampValidation: Boolean = false): Seq[ValidationErrorDetails] = {
    ...
    def isConnectingRoundabout(pls: Seq[ProjectLink]): Boolean = {
      ...
      roadParts.keys.exists(rp => TrackSectionOrder.isRoundabout(
        roadAddressService.getRoadAddressWithRoadPart(rp)))
    }
          ...
          interval =>
            ...
            if (nonTerminated.nonEmpty) {
              ...
              val projectNextRoadParts = (project.reservedParts++project.formedParts).filter(rp =>
                rp.roadPart.isAfter(last.roadPart) && rp.newLength.getOrElse(0L) > 0L && allProjectLinks.exists(l => l.roadPart == rp.roadPart))

              val nextProjectPart = projectNextRoadParts.map(_.roadPart).sorted.headOption
              val nextAddressPart = validRoadParts
                .filter(p => p > last.roadPart.partNumber).sorted
                .filterNot(
                  rp => allProjectLinks.exists(l => l.roadAddressRoadPart.getOrElse(RoadPart(0,0)) == RoadPart(roadNumber, rp))
                )
    ...
    /**
      * This will evaluate that the last link of the road part has EndOfRoad, ChangingEVKCode or Continuous discontinuity value as needed.
      *
      * @return
      */
        ...
        trackIntervals.flatMap {
          interval =>
            ...
            if (nonTerminated.nonEmpty) {
              ...
              val nextProjectPart = projectNextRoadParts.filter(np => np.newLength.getOrElse(0L) > 0L && allProjectLinks.exists(l => l.roadPart.partNumber == np.roadPart.partNumber))
                .map(_.roadPart.partNumber).sorted.headOption
              val nextAddressPart = validRoadParts
                .filter(p => p > last.roadPart.partNumber).sorted
                .find(p => roadAddressService.getRoadAddressesFiltered(RoadPart(last.roadPart.roadNumber, p))
                  .forall(ra => !allProjectLinks.exists(al => al.linearLocationId == ra.linearLocationId && al.roadPart.partNumber != ra.roadPart.partNumber)))
              if (!(nextProjectPart.isEmpty && nextAddressPart.isEmpty)) {
                val nextLinks = getNextLinksFromParts(allProjectLinks, last.roadPart.roadNumber, nextProjectPart, nextAddressPart)

                val rampDiscontinuity = if (isConnectingRoundabout(Seq(last)) && isRampValidation) {
                  discontinuity match {
                    case Discontinuity.EndOfRoad | Discontinuity.ChangingEVKCode | Discontinuity.Continuous =>
                      error(project.id, ValidationErrorList.RoadConnectingRoundabout,
                        s"Rampin tieosa ${last.roadPart} päättyy kiertoliittymään. Korjaa lievä epäjatkuvuus")(Seq(last))
                    case _ => None
                  }
                } else None
    ...
    /**
      * This will return the next link (being project link or road address) from a road part whether in this project or not
      *
  ...
  private def alterMessage(validationError: ValidationError,
    ...
    val formattedMessage =
      if (projectDate.nonEmpty && currentRoadPart.nonEmpty) {
        validationError.message.format(currentRoadPart.get, projectDate.get)
      } else if (currentRoadPart.nonEmpty && nextRoadPart.nonEmpty) {
        validationError.message.format(currentRoadPart.get, currentRoadPart.get, nextRoadPart.get.partNumber)
      } else {
        validationError.message.format(
          if (evkBorderData.nonEmpty)           evkBorderData.get.toSet.mkString(", ")
  ...
  private def alterShortMessage(validationError: ValidationError, currentRoadPart: Option[RoadPart] = Option.empty[RoadPart]) = {
    val formattedMessage =
      if (currentRoadPart.nonEmpty) {
        val roadPart = currentRoadPart.get
        val changedMsg = validationError.message.format(roadPart)
        changedMsg
      } else {
