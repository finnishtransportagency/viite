...
object RoadAddressSplitMapper extends RoadAddressMapper {
  def createAddressMap(splitProjectLinks: Seq[ProjectLink]): Seq[LinearLocationMapping] = {
    splitProjectLinks.groupBy(_.linearLocationId).flatMap { case (_, seq) =>
      val templateTerm = seq.find(_.status == LinkStatus.Terminated)
      ...
      (templateTerm, kept) match {
        case (Some(t), Some(k)) if GeometryUtils.areAdjacent(t.geometry.head, k.geometry.last) =>
          // t extends k
          Seq(LinearLocationMapping(t.linkId, t.linkId, t.id, t.startMValue, t.endMValue, t.startMValue, t.endMValue, t.geometry, t.geometry, None),
            LinearLocationMapping(t.linkId, k.linkId, t.id, 0.0, t.startMValue, k.startMValue, k.endMValue, k.geometry, k.geometry, None))
        case (Some(t), Some(k)) if GeometryUtils.areAdjacent(k.geometry.head, t.geometry.last) =>
          ...
          Seq(LinearLocationMapping(t.linkId, t.linkId, t.id, t.startMValue, t.endMValue, t.startMValue, t.endMValue, t.geometry, t.geometry, None),
            ...
            LinearLocationMapping(t.linkId, k.linkId, t.id, t.endMValue, t.endMValue + (k.endMValue - k.startMValue), k.startMValue, k.endMValue, k.geometry, k.geometry, None))
        case (Some(t), Some(k)) if GeometryUtils.areAdjacent(k.geometry.last, t.geometry.last) =>
          ...
          Seq(LinearLocationMapping(t.linkId, t.linkId, t.id, t.startMValue, t.endMValue, t.startMValue, t.endMValue, t.geometry, t.geometry, None),
            LinearLocationMapping(t.linkId, k.linkId, t.id, t.endMValue, t.endMValue + (k.endMValue - k.startMValue), k.endMValue, k.startMValue, k.geometry, k.geometry, None))
