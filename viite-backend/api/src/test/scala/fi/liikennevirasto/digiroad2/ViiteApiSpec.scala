...
import fi.vaylavirasto.viite.model.{AdministrativeClass, LifecycleStatus, LinkGeomSource, RoadLink, TrafficDirection, ArealRoadMaintainer}
...
class ViiteApiSpec extends AnyFunSuite with ScalatraSuite with BeforeAndAfter {
 override def header = response.header // IDE nagged about unimplemented variable
  ...
  private val testProjectId = roadNameService.runWithReadOnlySession { 
    projectDAO.fetchAllWithoutDeletedFilter().filter(p => p("statusCode") == 1L || p("statusCode") == 0L).head("id").toString.toLong 
  }
  ...
  test("Test /roadaddress API call returns some data in expected format for UI.") {
      get("/roadaddress?zoom=8&bbox=527800.0,6991096.0,538002.0,6992512.0") {
...
//    get("/roadaddress/linkid/4388185") {
//      println(body)
//      status should equal(200)
//      body should equal("""{"modifiedAt":"25.06.2015 03:00:00","linkId":"4388185","roadNameFi":"","roadPartNumber":0,"success":true,"endDate":"","linearLocationId":0,"roadwayNumber":0,"administrativeClassMML":"State","roadwayId":0,"municipalityCode":749,"middlePoint":{"x":424936.82,"y":6695689.112,"z":43.282671305168655},"calibrationCode":0,"roadNumber":0,"trackCode":99,"roadClass":99,"sideCode":9,"evkCode":6,"points":[{"x":424957.852,"y":6695689.141,"z":85.90899999999965},{"x":424996.173,"y":6695688.118,"z":0.0}],"newGeometry":[{"x":424957.852,"y":6695689.141,"z":85.90899999999965},{"x":424996.173,"y":6695688.118,"z":0.0}],"municipalityName":"Siilinjärvi","startMValue":0.0,"endMValue":38.335,"addrMRange":{"start":0,"end":0},"roadNameSe":"","calibrationPoints":[],"mmlId":"","startDate":"","modifiedBy":"kgv_modified","elyCode":8,"lifecycleStatus":3,"discontinuity":5,"administrativeClassId":1,"roadLinkSource":4}""")
//    }
//  }
...
//  }
  ...
  test("Test /roadnames") {
    get("/roadnames?roadNumber=5") {
      status should equal(200)
      body should include(""""name":"Helsinki-Sodankylä"""")
      body should include(""""roadNumber":5""")
      body should include(""""startDate":"01.01.1989, 00:00:00"""")
    }
    get("/roadnames?roadName=PORI-TAMPERE") {
      status should equal(200)
      body should include(""""name":"PORI-TAMPERE"""")
      body should include(""""endDate":"30.09.2001, 00:00:00"""")
      body should include(""""roadNumber":11""")
      body should include(""""startDate":"01.01.1996, 00:00:00"""")
    }
  }
  ...
  test("Test /roadlinks/roadname/:roadNumber/:projectID") {
    roadAddressService.runWithReadOnlySession {
      ...
      // TODO: FIX this test ProjectLinkName
