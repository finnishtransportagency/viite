...
class IntegrationApiSpec extends AnyFunSuite with ScalatraSuite with BeforeAndAfter {
  ...
  test("Test integrationApi.roadAddressLinksToApi() for administrative class When AdministrativeClass is State.") {
    val geometry = Seq(Point(0.0, 0.0), Point(1.0, 0.0, 0.5), Point(4.0, 4.0, 1.5))
    // This roadAddressLink has linearLocationId equal to zero, just to compile.
    val roadAdressLink = RoadAddressLink(63298, 0, 5171208.toString, geometry, GeometryUtils.geometryLength(geometry), AdministrativeClass.State, LifecycleStatus.InUse, LinkGeomSource.NormalLinkInterface, AdministrativeClass.State, None, BigInt(0), "", None, None, RoadPart(5, 205), 1,
  ...
  test("Test integrationApi.roadAddressLinksToApi() When for administrative class When AdministrativeClass is Private.") {
    val geometry = Seq(Point(0.0, 0.0), Point(1.0, 0.0, 0.5), Point(4.0, 4.0, 1.5))
    // This roadAddressLink has linearLocationId equal to zero, just to compile.
    val roadAdressLink = RoadAddressLink(63298, 0, 5171208.toString, geometry, GeometryUtils.geometryLength(geometry), AdministrativeClass.Private, LifecycleStatus.InUse, LinkGeomSource.NormalLinkInterface, AdministrativeClass.Private, None, BigInt(0), "", None, None, RoadPart(5, 205), 1,
      ArealRoadMaintainer.ARMInvalid, 0, AddrMRange(0, 6), "2015-01-01", "2015-12-31", 0.0, 0.0, SideCode.TowardsDigitizing, Some(ProjectCalibrationPoint(120.toString, 1, 2)), None, sourceId = "")
  ...
  test("Test integrationApi.roadAddressLinksToApi() When for administrative class When AdministrativeClass is Unknown.") {
    val geometry = Seq(Point(0.0, 0.0), Point(1.0, 0.0, 0.5), Point(4.0, 4.0, 1.5))
    // This roadAddressLink has linearLocationId equal to zero, just to compile.
    val roadAdressLink = RoadAddressLink(63298, 0, 5171208.toString, geometry, GeometryUtils.geometryLength(geometry), AdministrativeClass.Unknown, LifecycleStatus.InUse, LinkGeomSource.NormalLinkInterface, AdministrativeClass.Unknown, None, BigInt(0), "", None, None, RoadPart(5, 205), 1,
      ArealRoadMaintainer.ARMInvalid, 0, AddrMRange(0, 6), "2015-01-01", "2015-12-31", 0.0, 0.0, SideCode.TowardsDigitizing, Some(ProjectCalibrationPoint(120.toString, 1, 2)), None, sourceId = "")
