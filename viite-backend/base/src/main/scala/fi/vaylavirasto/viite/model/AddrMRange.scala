 ...
 *
 * @throws ViiteException at construction, if the startAddrM, or endAddrM are out of acceptable values.
 */
case class AddrMRange (startAddrM: Long, endAddrM: Long)// extends Ordered[AddrMRange]
{
  ...
  try {
    require((startAddrM>=minAddrMZero && startAddrM <= maxAddrM), s"A start address must be between $minAddrMZero-$maxAddrM.")
    require((  endAddrM>=minAddrMZero &&   endAddrM <= maxAddrM), s"An  end address must be between $minAddrMZero-$maxAddrM.")
    require((  startAddrM < endAddrM || (startAddrM==0 &&  endAddrM==0)), s"A start address (now $startAddrM) must be smaller than end address (now $endAddrM), or range must be undefined (0-0).")
  } catch  {
    case e: IllegalArgumentException => throw ViiteException(e.getMessage)
  }

  /** Returns the AddrMRange in the format "startAddrM-endAddrM", e.g. "0-145".
    * Overrides the very basic java.Object.toString.
    * @override [[java.object.toString]] */
  override def toString : String = {  s"$startAddrM-$endAddrM"  }
  ...
  /** A valid address range is one with positive endAddrM. (Construction time checks ensure that start address is at least 0.)
    * Returns true, if this endAddrM is greater than zero. Else false. */
  def isValid:     Boolean = {  this.endAddrM > 0  }
  /** Returns true, if this AddrMRange has both startAddrM, and endAddrM zeroes. Else false. */
  def isInvalid:   Boolean = {  this.startAddrM == 0 && this.endAddrM == 0  }
