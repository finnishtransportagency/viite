define(['chai', 'geometrycalculator'], function (chai, geometrycalculator) {
  ...
  describe('Geometry calculations: nearest point in line', function () {
    ...
    it('should return rigth value as distance', function () {
      ...
      assert.deepEqual({x: 1.12, y: 0.8400000000000001}, fut({start: {x: 0.0, y: 0.0}, end: {x: 4.0, y: 3.0}}, {
        x: 1.0,
        y: 1.0
      }));
    });

    it('should return right value as distance scenario2', function () {
      ...
      assert.deepEqual({x: 0.8400000000000001, y: 2.88}, fut({start: {x: 0.0, y: 4.0}, end: {x: 3.0, y: 0.0}}, {
        x: 5.0,
        y: 6.0
      }));
    });
  ...
  describe('Geometry calculations: centroid from points', function () {
    ...
    it('should return centroid for larger list of points', function () {
      assert.deepEqual({
        lon: 5,
        lat: 5
      }, gcp([assetLat1Lon1, assetLat2Lon2, assetLat6Lon6, assetLat7Lon7, assetLat9Lon9]));
    });
  });
});
