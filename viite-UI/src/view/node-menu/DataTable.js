(function (root) {
  const NodeMenuTableUtils = {
    ...
    toEvkGroups: function (items, rowMapper) {
      const grouped = _.groupBy(items || [], NodeMenuTableUtils.templateGroupKey);

      return _.reduce(_.sortBy(Object.keys(grouped)), function (result, groupKey) {
        result[groupKey] = {
          label: NodeMenuTableUtils.evkGroupLabel(groupKey),
          rows: _.map(NodeMenuTableUtils.sortedTemplateRows(grouped[groupKey]), rowMapper)
        };
        return result;
      }, {});
    }
