  ...
  root.ProjectLinkLayer = function (map, projectCollection, selectedProjectLinkProperty, roadLayer) {
    ...
    var suravageRoadProjectLayer = new ol.layer.Vector({
      ...
      style: function (feature) {
          return projectLinkStyler.getStyler(feature.linkData, {zoomLevel:Math.trunc(map.getView().getZoom())});
      },
      ...
      style: function (feature) {
        if (!_.isUndefined(feature.linkData))
          if (projectLinkStatusIn(feature.linkData, possibleStatusForSelection) || feature.linkData.roadClass === RoadClass.NoClass.value || feature.linkData.roadLinkSource === LinkGeomSource.SuravageLinkInterface.value) {
              return projectLinkStyler.getSelectionLinkStyle().getStyle(feature.linkData, {zoomLevel:Math.trunc(map.getView().getZoom())});
          }
      }
    });
      ...
      if (isActiveLayer) {
        _.defer(function () {
          if (applicationModel.getSelectedTool() !== 'Cut' && !event.originalEvent.ctrlKey &&
            selectedProjectLinkProperty.get().length === 0 && Math.trunc(map.getView().getZoom()) <= 13) {
    ...
    me.eventListener.listenTo(eventbus, 'projectLink:projectLinksCreateSuccess', function () {
      projectCollection.fetch(map.getView().calculateExtent(map.getSize()).join(','), Math.trunc(map.getView().getZoom()) + 1, undefined, projectCollection.getPublishableStatus());
    });
      ...
      _.each(partitioned[0], function (feature) {
        ...
        if (editedLink) {
          if (_.contains( _.pluck(toBeTerminated, 'id'), feature.linkData.linkId)) {
            ...
            var termination = projectLinkStyler.getStyler(feature.linkData, {zoomLevel:Math.trunc(map.getView().getZoom())});
    ...
    me.eventListener.listenTo(eventbus, 'roadAddressProject:selected', function (projId) {
      me.eventListener.listenToOnce(eventbus, 'roadAddressProject:projectFetched', function (projectInfo) {
        projectCollection.fetch(map.getView().calculateExtent(map.getSize()), Math.trunc(map.getView().getZoom()), projectInfo.id, projectInfo.publishable);
      });
    ...
    me.eventListener.listenTo(eventbus, 'roadAddressProject:fetch', function() {
      ...
      projectCollection.fetch(map.getView().calculateExtent(map.getSize()).join(','), Math.trunc(map.getView().getZoom()) + 1, projectId, projectCollection.getPublishableStatus());
    });
    ...
    me.eventListener.listenTo(eventbus, 'roadAddressProject:projectLinkSaved', function (projectId, isPublishable) {
      projectCollection.fetch(map.getView().calculateExtent(map.getSize()), Math.trunc(map.getView().getZoom()) + 1, projectId, isPublishable);
    });
