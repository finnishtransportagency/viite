  ...
  root.MapView = function(map, layers, instructionsPopup) {
    ...
    eventbus.on('coordinates:selected', function(position) {
      if (geometrycalculator.isInBounds(map.getView().calculateExtent(map.getSize()), position.lon, position.lat)) {
        ...
        map.getView().setZoom(zoomlevels.getAssetZoomLevelIfNotCloser(Math.abs(map.getView().getZoom())));
      } else {
    ...
    map.on('moveend', function() {
      applicationModel.refreshMap(Math.abs(map.getView().getZoom()), map.getLayers().getArray()[0].getExtent(), map.getView().getCenter());
