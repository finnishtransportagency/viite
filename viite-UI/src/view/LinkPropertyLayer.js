  ...
  root.LinkPropertyLayer = function(map, roadLayer, selectedLinkProperty, roadCollection, linkPropertiesModel, applicationModel) {
    ...
    var geometryChangedLayer = new ol.layer.Vector({
      ...
      style: function(feature) {
          return [roadLinkStyler.getRoadLinkStyle().getStyle(feature.linkData, {zoomLevel:Math.round(map.getView().getZoom())}),
              roadLinkStyler.getOverlayStyle().getStyle(feature.linkData, {zoomLevel:Math.round(map.getView().getZoom())})];
      },
    ...
    var zoomDoubleClickListener = function(event) {
      if (isActiveLayer)
        _.defer(function(){
          if (selectedLinkProperty.get().length === 0 && Math.round(map.getView().getZoom()) <= 13) {
    ...
    var selectSingleClick = new ol.interaction.Select({
      ...
      //The new/temporary layer needs to have a style function as well, we define it here.
      style: function(feature) {
          return [roadLinkStyler.getOverlayStyle().getStyle(feature.linkData, {zoomLevel:Math.round(map.getView().getZoom())}),
              roadLinkStyler.getRoadLinkStyle().getStyle(feature.linkData, {zoomLevel:Math.round(map.getView().getZoom())})];
      }
    });
    ...
    var unselectRoadLink = function() {
      ...
      _.map(roadLayer.layer.getSource().getFeatures(),function (feature){
        if (feature.linkData.gapTransfering) {
          ...
          var unknownRoadStyle = roadLinkStyler.getRoadLinkStyle().getStyle(feature.linkData, {zoomLevel: Math.round(map.getView().getZoom())});
    ...
    var redraw = function() {
      ...
      if(Math.round(map.getView().getZoom()) >= zoomlevels.minZoomForAssets) {
        ...
        _.each(directionRoadMarker, function(directionlink) {
          ...
          if(Math.round(map.getView().getZoom()) > zoomlevels.minZoomForDirectionalMarkers)
        ...
        if (Math.round(map.getView().getZoom()) >= zoomlevels.minZoomLevelForCalibrationPoints) {
    ...
    this.refreshView = function() {
      ...
      roadCollection.fetch(map.getView().calculateExtent(map.getSize()), Math.round(map.getView().getZoom()));
      ...
      if (greenFeaturesLinkId.length !== 0) {
        ...
        _.each(roadLayer.layer.getSource().getFeatures(), function (feature) {
            if (_.contains(greenFeaturesLinkId, feature.linkData.linkId)) {
                ...
                var greenRoadStyle = roadLinkStyler.getRoadLinkStyle().getStyle(feature.linkData, {zoomLevel: Math.round(map.getView().getZoom())});
    ...
    me.eventListener.listenTo(eventbus, 'linkProperties:highlightReservedRoads', function(reservedOL3Features){
      var styledFeatures = _.map(reservedOL3Features, function(feature) {
        feature.setStyle(roadLinkStyler.getRoadLinkStyle().getStyle(feature.linkData, {zoomLevel: Math.round(map.getView().getZoom())}));
        return feature;
      });
    ...
    me.eventListener.listenTo(eventbus, 'linkProperty:fetch', function() {
      map.getView().setZoom(Math.round(Math.round(map.getView().getZoom())));
      roadCollection.fetch(map.getView().calculateExtent(map.getSize()).join(','),Math.round(map.getView().getZoom()) + 1);
    });
