...
class JsonSerializerTest extends FunSuite with Matchers {
  ...
  test("testWriteReadCachedGeometry") {
    ...
    val roadLinks = Seq(RoadLink(1L.toString, Seq(Point(0.0, 1.0),Point(0.1, 2.0)), 1.1, AdministrativeClass.State, TrafficDirection.BothDirections, Option("yesterday"), modifiedBy = Option("someone"), Map(), municipalityCode = 257),
      RoadLink(2L.toString, Seq(Point(2.0, 1.0),Point(0.1, 2.0)), 1.1, AdministrativeClass.State, TrafficDirection.BothDirections, Option("yesterday"), modifiedBy = Option("someone"), Map(), municipalityCode = 257))
  ...
  ignore("testWriteHugeCachedGeometry") {
    val f = File.createTempFile("test", ".cache")
    val roadLink = RoadLink(1.toString, Seq(Point(0.0, 1.0),Point(0.1, 2.0)), 1.1, AdministrativeClass.State, TrafficDirection.BothDirections, None, modifiedBy = Option("someone"), Map("TO_RIGHT"->104,"versionstarttime"->new DateTime(1476468913000L).toString,"FROM_LEFT"->103,"MTKHEREFLIP"->1,"MTKID"->362888804,
        ...
        "starttime"->new DateTime(1446132842000L).toString,"GEOMETRY_EDITED_DATE"->1476468913000L,"HORIZONTALACCURACY"->3000,"ROADNUMBER"->1130), municipalityCode = 257)
