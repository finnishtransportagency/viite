...
DECLARE
    ...
    MISSING_JUNCTIONS	TABLE_OF_NUM_TYPE;
...
END IF;
    ...
    -- Insert missing junctions if some got excluded.
    select DISTINCT JUNCTION_ID BULK COLLECT
    INTO MISSING_JUNCTIONS
    from (SELECT JUNCTION_ID
          FROM JUNCTION_POINT
          where JUNCTION_ID not in (select id from JUNCTION));

    INSERT INTO junction SELECT * FROM junction@DEVTESTLINK3 j WHERE j.id IN (SELECT * FROM TABLE (MISSING_JUNCTIONS));
