...
class ProjectReservedPartDAOSpec extends FunSuite with Matchers {
  ...
  test("Test roadPartReservedByProject When removeReservedRoadPart is done in that same road part Then should not be returning that removed part") {
    runWithRollback {
      ...
      projectLinkDAO.create(projectLinks)
      val project = projectReservedPartDAO.fetchReservedPartInOtherProject(roadNumber1, roadPartNumber1)
      ...
      val projectAfter = projectReservedPartDAO.fetchReservedPartInOtherProject(roadNumber1, roadPartNumber1)
