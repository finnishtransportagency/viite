...
class RoadAddressChangesDAOSpec extends FunSuite with Matchers {
  ...
  test("confirm data fetching"){
    runWithRollback{
      ...
      sqlu""" insert into ROADWAY_CHANGES(project_id,change_type,new_road_number,new_road_part_number,new_track_code,new_start_addr_m,new_end_addr_m,new_discontinuity,new_road_type,new_ely) Values(100,1,6,1,1,0,10.5,1,1,8) """.execute
