...
class RoadwayDAOSpec extends FunSuite with Matchers {
  ...
  // TODO Ignored since currently database allows inserting of overlapping addresses
  ignore("Test create When insert roadways m-values overlapping Then throw exception") {
    runWithRollback {
      val error = intercept[SQLException] {
        dao.create(Seq(testRoadway1, testRoadway2.copy(roadPartNumber = testRoadway1.roadPartNumber,
          startAddrMValue = testRoadway1.endAddrMValue - 1)))
      }
