...
import fi.liikennevirasto.viite.{NewIdValue, NodesAndJunctionsService, ProjectService, RoadAddressService, RoadType}
...
class Viite_2517_spec extends FunSuite with Matchers with BeforeAndAfter {
 ...
 /* def withDynTransaction[T](f: => T): T = PostGISDatabase.withDynTransaction(f)
  ...
  test("Test projectService.updateProjectLinks() When reserving a road part, try to renumber it to the same number Then it should produce an error.") {
    runWithRollback {
      ...
      val road_101_5 = roadwayAddressMapper.getRoadAddressesByLinearLocation(linearLocationDAO.fetchByRoadways(roadwayDAO.fetchAllBySection(101, 5).map(_.roadwayNumber).toSet)).sortBy(_.startAddrMValue).toList
      val roadToReserve = road_101_5
      ...
      *//* -- load project*/
      ...
      val first_road_part_to_update = projectService_db.getProjectLinks(projectSaved.id).filter(
        x => x.track == Track(1) && x.roadPartNumber == 5 && x.endAddrMValue <= 965).toList
      val second_road_part_to_update = projectService_db.getProjectLinks(projectSaved.id).filter(
        x => x.track == Track(1) && x.roadPartNumber == 5 && x.endAddrMValue <= 965).toList
      val third_road_part_to_update = projectService_db.getProjectLinks(projectSaved.id).filter(
        x => x.track == Track(2) && x.roadPartNumber == 5 && x.startAddrMValue >= 965).toList
      val fourth_road_part_to_update = projectService_db.getProjectLinks(projectSaved.id).filter(
        x => x.track == Track(2) && x.roadPartNumber == 5 && x.startAddrMValue >= 965).toList

      val road_tracks_to_test = List(
                                      Test_config(first_road_part_to_update,  1,  LinkStatus.UnChanged),
                                      Test_config(second_road_part_to_update, 1,  LinkStatus.UnChanged),
                                      Test_config(third_road_part_to_update,  3,  LinkStatus.Transfer),
                                      Test_config(fourth_road_part_to_update, 3,  LinkStatus.Transfer)
                                     )
      ...
      for ((test_track, index) <- road_tracks_to_test.view.zipWithIndex) {
        ...
        projectService_db.updateProjectLinks(
          ...
          test_track.track_to_test.head.roadPartNumber,
          test_track.track_to_test.head.track.value,
          userDefinedEndAddressM = None,
          test_track.roadType,
          test_track.track_to_test.head.discontinuity.value,
          Some(2),
