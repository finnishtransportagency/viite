...
class Viite_PM1_mock_spec extends FunSuite with Matchers with BeforeAndAfter {
...
*/
  ...
  test("Test projectService.updateProjectLinks() When reserving a road part, try to renumber it to the same number Then it should produce an error.") {
    runWithRollback {
      ...
      /*val links = Seq(addressToRoadLink(road_420921(0)),
                      addressToRoadLink(road_420921(1)),
                      addressToRoadLink(road_420921(2)),
                      addressToRoadLink(road_420921(3)),
                      addressToRoadLink(road_420921(4))
                      )*/
      when(mockRoadLinkService.getRoadLinksHistoryFromVVH(any[Set[Long]])).thenReturn(Seq())
      ...
      *//* -- load project*/
      ...
      val allProjectLinks  = projectService.getProjectLinks(projectId.id).toList
    /*  (projectId: Long, ids: Set[Long], linkIds: Seq[Long], linkStatus: LinkStatus, userName: String,
      ...
      reversed: Boolean = false, roadName: Option[String] = None, coordinates: Option[ProjectCoordinates] = None)
*/

        projectService_db.updateProjectLinks(
          projectId,
          allProjectLinks.map(_.id).toSet,
          List(),
          linkStatus = LinkStatus.Transfer,
          userName = project.name,
          newRoadNumber = allProjectLinks.head.roadNumber,
          newRoadPartNumber = allProjectLinks.head.roadPartNumber,
          ...
          ely = Some(allProjectLinks.head.ely),
          reversed = false,
          roadName = "",
...
//      updatedLinks.foreach(println)
     ...
     // val x = validationErrorsAfterReverse(1)
      //x("validationError")
    //  x.take(2).last._2
