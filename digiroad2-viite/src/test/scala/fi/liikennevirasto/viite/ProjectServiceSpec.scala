...
class ProjectServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Test handleNewRoadNames - Test if a new RoadName is created from a project link") {
    runWithRollback {
      ...
      ProjectLinkNameDAO.create(project.id, roadNumber, testName)

      // Method to be tested
      projectService.handleNewRoadNames(changes)

      ProjectLinkNameDAO.get(project.id, roadNumber) should be (None)
      val roadnames = RoadNameDAO.getAllByRoadNumber(roadNumber)
      roadnames.foreach(rn => rn.roadName should be (testName))
    }
  }
