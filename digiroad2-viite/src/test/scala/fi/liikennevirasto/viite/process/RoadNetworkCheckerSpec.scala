...
class RoadNetworkCheckerSpec extends FunSuite with Matchers {
  ...
  test("Geometry subtracted by more than 1 meter should trigger floating check") {
    ...
    val roadAddressSeq = Seq(
      RoadAddress(1L, 1L, 12L, 1, RoadType.Unknown, Track.RightSide, Discontinuity.Continuous, 5L, 60L, None, None, None, 123, KMTKID("123", 0), 0.0, 54.948,
      ...
      RoadAddress(2L, 1L, 12L, 1, RoadType.Unknown, Track.RightSide, Discontinuity.Continuous, 60L, 75L, None, None, None, 123, KMTKID("123", 0), 54.948, 69.844,
  ...
  test("Automatic Merged road addresses that also overlap with the road link should not trigger a floating check") {
    ...
    val roadAddressSeq = Seq(RoadAddress(411362, 1L, 12819, 2, RoadType.PublicRoad, Track.Combined, Discontinuity.Continuous, 10, 92,
      Some(DateTime.parse("2015-08-25T00:00:00.000+03:00")), None, Some("Automatic_merged"), 5515411, KMTKID("5515411", 0), 0.0, 69.87700000000001,
