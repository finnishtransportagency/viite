...
class RoadAddressChangeInfoMapperSpec extends FunSuite with Matchers {
  ...
  test("resolve simple case") {
    ...
    val map = Seq(roadAddress1, roadAddress2).groupBy(ad => (ad.linkId, ad.roadwayId)).mapValues(s => LinkRoadAddressHistory(s, Seq()))
    val changes = Seq(
    ...
    results.values.flatten.map(_.roadwayId).toSet.size should be (1)
    results.values.flatten.map(_.roadwayId).toSet.head should be (roadAddress1.roadwayId)
  }
  ...
  test("no changes should apply") {
    ...
    val map = Seq(roadAddress1, roadAddress2, roadAddress3).groupBy(ad => (ad.linkId, ad.roadwayId)).mapValues(s => LinkRoadAddressHistory(s, Seq()))
    val changes = Seq(
      ChangeInfo(Some(roadLinkId1), Some(roadLinkId2), 123L, 2, Some(0.0), Some(1000.234), Some(399.648), Some(1399.882), changesVVHTimestamp),
      ChangeInfo(Some(roadLinkId2), Some(roadLinkId2), 123L, 1, Some(0.0), Some(399.648), Some(0.0), Some(399.648), changesVVHTimestamp),
  ...
  test("split a road address link into three") {
    ...
    val map = Seq(roadAddress1).groupBy(ad => (ad.linkId, ad.roadwayId)).mapValues(s => LinkRoadAddressHistory(s, Seq()))
    val changes = Seq(
      //Remain
  ...
  test("Lengthened changes with multiple road addresses at same road link") {
    ...
    val map = Seq(roadAddress1, roadAddress2, roadAddress3, roadAddress4).groupBy(ad => (ad.linkId, ad.roadwayId)).mapValues(s => LinkRoadAddressHistory(s, Seq()))
    val changes = Seq(
      //Old parts
      ChangeInfo(Some(roadLinkId1), Some(roadLinkId1), 123L, 3, Some(0.0), Some(960.434), Some(0.535), Some(960.969), changesVVHTimestamp),
      ChangeInfo(Some(roadLinkId2), Some(roadLinkId2), 456L, 3, Some(0.0), Some(201.333), Some(201.333), Some(0.0), changesVVHTimestamp),
  ...
  test("Removed link") {
    ...
    val roadAddress1 = roadAddr.copy(startAddrMValue = 400, endAddrMValue = 1400, linkId = roadLinkId1, endMValue = 960.434, roadwayId = 0,
  ...
  test("Multiple operations applied in order") {
    ...
    val roadAddress1 = roadAddr.copy(track = Track.Combined, startAddrMValue = 400, endAddrMValue = 1400, linkId = roadLinkId1, endMValue = 960.434,
      adjustedTimestamp = roadAdjustedTimestamp, geometry = Seq(Point(0.0, 0.0), Point(960.434, 0.0)), roadwayId = 0)
    val map = Seq(roadAddress1).groupBy(ad => (ad.linkId, ad.roadwayId)).mapValues(s => LinkRoadAddressHistory(s, Seq()))
    val changes = Seq(
