...
class RoadAddressChangeInfoMapperSpec extends FunSuite with Matchers {
  ...
  test("Shortened changes with multiple road addresses at same road link") {
    ...
    val roadAddress2 = roadAddr.copy(startAddrMValue = 800, endAddrMValue = 1400, linkId = roadLinkId1, endMValue = 400.969, geometry = Seq(Point(400.0, 0.0), Point(960.969, 0.0)), adjustedTimestamp = roadAdjustedTimestamp, track = Track.Combined)
    ...
    val addr1 = results(roadLinkId1).head
    addr1.startMValue should be (0.0)
    addr1.endMValue should be (399.465)
    addr1.startAddrMValue should be (400)
    ...
    val addr2 = addrAtLink1.head
    addr2.startMValue should be (399.465)
