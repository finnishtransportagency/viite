...
class RoadwayMapperSpec extends FunSuite with Matchers{
  ...
  test("Test roadwayAddressMapper.mapRoadAddresses() When giving it regular, current linear locations and roadways Then return the mapping of them in the form of Road Addresses."){
    ...
    val roadwayAddress = Roadway(2L, roadwayNumber, 1551, 2, AdministrativeClass.PublicRoad, Track.Combined, Discontinuity.Discontinuous, 140, 300, reversed = false, startDate, None, "test_user", None, 0, NoTermination, startDate)
  ...
  test("Test roadwayAddressMapper.mapRoadAddresses() When giving it regular, history linear locations and roadways Then return the mapping of them in the form of Road Addresses."){
    ...
    val currentRoadwayAddress = Roadway(2L, roadwayNumber, 1551, 2, AdministrativeClass.PublicRoad, Track.Combined, Discontinuity.Discontinuous, 140, 300, reversed = false, currentStartDate, None, "test_user", None, 0, NoTermination, currentStartDate)
    val historyRoadwayAddress = Roadway(3L, roadwayNumber, 1551, 2, AdministrativeClass.PublicRoad, Track.Combined, Discontinuity.Discontinuous, 240, 400, reversed = false, historyStartDate, Some(historyEndDate), "test_user", None, 0, NoTermination, historyStartDate)
    val linearLocations = Seq(
      LinearLocation(2L, 1, 125, 45.0, 105.0, SideCode.TowardsDigitizing, 0,
        (CalibrationPointReference.None, CalibrationPointReference.None)
        , Seq(Point(45.0,0.0), Point(105.0,0.0)), LinkGeomSource.NormalLinkInterface, roadwayNumber),
