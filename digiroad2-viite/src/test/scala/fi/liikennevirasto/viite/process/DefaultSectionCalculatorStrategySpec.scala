...
class DefaultSectionCalculatorStrategySpec extends FunSuite with Matchers {
  ...
  test("Test defaultSectionCalculatorStrategy.assignMValues() and the attribution of roadway_numbers for new Left Right sections with diff number of links Then " +
    "if there are for e.g. 2 (two) consecutive links with diff roadway_number (and all Transfer status), the opposite track, should split their link(s) if the amount of links is lower than two (with all New status) to have the same amount of links, with same end addresses as the other side and different roadway numbers") {
    runWithRollback {
