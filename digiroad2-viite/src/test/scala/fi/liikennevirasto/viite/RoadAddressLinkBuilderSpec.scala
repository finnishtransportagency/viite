...
class RoadAddressLinkBuilderSpec extends FunSuite with Matchers {
  ...
  test("Test RoadAddressLinkBuilder.build for roadlink and roadAddress input") {
    runWithRollback {
      ...
      val roadlink = RoadLinkFetched(linkId = 1L.toString, 91, List(Point(0.0, 0.0), Point(120.0, 0.0)), AdministrativeClass.Municipality, TrafficDirection.BothDirections, FeatureClass.AllOthers)
