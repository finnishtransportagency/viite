...
class RoadAddressLinkBuilderSpec extends FunSuite with Matchers {
  ...
  test("Test RoadAddressLinkBuilder.build for roadlink and roadAddress input") {
    runWithRollback {
      ...
      val roadlink = RoadLink(linkId = 1L.toString, List(Point(0.0, 0.0), Point(120.0, 0.0)), 120.0, AdministrativeClass.Municipality, -1, TrafficDirection.BothDirections, UnknownLinkType, None, None, attributes1)
