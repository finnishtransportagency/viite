...
class RoadAddressServiceSpec extends FunSuite with Matchers{
  ...
  test("Test SearchService") {
    ...
    val towardsDigitizingRoadLink = Seq(
      RoadLink(linkId, Seq(Point(0.0, 10.0), Point(0.0, 15.0)), 10.0, AdministrativeClass.Municipality, TrafficDirection.TowardsDigitizing, None, None, attributes = Map("MUNICIPALITYCODE" -> BigInt(235)), UnknownLifecycleStatus, NormalLinkInterface, 235)
    )
...
//  }
  ...
  test("Test roadAddressService.getAllByMunicipality() When asking for data for a specific municipality Then return all the Road addresses for that municipality."){
    ...
    val roadLinks = (Seq(
      RoadLink(123L.toString, newGeom0010, 17, AdministrativeClass.apply(2), TrafficDirection.TowardsDigitizing, None, None, Map("MUNICIPALITYCODE" -> BigInt.apply(99999), "MTKCLASS" -> BigInt.apply(12316)), municipalityCode = 99999),
      RoadLink(123L.toString, newGeom1020, 17, AdministrativeClass.apply(2), TrafficDirection.TowardsDigitizing, None, None, Map("MUNICIPALITYCODE" -> BigInt.apply(99999), "MTKCLASS" -> BigInt.apply(12141)), municipalityCode = 99999),
      RoadLink(124L.toString, newGeom3040, 17, AdministrativeClass.apply(2), TrafficDirection.TowardsDigitizing, None, None, Map("MUNICIPALITYCODE" -> BigInt.apply(99999), "MTKCLASS" -> BigInt.apply(12314)), municipalityCode = 99999),
      RoadLink(125L.toString, newGeom4050, 17, AdministrativeClass.apply(2), TrafficDirection.TowardsDigitizing, None, None, Map("MUNICIPALITYCODE" -> BigInt.apply(99999), "MTKCLASS" -> BigInt.apply(12312)), municipalityCode = 99999)
...
//    when(mockRoadLinkService.getComplementaryRoadLinksFromVVH(99999)).thenReturn(Seq())
  ...
  test("Test handleRoadwayPoints When Terminating one link and Transfer the rest Then roadway points should be handled/created properly") {
    runWithRollback {
      ...
      val link1 = dummyProjectLink(99, 2, Track.Combined, Discontinuity.Continuous, 0, 5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, LinkStatus.Terminated, projectId, AdministrativeClass.State, geom1, roadwayNumber1)
      val link2 = dummyProjectLink(99, 2, Track.Combined, Discontinuity.EndOfRoad, 5, 20, Some(DateTime.now()), None, 12346.toString, 0, 15, SideCode.TowardsDigitizing, LinkStatus.Transfer, projectId, AdministrativeClass.State, geom2, roadwayNumber2)
  ...
  test("Test handleRoadwayPoints When Terminating one link and Transfer the rest and reverse it Then roadway points should be handled/created properly") {
    runWithRollback {
      ...
      val linearLocationId = Sequences.nextLinearLocationId
      val link1 = dummyProjectLink(99, 2, Track.Combined, Discontinuity.Continuous, 0, 5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, LinkStatus.Terminated, projectId, AdministrativeClass.State, geom1, roadwayNumber1)
      val link2 = dummyProjectLink(99, 2, Track.Combined, Discontinuity.EndOfRoad, 5, 20, Some(DateTime.now()), None, 12346.toString, 0, 15, SideCode.AgainstDigitizing, LinkStatus.Transfer, projectId, AdministrativeClass.State, geom2, roadwayNumber2)
  ...
  test("Test handleRoadwayPoints When dual roadway point is handled") {
    runWithRollback {
      ...
      val projectId = Sequences.nextViiteProjectId
      val rap =  dummyProject(projectId, ProjectState.Incomplete, Seq(), None).copy(startDate = DateTime.parse("2019-10-10"))
      val link1 = dummyProjectLink(99, 1, Track.Combined, Discontinuity.EndOfRoad, 0, 5, Some(DateTime.now()), None, 12345.toString, 0, 5, SideCode.TowardsDigitizing, LinkStatus.UnChanged, projectId, AdministrativeClass.State, geom1, roadwayNumber2)
      val link2 = dummyProjectLink(99, 2, Track.Combined, Discontinuity.EndOfRoad, 0, 15, Some(DateTime.now()), None, 12346.toString, 0, 15, SideCode.TowardsDigitizing, LinkStatus.Transfer, projectId, AdministrativeClass.State, geom2, roadwayNumber3)
