...
-- [TYPE = 3] Includes templates, points where ADDR_M is equal to START_ADDR_M or END_ADDR_M of the road (road_number, road_part_number and track) and when ROAD_TYPE changes
		...
		WHEN (rp.ADDR_M = rw.END_ADDR_M AND rw.DISCONTINUITY IN (1,2,3,4,6)) THEN 3 /* There is DISCONTINUITY */
		WHEN (rp.ADDR_M = (SELECT MIN(roadAddr.START_ADDR_M) FROM ROADWAY roadAddr
				WHERE roadAddr.ROAD_NUMBER = rw.ROAD_NUMBER AND roadAddr.ROAD_PART_NUMBER = rw.ROAD_PART_NUMBER
				AND roadAddr.VALID_TO IS NULL AND roadAddr.END_DATE IS NULL)) THEN 3 /* ADDR_M is equal to START_ADDR_M */
		...
		WHEN ((SELECT DISTINCT(roadAddr.TRACK) FROM ROADWAY roadAddr
			...
			(SELECT DISTINCT(MAX(roadAddr.TRACK)) FROM ROADWAY roadAddr
				WHERE roadAddr.ROAD_NUMBER = rw.ROAD_NUMBER AND roadAddr.ROAD_PART_NUMBER = rw.ROAD_PART_NUMBER AND roadAddr.END_ADDR_M = rp.ADDR_M
				AND roadAddr.VALID_TO IS NULL AND roadAddr.END_DATE IS NULL AND roadAddr.TRACK IN (1,2)))  THEN 3 /* track changed on ADDR_M */
		WHEN ((SELECT DISTINCT(max(roadAddr.TRACK)) FROM ROADWAY roadAddr
			...
			(SELECT DISTINCT(roadAddr.TRACK) FROM ROADWAY roadAddr
				WHERE roadAddr.ROAD_NUMBER = rw.ROAD_NUMBER AND roadAddr.ROAD_PART_NUMBER = rw.ROAD_PART_NUMBER AND roadAddr.END_ADDR_M = rp.ADDR_M
				AND roadAddr.VALID_TO IS NULL AND roadAddr.END_DATE IS NULL AND roadAddr.TRACK = 0))  THEN 3 /* track changed on ADDR_M */
		ELSE 2
