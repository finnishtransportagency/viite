...
class RoadAddressService(roadLinkService: RoadLinkService, eventbus: DigiroadEventBus, frozenTimeVVHAPIServiceEnabled: Boolean = false) {
  ...
  def getRoadAddressLinks(boundingBoxResult: BoundingBoxResult,
    ...
    val combinedFuture=
      ...
      }  yield( changedRoadLinksF, (roadLinkFuture, complementaryFuture), suravageLinksF)

    val roadAddressResults = fetchRoadAddressesByBoundingBox(boundingRectangle)
    val (changedRoadLinks,((roadLinks, complementaryLinks)), suravageLinks) =
      withTiming(
    ...
    val filteredChangedRoadLinks = changedRoadLinks.filter(cr => addresses.exists(_.linkId == cr.oldId.getOrElse(0)))
  ...
  def applyChanges(roadLinks: Seq[RoadLink], changedRoadLinks: Seq[ChangeInfo], addresses: Map[(Long, Long), LinkRoadAddressHistory]): Seq[LinkRoadAddressHistory] = {
    if (changedRoadLinks.isEmpty)
      ...
      val ids = withDynTransaction {
        RoadAddressDAO.create(savedRoadAddresses).toSet ++ unchanged.map(_.id).toSet
      }
