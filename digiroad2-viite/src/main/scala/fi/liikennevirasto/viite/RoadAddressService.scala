...
class RoadAddressService(roadLinkService: RoadLinkService, eventbus: DigiroadEventBus, frozenTimeVVHAPIServiceEnabled: Boolean = false) {
  ...
  //TODO first version of VIITE-1549 this can be moved to a separated file also the code can be improved
  private def mapRoadAddresses(roadAddress: RoadwayAddress, linearLocations: Seq[LinearLocation]) : Seq[RoadAddress] = {
    ...
    def calculateHistoryCalibrationPoint(currentRoadwayAddress: RoadwayAddress, historyRoadwayAddress: RoadwayAddress)(calibrationAddress: Long): Long = {
    ...
    def recalculateHistoryCalibrationPoints(): Seq[LinearLocation] = {
    ...
    def recalculateBetweenCalibrationPoints(segmentLinearLocations: Seq[LinearLocation], startAddress: Long, endAddress: Long) = {
    ...
    def recursiveRecalculator(segmentLinearLocations: Seq[LinearLocation]) : Seq[RoadAddress] = {
      ...
      recalculateBetweenCalibrationPoints(toProcess, startAddrMValue, endAddrMValue)  ++ recursiveRecalculator(others)
    }
    ...
    val roadAddresses = recursiveRecalculator(if(roadAddress.endDate.nonEmpty) recalculateHistoryCalibrationPoints() else roadwayLinearLocations)
...
//    })
  ...
  private def getRoadAddressLinks(boundingBoxResult: BoundingBoxResult,
...
//    }
  ...
  private def setBlackUnderline(addresses: Seq[RoadAddressLink]): Seq[RoadAddressLink] = {
    time(logger, "Set the black underline") {
      //TODO move this decision to the client side styling
