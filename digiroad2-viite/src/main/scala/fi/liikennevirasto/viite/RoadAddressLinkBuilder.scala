...
class RoadAddressLinkBuilder(roadwayDAO: RoadwayDAO, linearLocationDAO: LinearLocationDAO, projectLinkDAO: ProjectLinkDAO) extends AddressLinkBuilder {
  ...
  def build(roadLink: RoadLinkLike, roadAddress: RoadAddress): RoadAddressLink = {
    ...
    val kmtkRoadName = getKMTKRoadName(roadLink.attributes)
    ...
    RoadAddressLink(roadAddress.id, roadAddress.linearLocationId, roadLink.linkId, geom,
      length, roadLink.administrativeClass, UnknownLinkType, roadLink.constructionType, roadLink.linkSource, roadType, kmtkRoadName, roadName, municipalityCode, municipalityName, extractModifiedAtKMTK(roadLink.attributes), Some("kmtk_modified"),
  ...
  def build(roadAddress: RoadAddress): RoadAddressLink = {
    ...
    RoadAddressLink(roadAddress.id, roadAddress.linearLocationId, roadAddress.linkId, geom,
      length, AdministrativeClass.apply(1), LinkType.apply(99), ConstructionType.apply(0), LinkGeomSource.apply(1), roadType, Some(""), roadAddress.roadName, municipalityCode, "", Some(""), Some("kmtk_modified"),
  ...
  private def getKMTKRoadName(link: Map[String, Any]): Option[String] = {
