...
object RoadAddressLinkPartitioner extends GraphPartitioner {
  ...
  def partition[T <: RoadAddressLinkLike](links: Seq[T]): Seq[Seq[T]] = {
    ...
    val groups = linkGroups.map(group => group._1._5 match {
      case true => partitionForFloatingsWithCalibration( Seq[Seq[T]](), Seq() ,group._2.sortBy(_.startAddressM)).map(g => (group._1, g))
      case _ => Seq(group)
    }).flatten.toSeq
