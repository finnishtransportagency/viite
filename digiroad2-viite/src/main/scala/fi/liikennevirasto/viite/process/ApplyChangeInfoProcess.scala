...
// TODO Make this work with KMTK
object ApplyChangeInfoProcess {

  private val logger = LoggerFactory.getLogger(getClass)

  private case class Projection(oldLinkId: Long, newLinkId: Long, oldStart: Double, oldEnd: Double, newStart: Double, newEnd: Double, timeStamp: Long, orderIncrement: Int = 0) {
    /**
  ...
  private def projectLinearLocation(linearLocation: LinearLocation, projection: Projection, mappedRoadLinks: Map[Long, RoadLinkLike]): LinearLocation = {
    ...
    linearLocation.copy(
      id = newId, linkId = projection.newLinkId, startMValue = newStartMeasure, endMValue = newEndMeasure, sideCode = newSideCode, geometry = geometry, adjustedTimestamp = projection.timeStamp,
