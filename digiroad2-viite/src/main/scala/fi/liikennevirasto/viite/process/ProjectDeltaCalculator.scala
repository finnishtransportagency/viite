...
//  }
  ...
  private def combineProjectLinks(pl1: ProjectLink, pl2: ProjectLink, allNonTerminatedProjectLinks: Seq[ProjectLink]): Seq[ProjectLink] = {
    ...
    val oppositePl1 = allNonTerminatedProjectLinks.filter( pl => pl.track != pl1.track && pl.track != Track.Combined && (pl.endAddrMValue == pl1.endAddrMValue || pl.originalEndAddrMValue == pl1.originalEndAddrMValue))
    val oppositePl2 = allNonTerminatedProjectLinks.filter( pl => pl.track != pl1.track && pl.track != Track.Combined && (pl.startAddrMValue == pl1.endAddrMValue || pl.originalStartAddrMValue == pl1.originalEndAddrMValue))
  ...
  def generateChangeTableRowsFromProjectLinks(projectLinksWithoutTerminated: Seq[ProjectLink], allProjectLinks: Seq[ProjectLink]): ChangeTableRows2 = {
    ...
    val averagedStarts = if (projectLinksWithoutTerminated.forall(_.status == LinkStatus.Terminated)) createAverageValuesForTransferedStarts(terminatedForAveraging) else createAverageValuesForTransferedStarts(startLinks)
    ...
    val grouped = if (allProjectLinks.exists(pl => pl.status == LinkStatus.Terminated && pl.originalStartAddrMValue == 0))
      (averagedStarts ++ projectLinksWithoutTerminated.filterNot(pl => averagedStarts.map(_.id).contains(pl.id))).groupBy(groupToSections)

    else
      projectLinksWithoutTerminated.groupBy(groupToSections)
    ...
    val sections = sectioned.map(sect => {
      ...
      val target                    = targetToMap.copy(projectLinks = projectLinksWithoutTerminated.filter(link => {
