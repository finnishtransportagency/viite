...
case class ProjectLink(id: Long, roadNumber: Long, roadPartNumber: Long, track: Track,
  ...
  lazy val startingPoint: Point = if (sideCode == SideCode.AgainstDigitizing) geometry.last else geometry.head
  lazy val endPoint: Point = if (sideCode == SideCode.AgainstDigitizing) geometry.head else geometry.last
