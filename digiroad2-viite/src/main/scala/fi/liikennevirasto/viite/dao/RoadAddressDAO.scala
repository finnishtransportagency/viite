...
import fi.liikennevirasto.viite.dao.RoadwayDAO
...
//      case (None, Some(cp2)) => (Option.empty[ProjectLinkCalibrationPoint], Option(ProjectLinkCalibrationPoint(linkId, endMValue, cp2, calibrationPointSource)))
//      case (Some(cp1), None) => (Option(ProjectLinkCalibrationPoint(linkId, startMValue, cp1, calibrationPointSource)), Option.empty[ProjectLinkCalibrationPoint])
//      case (Some(cp1), Some(cp2)) => (Option(ProjectLinkCalibrationPoint(linkId, startMValue, cp1, calibrationPointSource)), Option(ProjectLinkCalibrationPoint(linkId, endMValue, cp2, calibrationPointSource)))
//    }
//  }
//}
...
class RoadAddressDAO extends BaseDAO {
  ...
  private implicit val getRoadAddress: GetResult[RoadAddress] = new GetResult[RoadAddress] {
    def apply(r: PositionedResult) = {
      ...
      val startCalPoint = r.nextLongOption()
      val endCalPoint = r.nextLongOption()
      val calibrationPoints: (Option[CalibrationPoint], Option[CalibrationPoint]) = CalibrationPointsUtils.calibrations((startCalPoint, endCalPoint), linkId, startMValue, endMValue)
      val floating = FloatingReason.apply(r.nextInt())
      val geometry = Seq(Point(r.nextDouble(), r.nextDouble()), Point(r.nextDouble(), r.nextDouble()))
      val linkGeomSource = LinkGeomSource.apply(r.nextInt())
      val ely = r.nextLong()
      ...
      RoadAddress(id, linearLocationId, roadNumber, roadPartNumber, roadType, track, discontinuity, startAddrMValue,
        endAddrMValue, startDate, endDate, createdBy, linkId, startMValue, endMValue, sideCode, adjustedTimestamp,
        calibrationPoints, floating, geometry, linkGeomSource, ely, terminated, roadwayNumber, validFrom, validTo, roadName)
    }
  }
